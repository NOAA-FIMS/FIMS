...
std::function<bool()> rebuild_model;
...
// Laplace records on temporary graphs. Recreate every scalar from the XPtr
// modules on that graph, including constants not present in variable_map.
class XPtrModelAdapter {
 public:
  explicit XPtrModelAdapter(
  ...
  Type evaluate(const std::vector<Type>& parameters,
    ...
    if constexpr (std::is_same_v<Type, double>) {
      ...
      return evaluate_on_graph(parameters).val;
    } else {
      return evaluate_on_graph(parameters);
    }
  }
 ...
 private:
  template <typename Type>
  QUADRA_FIMS_TYPE evaluate_on_graph(const std::vector<Type>& parameters) {
    if (!rebuild_model || !rebuild_model()) {
      throw std::runtime_error("Unable to rebuild the XPtr model for Quadra.");
    }
...
/** Restore a persistent graph after Laplace's temporary graphs are destroyed.
 */
struct RestoreModelGraph {
  bool active;
  ~RestoreModelGraph() {
    if (!active) return;
    try {
      ...
      // The next call reports an unbuilt model rather than using stale scalars.
      fims_info::Information<QUADRA_FIMS_TYPE>::GetInstance()->Clear();
    }
  }
};
...
SEXP numeric_vector(const std::vector<double>& values) {
  SEXP result =
      PROTECT(Rf_allocVector(REALSXP, static_cast<R_xlen_t>(values.size())));
...
SEXP numeric_matrix(const Eigen::MatrixXd& values) {
  SEXP result = PROTECT(Rf_allocMatrix(REALSXP, static_cast<int>(values.rows()),
...
extern "C" SEXP fims_call_quadra_fit(SEXP fixed_sexp, SEXP random_sexp,
                                     SEXP method_sexp, SEXP max_iterations_sexp,
                                     SEXP tolerance_sexp) {
  try {
    ...
    fims_quadra::RestoreModelGraph restore{method == "laplace"};
    ...
    fims_quadra::XPtrModelAdapter model(info);
    ...
    if (method == "laplace" && !random.empty()) {
      ...
      auto result = quadra::stats::optimize_laplace(model, fixed, random,
                                                    partition, options);
      ...
      if (message.empty())
        message =
            converged ? "Quadra L-BFGS converged" : "Quadra L-BFGS stopped";
    }
...
extern "C" SEXP fims_call_quadra_evaluate(SEXP fixed_sexp, SEXP random_sexp) {
  try {
    ...
    std::vector<double> gradient_values(gradient.data(),
...
extern "C" SEXP fims_call_quadra_sdreport(SEXP fixed_sexp, SEXP random_sexp) {
  try {
    ...
    const bool positive_definite = factorization.info() == Eigen::Success;
    ...
    fims_quadra::set_list_element(
        ...
        fims_quadra::numeric_matrix(
            covariance.topLeftCorner(static_cast<Eigen::Index>(fixed.size()),
