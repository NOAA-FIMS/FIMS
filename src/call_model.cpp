...
void add_prior_internal(const std::string& module, uint32_t object_id,
  ...
  if (target == NULL)
    Rf_error("Could not find native prior target `%s.%u.%s`.", module.c_str(),
             object_id, parameter.c_str());
  const size_t target_size = target->size();
  if (target_size == 0)
    Rf_error("Native prior target `%s.%u.%s` is empty.", module.c_str(),
             object_id, parameter.c_str());
  if (mean_size != 1 && mean_size != target_size)
    Rf_error(
        "`mean` must have length 1 or match the prior target length (%zu).",
        target_size);
  if (log_sd_size != 1 && log_sd_size != target_size)
    Rf_error(
        "`log_sd` must have length 1 or match the prior target length (%zu).",
        target_size);
  ...
  for (size_t i = 0; i < target_size; ++i)
    prior->expected_values[i] = static_cast<Type>(0.0);
  for (size_t i = 0; i < mean_size; ++i) {
    prior->expected_mean[i] = static_cast<Type>(mean[i]);
    register_prior_parameter(
        info, prior->expected_mean[i], mean_estimation_type,
        distribution_name + "." + fims::to_string(prior->id) + ".mean." +
            fims::to_string(i));
  }
  for (size_t i = 0; i < log_sd_size; ++i) {
    ...
    register_prior_parameter(info, prior->log_sd[i], log_sd_estimation_type,
                             distribution_name + "." +
                                 fims::to_string(prior->id) + ".log_sd." +
                                 fims::to_string(i));
  }
  ...
  for (auto population_it = info->populations.begin();
    ...
    if (population->log_M.size() != n_age_year) {
      Rf_error(
          ...
          population->log_M.size(), n_age_year);
    }
    ...
    if (population->log_f_multiplier.size() != n_years) {
      Rf_error(
          "Population %u has log_f_multiplier size %zu but expected "
          "n_years=%zu. "
          "Refusing to resize estimable vectors after parameter registration.",
          static_cast<unsigned int>(population->GetId()),
          population->log_f_multiplier.size(), n_years);
    }
    ...
    if (population->log_init_naa.size() != n_ages) {
      Rf_error(
          ...
          "Refusing to resize estimable vectors after parameter registration.",
          static_cast<unsigned int>(population->GetId()),
          population->log_init_naa.size(), n_ages);
    }
    ...
    for (size_t fleet_index = 0; fleet_index < population->fleets.size();
      ...
      if (fleet->n_ages > 0 && fleet->age_to_length_conversion.size() > 0 &&
...
void validate_population_and_fleet_dimensions(
  ...
  const size_t expected_age_year_size =
  ...
  if (population->ages.size() != population->n_ages) {
    ...
    ss << "Population " << population->GetId()
       << " ages size mismatch: expected " << population->n_ages << ", got "
  ...
  if (population->M.size() != expected_age_year_size) {
    std::stringstream ss;
    ss << "Population " << population->GetId() << " M size mismatch: expected "
       << expected_age_year_size << ", got " << population->M.size() << ".";
    fail_dimension_check<Type>(ss.str());
  }
  ...
  if (population->recruitment->log_expected_recruitment.size() !=
      population->n_years) {
    std::stringstream ss;
  ...
  if (population->fleets.size() != population->n_fleets) {
    std::stringstream ss;
    ss << "Population " << population->GetId()
       << " fleet count mismatch: expected " << population->n_fleets << ", got "
       << population->fleets.size() << ".";
    fail_dimension_check<Type>(ss.str());
  }
  ...
  for (size_t fleet_index = 0; fleet_index < population->fleets.size();
    ...
    if (fleet->n_lengths > 0 && fleet->age_to_length_conversion.size() !=
...
template <typename Type>
void validate_backend_model_inputs(
    std::shared_ptr<fims_info::Information<Type>> info) {
  for (auto population_it = info->populations.begin();
       population_it != info->populations.end(); ++population_it) {
...
void assign_population_derived_vectors(
  ...
  derived_quantities["total_catch_numbers"] =
      fims::Vector<Type>(n_years, Type(), "total_catch_numbers");
  derived_quantities["mortality_F"] =
      fims::Vector<Type>(n_years * n_ages, Type(), "mortality_F");
  derived_quantities["mortality_M"] =
      fims::Vector<Type>(n_years * n_ages, Type(), "mortality_M");
  derived_quantities["mortality_Z"] =
      fims::Vector<Type>(n_years * n_ages, Type(), "mortality_Z");
  derived_quantities["numbers_at_age"] =
      fims::Vector<Type>((n_years + 1) * n_ages, Type(), "numbers_at_age");
  ...
  derived_quantities["unfished_spawning_biomass"] =
      fims::Vector<Type>(n_years + 1, Type(), "unfished_spawning_biomass");
  ...
  derived_quantities["expected_recruitment"] =
      fims::Vector<Type>(n_years + 1, Type(), "expected_recruitment");
  derived_quantities["sum_selectivity"] =
      fims::Vector<Type>(n_years * n_ages, Type(), "sum_selectivity");
}
...
void assign_fleet_derived_vectors(
  ...
  derived_quantities["catch_numbers_at_length"] = fims::Vector<Type>(
      n_years * n_lengths, Type(), "catch_numbers_at_length");
  ...
  derived_quantities["index_numbers_at_length"] = fims::Vector<Type>(
      n_years * n_lengths, Type(), "index_numbers_at_length");
...
void initialize_model_derived_quantity_maps(
  ...
  for (auto model_it = info->models_map.begin();
       model_it != info->models_map.end(); ++model_it) {
    ...
    for (auto population_id_it = model->population_ids.begin();
         population_id_it != model->population_ids.end(); ++population_id_it) {
      model->InitializePopulationDerivedQuantities(*population_id_it);
    }

    for (auto population_it = model->populations.begin();
         population_it != model->populations.end(); ++population_it) {
      ...
      for (auto fleet_id_it = (*population_it)->fleet_ids.begin();
           fleet_id_it != (*population_it)->fleet_ids.end(); ++fleet_id_it) {
    ...
    for (auto fleet_it = model->fleets.begin(); fleet_it != model->fleets.end();
      ...
      assign_fleet_derived_vectors<Type>(
          ...
          fleet_it->second->n_ages, fleet_it->second->n_lengths);
    }
  }
}
 ...
 * derived-quantity vectors used by the objective. The interface creates and
 * links its data/distributions before CreateTMBModel(); this helper provides
...
void prepare_backend_model_shell_internal() {
  ...
  if (info->populations.empty()) {
    Rf_error(
        "No populations are registered. Create at least one population before "
        "building likelihood components.");
  }
...
bool create_backend_model_internal() {
  ...
  if (info->populations.empty()) {
    Rf_error(
        "No populations are registered. Create at least one population before "
        "calling fims_call_create_model().");
  }
    ...
    for (auto population_it = info->populations.begin();
      ...
      if (population != nullptr) {
        for (auto fleet_id_it = population->fleet_ids.begin();
             fleet_id_it != population->fleet_ids.end(); ++fleet_id_it) {
          model->InitializeFleetDerivedQuantities(*fleet_id_it);
        }
      }
    }
...
int require_scalar_integer(SEXP sexp, const char* argument_name, int minimum) {
  ...
  const double value = Rf_asReal(sexp);
  if (!std::isfinite(value) || std::floor(value) != value ||
      value < static_cast<double>(minimum) ||
      value > static_cast<double>(std::numeric_limits<int>::max())) {
    Rf_error("`%s` must be a whole number greater than or equal to %d.",
             argument_name, minimum);
  }
...
std::shared_ptr<fims_data_object::DataObject<Type>> create_data_object_2d(
  ...
  if (static_cast<size_t>(XLENGTH(data_real)) != expected_size) {
    UNPROTECT(1);
    Rf_error("`%s` must have length n_years * n_bins = %zu.", argument_name,
             expected_size);
  }
...
std::shared_ptr<fims_popdy::Fleet<Type>> require_fleet(
  ...
  if (fleet_it == info->fleets.end() || fleet_it->second == nullptr) {
    Rf_error("No fleet with id %u was found for `%s`.",
             static_cast<unsigned int>(fleet_id), argument_name);
  }
...
fims::Vector<Type>* require_fleet_derived_quantity(
  ...
  if (derived_it == derived_quantities.end()) {
    Rf_error("Fleet %u is missing derived quantity `%s`.",
             static_cast<unsigned int>(fleet_id), derived_name.c_str());
  }
...
void add_continuous_data_component(
  ...
  if (expected_values->size() != n_years) {
    Rf_error(
        "Continuous-data expected-values size mismatch: expected %zu, got %zu.",
        n_years, expected_values->size());
  }

  if (sd.size() != 1 && sd.size() != n_years) {
    Rf_error("Distribution standard deviation must have length 1 or %zu.",
             n_years);
  }
...
void add_normal_observed_component(
  ...
  if (!std::isfinite(log_sd)) {
    Rf_error("Normal random-effects `log_sd` must be finite.");
  }
  ...
  distribution->log_sd[0] = static_cast<Type>(log_sd);
...
void add_multinomial_component(
  ...
  if (expected_values->size() != expected_size) {
    Rf_error(
        "Multinomial expected-values size mismatch: expected %zu, got %zu.",
        expected_size, expected_values->size());
  }
...
void build_default_likelihood_internal(
  ...
  bool enable_length_comp = !Rf_isNull(landings_length_comp_sexp) &&
                            !Rf_isNull(survey_length_comp_sexp) &&
                            n_lengths > 0;

  if (fishing_fleet->n_years != n_years || survey_fleet->n_years != n_years) {
    Rf_error(
        "`n_years` (%zu) does not match model fleet dimensions (%zu, %zu).",
        n_years, fishing_fleet->n_years, survey_fleet->n_years);
  }
  if (fishing_fleet->n_ages != n_ages || survey_fleet->n_ages != n_ages) {
    Rf_error("`n_ages` (%zu) does not match model fleet dimensions (%zu, %zu).",
             n_ages, fishing_fleet->n_ages, survey_fleet->n_ages);
  }
  if (enable_length_comp) {
    if (fishing_fleet->n_lengths == 0 && survey_fleet->n_lengths == 0) {
      Rf_warning(
          "Length composition data were provided but model fleets have "
          "n_lengths = 0. Skipping length-composition likelihood components.");
      enable_length_comp = false;
    } else if (fishing_fleet->n_lengths != n_lengths ||
               survey_fleet->n_lengths != n_lengths) {
      Rf_error(
          "`n_lengths` (%zu) does not match model fleet dimensions (%zu, %zu).",
          n_lengths, fishing_fleet->n_lengths, survey_fleet->n_lengths);
    }
  }
  ...
  // Recruitment deviations are the first likelihood component in the
  // established FIMS report contract. `recruitment_log_sd` is already on the
  // log scale and must not be logged a second time.
  if (!info->populations.empty()) {
    std::shared_ptr<fims_popdy::Population<Type>> population =
        info->populations.begin()->second;
    if (population != nullptr && population->recruitment != nullptr) {
      add_normal_observed_component<Type>(
          info, &(population->recruitment->log_recruit_devs),
          recruitment_log_sd, recruitment_log_sd_estimation_type);
    }
  }

  if (!Rf_isNull(landings_sexp) && XLENGTH(landings_sexp) > 0) {
    std::shared_ptr<fims_data_object::DataObject<Type>> fishing_landings_data =
        create_data_object_1d<Type>(info, landings_sexp, n_years, "landings");
    ...
    add_continuous_data_component<Type>(
        info, fishing_landings_data,
        require_fleet_derived_quantity<Type>(info, fishing_fleet_id,
                                             landings_distribution == "dnorm"
                                                 ? "catch_expected"
                                                 : "log_catch_expected"),
        n_years, landings_distribution, landings_sd);
  } else {
    fishing_fleet->fleet_observed_catch_data_id_m = -999;
    fishing_fleet->observed_catch_data.reset();
  }

  if (!Rf_isNull(landings_age_comp_sexp) &&
      XLENGTH(landings_age_comp_sexp) > 0) {
    std::shared_ptr<fims_data_object::DataObject<Type>> fishing_age_comp_data =
        create_data_object_2d<Type>(info, landings_age_comp_sexp, n_years,
                                    n_ages, "landings_age_comp");
  ...
  if (!Rf_isNull(survey_index_sexp) && XLENGTH(survey_index_sexp) > 0) {
    std::shared_ptr<fims_data_object::DataObject<Type>> survey_index_data =
        create_data_object_1d<Type>(info, survey_index_sexp, n_years,
                                    "survey_index");
    ...
    add_continuous_data_component<Type>(
        info, survey_index_data,
        require_fleet_derived_quantity<Type>(info, survey_fleet_id,
                                             survey_distribution == "dnorm"
                                                 ? "index_expected"
                                                 : "log_index_expected"),
        n_years, survey_distribution, survey_sd);
  } else {
  ...
  if (!Rf_isNull(survey_age_comp_sexp) && XLENGTH(survey_age_comp_sexp) > 0) {
    std::shared_ptr<fims_data_object::DataObject<Type>> survey_age_comp_data =
        create_data_object_2d<Type>(info, survey_age_comp_sexp, n_years, n_ages,
                                    "survey_age_comp");
  ...
  if (enable_length_comp && !Rf_isNull(survey_length_comp_sexp) &&
      XLENGTH(survey_length_comp_sexp) > 0) {
    std::shared_ptr<fims_data_object::DataObject<Type>>
        survey_length_comp_data =
            create_data_object_2d<Type>(info, survey_length_comp_sexp, n_years,
                                        n_lengths, "survey_length_comp");
...
extern "C" SEXP fims_call_build_default_likelihood(
  ...
  const uint32_t fishing_fleet_id = static_cast<uint32_t>(
      require_scalar_integer(fishing_fleet_id_sexp, "fishing_fleet_id", 0));
  const uint32_t survey_fleet_id = static_cast<uint32_t>(
      require_scalar_integer(survey_fleet_id_sexp, "survey_fleet_id", 0));
  const std::string landings_distribution = require_scalar_string(
      landings_distribution_sexp, "landings_distribution");
  const std::string survey_distribution =
  ...
  require_finite_values(survey_sd_sexp, "survey_sd");
  if (!Rf_isNumeric(recruitment_log_sd_sexp) ||
      XLENGTH(recruitment_log_sd_sexp) != 1) {
    Rf_error("`recruitment_log_sd` must be one numeric value.");
  }
  const double recruitment_log_sd = Rf_asReal(recruitment_log_sd_sexp);
  if (!std::isfinite(recruitment_log_sd))
    Rf_error("`recruitment_log_sd` must be finite.");
  const int recruitment_log_sd_estimation_type =
      require_scalar_integer(recruitment_log_sd_estimation_type_sexp,
                             "recruitment_log_sd_estimation_type", 0);
  const size_t n_years =
      static_cast<size_t>(require_scalar_integer(n_years_sexp, "n_years", 1));
  const size_t n_ages =
      static_cast<size_t>(require_scalar_integer(n_ages_sexp, "n_ages", 1));
  const size_t n_lengths = static_cast<size_t>(
      require_scalar_integer(n_lengths_sexp, "n_lengths", 0));

  build_default_likelihood_internal<double>(
      fishing_fleet_id, survey_fleet_id, landings_sexp, landings_distribution,
      landings_sd, landings_age_comp_sexp, landings_length_comp_sexp,
      survey_index_sexp, survey_distribution, survey_sd, survey_age_comp_sexp,
      survey_length_comp_sexp, recruitment_log_sd,
      recruitment_log_sd_estimation_type, n_years, n_ages, n_lengths);
...
extern "C" SEXP fims_call_add_prior(SEXP module_sexp, SEXP object_id_sexp,
  ...
  require_finite_values(mean_real, "mean");
  require_finite_values(log_sd_real, "log_sd");
  const std::string module = require_scalar_string(module_sexp, "module");
  const std::string parameter =
      require_scalar_string(parameter_sexp, "parameter");
  const std::string distribution =
      require_scalar_string(distribution_sexp, "distribution");
  const uint32_t object_id = static_cast<uint32_t>(
      require_scalar_integer(object_id_sexp, "object_id", 0));
  const int mean_estimation_type = require_scalar_integer(
      mean_estimation_type_sexp, "mean_estimation_type", 0);
...
extern "C" SEXP fims_call_information_get_random_effect_names() {
  ...
  const size_t n = info->random_effects_names.size();
  SEXP out = PROTECT(Rf_allocVector(STRSXP, static_cast<R_xlen_t>(n)));
  for (size_t i = 0; i < n; ++i) {
    SET_STRING_ELT(out, i, Rf_mkChar(info->random_effects_names[i].c_str()));
  }
