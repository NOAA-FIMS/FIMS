...
if [ "$MACHINE" == "Linux" ] && command -v sudo &> /dev/null; then
    ...
    sudo apt-get install -y --no-install-recommends build-essential libnode-dev libcairo2-dev libcurl4-openssl-dev libpoppler-cpp-dev libssl-dev libxml2-dev git wget perl xz-utils r-base libharfbuzz-dev libfribidi-dev libfreetype6-dev libpng-dev libtiff5-dev libjpeg-dev libwebp-dev liblapack-dev libblas-dev
