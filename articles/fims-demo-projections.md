...
Projections in FIMS can occur during the model-fitting process. That is,
you can extend your time series as many years into the future that you
wish with fixed values of catch or fixed fishing mortality values to
...
As we work to set up various inputs and outputs for necessary management
quantities for each region, we encourage you to join the [FIMS
Discussion Board](https://github.com/orgs/NOAA-FIMS/discussions) to
voice your thoughts about quantities that should be included. Second,
please report bugs in the code to our [Issues
...
The code below uses the built-in data within FIMS and sets up a simple
catch-at-age configuration object that will work with the data. These
data and configuration objects will be used for to create an extended
data set in the next section. More details about the chunk below can be
...
data_4_projections <- data_big_with_extra_year |>
  ...
  dplyr::bind_rows(
    dplyr::filter(
      ...
      dplyr::select(-timing) |>
      merge(
        data.frame(timing = max(data_big[["timing"]], na.rm = TRUE):(max(data_big[["timing"]], na.rm = TRUE) + years_of_projection - 1))
      )
  ) |>
...
The default configuration from a model without projections can be used
as the default configuration. After the parameter configuration is
...
Regardless if there are projections or not, the model is fit using the
[`FIMS::fit_fims()`](https://NOAA-FIMS.github.io/FIMS/reference/fit_fims.md),
which facilitates that the model output will be in the same format that
users are used to.
...
```
    ...
    ## ℹ Maximum gradient went from 0.00128 to 0.00037 after 3 steps.
    ## ✔ Finished optimization
    ## ✔ Finished sdreport
    ## ℹ FIMS model version: 0.10.0.9000
    ## ℹ Total run time was 14.10826 seconds
...
pass a single model to
[`stockplotr::plot_biomass()`](https://noaa-fisheries-integrated-toolbox.r-universe.dev/stockplotr/reference/plot_biomass.html)
but the list can contain multiple models, where the start and end year
of each model that is included need not align. Thus, if we had fit the
model without projections we could have plotted that here as well for
comparison.
