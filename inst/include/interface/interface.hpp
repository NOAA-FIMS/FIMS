...
inline had::AReal dmultinom(const vector<had::AReal>& x,
                            const vector<had::AReal>& p, int give_log = 0) {
  // Counts are observed data. Keep their gamma terms constant on the tape;
  // the generic lgamma AD helper has no compact-tape replay metadata.
  double total = 0.0;
  double log_coefficient = 0.0;
  had::AReal log_probability = 0.0;
  for (int i = 0; i < x.size(); ++i) {
    const double count = x[i].val;
    total += count;
    log_coefficient -= std::lgamma(count + 1.0);
    if (count != 0.0) log_probability += count * log(p[i]);
  }
  log_probability += log_coefficient + std::lgamma(total + 1.0);
...
#define FIMS_REPORT_F(name, F)                                           \
  if constexpr (isDouble<Type>::value)                                   \
    if (F->current_parallel_region < static_cast<Type>(0)) {             \
