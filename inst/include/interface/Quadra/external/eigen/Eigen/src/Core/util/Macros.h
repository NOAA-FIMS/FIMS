...
/// \internal EIGEN_COMP_CLANG set to major+minor version (e.g., 307 for
/// clang 3.7) if the compiler is clang
...
/// \internal EIGEN_COMP_ICC set to __INTEL_COMPILER if the compiler is Intel
...
/// \internal EIGEN_COMP_MSVC set to _MSC_VER if the compiler is Microsoft
/// Visual C++, 0 otherwise.
#if defined(_MSC_VER)
...
/// \internal EIGEN_COMP_MSVC_LANG set to _MSVC_LANG if the compiler is
...
// For the record, here is a table summarizing the possible values for
// EIGEN_COMP_MSVC_LANG: MSVC option                          Standard MSVC_LANG
// /std:c++14 (default as of VS 2019)   C++14     201402L
// /std:c++17                           C++17     201703L
// /std:c++latest                       >C++17    >201703L

/// \internal EIGEN_COMP_MSVC_STRICT set to 1 if the compiler is really
/// Microsoft Visual C++ and not ,e.g., ICC or clang-cl
...
/// \internal EIGEN_COMP_PGI set to PGI version if the compiler is Portland
/// Group Compiler
#if defined(__PGI)
...
/// \internal EIGEN_COMP_EMSCRIPTEN set to 1 if the compiler is Emscripten
/// Compiler
...
/// \internal EIGEN_GNUC_STRICT set to 1 if the compiler is really GCC and not a
/// compatible compiler (e.g., ICC, clang, mingw, etc.)
#if EIGEN_COMP_GNUC &&                                                         \
    !(EIGEN_COMP_CLANG || EIGEN_COMP_ICC || EIGEN_COMP_MINGW ||                \
      EIGEN_COMP_PGI || EIGEN_COMP_IBM || EIGEN_COMP_ARM ||                    \
...
/// \internal EIGEN_ARCH_ARM_OR_ARM64 set to 1 if the architecture is ARM or
/// ARM64
...
/// \internal EIGEN_HAS_ARM64_FP16_VECTOR_ARITHMETIC set to 1 if the
/// architecture supports Neon vector intrinsics for fp16.
#if EIGEN_ARCH_ARM64
...
/// \internal EIGEN_HAS_ARM64_FP16_SCALAR_ARITHMETIC set to 1 if the
/// architecture supports Neon scalar intrinsics for fp16.
#if EIGEN_ARCH_ARM64
...
// note: ANDROID is defined when using ndk_build, __ANDROID__ is defined when
...
/// \internal EIGEN_OS_GNULINUX set to 1 if the OS is GNU Linux and not
/// Linux-based OS (e.g., not android)
...
/// \internal EIGEN_OS_WIN_STRICT set to 1 if the OS is really Windows and not
/// some variants
...
// ++ host_defines.h which contains the defines for the __host__ and __device__
// macros
...
// For HIP (ROCm 3.5 and higher), we need to explicitly set the launch_bounds
// attribute value to 1024. The compiler assigns a default value of 256 when the
// attribute is not specified. This results in failures on the HIP platform, for
// cases when a GPU kernel without an explicit launch_bounds attribute is called
// with a threads_per_block value greater than 256.
//
// This is a regression in functioanlity and is expected to be fixed within the
// next couple of ROCm releases (compiler will go back to using 1024 value as
// the default)
//
// In the meantime, we will use a "only enabled for HIP" macro to set the
// launch_bounds attribute.
...
#define EIGEN_GPUCC
//
// EIGEN_HIPCC implies the HIP compiler and is used to tweak Eigen code for use
// in HIP kernels EIGEN_CUDACC implies the CUDA compiler and is used to tweak
// Eigen code for use in CUDA kernels
//
// In most cases the same tweaks are required to the Eigen code to enable in
// both the HIP and CUDA kernels. For those cases, the corresponding code should
// be guarded with
...
//
// For cases where the tweak is specific to CUDA, the code should be guarded
// with
//      #if defined(EIGEN_CUDACC)
//
#endif
...
//
// If either EIGEN_CUDA_ARCH or EIGEN_HIP_DEVICE_COMPILE is defined, then define
// EIGEN_GPU_COMPILE_PHASE
//
...
//
// Code that needs to enabled only during the either the "host" or "device"
// compilation phase needs to be guarded with a macro that indicates the current
// compilation phase
//
...
//
// In most cases, the "host" / "device" specific code is the same for both HIP
// and CUDA For those cases, the code should be guarded with
...
// EIGEN_USE_SYCL is a user-defined macro while __SYCL_DEVICE_ONLY__ is a
// compiler-defined macro. In most cases we want to check if both macros are
// defined which can be done using the define below.
...
// Do we support r-value references?
    ...
    (__has_feature(cxx_rvalue_references) || (EIGEN_COMP_CXXVER >= 11) ||      \
...
#define EIGEN_HAS_STD_INVOKE_RESULT 0
    ...
    (__has_feature(cxx_alignas) || EIGEN_HAS_CXX14 ||                          \
     (EIGEN_COMP_MSVC >= 1800) || (EIGEN_GNUC_AT_LEAST(4, 8)) ||               \
     (EIGEN_COMP_CLANG >= 305) || (EIGEN_COMP_ICC >= 1500) ||                  \
     (EIGEN_COMP_PGI >= 1500) || (EIGEN_COMP_SUNCC >= 0x5130))
...
// Does the compiler support variadic templates?
    ...
    (!defined(__NVCC__) || !EIGEN_ARCH_ARM_OR_ARM64 ||                         \
     (EIGEN_COMP_NVCC >= 80000))
// ^^ Disable the use of variadic templates when compiling with versions of nvcc
// older than 8.0 on ARM devices:
//    this prevents nvcc from crashing when compiling Eigen on Tegra X1
#define EIGEN_HAS_VARIADIC_TEMPLATES 1
#elif EIGEN_MAX_CPP_VER >= 11 && (EIGEN_COMP_CXXVER >= 11) &&                  \
...
#if defined(EIGEN_CUDACC)
// Const expressions are supported provided that c++11 is enabled and we're
// using either clang or nvcc 7.5 or above
#if EIGEN_MAX_CPP_VER >= 14 &&                                                 \
    (EIGEN_COMP_CXXVER >= 11 &&                                                \
     (EIGEN_COMP_CLANG || EIGEN_COMP_NVCC >= 70500))
#define EIGEN_HAS_CONSTEXPR 1
#endif
#elif EIGEN_MAX_CPP_VER >= 14 &&                                               \
    (__has_feature(cxx_relaxed_constexpr) || (EIGEN_COMP_CXXVER >= 14) ||      \
...
// Let's be conservative and enable the default C++11 implementation only if we
// are sure it exists
#ifndef EIGEN_HAS_CXX11_MATH
#if EIGEN_MAX_CPP_VER >= 11 &&                                                 \
    ((EIGEN_COMP_CXXVER > 11) ||                                               \
     (EIGEN_COMP_CXXVER == 11) &&                                              \
         (EIGEN_COMP_GNUC_STRICT || EIGEN_COMP_CLANG || EIGEN_COMP_MSVC ||     \
          EIGEN_COMP_ICC) &&                                                   \
         (EIGEN_ARCH_i386_OR_x86_64) &&                                        \
         (EIGEN_OS_GNULINUX || EIGEN_OS_WIN_STRICT || EIGEN_OS_MAC))
...
#if EIGEN_MAX_CPP_VER >= 11 &&                                                 \
    (__has_feature(cxx_noexcept) || (EIGEN_COMP_CXXVER > 11) ||                \
     ((EIGEN_COMP_CXXVER == 11) &&                                             \
      (EIGEN_COMP_GNUC_STRICT || EIGEN_COMP_CLANG || EIGEN_COMP_MSVC ||        \
       EIGEN_COMP_ICC >= 1400)))
...
#if EIGEN_MAX_CPP_VER >= 11 &&                                                 \
    (__has_feature(cxx_atomic) || (EIGEN_COMP_CXXVER > 11) ||                  \
     ((EIGEN_COMP_CXXVER == 11) &&                                             \
      (EIGEN_COMP_MSVC == 0 || EIGEN_COMP_MSVC >= 1700)))
...
// NOTE: the MSVC version is based on
// https://en.cppreference.com/w/cpp/compiler_support
//       and not tested.
#ifndef EIGEN_HAS_CXX17_OVERALIGN
#if EIGEN_MAX_CPP_VER >= 17 && EIGEN_COMP_CXXVER >= 17 &&                      \
    ((EIGEN_COMP_MSVC >= 1912) || (EIGEN_GNUC_AT_LEAST(7, 0)) ||               \
...
// While available already with c++11, this is useful mostly starting with c++14
// and relaxed constexpr rules
#if defined(__NVCC__)
// nvcc considers constexpr functions as __host__ __device__ with the option
...
// clang++ always considers constexpr functions as implicitly __host__
// __device__
#define EIGEN_CONSTEXPR_ARE_DEVICE_FUNC
#endif
#endif
...
// EIGEN_STRONG_INLINE is a stronger version of the inline, using __forceinline
// on MSVC, but it still doesn't use GCC's always_inline. This is useful in
...
// EIGEN_ALWAYS_INLINE is the stronget, it has the effect of making the function
...
//   Eval.h:91: sorry, unimplemented: inlining failed in call to 'const
//   Eigen::Eval<Derived> Eigen::MatrixBase<Scalar, Derived>::eval() const'
...
// this macro allows to get rid of linking errors about multiply defined
// functions.
//  - static is not very good because it prevents definitions from different
//  object files to be merged.
//           So static causes the resulting linked executable to be bloated with
//           multiple copies of the same function.
//  - inline is not perfect either as it unwantedly hints the compiler toward
...
// eigen_plain_assert is where we implement the workaround for the assert() bug
// in GCC <= 4.3, see bug 89
...
namespace Eigen {
namespace internal {
// trivial function copying a bool. Must be EIGEN_DONT_INLINE, so we implement
// it after including Eigen headers. see bug 89.
namespace {
...
#define eigen_plain_assert(x)                                                  \
  do {                                                                         \
    if (!Eigen::internal::copy_bool(x))                                        \
      Eigen::internal::assert_fail(EIGEN_MAKESTRING(x), __PRETTY_FUNCTION__,   \
                                   __FILE__, __LINE__);                        \
  } while (false)
#endif
#endif
...
#define EIGEN_USING_STD(FUNC) using std::FUNC;
    ...
    (EIGEN_COMP_MSVC < 1900 || (EIGEN_COMP_MSVC == 1900 && EIGEN_COMP_NVCC))
// For older MSVC versions, as well as 1900 && CUDA 8, using the base operator
// is necessary,
//   otherwise we get duplicate definition errors
// For later MSVC versions, we require explicit operator= definition, otherwise
// we get
...
#elif EIGEN_COMP_CLANG // workaround clang bug (see
                       // http://forum.kde.org/viewtopic.php?f=74&t=102653)
 ...
 * accidentally creating empty objects.
 *
 * Hiding the default destructor lead to problems in C++03 mode together with
...
// EIGEN_SIZE_MIN_PREFER_DYNAMIC gives the min between compile-time sizes. 0 has
// absolute priority, followed by 1, followed by Dynamic, followed by other
// finite values. The reason for giving Dynamic the priority over finite values
// is that min(3, Dynamic) should be Dynamic, since that could be anything
// between 0 and 3.
...
// EIGEN_SIZE_MIN_PREFER_FIXED is a variant of EIGEN_SIZE_MIN_PREFER_DYNAMIC
// comparing MaxSizes. The difference is that finite values now have priority
// over Dynamic, so that min(3, Dynamic) gives 3. Indeed, whatever the actual
// value is (between 0 and 3), it is not more than 3.
...
// see EIGEN_SIZE_MIN_PREFER_DYNAMIC. No need for a separate variant for
// MaxSizes here.
...
#define EIGEN_CWISE_BINARY_RETURN_TYPE(LHS, RHS, OPNAME)                       \
  CwiseBinaryOp<EIGEN_CAT(EIGEN_CAT(internal::scalar_, OPNAME), _op) <         \
...
#define EIGEN_MAKE_CWISE_BINARY_OP(METHOD, OPNAME)                             \
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const EIGEN_CWISE_BINARY_RETURN_TYPE(  \
      Derived, OtherDerived, OPNAME)(METHOD)(                                  \
    ...
    return EIGEN_CWISE_BINARY_RETURN_TYPE(Derived, OtherDerived,               \
                                          OPNAME)(derived(), other.derived()); \
  }

#define EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, TYPEA, TYPEB)                    \
  (Eigen::internal::has_ReturnType<Eigen::ScalarBinaryOpTraits<                \
       TYPEA, TYPEB,                                                           \
       EIGEN_CAT(EIGEN_CAT(Eigen::internal::scalar_, OPNAME), _op) < TYPEA,    \
       TYPEB>> > ::value)
...
// Workaround for MSVC 2010 (see ML thread "patch with compile for for MSVC
// 2010")
  ...
  EIGEN_MSVC10_WORKAROUND_BINARYOP_RETURN_TYPE(                                \
      const EIGEN_EXPR_BINARYOP_SCALAR_RETURN_TYPE(                            \
          ...
          typename internal::promote_scalar_arg<                               \
              Scalar EIGEN_COMMA T EIGEN_COMMA EIGEN_SCALAR_BINARY_SUPPORTED(  \
                  OPNAME, Scalar, T)>::type,                                   \
          OPNAME))(METHOD)(const T &scalar) const {                            \
    typedef typename internal::promote_scalar_arg<                             \
        Scalar, T, EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, Scalar, T)>::type     \
        PromotedT;                                                             \
    return EIGEN_EXPR_BINARYOP_SCALAR_RETURN_TYPE(Derived, PromotedT, OPNAME)( \
        ...
        typename internal::plain_constant_type<Derived, PromotedT>::type(      \
            ...
            internal::scalar_constant_op<PromotedT>(scalar)));                 \
  }

#define EIGEN_MAKE_SCALAR_BINARY_OP_ONTHELEFT(METHOD, OPNAME)                  \
  template <typename T>                                                        \
  EIGEN_DEVICE_FUNC                                                            \
      EIGEN_STRONG_INLINE friend EIGEN_MSVC10_WORKAROUND_BINARYOP_RETURN_TYPE( \
          const EIGEN_SCALAR_BINARYOP_EXPR_RETURN_TYPE(                        \
              typename internal::promote_scalar_arg<                           \
                  Scalar EIGEN_COMMA T EIGEN_COMMA                             \
                      EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, T, Scalar)>::type, \
              Derived, OPNAME))(METHOD)(const T &scalar,                       \
                                        const StorageBaseType &matrix) {       \
    typedef typename internal::promote_scalar_arg<                             \
        Scalar, T, EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, T, Scalar)>::type     \
        PromotedT;                                                             \
...
#define EIGEN_MAKE_SCALAR_BINARY_OP(METHOD, OPNAME)                            \
  ...
  EIGEN_MAKE_SCALAR_BINARY_OP_ONTHERIGHT(METHOD, OPNAME)
    ...
    !defined(EIGEN_CUDA_ARCH) && !defined(EIGEN_EXCEPTIONS) &&                 \
...
// The all function is used to enable a variadic version of eigen_assert which
// can take a parameter pack as its input.
namespace Eigen {
namespace internal {

inline bool all() { return true; }

template <typename T, typename... Ts> bool all(T t, Ts... ts) {
  return t && all(ts...);
}
