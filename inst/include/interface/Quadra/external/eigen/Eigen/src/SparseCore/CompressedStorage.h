...
public:
  ...
  CompressedStorage &operator=(const CompressedStorage &other) {
    ...
    if (other.size() > 0) {
      ...
      internal::smart_copy(other.m_indices, other.m_indices + m_size,
                           m_indices);
    }
    return *this;
  }
  ...
  void resize(Index size, double reserveSizeFactor = 0) {
    if (m_allocatedSize < size) {
      Index realloc_size =
          (std::min<Index>)(NumTraits<StorageIndex>::highest(),
  ...
  inline Scalar at(Index key, const Scalar &defaultValue = Scalar(0)) const {
    ...
    return ((id < m_size) && (m_indices[id] == key)) ? m_values[id]
                                                     : defaultValue;
  }
  ...
  inline Scalar &atWithInsertion(Index key,
    ...
    if (id >= m_size || m_indices[id] != key) {
      if (m_allocatedSize < m_size + 1) {
        ...
        // copy the rest
        if (m_size > id) {
          internal::smart_copy(m_values + id, m_values + m_size,
                               newValues.ptr() + id + 1);
          internal::smart_copy(m_indices + id, m_indices + m_size,
                               newIndices.ptr() + id + 1);
        }
        std::swap(m_values, newValues.ptr());
        std::swap(m_indices, newIndices.ptr());
      } else if (m_size > id) {
        internal::smart_memmove(m_values + id, m_values + m_size,
                                m_values + id + 1);
        internal::smart_memmove(m_indices + id, m_indices + m_size,
                                m_indices + id + 1);
      }
      m_size++;
  ...
  void moveChunk(Index from, Index to, Index chunkSize) {
    ...
    if (to > from && from + chunkSize > to) {
      // move backward
      internal::smart_memmove(m_values + from, m_values + from + chunkSize,
                              m_values + to);
      internal::smart_memmove(m_indices + from, m_indices + from + chunkSize,
                              m_indices + to);
    } else {
      internal::smart_copy(m_values + from, m_values + from + chunkSize,
                           m_values + to);
      internal::smart_copy(m_indices + from, m_indices + from + chunkSize,
                           m_indices + to);
    }
  }
