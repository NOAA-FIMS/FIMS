 ...
 * TriangularView with additional methods which are available for sparse
 * expressions only.
 *
...
public:
  ...
  typedef typename internal::remove_reference<MatrixTypeNested>::type
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void _solve_impl(const RhsType &rhs,
                                                         DstType &dst) const {
    if (!(internal::is_same<RhsType, DstType>::value &&
          internal::extract_data(dst) == internal::extract_data(rhs)))
      dst = rhs;
...
public:
  ...
  public:
    EIGEN_STRONG_INLINE InnerIterator(const unary_evaluator &xprEval,
                                      Index outer)
        : Base(xprEval.m_argImpl, outer), m_returnOne(false),
          m_containsDiag(Base::outer() < xprEval.m_arg.innerSize()) {
      if (SkipFirst) {
        while ((*this) && ((HasUnitDiag || SkipDiag) ? this->index() <= outer
                                                     : this->index() < outer))
          Base::operator++();
        if (HasUnitDiag)
          m_returnOne = m_containsDiag;
      } else if (HasUnitDiag &&
                 ((!Base::operator bool()) || Base::index() >= Base::outer())) {
    ...
    //       inline Index row() const { return (ArgType::Flags&RowMajorBit ?
    //       Base::outer() : this->index()); } inline Index col() const { return
    //       (ArgType::Flags&RowMajorBit ? this->index() : Base::outer()); }
