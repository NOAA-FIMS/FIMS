...
// parameters on which T itself depends. For example, traits<Map<const T> > !=
// traits<Map<T> >, but
...
template <typename Derived> struct accessors_level {
  enum {
    ...
    value = has_direct_access
                ? (has_write_access ? DirectWriteAccessors : DirectAccessors)
...
template <typename _Scalar, int _Rows, int _Cols,
          int _Options =
              ...
              // the innermost ?: ternary operator is misparsed. We write it
              // slightly differently and this makes gcc 3.4.6 happy, but it's
              // ugly. The error would only show up with
              // EIGEN_DEFAULT_TO_ROW_MAJOR is defined (when
              // EIGEN_DEFAULT_MATRIX_STORAGE_ORDER_OPTION is RowMajor)
...
class IndexedView;
template <typename XprType, int Rows = Dynamic, int Cols = Dynamic,
          int Order = 0>
class Reshaped;
...
template <typename MatrixType, int MapOptions = Unaligned,
          typename StrideType = Stride<0, 0>>
class Map;
template <typename Derived> class RefBase;
template <typename PlainObjectType, int Options = 0,
          typename StrideType = typename internal::conditional<
              PlainObjectType::IsVectorAtCompileTime, InnerStride<1>,
              OuterStride<>>::type>
class Ref;
...
namespace internal {
template <typename _Scalar, int Rows = Dynamic, int Cols = Dynamic,
          int Supers = Dynamic, int Subs = Dynamic, int Options = 0>
class BandMatrix;
}
...
template <typename T,
          int ProductTag =
              internal::product_type<typename T::Lhs, typename T::Rhs>::ret,
...
template <typename LhsScalar, typename RhsScalar, bool ConjLhs = false,
          bool ConjRhs = false>
struct conj_helper;
...
template <typename Scalar, bool is_complex, bool is_integer>
