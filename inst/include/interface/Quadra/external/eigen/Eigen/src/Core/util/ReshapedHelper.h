...
Index get_runtime_reshape_size(SizeType size, Index /*other*/,
                               Index /*total*/) {
...
struct get_compiletime_reshape_size<AutoSize_t, OtherSize, TotalSize> {
  enum {
    ...
    value = (TotalSize == Dynamic || other_size == Dynamic)
                ? Dynamic
                : TotalSize / other_size
  };
};

inline Index get_runtime_reshape_size(AutoSize_t /*size*/, Index other,
                                      Index total) {
  return total / other;
}

template <int Flags, int Order> struct get_compiletime_reshape_order {
