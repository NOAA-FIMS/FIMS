...
// helper function for dot(). The problem is that if we put that in the body of
// dot(), then upon calling dot with mismatched types, the compiler emits errors
// about failing to instantiate cwiseProduct BEFORE looking at the static
// assertions. Thus this is a trick to get better compile errors.
template <typename T, typename U,
          // the NeedToTranspose condition here is taken straight from Assign.h
          bool NeedToTranspose =
              T::IsVectorAtCompileTime && U::IsVectorAtCompileTime &&
              ((int(T::RowsAtCompileTime) == 1 &&
                int(U::ColsAtCompileTime) ==
                    1) | // FIXME | instead of || to please GCC 4.4.0 stupid
                         // warning "suggest parentheses around &&". revert to
 ...
 * \only_for_vectors
 *
 * \note If the scalar type is complex numbers, then this function returns the
 * hermitian (sesquilinear) dot product, conjugate-linear in the first variable
 * and linear in the second variable.
 *
 * \sa squaredNorm(), norm()
 */
template <typename Derived>
template <typename OtherDerived>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename ScalarBinaryOpTraits<
...
MatrixBase<Derived>::dot(const MatrixBase<OtherDerived> &other) const {
  ...
  typedef internal::scalar_conj_product_op<Scalar,
...
EIGEN_DEVICE_FUNC
  ...
  // NOTE: after extensive benchmarking, this conditional does not impact
  // performance, at least on recent x86 CPU
  if (z > RealScalar(0))
 ...
 * \only_for_vectors
 *
 * \warning If the input vector is too small (i.e., this->norm()==0), then \c
 ...
 * \only_for_vectors
 *
 * This method is analogue to the normalized() method, but it reduces the risk
 * of underflow and overflow when computing the norm.
 *
 * \warning If the input vector is too small (i.e., this->norm()==0),
 *          then this function returns a copy of the input.
 *
...
template <typename Derived> struct lpNorm_selector<Derived, Infinity> {
  ...
  EIGEN_DEVICE_FUNC
  static inline RealScalar run(const MatrixBase<Derived> &m) {
    if (Derived::SizeAtCompileTime == 0 ||
        (Derived::SizeAtCompileTime == Dynamic && m.size() == 0))
      return RealScalar(0);
 ...
 * the maximum of the absolute values of the coefficients of \c *this.
 *
 * In all cases, if \c *this is empty, then the value 0 is returned.
 *
 * \note For matrices, this function does not compute the <a
 * href="https://en.wikipedia.org/wiki/Operator_norm">operator-norm</a>. That
 * is, if \c *this is a matrix, then its coefficients are interpreted as a 1D
 * vector. Nonetheless, you can easily compute the 1-norm and \f$\infty\f$-norm
 * matrix operator norms using \link
 * TutorialReductionsVisitorsBroadcastingReductionsNorm partial reductions
 * \endlink.
 *
 * \sa norm()
 */
...
bool MatrixBase<Derived>::isOrthogonal(const MatrixBase<OtherDerived> &other,
  ...
  typename internal::nested_eval<OtherDerived, 2>::type otherNested(
      other.derived());
  return numext::abs2(nested.dot(otherNested)) <=
 ...
 * the name.
 *
 * \note This can be used to check whether a family of vectors forms an
 * orthonormal basis. Indeed, \c m.isUnitary() returns true if and only if the
 * columns (equivalently, the rows) of m form an orthonormal basis.
 *
...
bool MatrixBase<Derived>::isUnitary(const RealScalar &prec) const {
  ...
  for (Index i = 0; i < cols(); ++i) {
    if (!internal::isApprox(self.col(i).squaredNorm(),
                            static_cast<RealScalar>(1), prec))
      return false;
    for (Index j = 0; j < i; ++j)
      if (!internal::isMuchSmallerThan(self.col(i).dot(self.col(j)),
