 ...
 *
 * \tparam Derived is the derived type, e.g. a sparse matrix type, or an
 * expression, etc.
 *
...
public:
  ...
  enum {
    ...
    _HasDirectAccess =
        (int(Flags) & DirectAccessBit) ? 1 : 0 // workaround sunCC
#endif
  };
  ...
  typedef SparseMatrix<Scalar, Flags & RowMajorBit ? RowMajor : ColMajor,
                       StorageIndex>
      PlainObject;
...
public:
  friend std::ostream &operator<<(std::ostream &s, const SparseMatrixBase &m) {
    ...
    if (Flags & RowMajorBit) {
      ...
      for (Index row = 0; row < nm.outerSize(); ++row) {
        Index col = 0;
        for (typename internal::evaluator<NestedCleaned>::InnerIterator it(
                 thisEval, row);
             it; ++it) {
      ...
      if (m.cols() == 1) {
        ...
        s << static_cast<const SparseMatrixBase<
            SparseMatrix<Scalar, RowMajorBit, StorageIndex>> &>(trans);
      }
    }
    return s;
  }
  ...
  template <typename OtherDerived>
  EIGEN_STRONG_INLINE const typename CwiseProductDenseReturnType<
      OtherDerived>::Type
  ...
  // diagonal * sparse
  template <typename OtherDerived>
  friend const Product<OtherDerived, Derived>
  operator*(const DiagonalBase<OtherDerived> &lhs,
  ...
  SparseSymmetricPermutationProduct<Derived, Upper | Lower> twistedBy(
    ...
    return SparseSymmetricPermutationProduct<Derived, Upper | Lower>(derived(),
                                                                     perm);
  }
  ...
  inline typename ConstSelfAdjointViewReturnType<UpLo>::Type
  ...
  template <typename OtherDerived>
  bool
  isApprox(const SparseMatrixBase<OtherDerived> &other,
  ...
  inline const SparseView<Derived> pruned(
