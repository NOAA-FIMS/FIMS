...
template <typename TransformType, typename MatrixType,
          int Case = transform_traits<TransformType>::IsProjective ? 0
                     : int(MatrixType::RowsAtCompileTime) ==
                             int(transform_traits<TransformType>::HDim)
                         ? 1
                         : 2,
...
template <typename Lhs, typename Rhs,
          bool AnyProjective = transform_traits<Lhs>::IsProjective ||
 ...
 *              - #AffineCompact: the transformation is stored as a
 *(Dim)x(Dim+1) matrix.
 *              - #Projective: the transformation is stored as a (Dim+1)^2
 *matrix without any assumption.
 *              - #Isometry: same as #Affine with the additional assumption that
 *                           the linear part represents a rotation. This
 *assumption is exploited to speed up some functions such as inverse() and
 *rotation(). \tparam _Options has the same meaning as in class Matrix. It
 *allows to specify DontAlign and/or RowMajor. These Options are passed directly
 *to the underlying matrix type.
 *
 ...
 * with any other transformations
 *(Transform,Translation,RotationBase,DiagonalMatrix) and can be directly used
 *to transform implicit homogeneous vectors. All these operations are handled
 *via the operator*. For the composition of transformations, its principle
 *consists to first convert the right/left hand sides of the product to a
 *compatible (Dim+1)^2 matrix and then perform a pure matrix product. Of course,
 *internally, operator* tries to perform the minimal number of operations
 * according to the nature of each terms. Likewise, when applying the transform
 * to points, the latters are automatically promoted to homogeneous vectors
 * before doing the matrix product. The conventions to homogeneous
 *representations are performed as follow:
 *
 ...
 * homogeneous vectors.
 *
 * Since, for instance, a Dim x Dim matrix is interpreted as a linear
 *transformation, it is not possible to directly transform Dim vectors stored in
 *a Dim x Dim matrix. The solution is either to use a Dim x Dynamic matrix or
 *explicitly request a vector transformation by making the vector homogeneous:
 * \code
...
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF_VECTORIZABLE_FIXED_SIZE(
      _Scalar, _Dim == Dynamic ? Dynamic : (_Dim + 1) * (_Dim + 1))
  enum {
    Mode = _Mode,
    Options = _Options,
...
public:
  ...
  EIGEN_DEVICE_FUNC inline explicit Transform(
    ...
    check_template_params();
    internal::transform_construct_from_matrix<OtherDerived, Mode, Options, Dim,
  ...
  EIGEN_DEVICE_FUNC inline Transform(
    ...
    EIGEN_STATIC_ASSERT(
        EIGEN_IMPLIES(OtherMode == int(Projective), Mode == int(Projective)),
        YOU_PERFORMED_AN_INVALID_TRANSFORMATION_CONVERSION)

    // prevent conversions as:
    // Isometry = Affine | AffineCompact
    EIGEN_STATIC_ASSERT(EIGEN_IMPLIES(OtherMode == int(Affine) ||
                                          OtherMode == int(AffineCompact),
                                      Mode != int(Isometry)),
                        YOU_PERFORMED_AN_INVALID_TRANSFORMATION_CONVERSION)
    ...
    if (EIGEN_CONST_CONDITIONAL(ModeIsAffineCompact ==
                                OtherModeIsAffineCompact)) {
      // We need the block expression because the code is compiled for all
      // combinations of transformations and will trigger a compile time error
      // if one tries to assign the matrices directly
      m_matrix.template block<Dim, Dim + 1>(0, 0) =
          other.matrix().template block<Dim, Dim + 1>(0, 0);
      makeAffine();
    } else if (EIGEN_CONST_CONDITIONAL(OtherModeIsAffineCompact)) {
      typedef
          typename Transform<Scalar, Dim, OtherMode, OtherOptions>::MatrixType
              OtherMatrixType;
      internal::transform_construct_from_matrix<OtherMatrixType, Mode, Options,
                                                Dim, HDim>::run(this,
                                                                other.matrix());
    } else {
      // here we know that Mode == AffineCompact and OtherMode != AffineCompact.
      // if OtherMode were Projective, the static assert above would already
      // have caught it. So the only possibility is that OtherMode == Affine
      linear() = other.linear();
  ...
  EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR Index rows() const EIGEN_NOEXCEPT {
    return int(Mode) == int(Projective) ? m_matrix.cols()
                                        : (m_matrix.cols() - 1);
  }
   ...
   *
   * Moreover, if \c *this represents an affine transformation (i.e.,
   * Mode!=Projective), then \a other can also be: \li a point of size Dim
   * (computes: \code this->linear() * other + this->translation()\endcode), \li
   * a set of N points as a Dim x N matrix (computes: \code (this->linear() *
   * other).colwise() + this->translation()\endcode),
   *
   * In all cases, the return type is a matrix or vector of same sizes as the
   * right-hand-side \a other.
   *
   * If you want to interpret \a other as a linear or affine transformation,
   * then first convert it to a Transform<> type, or do your own cooking.
   *
   * Finally, if you want to apply Affine transformations to vectors, then
   * explicitly apply the linear part only: \code Affine3f A; Vector3f v1, v2;
   * v2 = A.linear() * v1;
   * \endcode
   *
   */
  // note: this function is defined here because some compilers cannot find the
  // respective declaration
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC
      EIGEN_STRONG_INLINE const typename internal::transform_right_product_impl<
  ...
  friend EIGEN_DEVICE_FUNC inline const typename internal::
      transform_left_product_impl<OtherDerived, Mode, Options, _Dim,
                                  _Dim + 1>::ResultType
      operator*(const EigenBase<OtherDerived> &a, const Transform &b) {
    return internal::transform_left_product_impl<OtherDerived, Mode, Options,
                                                 Dim, HDim>::run(a.derived(),
                                                                 b);
  }
   ...
   * The product results in a Transform of the same type (mode) as the lhs only
   * if the lhs mode is no isometry. In that case, the returned transform is an
   * affinity.
   */
  ...
  EIGEN_DEVICE_FUNC inline const Transform
...
private:
  ...
  //   "Eigen::Transform<double, 3, 32, 0>
  //             (const Eigen::Transform<double, 3, 2, 0> &) const"
  //  (the meaning of a name may have changed since the template declaration --
  //  the type of the template is:
  // "Eigen::internal::transform_transform_product_impl<Eigen::Transform<double,
  // 3, 32, 0>,
  //     Eigen::Transform<double, 3, Mode, Options>, <expression>>::ResultType
  //     (const Eigen::Transform<double, 3, Mode, Options> &) const")
  //
  template <int OtherMode, int OtherOptions> struct icc_11_workaround {
    typedef internal::transform_transform_product_impl<
        Transform, Transform<Scalar, Dim, OtherMode, OtherOptions>>
        ProductType;
...
public:
  ...
  operator*(
    ...
    typedef typename icc_11_workaround<OtherMode, OtherOptions>::ProductType
        ProductType;
  ...
  operator*(
      const Transform<Scalar, Dim, OtherMode, OtherOptions> &other) const {
    return internal::transform_transform_product_impl<
        Transform, Transform<Scalar, Dim, OtherMode, OtherOptions>>::run(*this,
                                                                         other);
  }
#endif
  ...
  EIGEN_DEVICE_FUNC
  inline Transform &operator*=(const UniformScaling<Scalar> &s) {
    return scale(s.factor());
  }

  EIGEN_DEVICE_FUNC
  inline TransformTimeDiagonalReturnType
  ...
  template <typename Derived>
  EIGEN_DEVICE_FUNC inline Transform &
  operator=(const RotationBase<Derived, Dim> &r);
  template <typename Derived>
  EIGEN_DEVICE_FUNC inline Transform &
  operator*=(const RotationBase<Derived, Dim> &r) {
  ...
  typedef typename internal::conditional<int(Mode) == Isometry, ConstLinearPart,
                                         const LinearMatrixType>::type
      RotationReturnType;
  EIGEN_DEVICE_FUNC RotationReturnType rotation() const;

  template <typename RotationMatrixType, typename ScalingMatrixType>
  EIGEN_DEVICE_FUNC void
  computeRotationScaling(RotationMatrixType *rotation,
                         ScalingMatrixType *scaling) const;
  template <typename ScalingMatrixType, typename RotationMatrixType>
  EIGEN_DEVICE_FUNC void
  computeScalingRotation(ScalingMatrixType *scaling,
                         RotationMatrixType *rotation) const;

  template <typename PositionDerived, typename OrientationType,
            typename ScaleDerived>
  EIGEN_DEVICE_FUNC Transform &
  fromPositionOrientationScale(const MatrixBase<PositionDerived> &position,
  ...
  EIGEN_DEVICE_FUNC inline typename internal::cast_return_type<
      Transform, Transform<NewScalarType, Dim, Mode, Options>>::type
  cast() const {
  ...
  isApprox(const Transform &other,
   ...
   */
  EIGEN_DEVICE_FUNC inline Block<MatrixType,
                                 int(Mode) == int(Projective) ? HDim : Dim, Dim>
  linearExt() {
   ...
   *          and the HDim x Dim part for projective transformations.
   */
  EIGEN_DEVICE_FUNC inline const Block<
      MatrixType, int(Mode) == int(Projective) ? HDim : Dim, Dim>
  linearExt() const {
    return m_matrix.template block < int(Mode) == int(Projective) ? HDim : Dim,
           Dim > (0, 0);
  }
   ...
   */
  EIGEN_DEVICE_FUNC inline Block<MatrixType,
                                 int(Mode) == int(Projective) ? HDim : Dim, 1>
  translationExt() {
    return m_matrix.template block < int(Mode) == int(Projective) ? HDim : Dim,
           1 > (0, Dim);
  }
   ...
   */
  EIGEN_DEVICE_FUNC inline const Block<
      MatrixType, int(Mode) == int(Projective) ? HDim : Dim, 1>
...
protected:
  ...
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE void check_template_params() {
    EIGEN_STATIC_ASSERT((Options & (DontAlign | RowMajor)) == Options,
...
Transform<Scalar, Dim, Mode, Options>::operator=(const QMatrix &other) {
  ...
  if (EIGEN_CONST_CONDITIONAL(Mode == int(AffineCompact)))
    m_matrix << other.m11(), other.m21(), other.dx(), other.m12(), other.m22(),
        other.dy();
  else
    m_matrix << other.m11(), other.m21(), other.dx(), other.m12(), other.m22(),
        other.dy(), 0, 0, 1;
...
Transform<Scalar, Dim, Mode, Options>::operator=(const QTransform &other) {
  ...
  if (EIGEN_CONST_CONDITIONAL(Mode == int(AffineCompact)))
    ...
    m_matrix << other.m11(), other.m21(), other.dx(), other.m12(), other.m22(),
        other.dy(), other.m13(), other.m23(), other.m33();
  return *this;
}
...
QTransform Transform<Scalar, Dim, Mode, Options>::toQTransform(void) const {
  ...
  if (EIGEN_CONST_CONDITIONAL(Mode == int(AffineCompact)))
    ...
    return QTransform(
        m_matrix.coeff(0, 0), m_matrix.coeff(1, 0), m_matrix.coeff(2, 0),
...
Transform<Scalar, Dim, Mode, Options>::scale(
    const MatrixBase<OtherDerived> &other) {
  EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(OtherDerived, int(Dim))
  EIGEN_STATIC_ASSERT(Mode != int(Isometry),
...
template <typename OtherDerived>
EIGEN_DEVICE_FUNC Transform<Scalar, Dim, Mode, Options> &
Transform<Scalar, Dim, Mode, Options>::prescale(
    const MatrixBase<OtherDerived> &other) {
  EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(OtherDerived, int(Dim))
  EIGEN_STATIC_ASSERT(Mode != int(Isometry),
                      THIS_METHOD_IS_ONLY_FOR_SPECIFIC_TRANSFORMATIONS)
...
template <typename OtherDerived>
EIGEN_DEVICE_FUNC Transform<Scalar, Dim, Mode, Options> &
Transform<Scalar, Dim, Mode, Options>::translate(
    const MatrixBase<OtherDerived> &other) {
  EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(OtherDerived, int(Dim))
...
template <typename OtherDerived>
EIGEN_DEVICE_FUNC Transform<Scalar, Dim, Mode, Options> &
Transform<Scalar, Dim, Mode, Options>::pretranslate(
    const MatrixBase<OtherDerived> &other) {
  EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(OtherDerived, int(Dim))
 ...
 *   - a AngleAxis (3D)
 *
 * This mechanism is easily extendable to support user types such as Euler
 * angles, or a pair of Quaternion for 4D rotations.
 *
 * \sa rotate(Scalar), class Quaternion, class AngleAxis,
 * prerotate(RotationType)
 */
template <typename Scalar, int Dim, int Mode, int Options>
template <typename RotationType>
EIGEN_DEVICE_FUNC Transform<Scalar, Dim, Mode, Options> &
...
Transform<Scalar, Dim, Mode, Options>::prerotate(const RotationType &rotation) {
  m_matrix.template block<Dim, HDim>(0, 0) =
      internal::toRotationMatrix<Scalar, Dim>(rotation) *
      m_matrix.template block<Dim, HDim>(0, 0);
  return *this;
}
...
template <typename Scalar, int Dim, int Mode, int Options>
EIGEN_DEVICE_FUNC inline Transform<Scalar, Dim, Mode, Options>
Transform<Scalar, Dim, Mode, Options>::operator*(
...
template <int Mode> struct transform_rotation_impl {
  ...
  EIGEN_DEVICE_FUNC static inline const typename TransformType::LinearMatrixType
  run(const TransformType &t) {
...
template <> struct transform_rotation_impl<Isometry> {
  template <typename TransformType>
  EIGEN_DEVICE_FUNC static inline typename TransformType::ConstLinearPart
  run(const TransformType &t) {
    return t.linear();
  }
};
...
template <typename RotationMatrixType, typename ScalingMatrixType>
EIGEN_DEVICE_FUNC void
Transform<Scalar, Dim, Mode, Options>::computeRotationScaling(
...
template <typename ScalingMatrixType, typename RotationMatrixType>
EIGEN_DEVICE_FUNC void
Transform<Scalar, Dim, Mode, Options>::computeScalingRotation(
...
template <typename PositionDerived, typename OrientationType,
          typename ScaleDerived>
EIGEN_DEVICE_FUNC Transform<Scalar, Dim, Mode, Options> &
Transform<Scalar, Dim, Mode, Options>::fromPositionOrientationScale(
...
struct projective_transform_inverse {
  EIGEN_DEVICE_FUNC static inline void run(const TransformType &,
                                           TransformType &) {}
};

template <typename TransformType>
struct projective_transform_inverse<TransformType, Projective> {
  EIGEN_DEVICE_FUNC static inline void run(const TransformType &m,
                                           TransformType &res) {
 ...
 * is known to be not a general transformation (optional). The possible values
 * are:
 ...
 *
 * \warning unless \a traits is always set to NoShear or NoScaling, this
 * function requires the generic inverse method of MatrixBase defined in the LU
 * module. If you forget to include this module, then you will get hard to debug
 * linking errors.
 *
...
Transform<Scalar, Dim, Mode, Options>::inverse(TransformTraits hint) const {
  ...
  if (hint == Projective) {
    ...
    res.makeAffine(); // we do need this, because in the beginning res is
                      // uninitialized
  }
  return res;
}
...
struct transform_take_affine_part<
    Transform<Scalar, Dim, AffineCompact, Options>> {
  typedef typename Transform<Scalar, Dim, AffineCompact, Options>::MatrixType
      MatrixType;
...
template <typename Other, int Mode, int Options, int Dim, int HDim>
struct transform_construct_from_matrix<Other, Mode, Options, Dim, HDim, Dim,
                                       Dim> {
  static inline void
  run(Transform<typename Other::Scalar, Dim, Mode, Options> *transform,
      const Other &other) {
...
template <typename Other, int Mode, int Options, int Dim, int HDim>
struct transform_construct_from_matrix<Other, Mode, Options, Dim, HDim, HDim,
                                       HDim> {
  static inline void
  run(Transform<typename Other::Scalar, Dim, Mode, Options> *transform,
      const Other &other) {
...
struct transform_construct_from_matrix<Other, AffineCompact, Options, Dim, HDim,
                                       HDim, HDim> {
  static inline void
  run(Transform<typename Other::Scalar, Dim, AffineCompact, Options> *transform,
      const Other &other) {
...
struct transform_right_product_impl<TransformType, MatrixType, 1, RhsCols> {
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ResultType
  run(const TransformType &T, const MatrixType &other) {
    EIGEN_STATIC_ASSERT(OtherRows == HDim,
                        YOU_MIXED_MATRICES_OF_DIFFERENT_SIZES);

    typedef Block<ResultType, Dim, OtherCols,
                  int(MatrixType::RowsAtCompileTime) == Dim>
        TopLeftLhs;
...
struct transform_right_product_impl<TransformType, MatrixType, 2, RhsCols> {
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ResultType
  run(const TransformType &T, const MatrixType &other) {
    EIGEN_STATIC_ASSERT(OtherRows == Dim,
                        YOU_MIXED_MATRICES_OF_DIFFERENT_SIZES);
...
template <typename Other, int Mode, int Options, int Dim, int HDim>
struct transform_left_product_impl<Other, Mode, Options, Dim, HDim, HDim,
                                   HDim> {
  typedef Transform<typename Other::Scalar, Dim, Mode, Options> TransformType;
  typedef typename TransformType::MatrixType MatrixType;
  typedef Transform<typename Other::Scalar, Dim, Projective, Options>
      ResultType;
...
template <typename Other, int Options, int Dim, int HDim>
struct transform_left_product_impl<Other, AffineCompact, Options, Dim, HDim,
                                   HDim, HDim> {
  typedef Transform<typename Other::Scalar, Dim, AffineCompact, Options>
      TransformType;
  typedef typename TransformType::MatrixType MatrixType;
  typedef Transform<typename Other::Scalar, Dim, Projective, Options>
      ResultType;
  static ResultType run(const Other &other, const TransformType &tr) {
    ResultType res;
    res.matrix().noalias() =
        other.template block<HDim, Dim>(0, 0) * tr.matrix();
...
template <typename Scalar, int Dim, int LhsMode, int LhsOptions, int RhsMode,
          int RhsOptions>
struct transform_transform_product_impl<
    Transform<Scalar, Dim, LhsMode, LhsOptions>,
...
struct transform_transform_product_impl<
    Transform<Scalar, Dim, AffineCompact, LhsOptions>,
...
template <typename Scalar, int Dim, int LhsOptions, int RhsOptions>
struct transform_transform_product_impl<
    Transform<Scalar, Dim, Projective, LhsOptions>,
