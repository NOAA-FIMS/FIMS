 ...
 * is used. In particular, this class is not aimed to be used to store a scaling
 * transformation, but rather to make easier the constructions and updates of
 * Transform objects.
 *
 * To represent an axis aligned scaling, use the DiagonalMatrix class.
 *
 * \sa Scaling(), class DiagonalMatrix, MatrixBase::asDiagonal(), class
 * Translation, class Transform
 */
...
public:
  ...
  operator*(const Transform<Scalar, Dim, Mode, Options> &t) const {
    typename internal::uniformscaling_times_affine_returntype<Scalar, Dim,
                                                              Mode>::type res =
        t;
    res.prescale(factor());
    return res;
