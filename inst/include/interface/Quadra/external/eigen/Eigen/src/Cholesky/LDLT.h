 ...
 *
 * \tparam _MatrixType the type of the matrix of which to compute the LDL^T
 * Cholesky decomposition \tparam _UpLo the triangular part that will be used
 * for the decompositon: Lower (default) or Upper. The other triangular part
 * won't be read.
 *
 * Perform a robust Cholesky decomposition of a positive semidefinite or
 * negative semidefinite matrix \f$ A \f$ such that \f$ A =  P^TLDL^*P \f$,
 ...
 *
 * Remember that Cholesky decompositions are not rank-revealing. Also, do not
 * use a Cholesky decomposition to determine whether a system of equations has a
 * solution.
 *
...
public:
   ...
   */
  LDLT()
      : m_matrix(), m_transpositions(), m_sign(internal::ZeroSign),
  ...
  explicit LDLT(Index size)
      : m_matrix(size, size), m_transpositions(size), m_temporary(size),
        m_sign(internal::ZeroSign), m_isInitialized(false) {}
  ...
  explicit LDLT(const EigenBase<InputType> &matrix)
      : m_matrix(matrix.rows(), matrix.cols()), m_transpositions(matrix.rows()),
        m_temporary(matrix.rows()), m_sign(internal::ZeroSign),
        m_isInitialized(false) {
  ...
  explicit LDLT(EigenBase<InputType> &matrix)
      : m_matrix(matrix.derived()), m_transpositions(matrix.rows()),
        m_temporary(matrix.rows()), m_sign(internal::ZeroSign),
        m_isInitialized(false) {
    compute(matrix.derived());
  }
   ...
   * decompositionObject.solve(x)</tt> .
   *
   * \note_about_checking_solutions
   *
   * More precisely, this method solves \f$ A x = b \f$ using the decomposition
   * \f$ A = P^T L D L^* P \f$ by solving the systems \f$ P^T y_1 = b \f$, \f$ L
   * y_2 = y_1 \f$, \f$ D y_3 = y_2 \f$, \f$ L^* y_4 = y_3 \f$ and \f$ P x = y_4
   * \f$ in succession. If the matrix \f$ A \f$ is singular, then \f$ D \f$ will
   ...
   * thus enabling generic code such as: \code x =
   * decomposition.adjoint().solve(b) \endcode
   */
...
template <> struct ldlt_inplace<Lower> {
  ...
  static bool unblocked(MatrixType &mat, TranspositionType &transpositions,
    ...
    for (Index k = 0; k < size; ++k) {
      ...
      // In some previous versions of Eigen (e.g., 3.2.1), the scaling was
      // omitted if the pivot was smaller than the cutoff value. However, since
      // LDLT is not rank-revealing we should only make sure that we do not
      ...
      if (k == 0 && !pivot_is_valid) {
        ...
        // except filling the transpositions, and checking whether the matrix is
        // zero.
        sign = ZeroSign;
  ...
  template <typename MatrixType, typename TranspositionType, typename Workspace,
            typename WType>
  static bool update(MatrixType &mat, const TranspositionType &transpositions,
                     Workspace &tmp, const WType &w,
                     const typename MatrixType::RealScalar &sigma = 1) {
...
template <> struct ldlt_inplace<Upper> {
  ...
  update(MatrixType &mat, TranspositionType &transpositions, Workspace &tmp,
         WType &w, const typename MatrixType::RealScalar &sigma = 1) {
    Transpose<MatrixType> matt(mat);
    return ldlt_inplace<Lower>::update(matt, transpositions, tmp, w.conjugate(),
                                       sigma);
  }
};
...
template <typename MatrixType> struct LDLT_Traits<MatrixType, Lower> {
  ...
  typedef const TriangularView<const typename MatrixType::AdjointReturnType,
                               UnitUpper>
      MatrixU;
...
LDLT<MatrixType, _UpLo>::compute(const EigenBase<InputType> &a) {
  ...
  m_info = internal::ldlt_inplace<UpLo>::unblocked(m_matrix, m_transpositions,
                                                   m_temporary, m_sign)
               ? Success
               : NumericalIssue;
 ...
 * Optional; default value is +1. \sa setZero()
 */
template <typename MatrixType, int _UpLo>
template <typename Derived>
LDLT<MatrixType, _UpLo> &LDLT<MatrixType, _UpLo>::rankUpdate(
  ...
  internal::ldlt_inplace<UpLo>::update(m_matrix, m_transpositions, m_temporary,
                                       w, sigma);
...
void LDLT<_MatrixType, _UpLo>::_solve_impl_transposed(const RhsType &rhs,
  ...
  // In some previous versions, tolerance was set to the max of 1/highest (or
  // rather numeric_limits::min()) and the maximal diagonal entry * epsilon as
  // motivated by LAPACK's xGELSS: RealScalar tolerance =
  // numext::maxi(vecD.array().abs().maxCoeff() *
  // NumTraits<RealScalar>::epsilon(),RealScalar(1) /
  // NumTraits<RealScalar>::highest()); However, LDLT is not rank revealing, and
  // so adjusting the tolerance wrt to the highest diagonal element is not well
  // justified and leads to numerical issues in some cases. Moreover, Lapack's
