...
// if EIGEN_STACK_ALLOCATION_LIMIT is defined to 0, then no limit
  ...
  EIGEN_STATIC_ASSERT(Size * sizeof(T) <= EIGEN_STACK_ALLOCATION_LIMIT,
...
template <typename T, int Size, int MatrixOrArrayOptions,
          int Alignment = (MatrixOrArrayOptions & DontAlign)
                              ? 0
...
// GCC 4.7 is too aggressive in its optimizations and remove the alignment test
// based on the fact the array is declared to be aligned. See this bug report:
// http://gcc.gnu.org/bugzilla/show_bug.cgi?id=53900 Hiding the origin of the
// array pointer behind a function argument seems to do the trick even if the
// function is inlined:
...
#define EIGEN_MAKE_UNALIGNED_ARRAY_ASSERT(sizemask)                            \
  eigen_assert((internal::UIntPtr(                                             \
                    eigen_unaligned_array_assert_workaround_gcc47(array)) &    \
                (sizemask)) == 0 &&                                            \
               "this assertion is explained here: "                            \
               "http://eigen.tuxfamily.org/dox-devel/"                         \
               "group__TopicUnalignedArrayAssert.html"                         \
               " **** READ THIS WEB PAGE !!! ****");
#else
#define EIGEN_MAKE_UNALIGNED_ARRAY_ASSERT(sizemask)                            \
...
struct plain_array_helper {
  ...
  copy(const plain_array<T, Size, MatrixOrArrayOptions, Alignment> &src,
  ...
  template <typename T, int Size, int MatrixOrArrayOptions, int Alignment>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE static void
  swap(plain_array<T, Size, MatrixOrArrayOptions, Alignment> &a,
...
public:
  EIGEN_DEVICE_FUNC DenseStorage(){EIGEN_INTERNAL_DENSE_STORAGE_CTOR_PLUGIN(
      Index size = Size)} EIGEN_DEVICE_FUNC
  ...
  DenseStorage(const DenseStorage &other)
      : m_data(other.m_data){EIGEN_INTERNAL_DENSE_STORAGE_CTOR_PLUGIN(
            Index size = Size)}
#else
  ...
  EIGEN_DEVICE_FUNC DenseStorage &
  operator=(DenseStorage &&other) EIGEN_NOEXCEPT {
    if (this != &other)
      m_data = std::move(other.m_data);
    return *this;
  }
  ...
  EIGEN_DEVICE_FUNC DenseStorage(Index size, Index rows, Index cols) {
    ...
    eigen_internal_assert(size == rows * cols && rows == _Rows &&
                          cols == _Cols);
  ...
  EIGEN_DEVICE_FUNC static EIGEN_CONSTEXPR Index cols(void) EIGEN_NOEXCEPT {
    return _Cols;
  }
...
public:
  EIGEN_DEVICE_FUNC DenseStorage() {}
  EIGEN_DEVICE_FUNC explicit DenseStorage(
      internal::constructor_without_unaligned_array_assert) {}
...
// more specializations for null matrices; these are necessary to resolve
// ambiguities
...
public:
  ...
  EIGEN_DEVICE_FUNC void swap(DenseStorage &other) {
    internal::plain_array_helper::swap(m_data, m_rows * m_cols, other.m_data,
...
public:
  ...
  EIGEN_DEVICE_FUNC void swap(DenseStorage &other) {
    internal::plain_array_helper::swap(m_data, m_rows * _Cols, other.m_data,
                                       other.m_rows * _Cols);
    numext::swap(m_rows, other.m_rows);
  }
...
public:
  ...
  EIGEN_DEVICE_FUNC void swap(DenseStorage &other) {
    internal::plain_array_helper::swap(m_data, _Rows * m_cols, other.m_data,
                                       _Rows * other.m_cols);
    numext::swap(m_cols, other.m_cols);
  }
...
public:
  ...
  EIGEN_DEVICE_FUNC DenseStorage(Index size, Index rows, Index cols)
      : m_data(
            internal::conditional_aligned_new_auto<T, (_Options & DontAlign) ==
                                                          0>(size)),
        m_rows(rows), m_cols(cols) {
    EIGEN_INTERNAL_DENSE_STORAGE_CTOR_PLUGIN({})
  ...
  EIGEN_DEVICE_FUNC DenseStorage(const DenseStorage &other)
    ...
    internal::smart_copy(other.m_data,
                         other.m_data + other.m_rows * other.m_cols, m_data);
  }
  EIGEN_DEVICE_FUNC DenseStorage &operator=(const DenseStorage &other) {
    if (this != &other) {
...
#endif
  EIGEN_DEVICE_FUNC ~DenseStorage() {
    internal::conditional_aligned_delete_auto<T, (_Options & DontAlign) == 0>(
  ...
  void conservativeResize(Index size, Index rows, Index cols) {
    m_data = internal::conditional_aligned_realloc_new_auto<
        T, (_Options & DontAlign) == 0>(m_data, size, m_rows * m_cols);
    m_rows = rows;
    m_cols = cols;
  }
  EIGEN_DEVICE_FUNC void resize(Index size, Index rows, Index cols) {
    if (size != m_rows * m_cols) {
      internal::conditional_aligned_delete_auto<T, (_Options & DontAlign) == 0>(
          m_data, m_rows * m_cols);
      if (size > 0) // >0 and not simply !=0 to let the compiler knows that size
                    // cannot be negative
        m_data =
            internal::conditional_aligned_new_auto<T, (_Options & DontAlign) ==
                                                          0>(size);
...
public:
  ...
  EIGEN_DEVICE_FUNC void conservativeResize(Index size, Index, Index cols) {
    m_data = internal::conditional_aligned_realloc_new_auto<
        T, (_Options & DontAlign) == 0>(m_data, size, _Rows * m_cols);
    m_cols = cols;
  }
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void resize(Index size, Index,
                                                    Index cols) {
...
public:
  ...
  void conservativeResize(Index size, Index rows, Index) {
    m_data = internal::conditional_aligned_realloc_new_auto<
        T, (_Options & DontAlign) == 0>(m_data, size, m_rows * _Cols);
    m_rows = rows;
  }
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void resize(Index size, Index rows,
                                                    Index) {
