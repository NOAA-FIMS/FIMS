...
static StorageIndex cs_wclear(StorageIndex mark, StorageIndex lemax,
                              StorageIndex *w, StorageIndex n) {
  StorageIndex k;
...
template <typename StorageIndex>
StorageIndex cs_tdfs(StorageIndex j, StorageIndex k, StorageIndex *head,
 ...
 *
 * \param[in] C the input selfadjoint matrix stored in compressed column major
 * format. \param[out] perm the permutation P reducing the fill-in of the input
 * matrix \a C
 *
 * Note that the input matrix \a C must be complete, that is both the upper and
 * lower parts have to be stored, as well as the diagonal entries. On exit the
 * values of C are destroyed */
template <typename Scalar, typename StorageIndex>
void minimum_degree_ordering(
    SparseMatrix<Scalar, ColMajor, StorageIndex> &C,
    PermutationMatrix<Dynamic, Dynamic, StorageIndex> &perm) {
  using std::sqrt;

  StorageIndex d, dk, dext, lemax = 0, e, elenk, eln, i, j, k, k1, k2, k3,
                            jlast, ln, dense, nzmax, mindeg = 0, nvi, nvj, nvk,
                            mark, wnvi, ok, nel = 0, p, p1, p2, p3, p4, pj, pk,
                            pk1, pk2, pn, q, t, h;

  StorageIndex n = StorageIndex(C.cols());
  dense = std::max<StorageIndex>(
      16, StorageIndex(10 * sqrt(double(n)))); /* find dense threshold */
  ...
  // get workspace
  ei_declare_aligned_stack_constructed_variable(StorageIndex, W, 8 * (n + 1),
                                                0);
  StorageIndex *len = W;
  ...
  {
    if (Cp[i] == -1)
      k = internal::cs_tdfs<StorageIndex>(i, k, head, next,
                                          perm.indices().data(), w);
  }
