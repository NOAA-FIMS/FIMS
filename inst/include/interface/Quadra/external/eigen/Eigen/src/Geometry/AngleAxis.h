 ...
 * \class AngleAxis
 *
 * \brief Represents a 3D rotation as a rotation angle around an arbitrary 3D
 * axis
 *
 * \param _Scalar the scalar type, i.e., the type of the coefficients.
 *
 * \warning When setting up an AngleAxis object, the axis vector \b must \b be
 * \b normalized.
 *
 ...
 * but rather to make easier the creation of other rotation (Quaternion,
 * rotation Matrix) and transformation objects.
 *
...
public:
   ...
   * radian and an \a axis which \b must \b be \b normalized.
   *
   * \warning If the \a axis vector is not normalized, then the angle-axis
   * object represents an invalid rotation. */
  template <typename Derived>
  EIGEN_DEVICE_FUNC inline AngleAxis(const Scalar &angle,
  ...
  EIGEN_DEVICE_FUNC inline explicit AngleAxis(
      const QuaternionBase<QuatDerived> &q) {
    *this = q;
  }
  /** Constructs and initialize the angle-axis rotation from a 3x3 rotation
   * matrix. */
  template <typename Derived>
  ...
  EIGEN_DEVICE_FUNC inline QuaternionType
  ...
  /** Concatenates two rotations */
  EIGEN_DEVICE_FUNC inline QuaternionType
  operator*(const QuaternionType &other) const {
    return QuaternionType(*this) * other;
  }

  /** Concatenates two rotations */
  friend EIGEN_DEVICE_FUNC inline QuaternionType
  operator*(const QuaternionType &a, const AngleAxis &b) {
    return a * QuaternionType(b);
  }

  /** \returns the inverse rotation, i.e., an angle-axis with opposite rotation
   * angle */
  ...
  EIGEN_DEVICE_FUNC inline
      typename internal::cast_return_type<AngleAxis,
                                          AngleAxis<NewScalarType>>::type
      cast() const {
    return typename internal::cast_return_type<
        AngleAxis, AngleAxis<NewScalarType>>::type(*this);
  }
  ...
  isApprox(const AngleAxis &other,
 ...
 *
 * The resulting axis is normalized, and the computed angle is in the [0,pi]
 * range.
 *
 * This function implicitly normalizes the quaternion \a q.
 */
 ...
 */
template <typename Scalar>
typename AngleAxis<Scalar>::Matrix3 EIGEN_DEVICE_FUNC
