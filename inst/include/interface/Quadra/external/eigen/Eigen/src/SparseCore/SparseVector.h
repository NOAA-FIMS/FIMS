 ...
 *
 * See http://www.netlib.org/linalg/html_templates/node91.html for details on
 * the storage scheme.
 *
...
struct traits<SparseVector<_Scalar, _Options, _StorageIndex>> {
  ...
  enum {
    ...
    Flags = _Options | NestByRefBit | LvalueBit |
            (IsColVector ? 0 : RowMajorBit) | CompressedAccessBit,
...
public:
  ...
  inline Scalar coeff(Index row, Index col) const {
    eigen_assert(IsColVector ? (col == 0 && row >= 0 && row < m_size)
                             : (row == 0 && col >= 0 && col < m_size));
   ...
   * performed.
   *
   * This insertion might be very costly if the number of nonzeros above \a i is
   * large.
   */
...
public:
   ...
   * bounds coefficients is kept and reserved. Call .data().squeeze() to free
   * extra memory.
   *
   * \sa reserve(), setZero()
   */
  ...
  explicit inline SparseVector(Index size) : m_size(0) {
    check_template_parameters();
    resize(size);
  }
  ...
  inline SparseVector(const SparseVector &other) : Base(other), m_size(0) {
    check_template_parameters();
    *this = other.derived();
  }
  ...
  inline SparseVector &operator=(const SparseMatrixBase<OtherDerived> &other) {
    ...
    internal::sparse_vector_assign_selector<SparseVector, OtherDerived>::run(
        tmp, other.derived());
    this->swap(tmp);
    return *this;
  }
...
protected:
  static void check_template_parameters() {
    ...
    EIGEN_STATIC_ASSERT((_Options & (ColMajor | RowMajor)) == Options,
