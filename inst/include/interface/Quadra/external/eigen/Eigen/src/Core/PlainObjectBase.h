...
template <> struct check_rows_cols_for_overflow<Dynamic> {
  template <typename Index>
  EIGEN_DEVICE_FUNC static EIGEN_ALWAYS_INLINE void run(Index rows,
                                                        Index cols) {
...
// This is a workaround to doxygen not being able to understand the inheritance
// logic when it is hidden by the dense_xpr_base helper struct. Moreover,
// doxygen fails to include members that are not documented in the declaration
// body of MatrixBase if we inherits MatrixBase<Matrix<_Scalar, _Rows, _Cols,
...
protected:
  DenseStorage<Scalar, Base::MaxSizeAtCompileTime, Base::RowsAtCompileTime,
...
public:
   ...
   *
   * See DenseCoeffsBase<Derived,ReadOnlyAccessors>::coeff(Index) const for
   * details. */
  EIGEN_DEVICE_FUNC
   ...
   * saving compilation efforts.
   *
   * See DenseCoeffsBase<Derived,WriteAccessors>::coeffRef(Index,Index) const
   * for details. */
  EIGEN_DEVICE_FUNC
   ...
   * compilation efforts.
   *
   * See DenseCoeffsBase<Derived,WriteAccessors>::coeffRef(Index) const for
   * details. */
  EIGEN_DEVICE_FUNC
   ...
   * you only want to change the number of rows and/or of columns, you can use
   * resize(NoChange_t, Index), resize(Index, NoChange_t).
   *
   ...
   *
   * \sa resize(Index) for vectors, resize(NoChange_t, Index), resize(Index,
   * NoChange_t)
   */
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE void resize(Index rows, Index cols) {
    eigen_assert(EIGEN_IMPLIES(RowsAtCompileTime != Dynamic,
                               rows == RowsAtCompileTime) &&
                 EIGEN_IMPLIES(ColsAtCompileTime != Dynamic,
                               cols == ColsAtCompileTime) &&
                 EIGEN_IMPLIES(RowsAtCompileTime == Dynamic &&
    ...
    internal::check_rows_cols_for_overflow<MaxSizeAtCompileTime>::run(rows,
                                                                      cols);
   ...
   * change the number of rows and/or of columns, you can use
   * conservativeResize(NoChange_t, Index) or conservativeResize(Index,
   * NoChange_t).
   *
   * Matrices are resized relative to the top-left element. In case values need
   * to be appended to the matrix they will be uninitialized.
   */
  EIGEN_DEVICE_FUNC
   ...
   * leaves the number of columns unchanged.
   *
   ...
   *
   * As opposed to conservativeResize(Index rows, Index cols), this version
   * leaves the number of rows unchanged.
   *
   ...
   *
   * Matrices are resized relative to the top-left element. In case values need
   * to be appended to the matrix they will copied from \c other.
   */
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  conservativeResizeLike(const DenseBase<OtherDerived> &other) {
    internal::conservative_resize_like_impl<Derived, OtherDerived>::run(*this,
                                                                        other);
  }
...
protected:
   ...
   * There exists C++98 analogue constructors for fixed-size array/vector having
   * 1, 2, 3, or 4 coefficients.
   *
   * \warning To construct a column (resp. row) vector of fixed length, the
   * number of values passed to this constructor must match the the fixed number
   * of rows (resp. columns) of \c *this.
   */
  template <typename... ArgTypes>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PlainObjectBase(const Scalar &a0,
                                                        const Scalar &a1,
                                                        const Scalar &a2,
                                                        const Scalar &a3,
                                                        const ArgTypes &...args)
      : m_storage() {
    _check_template_params();
    EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(PlainObjectBase,
                                             sizeof...(args) + 4);
    m_storage.data()[0] = a0;
    m_storage.data()[1] = a1;
    m_storage.data()[2] = a2;
  ...
  explicit EIGEN_STRONG_INLINE PlainObjectBase(
    ...
    if (ColsAtCompileTime == 1 && list.size() == 1) {
      eigen_assert(list_size == static_cast<size_t>(RowsAtCompileTime) ||
                   RowsAtCompileTime == Dynamic);
      resize(list_size, ColsAtCompileTime);
      std::copy(list.begin()->begin(), list.begin()->end(), m_storage.data());
    } else {
      eigen_assert(list.size() == static_cast<size_t>(RowsAtCompileTime) ||
                   RowsAtCompileTime == Dynamic);
      eigen_assert(list_size == static_cast<size_t>(ColsAtCompileTime) ||
...
public:
  ...
  static inline ConstAlignedMapType MapAligned(const Scalar *data, Index rows,
                                               Index cols) {
    return ConstAlignedMapType(data, rows, cols);
  }
  static inline AlignedMapType MapAligned(Scalar *data, Index rows,
                                          Index cols) {
  ...
  Map(const Scalar *data, const Stride<Outer, Inner> &stride) {
    return
        typename StridedConstMapType<Stride<Outer, Inner>>::type(data, stride);
  }
  template <int Outer, int Inner>
  ...
  Map(const Scalar *data, Index size, const Stride<Outer, Inner> &stride) {
    return typename StridedConstMapType<Stride<Outer, Inner>>::type(data, size,
                                                                    stride);
  }
  template <int Outer, int Inner>
  static inline typename StridedMapType<Stride<Outer, Inner>>::type
  Map(Scalar *data, Index size, const Stride<Outer, Inner> &stride) {
    return
        typename StridedMapType<Stride<Outer, Inner>>::type(data, size, stride);
  }
  template <int Outer, int Inner>
  static inline typename StridedConstMapType<Stride<Outer, Inner>>::type
  ...
  Map(Scalar *data, Index rows, Index cols,
      const Stride<Outer, Inner> &stride) {
    return typename StridedMapType<Stride<Outer, Inner>>::type(data, rows, cols,
                                                               stride);
  }
  ...
  MapAligned(Scalar *data, const Stride<Outer, Inner> &stride) {
    return typename StridedAlignedMapType<Stride<Outer, Inner>>::type(data,
                                                                      stride);
  }
  template <int Outer, int Inner>
  static inline typename StridedConstAlignedMapType<Stride<Outer, Inner>>::type
  ...
  EIGEN_DEVICE_FUNC Derived &setConstant(Index rows, Index cols,
                                         const Scalar &val);
  EIGEN_DEVICE_FUNC Derived &setConstant(NoChange_t, Index cols,
                                         const Scalar &val);
  EIGEN_DEVICE_FUNC Derived &setConstant(Index rows, NoChange_t,
                                         const Scalar &val);
...
protected:
  ...
  // so at this stage we have to assume aliasing... and resising has to be done
  // later.
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Derived &
  ...
  _set_noalias(const DenseBase<OtherDerived> &other) {
    // I don't think we need this resize call since the lazyAssign will anyways
    // resize and lazyAssign will be called by the assign selector.
    //_resize_to_match(other);
    // the 'false' below means to enforce lazy evaluation. We don't use
    // lazyAssign() because it wouldn't allow to copy a row-vector into a
    // column-vector.
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void _init2(
      ...
      typename internal::enable_if<Base::SizeAtCompileTime != 2, T0>::type * =
          0) {
  ...
  // The argument is convertible to the Index type and we either have a non 1x1
  // Matrix, or a dynamic-sized Array, then the argument is meant to be the size
  // of the object.
  template <typename T>
      ...
      typename internal::enable_if<
          (Base::SizeAtCompileTime != 1 ||
           !internal::is_convertible<T, Scalar>::value) &&
              ((!internal::is_same<typename internal::traits<Derived>::XprKind,
                                   ArrayXpr>::value ||
  ...
  // We have a 1x1 matrix/array => the argument is interpreted as the value of
  // the unique coefficient (case where scalar type can be implicitly converted)
  template <typename T>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  ...
  // We have a 1x1 matrix/array => the argument is interpreted as the value of
  // the unique coefficient (case where scalar type match the index type)
  template <typename T>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  ...
  // Initialize an arbitrary matrix from an object convertible to the Derived
  // type.
  template <typename T>
...
public:
  ...
  swap(DenseBase<OtherDerived> &other) {
    enum {SwapPointers = internal::is_same<Derived, OtherDerived>::value &&
                         Base::SizeAtCompileTime == Dynamic};
    internal::matrix_swap_impl<Derived, OtherDerived, bool(SwapPointers)>::run(
        this->derived(), other.derived());
  }
...
public:
  ...
  // for EIGEN_MAX_ALIGN_BYTES==0, AlignedMax==Unaligned, and many compilers
  // generate warnings for friend-ing a class twice.
...
struct conservative_resize_like_impl {
  ...
  static void run(DenseBase<Derived> &_this, Index rows, Index cols) {
    ...
    if (IsRelocatable &&
        ((Derived::IsRowMajor &&
          _this.cols() ==
              cols) || // row-major and we change only the number of rows
         (!Derived::IsRowMajor &&
          _this.rows() ==
              rows))) // column-major and we change only the number of columns
    {
  ...
  static void run(DenseBase<Derived> &_this,
    ...
    // ColsAtCompileTime must be Dynamic. If only one of the dimensions is
    // dynamic, one could use either conservativeResize(Index rows, NoChange_t)
    // or conservativeResize(NoChange_t, Index cols). For these methods new
    // static asserts like EIGEN_STATIC_ASSERT_DYNAMIC_ROWS and
    ...
    if (IsRelocatable &&
        ((Derived::IsRowMajor &&
          _this.cols() == other.cols()) || // row-major and we change only the
                                           // number of rows
         (!Derived::IsRowMajor &&
          _this.rows() == other.rows()))) // column-major and we change only the
                                          // number of columns
    {
      const Index new_rows = other.rows() - _this.rows();
      const Index new_cols = other.cols() - _this.cols();
      _this.derived().m_storage.conservativeResize(other.size(), other.rows(),
                                                   other.cols());
      if (new_rows > 0)
...
struct conservative_resize_like_impl<Derived, OtherDerived, true>
  ...
  static void run(DenseBase<Derived> &_this,
    ...
    if (IsRelocatable)
      _this.derived().m_storage.conservativeResize(other.size(), new_rows,
                                                   new_cols);
    else
      Base::run(_this.derived(), new_rows, new_cols);
