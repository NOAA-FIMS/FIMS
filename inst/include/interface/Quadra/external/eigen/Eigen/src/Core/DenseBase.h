...
// The index type defined by EIGEN_DEFAULT_DENSE_INDEX_TYPE must be a signed
// type. This dummy function simply aims at checking that at compile time.
static inline void check_DenseIndex_is_signed() {
  EIGEN_STATIC_ASSERT(NumTraits<DenseIndex>::IsSigned,
 ...
 *
 * This class is the base that is inherited by all dense objects (matrix,
 * vector, arrays, and related expression types). The common Eigen API for dense
 * objects is contained in this class.
 *
...
public:
   ...
   * Eigen::Index \sa \blank \ref TopicPreprocessorDirectives, Eigen::Index,
   * SparseMatrixBase.
   */
  ...
  typedef DenseCoeffsBase<Derived, internal::accessors_level<Derived>::value>
      Base;
  ...
  enum {
     ...
     *
     * This value is useful to know when evaluating an expression, in order to
     * determine whether it is possible to avoid doing a dynamic memory
     * allocation.
     *
  ...
  typedef typename internal::find_best_packet<Scalar, SizeAtCompileTime>::type
      PacketScalar;
  ...
  typedef Matrix<typename internal::traits<Derived>::Scalar,
                 ...
                 AutoAlign | (internal::traits<Derived>::Flags & RowMajorBit
                                  ? RowMajor
                                  : ColMajor),
                 internal::traits<Derived>::MaxRowsAtCompileTime,
  ...
  typedef Array<typename internal::traits<Derived>::Scalar,
                ...
                internal::traits<Derived>::ColsAtCompileTime,
                AutoAlign |
                    (internal::traits<Derived>::Flags & RowMajorBit ? RowMajor
                                                                    : ColMajor),
                internal::traits<Derived>::MaxRowsAtCompileTime,
                internal::traits<Derived>::MaxColsAtCompileTime>
      PlainArray;
   ...
   * not a matrix! It is however guaranteed that the return type of eval() is
   * either PlainObject or const PlainObject&.
   */
   ...
   * order", i.e., the number of columns for a column-major matrix, and the
   * number of rows for a row-major matrix. */
  EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR Index outerSize() const {
    return IsVectorAtCompileTime ? 1
           : int(IsRowMajor)     ? this->rows()
                                 : this->cols();
  }
   ...
   * order", i.e., the number of rows for a column-major matrix, and the number
   * of columns for a row-major matrix. */
  EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR Index innerSize() const {
    return IsVectorAtCompileTime ? this->size()
           : int(IsRowMajor)     ? this->cols()
                                 : this->rows();
  }
  ...
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC CommaInitializer<Derived>
  operator<<(const DenseBase<OtherDerived> &other);
  ...
  EIGEN_DEVICE_FUNC static const ConstantReturnType Zero(Index rows,
                                                         Index cols);
  EIGEN_DEVICE_FUNC static const ConstantReturnType Zero(Index size);
  EIGEN_DEVICE_FUNC static const ConstantReturnType Zero();
  EIGEN_DEVICE_FUNC static const ConstantReturnType Ones(Index rows,
                                                         Index cols);
  ...
  EIGEN_DEVICE_FUNC Derived &setLinSpaced(Index size, const Scalar &low,
                                          const Scalar &high);
  EIGEN_DEVICE_FUNC Derived &setLinSpaced(const Scalar &low,
                                          const Scalar &high);
  ...
  isApprox(const DenseBase<OtherDerived> &other,
  ...
  EIGEN_DEVICE_FUNC bool isConstant(
      ...
      const RealScalar &prec = NumTraits<Scalar>::dummy_precision()) const;
  EIGEN_DEVICE_FUNC bool
  isZero(const RealScalar &prec = NumTraits<Scalar>::dummy_precision()) const;
  EIGEN_DEVICE_FUNC bool
  isOnes(const RealScalar &prec = NumTraits<Scalar>::dummy_precision()) const;
   ...
   * copy.
   *
   * \warning Be careful with eval() and the auto C++ keyword, as detailed in
   * this \link TopicPitfalls_auto_keyword page \endlink.
   */
  EIGEN_DEVICE_FUNC
  ...
  swap(const DenseBase<OtherDerived> &other) {
    EIGEN_STATIC_ASSERT(!OtherDerived::IsPlainObjectBase,
  ...
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  swap(PlainObjectBase<OtherDerived> &other) {
    eigen_assert(rows() == other.rows() && cols() == other.cols());
  ...
  EIGEN_DEVICE_FUNC inline const typename internal::conditional<
      Enable, ForceAlignedAccess<Derived>, Derived &>::type
  forceAlignedAccessIf() const;
  template <bool Enable>
  EIGEN_DEVICE_FUNC inline
      typename internal::conditional<Enable, ForceAlignedAccess<Derived>,
                                     Derived &>::type
  ...
  template <typename Visitor> EIGEN_DEVICE_FUNC void visit(Visitor &func) const;
  ...
  typedef CwiseNullaryOp<internal::scalar_random_op<Scalar>, PlainObject>
      RandomReturnType;
  ...
  inline EIGEN_DEVICE_FUNC const
  ...
  inline EIGEN_DEVICE_FUNC const
      ...
      select(const typename ElseDerived::Scalar &thenScalar,
             const DenseBase<ElseDerived> &elseMatrix) const;
   ...
   *
   * \sa VectorwiseOp::replicate(), DenseBase::replicate<int,int>(), class
   * Replicate
   */
  // Code moved here due to a CUDA compiler bug
  EIGEN_DEVICE_FUNC
  const Replicate<Derived, Dynamic, Dynamic> replicate(Index rowFactor,
    ...
    return Replicate<Derived, Dynamic, Dynamic>(derived(), rowFactor,
                                                colFactor);
  }
...
#else
  typedef typename internal::conditional<
      (Flags & DirectAccessBit) == DirectAccessBit,
  ...
  typedef typename internal::conditional<IsVectorAtCompileTime, iterator_type,
                                         void>::type iterator;

  typedef
      typename internal::conditional<IsVectorAtCompileTime, const_iterator_type,
...
#undef EIGEN_DOC_UNARY_ADDONS
  ...
  template <typename Dest> EIGEN_DEVICE_FUNC inline void evalTo(Dest &) const {
    EIGEN_STATIC_ASSERT(
        (internal::is_same<Dest, void>::value),
