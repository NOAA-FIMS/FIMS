...
struct simplicial_cholesky_grab_input {
  ...
  static void run(const InputMatrixType &input, ConstCholMatrixPtr &pmat,
                  CholMatrixType &tmp) {
    tmp = input;
    pmat = &tmp;
  }
 ...
 * matrices that are selfadjoint and positive definite. These factorizations
 * allow for solving A.X = B where X and B can be either dense or sparse.
 *
 * In order to reduce the fill-in, a symmetric permutation P is applied prior to
 * the factorization such that the factorized matrix is P A P^-1.
 *
 * \tparam Derived the type of the derived class, that is the actual
 * factorization type.
 *
 */
...
public:
  ...
  SimplicialCholeskyBase()
      : m_info(Success), m_factorizationIsOk(false), m_analysisIsOk(false),
        m_shiftOffset(0), m_shiftScale(1) {}
   ...
   * transformed by the following linear model:\n \c d_ii = \a offset + \a scale
   * * \c d_ii
   *
   * The default is the identity transformation with \a offset=0, and \a
   * scale=1.
   *
  ...
  template <typename Stream> void dumpMemory(Stream &s) {
    int total = 0;
    s << "  L:        "
      << ((total += (m_matrix.cols() + 1) * sizeof(int) +
                    m_matrix.nonZeros() * (sizeof(int) + sizeof(Scalar))) >>
          20)
      << "Mb" << "\n";
    s << "  diag:     " << ((total += m_diag.size() * sizeof(Scalar)) >> 20)
      << "Mb" << "\n";
    s << "  tree:     " << ((total += m_parent.size() * sizeof(int)) >> 20)
      << "Mb" << "\n";
    s << "  nonzeros: "
    ...
    s << "  perm:     " << ((total += m_P.size() * sizeof(int)) >> 20) << "Mb"
      << "\n";
    s << "  perm^-1:  " << ((total += m_Pinv.size() * sizeof(int)) >> 20)
      << "Mb" << "\n";
    s << "  TOTAL:    " << (total >> 20) << "Mb" << "\n";
  }
...
protected:
  ...
  template <bool DoLDLT> void factorize(const MatrixType &a) {
    ...
    if (m_P.size() == 0 && (int(UpLo) & int(Upper)) == Upper) {
      // If there is no ordering, try to directly use the input matrix without
      // any copy
      internal::simplicial_cholesky_grab_input<CholMatrixType, MatrixType>::run(
          a, pmat, tmp);
    } else {
  ...
  template <bool DoLDLT> void factorize_preordered(const CholMatrixType &a);
  ...
  PermutationMatrix<Dynamic, Dynamic, StorageIndex>
      m_Pinv; // the inverse permutation
...
struct traits<SimplicialLLT<_MatrixType, _UpLo, _Ordering>> {
  ...
  static inline MatrixU getU(const CholMatrixType &m) {
 ...
 *
 * This class provides a LL^T Cholesky factorizations of sparse matrices that
 * are selfadjoint and positive definite. The factorization allows for solving
 * A.X = B where X and B can be either dense or sparse.
 *
 * In order to reduce the fill-in, a symmetric permutation P is applied prior to
 * the factorization such that the factorized matrix is P A P^-1.
 *
 ...
 * computations. It can be Lower or Upper. Default is Lower. \tparam _Ordering
 * The ordering method to use, either AMDOrdering<> or NaturalOrdering<>.
 * Default is AMDOrdering<>
 *
 ...
 *
 * This class provides a LDL^T Cholesky factorizations without square root of
 * sparse matrices that are selfadjoint and positive definite. The factorization
 * allows for solving A.X = B where X and B can be either dense or sparse.
 *
 * In order to reduce the fill-in, a symmetric permutation P is applied prior to
 * the factorization such that the factorized matrix is P A P^-1.
 *
...
public:
  ...
  explicit SimplicialCholesky(const MatrixType &matrix) : Base(), m_LDLT(true) {
    compute(matrix);
  }
...
template <typename Derived>
void SimplicialCholeskyBase<Derived>::ordering(const MatrixType &a,
                                               ConstCholMatrixPtr &pmat,
                                               CholMatrixType &ap) {
  eigen_assert(a.rows() == a.cols());
  ...
  if (!internal::is_same<OrderingType, NaturalOrdering<Index>>::value) {
    ...
    ap.resize(size, size);
    ap.template selfadjointView<Upper>() =
        a.template selfadjointView<UpLo>().twistedBy(m_P);
  } else {
    m_Pinv.resize(0);
    m_P.resize(0);
