 ...
 * to distinguish between a very expensive and very very expensive expressions.
 * It thus must also be large enough to make sure unrolling won't happen and
 * that sub expressions will be evaluated, but not too large to avoid overflow.
 */
 ...
 *
 * It is important to note that these flags are a purely compile-time notion.
 * They are a compile-time property of an expression type, implemented as
 * enum's. They are not stored in memory at runtime, and they do not incur any
 * runtime overhead.
 *
 * \sa MatrixBase::Flags
 */
 ...
 * of the present CPU architecture for optimized packet access. In the
 * fixed-size case, there is the additional condition that it be possible to
 * access all the coefficients by packets (this implies the requirement that the
 * size be a multiple of 16 bytes, and that any nontrivial strides don't break
 * the alignment). In the dynamic-size case, there is no such condition on the
 * total size and strides, so it might not be possible to access all coeffs by
 * packets.
 *
 * \note This bit can be set regardless of whether vectorization is actually
 * enabled. To check for actual vectorizability, see \a ActualPacketAccessBit.
 */
 ...
 *
 * If vectorization is not enabled (EIGEN_VECTORIZE is not defined) this
 ...
 * of this expression by coeff(int), and coeffRef(int) in the case of a lvalue
 * expression. These index-based access methods are guaranteed to not have to do
 * any runtime computation of a (row, col)-pair from the index, so that it is
 * guaranteed that whenever it is available, index-based access is at least as
 * fast as (row,col)-based access. Expressions for which that isn't possible
 * don't have the LinearAccessBit.
 *
 * If both PacketAccessBit and LinearAccessBit are set, then the
 * packets of this expression can be accessed by packet(int), and
 * writePacket(int) in the case of a lvalue expression.
 *
 * Typically, all vector expressions have the LinearAccessBit, but there is one
 * exception: Product expressions don't have it, because it would be troublesome
 * for vectorization, even when the Product is a vector expression. Thus, vector
 * Product expressions allow index-based coefficient access but not index-based
 * packet access, so they don't have the LinearAccessBit.
 */
 ...
 * expressions.
 *
 * Note that DirectAccessBit and LvalueBit are mutually orthogonal, as there are
 * examples of expression having one but note the other: \li writable
 * expressions that don't have a very simple memory layout as a strided array,
 * have LvalueBit but not DirectAccessBit \li Map-to-const expressions, for
 * example Map<const Matrix>, have DirectAccessBit but not LvalueBit
 *
 * Expressions having LvalueBit also have their coeff() method returning a const
 * reference instead of returning a new value.
 */
 ...
 * exactly the natural one suggested by rows(), cols(), outerStride(),
 * innerStride(), and the RowMajorBit. This rules out expressions such as
 * Diagonal, whose coefficients, though referencable, do not have such a regular
 * memory layout.
 *
 * See the comment on LvalueBit for an explanation of how LvalueBit and
 * DirectAccessBit are mutually orthogonal.
 */
 ...
 * In other words, this means we are allow to perform an aligned packet access
 * to the first element regardless of the expression kind: \code
 ...
 * enum/integer types. However, this leads to "variable declared but never
 * referenced" warnings on Intel Composer XE, and we do not know how to get rid
 * of them (bug 450).
 */
