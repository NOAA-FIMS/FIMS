 ...
 * symbolically reference the last element/row/columns of the underlying vector
 * or matrix once passed to DenseBase::operator()(const RowIndices&, const
 * ColIndices&).
 *
 ...
 *
 * Can be used as a parameter to Eigen::seq and Eigen::seqN functions to
 * symbolically reference the last+1 element/row/columns of the underlying
 * vector or matrix once passed to DenseBase::operator()(const RowIndices&,
 * const ColIndices&).
 *
 * This symbolic placeholder supports standard arithmetic operations.
...
static const symbolic::AddExpr<
    symbolic::SymbolExpr<internal::symbolic_last_tag>,
...
template <typename T> EIGEN_CONSTEXPR Index first(const T &x) EIGEN_NOEXCEPT {
  return x.first();
}

// IndexedViewCompatibleType/makeIndexedViewCompatible turn an arbitrary object
// of type T into something usable by MatrixSlice The generic implementation is
// a no-op
...
// Turn a single index into something that looks like an array (i.e., that
// exposes a .size(), and operator[](int) methods)
template <typename T, int XprSize>
struct IndexedViewCompatibleType<
    T, XprSize,
    typename internal::enable_if<internal::is_integral<T>::value>::type> {
  // Here we could simply use Array, but maybe it's less work for the compiler
  // to use a simpler wrapper as SingleRange
...
template <typename T, int XprSize>
struct IndexedViewCompatibleType<
    T, XprSize, typename enable_if<symbolic::is_symbolic<T>::value>::type> {
  typedef SingleRange type;
};
...
template <int XprSize> struct AllRange {
...
template <int XprSize> struct IndexedViewCompatibleType<all_t, XprSize> {
  typedef AllRange<XprSize> type;
};

template <typename XprSizeType>
inline AllRange<get_fixed_value<XprSizeType>::value>
makeIndexedViewCompatible(all_t, XprSizeType size, SpecializedType) {
 ...
 */
static const Eigen::internal::all_t
    all; // PLEASE use Eigen::all instead of Eigen::placeholders::all
...
EIGEN_DEPRECATED static const all_t all =
    Eigen::all; // PLEASE use Eigen::all    instead of Eigen::placeholders::all
EIGEN_DEPRECATED static const last_t last =
    Eigen::last; // PLEASE use Eigen::last   instead of
                 // Eigen::placeholders::last
EIGEN_DEPRECATED static const end_t end =
    Eigen::lastp1; // PLEASE use Eigen::lastp1 instead of
