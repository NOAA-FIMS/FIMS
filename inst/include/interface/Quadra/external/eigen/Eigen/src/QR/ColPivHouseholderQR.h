 ...
 * matrices \b P, \b Q and \b R such that \f[ \mathbf{A} \, \mathbf{P} =
 * \mathbf{Q} \, \mathbf{R} \f] by using Householder transformations. Here, \b P
 * is a permutation matrix, \b Q a unitary matrix and \b R an upper triangular
 * matrix.
 *
 * This decomposition performs column pivoting in order to be rank-revealing and
 * improve numerical stability. It is slower than HouseholderQR, and faster than
...
public:
   ...
   */
  ColPivHouseholderQR()
      : m_qr(), m_hCoeffs(), m_colsPermutation(), m_colsTranspositions(),
        m_temp(), m_colNormsUpdated(), m_colNormsDirect(),
  ...
  ColPivHouseholderQR(Index rows, Index cols)
      : m_qr(rows, cols), m_hCoeffs((std::min)(rows, cols)),
        m_colsPermutation(PermIndexType(cols)), m_colsTranspositions(cols),
        m_temp(cols), m_colNormsUpdated(cols), m_colNormsDirect(cols),
        m_isInitialized(false), m_usePrescribedThreshold(false) {}
  ...
  explicit ColPivHouseholderQR(const EigenBase<InputType> &matrix)
      : m_qr(matrix.rows(), matrix.cols()),
        ...
        m_colsTranspositions(matrix.cols()), m_temp(matrix.cols()),
        m_colNormsUpdated(matrix.cols()), m_colNormsDirect(matrix.cols()),
        m_isInitialized(false), m_usePrescribedThreshold(false) {
   ...
   * stored \warning The strict lower part of this matrix contains internal
   * values. Only the upper triangular part should be referenced. To get it, use
...
void ColPivHouseholderQR<MatrixType>::computeInPlace() {
  ...
  RealScalar threshold_helper =
      numext::abs2<RealScalar>(m_colNormsUpdated.maxCoeff() *
                               NumTraits<RealScalar>::epsilon()) /
      RealScalar(rows);
  RealScalar norm_downdate_threshold =
      numext::sqrt(NumTraits<RealScalar>::epsilon());
  ...
  for (Index k = 0; k < size; ++k) {
    // first, we look up in our table m_colNormsUpdated which column has the
    // biggest norm
    Index biggest_col_index;
    RealScalar biggest_col_sq_norm = numext::abs2(
        m_colNormsUpdated.tail(cols - k).maxCoeff(&biggest_col_index));
    biggest_col_index += k;

    // Track the number of meaningful pivots but do not stop the decomposition
    // to make sure that the initial matrix is properly reproduced. See bug 941.
    if (m_nonzero_pivots == size &&
        biggest_col_sq_norm < threshold_helper * RealScalar(rows - k))
      m_nonzero_pivots = k;
    ...
    if (k != biggest_col_index) {
      ...
      std::swap(m_colNormsUpdated.coeffRef(k),
                m_colNormsUpdated.coeffRef(biggest_col_index));
    ...
    for (Index j = k + 1; j < cols; ++j) {
      ...
      if (m_colNormsUpdated.coeffRef(j) != RealScalar(0)) {
        ...
        RealScalar temp2 =
            temp * numext::abs2<RealScalar>(m_colNormsUpdated.coeffRef(j) /
  ...
  for (PermIndexType k = 0; k < size /*m_nonzero_pivots*/; ++k)
    m_colsPermutation.applyTranspositionOnTheRight(
        k, PermIndexType(m_colsTranspositions.coeff(k)));
...
template <typename _MatrixType>
template <typename RhsType, typename DstType>
void ColPivHouseholderQR<_MatrixType>::_solve_impl(const RhsType &rhs,
                                                   DstType &dst) const {
...
void ColPivHouseholderQR<_MatrixType>::_solve_impl_transposed(
  ...
  dst.applyOnTheLeft(householderQ()
...
struct Assignment<
    DstXprType, Inverse<ColPivHouseholderQR<MatrixType>>,
    internal::assign_op<typename DstXprType::Scalar,
                        typename ColPivHouseholderQR<MatrixType>::Scalar>,
    Dense2Dense> {
