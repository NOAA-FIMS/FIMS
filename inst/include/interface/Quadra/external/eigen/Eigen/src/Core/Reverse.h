 ...
 * \tparam Direction defines the direction of the reverse operation, can be
 * Vertical, Horizontal, or BothDirections
 *
...
protected:
  ...
  typedef internal::reverse_packet_cond<PacketScalar, ReversePacket>
      reverse_packet;
 ...
 *  - less error prone: doing the same operation with .reverse() requires
 * special care: \code m = m.reverse().eval(); \endcode
...
template <> struct vectorwise_reverse_inplace_impl<Vertical> {
  template <typename ExpressionType> static void run(ExpressionType &xpr) {
    const int HalfAtCompileTime = ExpressionType::RowsAtCompileTime == Dynamic
                                      ? Dynamic
                                      : ExpressionType::RowsAtCompileTime / 2;
...
template <> struct vectorwise_reverse_inplace_impl<Horizontal> {
  template <typename ExpressionType> static void run(ExpressionType &xpr) {
    const int HalfAtCompileTime = ExpressionType::ColsAtCompileTime == Dynamic
                                      ? Dynamic
