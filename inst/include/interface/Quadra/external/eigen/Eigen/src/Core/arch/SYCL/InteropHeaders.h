...
template <typename PacketReturnType> struct PacketWrapper<PacketReturnType, 4> {
  typedef typename ::Eigen::internal::unpacket_traits<PacketReturnType>::type
      Scalar;
  template <typename Index>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE static Scalar
  scalarize(Index index, PacketReturnType &in) {
    switch (index) {
    case 0:
      return in.x();
    case 1:
    ...
    case 3:
      return in.w();
    default:
      // INDEX MUST BE BETWEEN 0 and 3.There is no abort function in SYCL
      // kernel. so we cannot use abort here.
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE static PacketReturnType
  ...
  set_packet(PacketReturnType &lhs, Scalar *rhs) {
...
template <typename PacketReturnType> struct PacketWrapper<PacketReturnType, 1> {
  ...
  template <typename Index>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE static Scalar
  scalarize(Index, PacketReturnType &in) {
    return in;
  }
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE static PacketReturnType
  convert_to_packet_type(Scalar in, Scalar) {
...
template <typename PacketReturnType> struct PacketWrapper<PacketReturnType, 2> {
  ...
  scalarize(Index index, PacketReturnType &in) {
    ...
    case 1:
      return in.y();
    default:
      // INDEX MUST BE BETWEEN 0 and 1.There is no abort function in SYCL
      // kernel. so we cannot use abort here.
      // The code will never reach here
      __builtin_unreachable();
    }
