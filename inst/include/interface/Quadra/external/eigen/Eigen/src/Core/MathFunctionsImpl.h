...
/** \internal \returns the hyperbolic tan of \a a (coeff-wise)
    ...
    the approxmation tanh(x) ~= x is used for better accuracy as x tends to
   zero.
...
EIGEN_DEVICE_FUNC std::complex<T> complex_rsqrt(const std::complex<T> &z) {
  ...
  return abs_z == zero ? std::complex<T>(NumTraits<T>::infinity(),
                                         NumTraits<T>::quiet_NaN())
         : ((numext::isinf)(x) || (numext::isinf)(y))
             ? std::complex<T>(zero, zero)
         : x == zero ? std::complex<T>(woz, y < zero ? woz : -woz)
         : x > zero  ? std::complex<T>(woz, -y / (2 * w * abs_z))
                     : std::complex<T>(numext::abs(y) / (2 * w * abs_z),
                                      y < zero ? woz : -woz);
}
