...
void SimplicialCholeskyBase<Derived>::analyzePattern_preordered(
    const CholMatrixType &ap, bool doLDLT) {
...
void SimplicialCholeskyBase<Derived>::factorize_preordered(
  ...
  for (StorageIndex k = 0; k < size; ++k) {
    ...
    for (typename CholMatrixType::InnerIterator it(ap, k); it; ++it) {
      ...
      if (i <= k) {
        y[i] += numext::conj(
            it.value()); /* scatter A(i,k) into Y (sum duplicates) */
        Index len;
    ...
    RealScalar d =
        numext::real(y[k]) * m_shiftScale +
        m_shiftOffset; // get D(k,k), apply the shift function, and clear Y(k)
    y[k] = Scalar(0);
    for (; top < size; ++top) {
