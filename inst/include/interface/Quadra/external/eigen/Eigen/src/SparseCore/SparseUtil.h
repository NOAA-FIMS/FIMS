...
#define EIGEN_SPARSE_INHERIT_ASSIGNMENT_OPERATOR(Derived, Op)                  \
  ...
  EIGEN_STRONG_INLINE Derived &operator Op(                                    \
      const Eigen::SparseMatrixBase<OtherDerived> &other) {                    \
...
struct eval<T, Sparse>
    : sparse_eval<T, traits<T>::RowsAtCompileTime, traits<T>::ColsAtCompileTime,
                  traits<T>::Flags> {};
...
struct solve_traits<Decomposition, RhsType, Sparse> {
  typedef typename sparse_eval<RhsType, RhsType::RowsAtCompileTime,
...
public:
  ...
  Triplet(const StorageIndex &i, const StorageIndex &j,
