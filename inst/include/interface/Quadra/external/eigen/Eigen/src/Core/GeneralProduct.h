...
template <int Size, int MaxSize> struct product_size_category {
  enum {
    ...
    is_large =
        MaxSize == Dynamic || Size >= EIGEN_CACHEFRIENDLY_PRODUCT_THRESHOLD ||
...
template <typename Lhs, typename Rhs> struct product_type {
  ...
  // the splitting into different lines of code here, introducing the _select
  // enums and the typedef below, is to work around an internal compiler error
...
// product ends up to a row-vector times col-vector product... To tackle this
 ...
 * other cases are handled using a simple loop along the outer-storage
 * direction. Therefore we need a lower level meta selector. Furthermore, if the
 * matrix is the rhs, then the product has to be transposed.
 */
namespace internal {
...
struct gemv_static_vector_if<Scalar, Size, MaxSize, true> {
  ...
  internal::plain_array<Scalar, EIGEN_SIZE_MIN_PREFER_FIXED(Size, MaxSize), 0,
                        EIGEN_PLAIN_ENUM_MIN(AlignedMax, PacketSize)>
      m_data;
  EIGEN_STRONG_INLINE Scalar *data() { return m_data.array; }
#else
  // Some architectures cannot align on the stack,
  // => let's manually enforce alignment by allocating more data and return the
  // address of the first aligned element.
  internal::plain_array<Scalar,
                        EIGEN_SIZE_MIN_PREFER_FIXED(Size, MaxSize) +
                            (ForceAlignment ? EIGEN_MAX_ALIGN_BYTES : 0),
                        0>
      m_data;
  EIGEN_STRONG_INLINE Scalar *data() {
    return ForceAlignment ? reinterpret_cast<Scalar *>(
                                (internal::UIntPtr(m_data.array) &
                                 ~(std::size_t(EIGEN_MAX_ALIGN_BYTES - 1))) +
...
struct gemv_dense_selector<OnTheLeft, StorageOrder, BlasCompatible> {
  ...
  static void run(const Lhs &lhs, const Rhs &rhs, Dest &dest,
...
template <> struct gemv_dense_selector<OnTheRight, ColMajor, true> {
  ...
  static inline void run(const Lhs &lhs, const Rhs &rhs, Dest &dest,
    ...
    typedef Map<Matrix<ResScalar, Dynamic, 1>,
                EIGEN_PLAIN_ENUM_MIN(AlignedMax,
                                     internal::packet_traits<ResScalar>::size)>
        MappedDest;
    ...
    typedef typename conditional<Dest::IsVectorAtCompileTime, Dest,
                                 typename Dest::ColXpr>::type ActualDest;

    enum {
      // FIXME find a way to allow an inner stride on the result if
      // packet_traits<Scalar>::size==1
      // on, the other hand it is good for the cache to pack the vector
      // anyways...
      EvalToDestAtCompileTime = (ActualDest::InnerStrideAtCompileTime == 1),
      ComplexByReal = (NumTraits<LhsScalar>::IsComplex) &&
                      (!NumTraits<RhsScalar>::IsComplex),
      MightCannotUseDest = ((!EvalToDestAtCompileTime) || ComplexByReal) &&
                           (ActualDest::MaxSizeAtCompileTime != 0)
    };
    ...
    if (!MightCannotUseDest) {
      ...
      // this ease the compiler to generate cleaner and more optimzized code for
      // most common cases
      general_matrix_vector_product<Index, LhsScalar, LhsMapper, ColMajor,
                                    LhsBlasTraits::NeedToConjugate, RhsScalar,
      ...
      gemv_static_vector_if<ResScalar, ActualDest::SizeAtCompileTime,
                            ActualDest::MaxSizeAtCompileTime,
                            MightCannotUseDest>
          static_dest;

      const bool alphaIsCompatible =
          (!ComplexByReal) || (numext::imag(actualAlpha) == RealScalar(0));
      const bool evalToDest = EvalToDestAtCompileTime && alphaIsCompatible;

      ei_declare_aligned_stack_constructed_variable(
          ResScalar, actualDestPtr, dest.size(),
...
template <> struct gemv_dense_selector<OnTheRight, RowMajor, true> {
  ...
  static void run(const Lhs &lhs, const Rhs &rhs, Dest &dest,
    ...
    typename add_const<ActualLhsType>::type actualLhs =
        LhsBlasTraits::extract(lhs);
    typename add_const<ActualRhsType>::type actualRhs =
    ...
    enum {
      ...
      DirectlyUseRhs = ActualRhsTypeCleaned::InnerStrideAtCompileTime == 1 ||
                       ActualRhsTypeCleaned::MaxSizeAtCompileTime == 0
    };
    ...
    general_matrix_vector_product<Index, LhsScalar, LhsMapper, RowMajor,
                                  LhsBlasTraits::NeedToConjugate, RhsScalar,
                                  RhsMapper, RhsBlasTraits::NeedToConjugate>::
        run(actualLhs.rows(), actualLhs.cols(),
            LhsMapper(actualLhs.data(), actualLhs.outerStride()),
            RhsMapper(actualRhsPtr, 1), dest.data(),
            dest.col(0).innerStride(), // NOTE  if dest is not a vector at
                                       // compile-time, then dest.innerStride()
...
template <> struct gemv_dense_selector<OnTheRight, ColMajor, false> {
  ...
  static void run(const Lhs &lhs, const Rhs &rhs, Dest &dest,
                  const typename Dest::Scalar &alpha) {
    EIGEN_STATIC_ASSERT(
        (!nested_eval<Lhs, 1>::Evaluate),
        EIGEN_INTERNAL_COMPILATION_ERROR_OR_YOU_MADE_A_PROGRAMMING_MISTAKE);
    // TODO if rhs is large enough it might be beneficial to make sure that dest
    // is sequentially stored in memory, otherwise use a temp
...
MatrixBase<Derived>::operator*(const MatrixBase<OtherDerived> &other) const {
  // A note regarding the function declaration: In MSVC, this function will
  // sometimes not be inlined since DenseStorage is an unwindable object for
  // dynamic matrices and product types are holding a member to store the
  // result. Thus it does not help tagging this function with
  // EIGEN_STRONG_INLINE.
  enum {
    ProductIsValid =
 ...
 * of the product will be computed once at a time as requested. This might be
 * useful in some extremely rare cases when only a small and no coherent
 * fraction of the result's coefficients have to be computed.
 *
 * \warning This version of the matrix product can be much much slower. So use
 * it only if you know what you are doing and that you measured a true speed
 * improvement.
 *
...
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const
    ...
    MatrixBase<Derived>::lazyProduct(
  ...
  return Product<Derived, OtherDerived, LazyProduct>(derived(),
                                                     other.derived());
}
