...
template <> struct cholmod_configure_matrix<double> {
  template <typename CholmodType> static void run(CholmodType &mat) {
...
viewAsCholmod(const SparseMatrix<_Scalar, _Options, _Index> &mat) {
  cholmod_sparse res = viewAsCholmod(
      Ref<SparseMatrix<_Scalar, _Options, _Index>>(mat.const_cast_derived()));
  return res;
}
...
cholmod_sparse viewAsCholmod(
    const SparseSelfAdjointView<const SparseMatrix<_Scalar, _Options, _Index>,
                                UpLo> &mat) {
  cholmod_sparse res =
      viewAsCholmod(Ref<SparseMatrix<_Scalar, _Options, _Index>>(
          mat.matrix().const_cast_derived()));
...
cholmod_dense viewAsCholmod(MatrixBase<Derived> &mat) {
  ...
  res.d = Derived::IsVectorAtCompileTime ? mat.derived().size()
...
public:
  CholmodBase()
      : m_cholmodFactor(0), m_info(Success), m_factorizationIsOk(false),
  ...
  void _solve_impl(const SparseMatrixBase<RhsDerived> &b,
    ...
    Ref<SparseMatrix<typename RhsDerived::Scalar, ColMajor,
  ...
  Scalar logDeterminant() const {
    ...
    if (m_cholmodFactor->is_super) {
      // Supernodal factorization stored as a packed list of dense column-major
      // blocs, as described by the following structure:
      ...
      for (Index k = 0; k < nb_super_nodes; ++k) {
        ...
        Map<const Array<Scalar, 1, Dynamic>, 0, InnerStride<>> sk(
  ...
  template <typename Stream> void dumpMemory(Stream & /*s*/) {}
 ...
 * simplicial LL^T Cholesky factorization using the Cholmod library. This
 * simplicial variant is equivalent to Eigen's built-in SimplicialLLT class.
 * Therefore, it has little practical interest. The sparse matrix A must be
 * selfadjoint and positive definite. The vectors or matrices X and B can be
 * either dense or sparse.
 *
 * \tparam _MatrixType the type of the sparse matrix A, it must be a
 * SparseMatrix<> \tparam _UpLo the triangular part that will be used for the
 * computations. It can be Lower or Upper. Default is Lower.
 *
 * \implsparsesolverconcept
 *
 * This class supports all kind of SparseMatrix<>: row or column major; upper,
 * lower, or both; compressed or non compressed.
 *
 * \warning Only double precision real and complex scalar types are supported by
 * Cholmod.
 *
 ...
 *
 * This class allows to solve for A.X = B sparse linear problems via a
 * simplicial LDL^T Cholesky factorization using the Cholmod library. This
 * simplicial variant is equivalent to Eigen's built-in SimplicialLDLT class.
 * Therefore, it has little practical interest. The sparse matrix A must be
 * selfadjoint and positive definite. The vectors or matrices X and B can be
 * either dense or sparse.
 *
 ...
 *
 * This class allows to solve for A.X = B sparse linear problems via a
 * supernodal LL^T Cholesky factorization using the Cholmod library. This
 * supernodal variant performs best on dense enough problems, e.g., 3D FEM, or
 * very high order 2D FEM. The sparse matrix A must be selfadjoint and positive
 * definite. The vectors or matrices X and B can be either dense or sparse.
 *
 * \tparam _MatrixType the type of the sparse matrix A, it must be a
 * SparseMatrix<> \tparam _UpLo the triangular part that will be used for the
 * computations. It can be Lower or Upper. Default is Lower.
 ...
 *
 * This class allows to solve for A.X = B sparse linear problems via a LL^T or
 * LDL^T Cholesky factorization using the Cholmod library. The sparse matrix A
 * must be selfadjoint and positive definite. The vectors or matrices X and B
 * can be either dense or sparse.
 *
 * This variant permits to change the underlying Cholesky method at runtime.
 * On the other hand, it does not provide access to the result of the
 * factorization. The default is to let Cholmod automatically choose between a
 * simplicial and supernodal factorization.
 *
 * \tparam _MatrixType the type of the sparse matrix A, it must be a
 * SparseMatrix<> \tparam _UpLo the triangular part that will be used for the
 * computations. It can be Lower or Upper. Default is Lower.
