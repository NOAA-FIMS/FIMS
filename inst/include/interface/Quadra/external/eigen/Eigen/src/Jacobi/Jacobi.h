 ...
 * \f$ J = \left ( \begin{array}{cc} c & \overline s \\ -s  & \overline c
 * \end{array} \right ) \f$
 *
 * You can apply the respective counter-clockwise rotation to a column vector \c
 * v by applying its adjoint on the left: \f$ v = J^* v \f$ that translates to
 * the following Eigen code: \code v.applyOnTheLeft(J.adjoint()); \endcode
 *
...
public:
  ...
  JacobiRotation operator*(const JacobiRotation &other) {
    ...
    return JacobiRotation(
        m_c * other.m_c - conj(m_s) * other.m_s,
  ...
  EIGEN_DEVICE_FUNC bool makeJacobi(const MatrixBase<Derived> &, Index p,
                                    Index q);
  EIGEN_DEVICE_FUNC
 ...
 *
 * \sa MatrixBase::makeJacobi(const MatrixBase<Derived>&, Index, Index),
 * MatrixBase::applyOnTheLeft(), MatrixBase::applyOnTheRight()
 */
template <typename Scalar>
EIGEN_DEVICE_FUNC bool JacobiRotation<Scalar>::makeJacobi(const RealScalar &x,
                                                          const Scalar &y,
                                                          const RealScalar &z) {
  using std::abs;
  using std::sqrt;
 ...
 *
 * \sa JacobiRotation::makeJacobi(RealScalar, Scalar, RealScalar),
 * MatrixBase::applyOnTheLeft(), MatrixBase::applyOnTheRight()
 */
template <typename Scalar>
template <typename Derived>
EIGEN_DEVICE_FUNC inline bool
JacobiRotation<Scalar>::makeJacobi(const MatrixBase<Derived> &m, Index p,
                                   Index q) {
  return makeJacobi(numext::real(m.coeff(p, p)), m.coeff(p, q),
 ...
 * found in Anderson (2000), Discontinuous Plane Rotations and the Symmetric
 * Eigenvalue Problem. LAPACK Working Note 150, University of Tennessee,
 * UT-CS-00-454, December 4, 2000.
 *
 * \sa MatrixBase::applyOnTheLeft(), MatrixBase::applyOnTheRight()
 */
template <typename Scalar>
EIGEN_DEVICE_FUNC void JacobiRotation<Scalar>::makeGivens(const Scalar &p,
                                                          const Scalar &q,
                                                          Scalar *r) {
  makeGivens(p, q, r,
...
EIGEN_DEVICE_FUNC void
apply_rotation_in_the_plane(DenseBase<VectorX> &xpr_x,
 ...
 *
 * \sa class JacobiRotation, MatrixBase::applyOnTheRight(),
 * internal::apply_rotation_in_the_plane()
 */
template <typename Derived>
template <typename OtherScalar>
EIGEN_DEVICE_FUNC inline void
MatrixBase<Derived>::applyOnTheLeft(Index p, Index q,
                                    const JacobiRotation<OtherScalar> &j) {
  RowXpr x(this->row(p));
  RowXpr y(this->row(q));
 ...
 *
 * \sa class JacobiRotation, MatrixBase::applyOnTheLeft(),
 * internal::apply_rotation_in_the_plane()
 */
template <typename Derived>
template <typename OtherScalar>
EIGEN_DEVICE_FUNC inline void
MatrixBase<Derived>::applyOnTheRight(Index p, Index q,
                                     const JacobiRotation<OtherScalar> &j) {
  ColXpr x(this->col(p));
  ColXpr y(this->col(q));
...
template <typename Scalar, typename OtherScalar, int SizeAtCompileTime,
          int MinAlignment, bool Vectorizable>
struct apply_rotation_in_the_plane_selector {
  static EIGEN_DEVICE_FUNC inline void run(Scalar *x, Index incrx, Scalar *y,
                                           Index incry, Index size,
...
struct apply_rotation_in_the_plane_selector<Scalar, OtherScalar,
  ...
  static inline void run(Scalar *x, Index incrx, Scalar *y, Index incry,
    ...
    if (SizeAtCompileTime == Dynamic &&
      ...
      conj_helper<OtherPacket, Packet, NumTraits<OtherScalar>::IsComplex, false>
          pcj;
      ...
      if (internal::first_default_aligned(x, size) == alignedStart) {
        ...
        Index peelingEnd =
            alignedStart + ((size - alignedStart) / (Peeling * PacketSize)) *
                               (Peeling * PacketSize);
        for (Index i = alignedStart; i < peelingEnd;
                 ...
                 0) // FIXME should be compared to the required alignment
    {
      const OtherPacket pc = pset1<OtherPacket>(c);
      const OtherPacket ps = pset1<OtherPacket>(s);
      conj_helper<OtherPacket, Packet, NumTraits<OtherPacket>::IsComplex, false>
          pcj;
      conj_helper<OtherPacket, Packet, false, false> pm;
    ...
    /*** non-vectorized path ***/
      ...
      apply_rotation_in_the_plane_selector<Scalar, OtherScalar,
                                           SizeAtCompileTime, MinAlignment,
                                           false>::run(x, incrx, y, incry, size,
                                                       c, s);
    }
  }
};
...
apply_rotation_in_the_plane(DenseBase<VectorX> &xpr_x,
  ...
  const bool Vectorizable =
      (int(VectorX::Flags) & int(VectorY::Flags) & PacketAccessBit) &&
  ...
  apply_rotation_in_the_plane_selector<
      Scalar, OtherScalar, VectorX::SizeAtCompileTime,
