...
template <> EIGEN_STRONG_INLINE Packet2cf pconj(const Packet2cf &a) {
  const __m128 mask = _mm_castsi128_ps(
      _mm_setr_epi32(0x00000000, 0x80000000, 0x00000000, 0x80000000));
  return Packet2cf(_mm_xor_ps(a.v, mask));
}
...
EIGEN_STRONG_INLINE Packet2cf pmul<Packet2cf>(const Packet2cf &a,
  ...
  //   return Packet2cf(_mm_addsub_ps(_mm_mul_ps(vec4f_swizzle1(a.v, 0, 0, 2,
  //   2), b.v),
  //                                  _mm_mul_ps(vec4f_swizzle1(a.v, 1, 1, 3,
  //                                  3),
  //                                             vec4f_swizzle1(b.v, 1, 0, 3,
  //                                             2))));
#else
  const __m128 mask = _mm_castsi128_ps(
      _mm_setr_epi32(0x80000000, 0x00000000, 0x80000000, 0x00000000));
  return Packet2cf(
...
template <> EIGEN_STRONG_INLINE Packet2cf ptrue<Packet2cf>(const Packet2cf &a) {
...
#ifdef EIGEN_VECTORIZE_SSE3
  res.v =
      _mm_castpd_ps(_mm_loaddup_pd(reinterpret_cast<double const *>(&from)));
#else
...
pgather<std::complex<float>, Packet2cf>(const std::complex<float> *from,
                                        Index stride) {
  return Packet2cf(
      _mm_set_ps(std::imag(from[1 * stride]), std::real(from[1 * stride]),
...
pscatter<std::complex<float>, Packet2cf>(std::complex<float> *to,
  ...
  to[stride * 0] =
      std::complex<float>(_mm_cvtss_f32(_mm_shuffle_ps(from.v, from.v, 0)),
                          _mm_cvtss_f32(_mm_shuffle_ps(from.v, from.v, 1)));
  to[stride * 1] =
      std::complex<float>(_mm_cvtss_f32(_mm_shuffle_ps(from.v, from.v, 2)),
...
EIGEN_STRONG_INLINE std::complex<float> pfirst<Packet2cf>(const Packet2cf &a) {
  ...
  // Workaround gcc 4.2 ICE - this is not performance wise ideal, but who
  // cares... This workaround also fix invalid code generation with gcc 4.3
...
EIGEN_STRONG_INLINE Packet2cf pdiv<Packet2cf>(const Packet2cf &a,
  ...
  return Packet2cf(
      _mm_div_ps(res.v, _mm_add_ps(s, vec4f_swizzle1(s, 1, 0, 3, 2))));
}
...
EIGEN_STRONG_INLINE Packet1cd pmul<Packet1cd>(const Packet1cd &a,
                                              const Packet1cd &b) {
#ifdef EIGEN_VECTORIZE_SSE3
  return Packet1cd(_mm_addsub_pd(
      _mm_mul_pd(_mm_movedup_pd(a.v), b.v),
...
template <> EIGEN_STRONG_INLINE Packet1cd ptrue<Packet1cd>(const Packet1cd &a) {
...
EIGEN_STRONG_INLINE Packet2cf pblend(const Selector<2> &ifPacket,
  ...
  __m128d result = pblend<Packet2d>(ifPacket, _mm_castps_pd(thenPacket.v),
                                    _mm_castps_pd(elsePacket.v));
