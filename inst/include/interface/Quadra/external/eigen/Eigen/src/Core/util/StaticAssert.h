 ...
 *    in that case, the static assertion is converted to the following runtime
 * assert: eigen_assert(CONDITION && "MSG")
 *
 ...
 */
    ...
    (__has_feature(cxx_static_assert) || (EIGEN_COMP_CXXVER >= 11) ||          \
...
#define EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(TYPE, SIZE)                   \
  EIGEN_STATIC_ASSERT(TYPE::IsVectorAtCompileTime &&TYPE::SizeAtCompileTime == \
                          SIZE,                                                \
...
// static assertion failing if the two vector expression types are not
// compatible (same fixed-size or dynamic size)
...
#define EIGEN_STATIC_ASSERT_NON_INTEGER(TYPE)                                  \
  EIGEN_STATIC_ASSERT(!Eigen::NumTraits<TYPE>::IsInteger,                      \
                      THIS_FUNCTION_IS_NOT_FOR_INTEGER_NUMERIC_TYPES)

// static assertion failing if it is guaranteed at compile-time that the two
// matrix expression types have different sizes
#define EIGEN_STATIC_ASSERT_SAME_MATRIX_SIZE(TYPE0, TYPE1)                     \
  EIGEN_STATIC_ASSERT(EIGEN_PREDICATE_SAME_MATRIX_SIZE(TYPE0, TYPE1),          \
                      YOU_MIXED_MATRICES_OF_DIFFERENT_SIZES)

#define EIGEN_STATIC_ASSERT_SIZE_1x1(TYPE)                                     \
  EIGEN_STATIC_ASSERT((TYPE::RowsAtCompileTime == 1 ||                         \
                       TYPE::RowsAtCompileTime == Eigen::Dynamic) &&           \
                          (TYPE::ColsAtCompileTime == 1 ||                     \
...
#define EIGEN_STATIC_ASSERT_ARRAYXPR(Derived)                                  \
  EIGEN_STATIC_ASSERT((Eigen::internal::is_same<                               \
                          typename Eigen::internal::traits<Derived>::XprKind,  \
                          ArrayXpr>::value),                                   \
...
#define EIGEN_STATIC_ASSERT_SAME_XPR_KIND(Derived1, Derived2)                  \
  EIGEN_STATIC_ASSERT(                                                         \
      (Eigen::internal::is_same<                                               \
          typename Eigen::internal::traits<Derived1>::XprKind,                 \
          typename Eigen::internal::traits<Derived2>::XprKind>::value),        \
      YOU_CANNOT_MIX_ARRAYS_AND_MATRICES)

// Check that a cost value is positive, and that is stay within a reasonable
// range
// TODO this check could be enabled for internal debugging only
#define EIGEN_INTERNAL_CHECK_COST_VALUE(C)                                     \
  EIGEN_STATIC_ASSERT(                                                         \
      (C) >= 0 && (C) <= HugeCost * HugeCost,                                  \
