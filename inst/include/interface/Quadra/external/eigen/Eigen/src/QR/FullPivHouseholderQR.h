 ...
 *
 * \brief Householder rank-revealing QR decomposition of a matrix with full
 * pivoting
 *
 * \tparam _MatrixType the type of the matrix of which we are computing the QR
 * decomposition
 *
 * This class performs a rank-revealing QR decomposition of a matrix \b A into
 * matrices \b P, \b P', \b Q and \b R such that \f[ \mathbf{P} \, \mathbf{A} \,
 * \mathbf{P}' = \mathbf{Q} \, \mathbf{R} \f] by using Householder
 * transformations. Here, \b P and \b P' are permutation matrices, \b Q a
 * unitary matrix and \b R an upper triangular matrix.
 *
 * This decomposition performs a very prudent full pivoting in order to be
 * rank-revealing and achieve optimal numerical stability. The trade-off is that
 * it is slower than HouseholderQR and ColPivHouseholderQR.
 *
...
public:
  ...
  typedef internal::FullPivHouseholderQRMatrixQReturnType<MatrixType>
      MatrixQReturnType;
  ...
  FullPivHouseholderQR()
      : m_qr(), m_hCoeffs(), m_rows_transpositions(), m_cols_transpositions(),
        m_cols_permutation(), m_temp(), m_isInitialized(false),
  ...
  FullPivHouseholderQR(Index rows, Index cols)
      : m_qr(rows, cols), m_hCoeffs((std::min)(rows, cols)),
        m_rows_transpositions((std::min)(rows, cols)),
        m_cols_transpositions((std::min)(rows, cols)), m_cols_permutation(cols),
        m_temp(cols), m_isInitialized(false), m_usePrescribedThreshold(false) {}
  ...
  explicit FullPivHouseholderQR(const EigenBase<InputType> &matrix)
      : m_qr(matrix.rows(), matrix.cols()),
        ...
        m_cols_permutation(matrix.cols()), m_temp(matrix.cols()),
   ...
   *
   * \returns the exact or least-square solution if the rank is greater or equal
   * to the number of columns of A, and an arbitrary solution otherwise.
   *
...
void FullPivHouseholderQR<MatrixType>::computeInPlace() {
  ...
  for (Index k = 0; k < size; ++k) {
    ...
    RealScalar biggest_in_corner = internal::abs_knowing_score<Scalar>()(
        m_qr(row_of_biggest_in_corner, col_of_biggest_in_corner), score);
    if (k == 0)
      biggest = biggest_in_corner;

    // if the corner is negligible, then we have less than full rank, and we can
    // finish early
    if (internal::isMuchSmallerThan(biggest_in_corner, biggest, m_precision)) {
      m_nonzero_pivots = k;
      for (Index i = k; i < size; i++) {
        m_rows_transpositions.coeffRef(i) =
            internal::convert_index<StorageIndex>(i);
        m_cols_transpositions.coeffRef(i) =
            internal::convert_index<StorageIndex>(i);
    ...
    m_cols_transpositions.coeffRef(k) =
        internal::convert_index<StorageIndex>(col_of_biggest_in_corner);
    if (k != row_of_biggest_in_corner) {
      m_qr.row(k).tail(cols - k).swap(
          m_qr.row(row_of_biggest_in_corner).tail(cols - k));
    ...
    RealScalar beta;
    m_qr.col(k).tail(rows - k).makeHouseholderInPlace(m_hCoeffs.coeffRef(k),
                                                      beta);
    ...
    m_qr.bottomRightCorner(rows - k, cols - k - 1)
        .applyHouseholderOnTheLeft(m_qr.col(k).tail(rows - k - 1),
  ...
  for (Index k = 0; k < size; ++k)
    m_cols_permutation.applyTranspositionOnTheRight(
        k, m_cols_transpositions.coeff(k));
...
template <typename _MatrixType>
template <typename RhsType, typename DstType>
void FullPivHouseholderQR<_MatrixType>::_solve_impl(const RhsType &rhs,
                                                    DstType &dst) const {
  ...
  Matrix<typename RhsType::Scalar, 1, RhsType::ColsAtCompileTime> temp(
      rhs.cols());
...
struct Assignment<
    DstXprType, Inverse<FullPivHouseholderQR<MatrixType>>,
    internal::assign_op<typename DstXprType::Scalar,
                        typename FullPivHouseholderQR<MatrixType>::Scalar>,
    Dense2Dense> {
...
public:
  typedef typename FullPivHouseholderQR<MatrixType>::IntDiagSizeVectorType
      IntDiagSizeVectorType;
  typedef typename internal::plain_diag_type<MatrixType>::type HCoeffsType;
  typedef Matrix<typename MatrixType::Scalar, 1, MatrixType::RowsAtCompileTime,
                 RowMajor, 1, MatrixType::MaxRowsAtCompileTime>
      WorkVectorType;

  FullPivHouseholderQRMatrixQReturnType(
      const MatrixType &qr, const HCoeffsType &hCoeffs,
...
//  : public
//  evaluator<ReturnByValue<FullPivHouseholderQRMatrixQReturnType<MatrixType> >
//  >
// {};
