...
pcast<half2, float4>(const half2 &a, const half2 &b) {
...
pcast<float4, Packet4h2>(const float4 &a, const float4 &b) {
  Packet4h2 r;
...
template <>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE float4
pcast<Packet4h2, float4>(const Packet4h2 &a) {
...
pcast<float4, half2>(const float4 &a) {
  // Simply discard the second half of the input
