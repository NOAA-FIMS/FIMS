...
struct traits<PartialPivLU<_MatrixType>> : traits<_MatrixType> {
 ...
 * \class PartialPivLU
 *
 * \brief LU decomposition of a matrix with partial pivoting, and related
 * features
 *
 * \tparam _MatrixType the type of the matrix of which we are computing the LU
 * decomposition
 *
 * This class represents a LU decomposition of a \b square \b invertible matrix,
 * with partial pivoting: the matrix A is decomposed as A = PLU where L is
 * unit-lower-triangular, U is upper-triangular, and P is a permutation matrix.
 *
 * Typically, partial pivoting LU decomposition is only considered numerically
 * stable for square invertible matrices. Thus LAPACK's dgesv and dgesvx require
 * the matrix to be square and invertible. The present class does the same. It
 * will assert that the matrix is square, but it won't (actually it can't) check
 * that the matrix is invertible: it is your task to check that you only use
 * this decomposition on invertible matrices.
 *
 * The guaranteed safe alternative, working for all matrices, is the full
 * pivoting LU decomposition, provided by class FullPivLU.
 *
 * This is \b not a rank-revealing LU decomposition. Many features are
 * intentionally absent from this class, such as rank computation. If you need
 * these features, use class FullPivLU.
 *
 * This LU decomposition is suitable to invert invertible matrices. It is what
 * MatrixBase::inverse() uses in the general case. On the other hand, it is \b
 * not suitable to determine whether a given matrix is invertible.
 *
 * The data of the LU decomposition can be directly accessed through the methods
 * matrixLU(), permutationP().
 *
 * This class supports the \link InplaceDecomposition inplace decomposition
 * \endlink mechanism.
 *
 * \sa MatrixBase::partialPivLu(), MatrixBase::determinant(),
 * MatrixBase::inverse(), MatrixBase::computeInverse(), class FullPivLU
 */
template <typename _MatrixType>
class PartialPivLU : public SolverBase<PartialPivLU<_MatrixType>> {
public:
   ...
   *
   * \warning The matrix should have full rank (e.g. if it's square, it should
   * be invertible). If you need to deal with non-full rank, use class FullPivLU
   * instead.
   */
   ...
   *
   * Since this PartialPivLU class assumes anyway that the matrix A is
   * invertible, the solution theoretically exists and is unique regardless of
   * b.
   *
  ...
  EIGEN_DEVICE_FUNC void _solve_impl_transposed(const RhsType &rhs,
    ...
    // Step 1
    dst = m_lu.template triangularView<Upper>()
              .transpose()
...
PartialPivLU<MatrixType>::PartialPivLU()
    : m_lu(), m_p(), m_rowsTranspositions(), m_l1_norm(0), m_det_p(0),
      m_isInitialized(false) {}

template <typename MatrixType>
PartialPivLU<MatrixType>::PartialPivLU(Index size)
    : m_lu(size, size), m_p(size), m_rowsTranspositions(size), m_l1_norm(0),
      m_det_p(0), m_isInitialized(false) {}

template <typename MatrixType>
template <typename InputType>
PartialPivLU<MatrixType>::PartialPivLU(const EigenBase<InputType> &matrix)
    : m_lu(matrix.rows(), matrix.cols()), m_p(matrix.rows()),
      m_rowsTranspositions(matrix.rows()), m_l1_norm(0), m_det_p(0),
      m_isInitialized(false) {
...
PartialPivLU<MatrixType>::PartialPivLU(EigenBase<InputType> &matrix)
    : m_lu(matrix.derived()), m_p(matrix.rows()),
      m_rowsTranspositions(matrix.rows()), m_l1_norm(0), m_det_p(0),
      m_isInitialized(false) {
  compute();
}
...
struct partial_lu_impl {
  ...
  static const int ActualSizeAtCompileTime =
      UnBlockedAtCompileTime ? SizeAtCompileTime : Dynamic;
  ...
  typedef Matrix<Scalar, ActualSizeAtCompileTime, ActualSizeAtCompileTime,
                 StorageOrder>
      MatrixType;
   ...
   *
   * \returns The index of the first pivot which is exactly zero if any, or a
   * negative number otherwise.
   */
  static Index unblocked_lu(MatrixTypeRef &lu, PivIndex *row_transpositions,
    ...
    // For small compile-time matrices it is worth processing the last row
    // separately:
    ...
    for (Index k = 0; k < endk; ++k) {
      ...
      Score biggest_in_corner =
          lu.col(k).tail(rows - k).unaryExpr(Scoring()).maxCoeff(
              &row_of_biggest_in_col);
      ...
      if (biggest_in_corner != Score(0)) {
        ...
        // the pivot is exactly zero, we record the index of the first pivot
        // which is exactly 0, and continue the factorization such we still have
        // A = PLU
        first_zero_pivot = k;
      }
   ...
   *   2 - avoid infinite recursion of the instantiations with
   * Block<Block<Block<...> > >
   */
  static Index blocked_lu(Index rows, Index cols, Scalar *lu_data,
    ...
    for (Index k = 0; k < size; k += blockSize) {
      ...
      for (Index i = k; i < k + bs; ++i) {
        Index piv =
            (row_transpositions[i] += internal::convert_index<PivIndex>(k));
...
void partial_lu_inplace(
  ...
  partial_lu_impl<typename MatrixType::Scalar,
                  ...
                  EIGEN_SIZE_MIN_PREFER_FIXED(MatrixType::RowsAtCompileTime,
                                              MatrixType::ColsAtCompileTime)>::
      blocked_lu(lu.rows(), lu.cols(), &lu.coeffRef(0, 0), lu.outerStride(),
                 &row_transpositions.coeffRef(0), nb_transpositions);
}
...
template <typename MatrixType>
typename PartialPivLU<MatrixType>::Scalar
PartialPivLU<MatrixType>::determinant() const {
  eigen_assert(m_isInitialized && "PartialPivLU is not initialized.");
...
struct Assignment<
    DstXprType, Inverse<PartialPivLU<MatrixType>>,
