 ...
 *
 * \brief Reduces a square matrix to Hessenberg form by an orthogonal similarity
 * transformation
 *
...
public:
  ...
  explicit HessenbergDecomposition(const EigenBase<InputType> &matrix)
      : m_matrix(matrix.derived()), m_temp(matrix.rows()),
   ...
   *householderCoefficients(), allows to reconstruct the matrix Q as \f$ Q =
   *H_{N-1} \ldots H_1 H_0 \f$. Here, the matrices \f$ H_i \f$ are the
   *Householder transformations \f$ H_i = (I - h_i v_i v_i^T) \f$ where \f$ h_i
   *\f$ is the \f$ i \f$th Householder coefficient and \f$ v_i \f$ is the
   *Householder vector defined by \f$ v_i = [ 0, \ldots, 0, 1, M(i+2,i), \ldots,
   *M(N-1,i) ]^T \f$ with M the matrix returned by this function.
   *
...
private:
  typedef Matrix<Scalar, 1, Size, int(Options) | int(RowMajor), 1, MaxSize>
      VectorType;
...
void HessenbergDecomposition<MatrixType>::_compute(MatrixType &matA,
  ...
  for (Index i = 0; i < n - 1; ++i) {
    ...
    matA.bottomRightCorner(remainingSize, remainingSize)
        .applyHouseholderOnTheLeft(matA.col(i).tail(remainingSize - 1), h,
                                   &temp.coeffRef(0));
...
public:
  ...
  template <typename ResultType> inline void evalTo(ResultType &result) const {
    ...
    Index n = result.rows();
    if (n > 2)
      result.bottomLeftCorner(n - 2, n - 2)
