...
void parallelize_gemm(const Functor &func, Index rows, Index cols, Index depth,
  ...
  // See bug 1572.
    ...
    ((!EIGEN_HAS_CXX11_ATOMIC) && !(EIGEN_ARCH_i386_OR_x86_64))
  ...
  // This first heuristic takes into account that the product kernel is fully
  ...
  pb_max_threads = std::max<Index>(
      1,
      std::min<Index>(pb_max_threads, static_cast<Index>(work / kMinTaskSize)));

  // compute the number of threads we are going to use
  Index threads = std::min<Index>(nbThreads(), pb_max_threads);

  // if multi-threading is explicitly disabled, not useful, or if we already are
  // in a parallel session, then abort multi-threading
  // FIXME omp_get_num_threads()>1 only works for openmp, what if the user does
  // not use openmp?
  ...
  ei_declare_aligned_stack_constructed_variable(GemmParallelInfo<Index>, info,
                                                threads, 0);
