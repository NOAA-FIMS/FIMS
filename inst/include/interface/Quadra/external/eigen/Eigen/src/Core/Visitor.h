...
public:
  EIGEN_DEVICE_FUNC
  explicit visitor_evaluator(const XprType &xpr)
 ...
 * following interface: \code struct MyVisitor {
  ...
  enum {
    unroll = SizeAtCompileTime != Dynamic &&
             SizeAtCompileTime * int(ThisEvaluator::CoeffReadCost) +
                     (SizeAtCompileTime - 1) *
                         int(internal::functor_traits<Visitor>::Cost) <=
...
template <typename Derived> struct coeff_visitor {
  // default initialization to avoid countless invalid maybe-uninitialized
  // warnings by gcc
  EIGEN_DEVICE_FUNC
...
struct min_coeff_visitor<Derived, PropagateNumbers> : coeff_visitor<Derived> {
  ...
  EIGEN_DEVICE_FUNC
  void operator()(const Scalar &value, Index i, Index j) {
    if ((numext::isnan)(this->res) ||
        (!(numext::isnan)(value) && value < this->res)) {
      this->res = value;
      this->row = i;
      this->col = j;
    }
 ...
 * triggered.
 *
 * \sa DenseBase::minCoeff(Index*), DenseBase::maxCoeff(Index*,Index*),
 * DenseBase::visit(), DenseBase::minCoeff()
 */
template <typename Derived>
 ...
 * triggered.
 *
 * \sa DenseBase::minCoeff(IndexType*,IndexType*),
 * DenseBase::maxCoeff(IndexType*,IndexType*), DenseBase::visit(),
 * DenseBase::minCoeff()
 */
template <typename Derived>
template <int NaNPropagation, typename IndexType>
...
DenseBase<Derived>::minCoeff(IndexType *index) const {
  ...
  this->visit(minVisitor);
  *index =
      IndexType((RowsAtCompileTime == 1) ? minVisitor.col : minVisitor.row);
  return minVisitor.res;
}
 ...
 * triggered.
 *
 * \sa DenseBase::minCoeff(IndexType*,IndexType*), DenseBase::visit(),
 * DenseBase::maxCoeff()
 */
template <typename Derived>
template <int NaNPropagation, typename IndexType>
 ...
 * triggered.
 *
 * \sa DenseBase::maxCoeff(IndexType*,IndexType*),
 * DenseBase::minCoeff(IndexType*,IndexType*), DenseBase::visitor(),
 * DenseBase::maxCoeff()
 */
template <typename Derived>
template <int NaNPropagation, typename IndexType>
