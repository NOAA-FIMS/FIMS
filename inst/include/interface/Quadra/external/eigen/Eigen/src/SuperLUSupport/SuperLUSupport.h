...
#define DECL_GSSVX(PREFIX, FLOATTYPE, KEYTYPE)                                 \
  extern "C" {                                                                 \
  extern void PREFIX##gssvx(superlu_options_t *, SuperMatrix *, int *, int *,  \
                            int *, char *, FLOATTYPE *, FLOATTYPE *,           \
                            SuperMatrix *, SuperMatrix *, void *, int,         \
                            SuperMatrix *, SuperMatrix *, FLOATTYPE *,         \
  ...
  inline float SuperLU_gssvx(                                                  \
      ...
      int *etree, char *equed, FLOATTYPE *R, FLOATTYPE *C, SuperMatrix *L,     \
      SuperMatrix *U, void *work, int lwork, SuperMatrix *B, SuperMatrix *X,   \
      FLOATTYPE *recip_pivot_growth, FLOATTYPE *rcond, FLOATTYPE *ferr,        \
      FLOATTYPE *berr, SuperLUStat_t *stats, int *info, KEYTYPE) {             \
...
#define DECL_GSISX(PREFIX, FLOATTYPE, KEYTYPE)                                 \
  extern "C" {                                                                 \
  extern void PREFIX##gsisx(superlu_options_t *, SuperMatrix *, int *, int *,  \
                            int *, char *, FLOATTYPE *, FLOATTYPE *,           \
                            SuperMatrix *, SuperMatrix *, void *, int,         \
                            SuperMatrix *, SuperMatrix *, FLOATTYPE *,         \
                            FLOATTYPE *, mem_usage_t *, SuperLUStat_t *,       \
                            int *);                                            \
  }                                                                            \
  inline float SuperLU_gsisx(                                                  \
      superlu_options_t *options, SuperMatrix *A, int *perm_c, int *perm_r,    \
      int *etree, char *equed, FLOATTYPE *R, FLOATTYPE *C, SuperMatrix *L,     \
      SuperMatrix *U, void *work, int lwork, SuperMatrix *B, SuperMatrix *X,   \
      FLOATTYPE *recip_pivot_growth, FLOATTYPE *rcond, SuperLUStat_t *stats,   \
      int *info, KEYTYPE) {                                                    \
    mem_usage_t mem_usage;                                                     \
...
template <typename MatrixType> struct SluMatrixMapHelper;
 ...
 *
 * This wrapper class mainly aims to avoids the need of dynamic allocation of
...
struct SluMatrix : SuperMatrix {
  ...
  template <typename Scalar> void setScalarType() {
  ...
  static SluMatrix Map(MatrixBase<MatrixType> &_mat) {
    ...
    res.storage.lda = internal::convert_index<int>(
        MatrixType::IsVectorAtCompileTime ? mat.size() : mat.outerStride());
...
template <typename Scalar, int Rows, int Cols, int Options, int MRows,
          int MCols>
...
template <typename MatrixType> SluMatrix asSluMatrix(MatrixType &mat) {
 ...
 * factorization using the SuperLU library. The sparse matrix A must be squared
 * and invertible. The vectors or matrices X and B can be either dense or
 * sparse.
 *
 * \tparam _MatrixType the type of the sparse matrix A, it must be a
 * SparseMatrix<>
 *
 * \warning This class is only for the 4.x versions of SuperLU. The 3.x and 5.x
...
void SuperLU<MatrixType>::factorize(const MatrixType &a) {
  ...
  SuperLU_gssvx(&m_sluOptions, &m_sluA, m_q.data(), m_p.data(), &m_sluEtree[0],
                &m_sluEqued, &m_sluRscale[0], &m_sluCscale[0], &m_sluL, &m_sluU,
                NULL, 0, &m_sluB, &m_sluX, &recip_pivot_growth, &rcond, &ferr,
...
template <typename MatrixType>
template <typename Rhs, typename Dest>
void SuperLU<MatrixType>::_solve_impl(const MatrixBase<Rhs> &b,
...
void SuperLUBase<MatrixType, Derived>::extractData() const {
  eigen_assert(
      m_factorizationIsOk &&
      "The decomposition is not in a valid state for extracting factors, you "
...
typename SuperLU<MatrixType>::Scalar SuperLU<MatrixType>::determinant() const {
  eigen_assert(m_factorizationIsOk &&
               "The decomposition is not in a valid state for computing the "
               "determinant, you must first call either compute() or "
  ...
  if (PermutationMap(m_p.data(), m_p.size()).determinant() *
          PermutationMap(m_q.data(), m_q.size()).determinant() <
      0)
    det = -det;
  if (m_sluEqued != 'N')
 ...
 * SuperLU library
 *
 * This class allows to solve for an approximate solution of A.X = B sparse
 * linear problems via an incomplete LU factorization using the SuperLU library.
 * This class is aimed to be used as a preconditioner of the iterative linear
 * solvers.
 *
 * \warning This class is only for the 4.x versions of SuperLU. The 3.x and 5.x
 * versions are not supported.
 ...
 * \implsparsesolverconcept
 *
 * \sa \ref TutorialSparseSolverConcept, class IncompleteLUT, class
 * ConjugateGradient, class BiCGSTAB
 */
...
template <typename MatrixType>
template <typename Rhs, typename Dest>
void SuperILU<MatrixType>::_solve_impl(const MatrixBase<Rhs> &b,
                                       MatrixBase<Dest> &x) const {
  eigen_assert(
      m_factorizationIsOk &&
      "The decomposition is not in a valid state for solving, you must first "
