 ...
 * and manipulate the supernodes during the factorization and solution phase of
 * Sparse LU. Only the lower triangular matrix has supernodes.
 *
...
public:
  ...
  MappedSuperNodalMatrix(Index m, Index n, ScalarVector &nzval,
                         IndexVector &nzval_colptr, IndexVector &rowind,
                         IndexVector &rowind_colptr, IndexVector &col_to_sup,
                         IndexVector &sup_to_col) {
    setInfos(m, n, nzval, nzval_colptr, rowind, rowind_colptr, col_to_sup,
             sup_to_col);
  }
...
protected:
  ...
  StorageIndex *m_nzval_colptr; // nzval_colptr[j] Stores the location in
  ...
  StorageIndex *m_col_to_sup; // col_to_sup[j] is the supernode number to which
                              // column j belongs
  StorageIndex *m_sup_to_col; // sup_to_col[s] points to the starting column of
                              // the s-th supernode
...
public:
  InnerIterator(const MappedSuperNodalMatrix &mat, Index outer)
      : m_matrix(mat), m_outer(outer), m_supno(mat.colToSup()[outer]),
        m_idval(mat.colIndexPtr()[outer]), m_startidval(m_idval),
        m_endidval(mat.colIndexPtr()[outer + 1]),
        m_idrow(mat.rowIndexPtr()[mat.supToCol()[mat.colToSup()[outer]]]),
        m_endidrow(
            mat.rowIndexPtr()[mat.supToCol()[mat.colToSup()[outer]] + 1]) {}
...
void MappedSuperNodalMatrix<Scalar, Index_>::solveInPlace(
  ...
  Matrix<Scalar, Dynamic, Dest::ColsAtCompileTime, ColMajor> work(
      n, nrhs); // working vector
  work.setZero();
  for (Index k = 0; k <= nsuper(); k++) {
    Index fsupc = supToCol()[k]; // First column of the current supernode
    Index istart = rowIndexPtr()[fsupc]; // Pointer index to the subscript of
                                         // the current column
    Index nsupr = rowIndexPtr()[fsupc + 1] -
                  istart; // Number of rows in the current supernode
    Index nsupc =
        supToCol()[k + 1] - fsupc; // Number of columns in the current supernode
    Index nrow =
        nsupr -
        nsupc;  // Number of rows in the non-diagonal part of the supernode
    ...
    if (nsupc == 1) {
      ...
      Map<const Matrix<Scalar, Dynamic, Dynamic, ColMajor>, 0, OuterStride<>> A(
          &(Lval[luptr]), nsupc, nsupc, OuterStride<>(lda));
      ...
      new (&A) Map<const Matrix<Scalar, Dynamic, Dynamic, ColMajor>, 0,
                   OuterStride<>>(&(Lval[luptr + nsupc]), nrow, nsupc,
                                  OuterStride<>(lda));
...
void MappedSuperNodalMatrix<Scalar, Index_>::solveTransposedInPlace(
  ...
  for (Index k = nsuper(); k >= 0; k--) {
    ...
    if (nsupc == 1) {
      ...
      Map<const Matrix<Scalar, Dynamic, Dynamic, ColMajor>, 0, OuterStride<>> A(
          &(Lval[luptr + nsupc]), nrow, nsupc, OuterStride<>(lda));
      Map<Matrix<Scalar, Dynamic, Dest::ColsAtCompileTime, ColMajor>, 0,
          OuterStride<>>
          U(&(X(fsupc, 0)), nsupc, nrhs, OuterStride<>(n));
      if (Conjugate)
      ...
      new (&A)
          Map<const Matrix<Scalar, Dynamic, Dynamic, ColMajor>, 0,
              OuterStride<>>(&(Lval[luptr]), nsupc, nsupc, OuterStride<>(lda));
      if (Conjugate)
