...
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const CwiseBinaryOp<
    internal::scalar_quotient_op<Scalar, typename OtherDerived::Scalar>,
...
EIGEN_STRONG_INLINE const CwiseBinaryOp<
    internal::scalar_min_op<Scalar, Scalar>, const Derived,
...
EIGEN_DEVICE_FUNC
EIGEN_STRONG_INLINE const CwiseBinaryOp<
    internal::scalar_max_op<Scalar, Scalar>, const Derived,
    const CwiseNullaryOp<internal::scalar_constant_op<Scalar>, PlainObject>>
#ifdef EIGEN_PARSED_BY_DOXYGEN
max
#else
...
EIGEN_DEVICE_FUNC
EIGEN_STRONG_INLINE const CwiseBinaryOp<
    internal::scalar_absolute_difference_op<Scalar, Scalar>, const Derived,
    const CwiseNullaryOp<internal::scalar_constant_op<Scalar>, PlainObject>>
#ifdef EIGEN_PARSED_BY_DOXYGEN
absolute_difference
#else
...
#endif
    (const Scalar &other) const {
  return (absolute_difference)(Derived::PlainObject::Constant(rows(), cols(),
                                                              other));
}
 ...
 *
 * This function computes the coefficient-wise power. The function
 * MatrixBase::pow() in the unsupported module MatrixFunctions computes the
 * matrix power.
 *
...
#define EIGEN_MAKE_CWISE_COMP_OP(OP, COMPARATOR)                               \
  template <typename OtherDerived>                                             \
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const CwiseBinaryOp<                   \
      internal::scalar_cmp_op<Scalar, typename OtherDerived::Scalar,           \
                              internal::cmp_##COMPARATOR>,                     \
      const Derived, const OtherDerived>                                       \
  ...
  typedef CwiseBinaryOp<                                                       \
      internal::scalar_cmp_op<Scalar, Scalar, internal::cmp_##COMPARATOR>,     \
      const Derived,                                                           \
      const CwiseNullaryOp<internal::scalar_constant_op<Scalar>, PlainObject>> \
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const Cmp##COMPARATOR##ReturnType OP(  \
      const Scalar &s) const {                                                 \
    return this->OP(Derived::PlainObject::Constant(rows(), cols(), s));        \
  }                                                                            \
  EIGEN_DEVICE_FUNC friend EIGEN_STRONG_INLINE const                           \
      RCmp##COMPARATOR##ReturnType                                             \
      OP(const Scalar &s,                                                      \
         const EIGEN_CURRENT_STORAGE_BASE_CLASS<Derived> &d) {                 \
...
#define EIGEN_MAKE_CWISE_COMP_R_OP(OP, R_OP, RCOMPARATOR)                      \
  template <typename OtherDerived>                                             \
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const CwiseBinaryOp<                   \
      internal::scalar_cmp_op<typename OtherDerived::Scalar, Scalar,           \
                              internal::cmp_##RCOMPARATOR>,                    \
  ...
  EIGEN_DEVICE_FUNC                                                            \
  inline const RCmp##RCOMPARATOR##ReturnType OP(const Scalar &s) const {       \
    return Derived::PlainObject::Constant(rows(), cols(), s).R_OP(*this);      \
  }                                                                            \
  friend inline const Cmp##RCOMPARATOR##ReturnType OP(const Scalar &s,         \
                                                      const Derived &d) {      \
 ...
 */
template <typename OtherDerived>
EIGEN_DEVICE_FUNC inline const CwiseBinaryOp<internal::scalar_boolean_xor_op,
                                             const Derived, const OtherDerived>
operator^(const EIGEN_CURRENT_STORAGE_BASE_CLASS<OtherDerived> &other) const {
  EIGEN_STATIC_ASSERT(
      (internal::is_same<bool, Scalar>::value &&
       internal::is_same<bool, typename OtherDerived::Scalar>::value),
      THIS_METHOD_IS_ONLY_FOR_EXPRESSIONS_OF_BOOL);
  return CwiseBinaryOp<internal::scalar_boolean_xor_op, const Derived,
 ...
 * other scalar types, the user has to provide implementations of zeta(T,T) for
...
inline const CwiseBinaryOp<internal::scalar_zeta_op<Scalar>, const Derived,
                           const DerivedQ>
zeta(const EIGEN_CURRENT_STORAGE_BASE_CLASS<DerivedQ> &q) const {
  return CwiseBinaryOp<internal::scalar_zeta_op<Scalar>, const Derived,
