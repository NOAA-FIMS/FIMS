 ...
 *
 * Both strides can be negative, however, a negative stride of -1 cannot be
 * specified at compiletime because of the ambiguity with Dynamic which is
 * defined to -1 (historically, negative strides were not allowed).
 *
...
public:
  ...
  Stride()
    ...
    eigen_assert(InnerStrideAtCompileTime != Dynamic &&
...
public:
  ...
  EIGEN_DEVICE_FUNC OuterStride(Index v)
      : Base(v, 0) {} // FIXME making this explicit could break valid code
};
