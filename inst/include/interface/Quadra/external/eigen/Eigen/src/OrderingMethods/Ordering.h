 ...
 * \tparam  StorageIndex The type of indices of the matrix
 *
 * Functor computing the \em column \em approximate \em minimum \em degree
 * ordering The matrix should be in column-major and \b compressed format (see
...
public:
  ...
  void operator()(const MatrixType &mat, PermutationType &perm) {
    eigen_assert(mat.isCompressed() &&
                 "COLAMDOrdering requires a sparse matrix in compressed mode. "
                 "Call .makeCompressed() before passing it to COLAMDOrdering");
    ...
    StorageIndex info = internal::Colamd::compute_ordering(
        m, n, Alen, A.data(), p.data(), knobs, stats);
