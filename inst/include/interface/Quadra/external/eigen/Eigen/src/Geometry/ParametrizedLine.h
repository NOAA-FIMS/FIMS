 ...
 * unit direction vector \f$ \mathbf{d} \f$ such that the line corresponds to
...
public:
  ...
  EIGEN_DEVICE_FUNC
  ParametrizedLine(const ParametrizedLine<Scalar, AmbientDimAtCompileTime,
                                          OtherOptions> &other)
  ...
  /** Initializes a parametrized line of direction \a direction and origin \a
   * origin. \warning the vector direction is assumed to be normalized.
   */
  EIGEN_DEVICE_FUNC ParametrizedLine(const VectorType &origin,
  ...
  Through(const VectorType &p0, const VectorType &p1) {
  ...
  cast() const {
    return typename internal::cast_return_type<
        ParametrizedLine,
        ParametrizedLine<NewScalarType, AmbientDimAtCompileTime,
                         Options>>::type(*this);
  }
  ...
  EIGEN_DEVICE_FUNC inline explicit ParametrizedLine(
      const ParametrizedLine<OtherScalarType, AmbientDimAtCompileTime,
                             OtherOptions> &other) {
  ...
  isApprox(const ParametrizedLine &other,
 ...
 *
 * \warning the ambient space must have dimension 2 such that the hyperplane
 * actually describes a line
 */
template <typename _Scalar, int _AmbientDim, int _Options>
template <int OtherOptions>
EIGEN_DEVICE_FUNC inline ParametrizedLine<_Scalar, _AmbientDim, _Options>::
    ParametrizedLine(
        const Hyperplane<_Scalar, _AmbientDim, OtherOptions> &hyperplane) {
...
EIGEN_DEVICE_FUNC inline
    typename ParametrizedLine<_Scalar, _AmbientDim, _Options>::VectorType
    ParametrizedLine<_Scalar, _AmbientDim, _Options>::pointAt(
        const _Scalar &t) const {
  return origin() + (direction() * t);
}

/** \returns the parameter value of the intersection between \c *this and the
 * given \a hyperplane
 */
template <typename _Scalar, int _AmbientDim, int _Options>
template <int OtherOptions>
...
/** \returns the point of the intersection between \c *this and the given
 * hyperplane
 */
template <typename _Scalar, int _AmbientDim, int _Options>
template <int OtherOptions>
