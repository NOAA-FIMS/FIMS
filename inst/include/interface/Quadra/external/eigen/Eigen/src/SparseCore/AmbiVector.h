 ...
 */
template <typename _Scalar, typename _StorageIndex> class AmbiVector {
public:
  ...
  explicit AmbiVector(Index size)
      : m_buffer(0), m_zero(0), m_size(0), m_end(0), m_allocatedSize(0),
        m_allocatedElements(0), m_mode(-1) {
    resize(size);
  }
...
protected:
  ...
  void reallocate(Index size) {
    // if the size of the matrix is not too large, let's allocate a bit more
    // than needed such that we can handle dense vector even in sparse mode.
    delete[] m_buffer;
    if (size < 1000) {
      Index allocSize =
          (size * sizeof(ListEl) + sizeof(Scalar) - 1) / sizeof(Scalar);
      m_allocatedElements =
          convert_index((allocSize * sizeof(Scalar)) / sizeof(ListEl));
      m_buffer = new Scalar[allocSize];
    } else {
      m_allocatedElements =
          convert_index((size * sizeof(Scalar)) / sizeof(ListEl));
...
void AmbiVector<_Scalar, _StorageIndex>::init(int mode) {
  m_mode = mode;
  // This is only necessary in sparse mode, but we set these unconditionally to
  // avoid some maybe-uninitialized warnings if (m_mode==IsSparse)
  {
    m_llSize = 0;
    m_llStart = -1;
  }
  ...
  if (m_mode == IsDense)
    ...
    if (m_llSize == 0) {
      ...
      eigen_assert(i >= llElements[m_llCurrent].index &&
                   "you must call restart() before inserting an element with "
      ...
      if (llElements[m_llCurrent].index == i) {
        ...
        eigen_internal_assert(m_llSize < m_allocatedElements &&
...
public:
  ...
  explicit Iterator(const AmbiVector &vec, const RealScalar &epsilon = 0)
    ...
    if (m_isDense) {
      ...
      ListEl *EIGEN_RESTRICT llElements =
          reinterpret_cast<ListEl *>(m_vector.m_buffer);
