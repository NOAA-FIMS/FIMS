...
struct general_matrix_matrix_product<Index, LhsScalar, LhsStorageOrder,
                                     ConjugateLhs, RhsScalar, RhsStorageOrder,
  ...
  run(Index rows, Index cols, Index depth, const LhsScalar *lhs,
    ...
    general_matrix_matrix_product<
        ...
        ColMajor, ResInnerStride>::run(cols, rows, depth, rhs, rhsStride, lhs,
...
struct general_matrix_matrix_product<Index, LhsScalar, LhsStorageOrder,
  ...
  static void run(Index rows, Index cols, Index depth, const LhsScalar *_lhs,
                  Index lhsStride, const RhsScalar *_rhs, Index rhsStride,
    ...
    Index mc = (std::min)(
        rows, blocking.mc()); // cache block size along the M direction
    Index nc = (std::min)(
        cols, blocking.nc()); // cache block size along the N direction
    ...
    if (info) {
      ...
      // For each horizontal panel of the rhs, and corresponding vertical panel
      // of the lhs...
      for (Index k = 0; k < depth; k += kc) {
        const Index actual_kc =
            (std::min)(k + kc, depth) - k; // => rows of B', and cols of the A'

        // In order to reduce the chance that a thread has to wait for the
        // other, let's start by packing B'.
        pack_rhs(blockB, rhs.getSubMapper(k, 0), actual_kc, nc);

        // Pack A_k to A' in a parallel fashion:
        // each thread packs the sub block A_k,i to A'_i where i is the thread
        // id.

        // However, before copying to A'_i, we have to make sure that no other
        // thread is still using it, i.e., we test that info[tid].users equals
        // 0. Then, we set info[tid].users to the number of threads to mark that
        // all other threads are going to use it.
        while (info[tid].users != 0) {
        }
        info[tid].users = threads;

        pack_lhs(blockA + info[tid].lhs_start * actual_kc,
                 lhs.getSubMapper(info[tid].lhs_start, k), actual_kc,
                 info[tid].lhs_length);
        ...
        for (int shift = 0; shift < threads; ++shift) {
          ...
          // At this point we have to make sure that A'_i has been updated by
          // the thread i, we use testAndSetOrdered to mimic a volatile access.
          // However, no need to wait for the B' part which has been updated by
          // the current thread!
          if (shift > 0) {
            while (info[i].sync != k) {
            }
          ...
          gebp(res.getSubMapper(info[i].lhs_start, 0),
               blockA + info[i].lhs_start * actual_kc, blockB,
               info[i].lhs_length, actual_kc, nc, alpha);
        }
        ...
        for (Index j = nc; j < cols; j += nc) {
          ...
          // C_j += A' * B'
          gebp(res.getSubMapper(0, j), blockA, blockB, rows, actual_kc,
               actual_nc, alpha);
        }
      ...
      for (Index i2 = 0; i2 < rows; i2 += mc) {
        ...
        for (Index k2 = 0; k2 < depth; k2 += kc) {
          ...
          for (Index j2 = 0; j2 < cols; j2 += nc) {
            ...
            gebp(res.getSubMapper(i2, j2), blockA, blockB, actual_mc, actual_kc,
                 actual_nc, alpha);
          }
        }
      }
...
template <typename Scalar, typename Index, typename Gemm, typename Lhs,
          typename Rhs, typename Dest, typename BlockingType>
struct gemm_functor {
  gemm_functor(const Lhs &lhs, const Rhs &rhs, Dest &dest,
               const Scalar &actualAlpha, BlockingType &blocking)
      : m_lhs(lhs), m_rhs(rhs), m_dest(dest), m_actualAlpha(actualAlpha),
        m_blocking(blocking) {}

  void initParallelSession(Index num_threads) const {
    m_blocking.initParallel(m_lhs.rows(), m_rhs.cols(), m_lhs.cols(),
                            num_threads);
  ...
  void operator()(Index row, Index rows, Index col = 0, Index cols = -1,
    ...
    Gemm::run(rows, cols, m_lhs.cols(), &m_lhs.coeffRef(row, 0),
              m_lhs.outerStride(), &m_rhs.coeffRef(0, col), m_rhs.outerStride(),
              (Scalar *)&(m_dest.coeffRef(row, col)), m_dest.innerStride(),
              m_dest.outerStride(), m_actualAlpha, m_blocking, info);
  }
...
template <int StorageOrder, typename LhsScalar, typename RhsScalar, int MaxRows,
          int MaxCols, int MaxDepth, int KcFactor = 1,
          bool FiniteAtCompileTime =
              MaxRows != Dynamic && MaxCols != Dynamic && MaxDepth != Dynamic>
...
class gemm_blocking_space<StorageOrder, _LhsScalar, _RhsScalar, MaxRows,
    ...
    : public level3_blocking<
          typename conditional<StorageOrder == RowMajor, _RhsScalar,
                               _LhsScalar>::type,
          typename conditional<StorageOrder == RowMajor, _LhsScalar,
                               _RhsScalar>::type> {
  enum {
  ...
  typedef
      typename conditional<Transpose, _RhsScalar, _LhsScalar>::type LhsScalar;
  typedef
      typename conditional<Transpose, _LhsScalar, _RhsScalar>::type RhsScalar;
  typedef gebp_traits<LhsScalar, RhsScalar> Traits;
...
#else
  EIGEN_ALIGN_MAX char
      m_staticA[SizeA * sizeof(LhsScalar) + EIGEN_DEFAULT_ALIGN_BYTES - 1];
  EIGEN_ALIGN_MAX char
      m_staticB[SizeB * sizeof(RhsScalar) + EIGEN_DEFAULT_ALIGN_BYTES - 1];
#endif
...
public:
  gemm_blocking_space(Index rows, Index cols, Index depth, Index num_threads,
    ...
    if (l3_blocking) {
      computeProductBlockingSizes<LhsScalar, RhsScalar, KcFactor>(
          this->m_kc, this->m_mc, this->m_nc, num_threads);
    } else // no l3 blocking
    {
      Index n = this->m_nc;
      computeProductBlockingSizes<LhsScalar, RhsScalar, KcFactor>(
          this->m_kc, this->m_mc, n, num_threads);
    }
  ...
  void initParallel(Index rows, Index cols, Index depth, Index num_threads) {
    ...
    Index m = this->m_mc;
    computeProductBlockingSizes<LhsScalar, RhsScalar, KcFactor>(
        this->m_kc, m, this->m_nc, num_threads);
    m_sizeA = this->m_mc * this->m_kc;
    m_sizeB = this->m_kc * this->m_nc;
  }
...
struct generic_product_impl<Lhs, Rhs, DenseShape, DenseShape, GemmProduct>
  ...
  static void evalTo(Dst &dst, const Lhs &lhs, const Rhs &rhs) {
    ...
    if ((rhs.rows() + dst.rows() + dst.cols()) <
        ...
        rhs.rows() > 0)
      lazyproduct::eval_dynamic(
          dst, lhs, rhs, internal::assign_op<typename Dst::Scalar, Scalar>());
    else {
      dst.setZero();
  ...
  static void scaleAndAddTo(Dest &dst, const Lhs &a_lhs, const Rhs &a_rhs,
    ...
    if (dst.cols() == 1) {
      ...
      return internal::generic_product_impl<
          Lhs, typename Rhs::ConstColXpr, DenseShape, DenseShape,
