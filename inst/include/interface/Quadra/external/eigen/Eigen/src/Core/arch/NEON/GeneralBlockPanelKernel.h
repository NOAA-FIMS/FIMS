...
struct gebp_traits<float, float, false, false, Architecture::NEON,
  ...
  EIGEN_STRONG_INLINE void acc(const AccPacket &c, const ResPacket &alpha,
    ...
    asm volatile("vmla.f32 %q[r], %q[c], %q[alpha]"
                 : [r] "+w"(r)
                 : [c] "w"(c), [alpha] "w"(alpha)
                 :);
  }
  ...
  template <typename LaneIdType>
  EIGEN_STRONG_INLINE void madd(const Packet4f &a,
                                const QuadPacket<Packet4f> &b, Packet4f &c,
                                Packet4f &tmp, const LaneIdType &lane) const {
...
struct gebp_traits<float, float, false, false, Architecture::NEON,
  ...
  // "candidate template ignored: could not match 'FixedInt<N>' against
...
private:
  template <int LaneID>
  EIGEN_STRONG_INLINE void madd_helper(const LhsPacket &a, const RhsPacketx4 &b,
                                       AccPacket &c) const {
