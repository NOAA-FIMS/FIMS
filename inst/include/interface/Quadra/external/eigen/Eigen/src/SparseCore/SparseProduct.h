 ...
 * algorithm is employed: \code C = (A*B).pruned();             // suppress
 * numerical zeros (exact) C = (A*B).pruned(ref); C = (A*B).pruned(ref,epsilon);
 * \endcode
...
SparseMatrixBase<Derived>::operator*(
    const SparseMatrixBase<OtherDerived> &other) const {
  return Product<Derived, OtherDerived, AliasFreeProduct>(derived(),
                                                          other.derived());
}
...
struct generic_product_impl<Lhs, Rhs, SparseShape, SparseShape, ProductType> {
  ...
  static void
  addTo(Dest &dst, const ActualLhs &lhs, const Rhs &rhs,
        typename enable_if<
            is_same<typename evaluator_traits<Dest>::Shape, DenseShape>::value,
            int *>::type * = 0) {
    ...
    internal::sparse_sparse_to_dense_product_selector<
        typename remove_all<LhsNested>::type,
        typename remove_all<RhsNested>::type, Dest>::run(lhsNested, rhsNested,
                                                         dst);
  }
  ...
  template <typename Dest>
  static void
  subTo(Dest &dst, const Lhs &lhs, const Rhs &rhs,
        typename enable_if<
            is_same<typename evaluator_traits<Dest>::Shape, DenseShape>::value,
            int *>::type * = 0) {
    addTo(dst, -lhs, rhs);
...
template <typename Lhs, typename Rhs, int ProductType>
struct generic_product_impl<Lhs, Rhs, SparseShape, SparseTriangularShape,
                            ProductType>
    : public generic_product_impl<Lhs, Rhs, SparseShape, SparseShape,
                                  ProductType> {};

// sparse-triangular * sparse
template <typename Lhs, typename Rhs, int ProductType>
struct generic_product_impl<Lhs, Rhs, SparseTriangularShape, SparseShape,
                            ProductType>
    : public generic_product_impl<Lhs, Rhs, SparseShape, SparseShape,
                                  ProductType> {};
...
struct Assignment<
    ...
    internal::assign_op<typename DstXprType::Scalar,
                        typename Product<Lhs, Rhs, AliasFreeProduct>::Scalar>,
    Sparse2Dense> {
...
// dense += sparse-product (can be sparse*sparse, sparse*perm, etc.)
template <typename DstXprType, typename Lhs, typename Rhs>
struct Assignment<DstXprType, Product<Lhs, Rhs, AliasFreeProduct>,
                  internal::add_assign_op<
...
struct unary_evaluator<SparseView<Product<Lhs, Rhs, Options>>, IteratorBased>
    : public evaluator<
          typename Product<Lhs, Rhs, DefaultProduct>::PlainObject> {
  ...
  explicit unary_evaluator(const XprType &xpr)
    ...
    internal::sparse_sparse_product_with_pruning_selector<
        typename remove_all<LhsNested>::type,
        typename remove_all<RhsNested>::type,
