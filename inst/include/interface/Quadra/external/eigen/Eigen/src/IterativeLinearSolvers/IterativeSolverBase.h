...
template <typename MatrixType,
          bool MatrixFree = !internal::is_ref_compatible<MatrixType>::value>
...
public:
  ...
  template <int UpLo> struct ConstSelfAdjointViewReturnType {
    typedef typename ActualMatrixType::template ConstSelfAdjointViewReturnType<
        UpLo>::Type Type;
  };
...
public:
   ...
   * In the future we might, for instance, implement column reordering for
   ...
   * the future we might, for instance, implement column reordering for faster
  ...
  void _solve_with_guess_impl(const Rhs &b,
    ...
    // We do not directly fill dest because sparse expressions have to be free
    // of aliasing issue. For non square least-square problems, b and dest might
    // not have the same size whereas they might alias each-other.
    ...
    for (Index k = 0; k < rhsCols; ++k) {
      ...
      // The call to _solve_vector_with_guess_impl updates m_info, so if it
      // failed for a previous column we need to restore it to the worst value.
  ...
  template <typename Rhs, typename DestDerived>
  typename internal::enable_if<Rhs::ColsAtCompileTime != 1 &&
                               DestDerived::ColsAtCompileTime != 1>::type
  _solve_with_guess_impl(const Rhs &b, MatrixBase<DestDerived> &aDest) const {
    ...
    for (Index k = 0; k < rhsCols; ++k) {
      ...
      // The call to _solve_vector_with_guess updates m_info, so if it failed
      // for a previous column we need to restore it to the worst value.
      if (m_info == NumericalIssue)
        global_info = NumericalIssue;
      else if (m_info == NoConvergence)
        global_info = NoConvergence;
...
protected:
  ...
  template <typename InputType> void grab(const InputType &A) {
