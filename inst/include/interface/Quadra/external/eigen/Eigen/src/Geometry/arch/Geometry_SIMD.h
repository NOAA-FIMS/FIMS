...
struct cross3_impl<Architecture::Target, VectorLhs, VectorRhs, float, true> {
  ...
  run(const VectorLhs &lhs, const VectorRhs &rhs) {
    ...
    Packet4f a =
        lhs_eval.template packet<traits<VectorLhs>::Alignment, Packet4f>(0);
    Packet4f b =
        rhs_eval.template packet<traits<VectorRhs>::Alignment, Packet4f>(0);
    Packet4f mul1 =
...
struct quat_product<Architecture::Target, Derived, OtherDerived, double> {
  ...
  static inline Quaternion<double> run(const QuaternionBase<Derived> &_a,
    ...
    pstoret<double, Packet2d, ResAlignment>(
        &res.z(), preverse(paddsub(preverse(t1), t2)));
