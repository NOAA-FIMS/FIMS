...
struct traits<Array<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols>>
  ...
  typedef ArrayBase<Array<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols>>
      XprBase;
};
} // namespace internal
 ...
 * operations.
 *
 * See documentation of class Matrix for detailed information on the template
...
template <typename _Scalar, int _Rows, int _Cols, int _Options, int _MaxRows,
          int _MaxCols>
class Array : public PlainObjectBase<
...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Array(const Scalar &a0,
                                              const Scalar &a1,
                                              const Scalar &a2,
                                              const Scalar &a3,
                                              const ArgTypes &...args)
   ...
   *
   * In the case of a compile-time column 1D array, implicit transposition from
   * a single row is allowed. Therefore <code>
   * Array<int,Dynamic,1>{{1,2,3,4,5}}</code> is legal and the more verbose
   * syntax <code>Array<int,Dynamic,1>{{1},{2},{3},{4},{5}}</code> can be
   * avoided:
   *
   * Example: \include Array_initializer_list_vector_cxx11.cpp
   * Output: \verbinclude Array_initializer_list_vector_cxx11.out
   *
   * In the case of fixed-sized arrays, the initializer list sizes must exactly
   * match the array sizes, and implicit transposition is allowed for
   * compile-time 1D arrays only.
   *
   ...
   * to use the default constructor Array() instead.
   */
  EIGEN_DEVICE_FUNC
   ...
   */
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE Array(const Scalar &val0, const Scalar &val1,
                            const Scalar &val2) {
    Base::_check_template_params();
...
public:
  ...
  Array(const EigenBase<OtherDerived> &other,
        typename internal::enable_if<
            internal::is_convertible<typename OtherDerived::Scalar,
                                     Scalar>::value,
 ...
 *
 * \c ArrayRowsColsType where \c Rows and \c Cols can be \c 2,\c 3,\c 4 for
 * fixed size square matrices or \c X for dynamic size, and where \c Type can be
 * \c i for integer, \c f for float, \c d for double, \c cf for complex float,
 * \c cd for complex double.
 *
 * For example, \c Array33d is a fixed-size 3x3 array type of doubles, and \c
 * ArrayXXf is a dynamic-size matrix of floats.
 *
 * There are also \c ArraySizeType which are self-explanatory. For example, \c
 * Array4cf is a fixed-size 1D array of 4 complex floats.
 *
 ...
 *   - `ArrayRowsCols<Type>` where `Rows` and `Cols` can be \c 2,\c 3,\c 4, or
 * \c X for fixed or dynamic size.
 *   - `ArraySize<Type>` where `Size` can be \c 2,\c 3,\c 4 or \c X for fixed or
 * dynamic size 1D arrays.
 *
 * \sa class Array
 */
