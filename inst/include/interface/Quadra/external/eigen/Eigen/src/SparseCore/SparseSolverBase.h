...
typename enable_if<Rhs::ColsAtCompileTime != 1 &&
                   Dest::ColsAtCompileTime != 1>::type
solve_sparse_through_dense_panels(const Decomposition &dec, const Rhs &rhs,
                                  Dest &dest) {
  EIGEN_STATIC_ASSERT((Dest::Flags & RowMajorBit) == 0,
                      THIS_METHOD_IS_ONLY_FOR_COLUMN_MAJOR_MATRICES);
  typedef typename Dest::Scalar DestScalar;
  // we process the sparse rhs per block of NbColsAtOnce columns temporarily
  // stored into a dense matrix.
...
public:
  ...
  void _solve_impl(const SparseMatrixBase<Rhs> &b,
    ...
    internal::solve_sparse_through_dense_panels(derived(), b.derived(),
                                                dest.derived());
  }
