...
#define EIGEN_DEFINE_STL_DEQUE_SPECIALIZATION(...)                             \
  ...
  class deque<__VA_ARGS__, std::allocator<__VA_ARGS__>>                        \
    ...
    typedef deque<__VA_ARGS__, EIGEN_ALIGNED_ALLOCATOR<__VA_ARGS__>>           \
        deque_base;                                                            \
                                                                               \
  public:                                                                      \
    ...
    explicit deque(size_type num, const value_type &val = value_type())        \
        : deque_base(num, val) {}                                              \
...
class deque<T, EIGEN_ALIGNED_ALLOCATOR<T>>
  ...
  void resize(size_type new_size, const value_type &x) {
    if (deque_base::size() < new_size)
      deque_base::_Insert_n(deque_base::end(), new_size - deque_base::size(),
                            x);
    else if (new_size < deque_base::size())
      deque_base::erase(deque_base::begin() + new_size, deque_base::end());
  }
  void push_back(const value_type &x) { deque_base::push_back(x); }
  void push_front(const value_type &x) { deque_base::push_front(x); }
  using deque_base::insert;
  iterator insert(const_iterator position, const value_type &x) {
    return deque_base::insert(position, x);
  }
  void insert(const_iterator position, size_type new_size,
              const value_type &x) {
