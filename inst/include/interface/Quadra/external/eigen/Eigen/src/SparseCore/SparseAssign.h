...
template <typename OtherDerived>
Derived &
SparseMatrixBase<Derived>::operator=(const ReturnByValue<OtherDerived> &other) {
...
template <typename Derived>
template <typename OtherDerived>
inline Derived &SparseMatrixBase<Derived>::operator=(
...
void assign_sparse_to_sparse(DstXprType &dst, const SrcXprType &src) {
  ...
  const bool transpose = (DstEvaluatorType::Flags & RowMajorBit) !=
                         (SrcEvaluatorType::Flags & RowMajorBit);
  const Index outerEvaluationSize =
      (SrcEvaluatorType::Flags & RowMajorBit) ? src.rows() : src.cols();
  if ((!transpose) && src.isRValue()) {
    // eval without temporary
    dst.resize(src.rows(), src.cols());
    dst.setZero();
    dst.reserve((std::min)(src.rows() * src.cols(),
                           (std::max)(src.rows(), src.cols()) * 2));
    for (Index j = 0; j < outerEvaluationSize; ++j) {
      dst.startVec(j);
      for (typename SrcEvaluatorType::InnerIterator it(srcEvaluator, j); it;
           ++it) {
        Scalar v = it.value();
    ...
    eigen_assert((((internal::traits<DstXprType>::SupportedAccessPatterns &
                    OuterRandomAccessPattern) == OuterRandomAccessPattern) ||
                  (!((DstEvaluatorType::Flags & RowMajorBit) !=
                     (SrcEvaluatorType::Flags & RowMajorBit)))) &&
                 "the transpose operation is supposed to be handled in "
                 "SparseMatrix::operator=");

    enum {
      Flip = (DstEvaluatorType::Flags & RowMajorBit) !=
             (SrcEvaluatorType::Flags & RowMajorBit)
    };

    DstXprType temp(src.rows(), src.cols());

    temp.reserve((std::min)(src.rows() * src.cols(),
                            (std::max)(src.rows(), src.cols()) * 2));
    for (Index j = 0; j < outerEvaluationSize; ++j) {
      temp.startVec(j);
      for (typename SrcEvaluatorType::InnerIterator it(srcEvaluator, j); it;
           ++it) {
        Scalar v = it.value();
        temp.insertBackByOuterInner(Flip ? it.index() : j,
                                    Flip ? j : it.index()) = v;
      }
    }
    temp.finalize();
...
struct Assignment<DstXprType, SrcXprType, Functor, Sparse2Dense, Weak> {
  static void run(DstXprType &dst, const SrcXprType &src, const Functor &func) {
    if (internal::is_same<
            Functor, internal::assign_op<typename DstXprType::Scalar,
    ...
    const Index outerEvaluationSize =
        (internal::evaluator<SrcXprType>::Flags & RowMajorBit) ? src.rows()
                                                               : src.cols();
    for (Index j = 0; j < outerEvaluationSize; ++j)
      for (typename internal::evaluator<SrcXprType>::InnerIterator i(srcEval,
                                                                     j);
           i; ++i)
...
struct assignment_from_dense_op_sparse {
  ...
  // Specialization for dense1 = sparse + dense2; -> dense1 = dense2; dense1 +=
  // sparse;
  template <typename Lhs, typename Rhs, typename Scalar>
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename internal::enable_if<
      internal::is_same<typename internal::evaluator_traits<Rhs>::Shape,
                        DenseShape>::value>::type
  run(DstXprType &dst,
      const CwiseBinaryOp<internal::scalar_sum_op<Scalar, Scalar>, const Lhs,
                          const Rhs> &src,
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename internal::enable_if<
      internal::is_same<typename internal::evaluator_traits<Rhs>::Shape,
                        DenseShape>::value>::type
  run(DstXprType &dst,
      const CwiseBinaryOp<internal::scalar_difference_op<Scalar, Scalar>,
                          const Lhs, const Rhs> &src,
      const internal::assign_op<typename DstXprType::Scalar, Scalar>
          & /*func*/) {
    ...
    call_assignment_no_alias(
        dst, src.lhs(),
        add_assign_op<typename DstXprType::Scalar, typename Lhs::Scalar>());
  }
};
...
#define EIGEN_CATCH_ASSIGN_DENSE_OP_SPARSE(ASSIGN_OP, BINOP, ASSIGN_OP2)       \
  ...
  struct Assignment<                                                           \
      ...
      typename internal::enable_if<                                            \
          internal::is_same<typename internal::evaluator_traits<Lhs>::Shape,   \
                            DenseShape>::value ||                              \
          internal::is_same<typename internal::evaluator_traits<Rhs>::Shape,   \
                            DenseShape>::value>::type>                         \
      : assignment_from_dense_op_sparse<                                       \
            DstXprType,                                                        \
            internal::ASSIGN_OP<typename DstXprType::Scalar,                   \
                                typename Lhs::Scalar>,                         \
            internal::ASSIGN_OP2<typename DstXprType::Scalar,                  \
                                 typename Rhs::Scalar>> {}
...
EIGEN_CATCH_ASSIGN_DENSE_OP_SPARSE(assign_op, scalar_difference_op,
                                   sub_assign_op);
EIGEN_CATCH_ASSIGN_DENSE_OP_SPARSE(add_assign_op, scalar_difference_op,
                                   sub_assign_op);
EIGEN_CATCH_ASSIGN_DENSE_OP_SPARSE(sub_assign_op, scalar_difference_op,
                                   add_assign_op);

// Specialization for "dst = dec.solve(rhs)"
// NOTE we need to specialize it for Sparse2Sparse to avoid ambiguous
// specialization error
template <typename DstXprType, typename DecType, typename RhsType,
          typename Scalar>
struct Assignment<DstXprType, Solve<DecType, RhsType>,
                  internal::assign_op<Scalar, Scalar>, Sparse2Sparse> {
...
struct Assignment<DstXprType, SrcXprType, Functor, Diagonal2Sparse> {
  ...
  static void run(SparseMatrix<Scalar, Options, StorageIndex> &dst,
  ...
  static void
  run(SparseMatrixBase<DstDerived> &dst, const SrcXprType &src,
      const internal::assign_op<typename DstXprType::Scalar,
                                typename SrcXprType::Scalar> & /*func*/) {
