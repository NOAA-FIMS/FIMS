...
// page 114, "[The] LP64 model [...] is used by all 64-bit UNIX ports" so it's
// indeed quite safe, at least within the context of glibc, to equate 64-bit
// with LP64.
...
// FreeBSD 7 seems to have 16-byte aligned malloc except on ARM and MIPS
// architectures
//   See
//   http://svn.freebsd.org/viewvc/base/stable/7/lib/libc/stdlib/malloc.c?view=markup
#if defined(__FreeBSD__) && !(EIGEN_ARCH_ARM || EIGEN_ARCH_MIPS) &&            \
    (EIGEN_DEFAULT_ALIGN_BYTES == 16)
...
inline void throw_std_bad_alloc() {
  ...
  //
  // calls to "::operator new" are to be treated as opaque function calls (i.e
  // no inlining), and as a consequence the code in the #else block triggers the
  // hipcc warning : "no overloaded function has restriction specifiers that are
  // compatible with the ambient context"
  //
  // "throw_std_bad_alloc" has the EIGEN_DEVICE_FUNC attribute, so it seems that
  // hipcc expects the same on "operator new" Reverting code back to the old
  // version in this #if block for the hipcc compiler
  //
  new int[huge];
#else
...
handmade_aligned_malloc(std::size_t size,
                        std::size_t alignment = EIGEN_DEFAULT_ALIGN_BYTES) {
  eigen_assert(alignment >= sizeof(void *) &&
 ...
 */
inline void *handmade_aligned_realloc(void *ptr, std::size_t size,
                                      std::size_t = 0) {
  if (ptr == 0)
    return handmade_aligned_malloc(size);
  void *original = *(reinterpret_cast<void **>(ptr) - 1);
  std::ptrdiff_t previous_offset =
      static_cast<char *>(ptr) - static_cast<char *>(original);
...
EIGEN_DEVICE_FUNC inline bool is_malloc_allowed_impl(bool update,
...
EIGEN_DEVICE_FUNC inline bool is_malloc_allowed() {
  return is_malloc_allowed_impl(false);
}
EIGEN_DEVICE_FUNC inline bool set_is_malloc_allowed(bool new_value) {
  return is_malloc_allowed_impl(true, new_value);
}
EIGEN_DEVICE_FUNC inline void check_that_malloc_is_allowed() {
  eigen_assert(is_malloc_allowed() &&
               "heap allocation is forbidden (EIGEN_RUNTIME_NO_MALLOC is "
               "defined and g_is_malloc_allowed is false)");
}
#else
EIGEN_DEVICE_FUNC inline void check_that_malloc_is_allowed() {}
#endif
...
EIGEN_DEVICE_FUNC inline void *aligned_malloc(std::size_t size) {
  ...
  eigen_assert((size < 16 || (std::size_t(result) % 16) == 0) &&
               "System's malloc returned an unaligned pointer. Compile with "
               "EIGEN_MALLOC_ALREADY_ALIGNED=0 to fallback to handmade aligned "
               "memory allocator.");
#endif
#else
...
template <bool Align>
inline void *conditional_aligned_realloc(void *ptr, std::size_t new_size,
                                         std::size_t old_size) {
  return aligned_realloc(ptr, new_size, old_size);
}

template <>
inline void *conditional_aligned_realloc<false>(void *ptr, std::size_t new_size,
                                                std::size_t) {
 ...
 */
template <typename T>
EIGEN_DEVICE_FUNC inline void destruct_elements_of_array(T *ptr,
                                                         std::size_t size) {
 ...
 */
template <typename T>
EIGEN_DEVICE_FUNC inline T *construct_elements_of_array(T *ptr,
                                                        std::size_t size) {
  std::size_t i;
  EIGEN_TRY {
    for (i = 0; i < size; ++i)
 ...
 */
template <typename T, bool Align>
EIGEN_DEVICE_FUNC inline void conditional_aligned_delete(T *ptr,
                                                         std::size_t size) {
  destruct_elements_of_array<T>(ptr, size);
...
conditional_aligned_realloc_new(T *pts, std::size_t new_size,
                                std::size_t old_size) {
...
template <typename T, bool Align>
inline T *conditional_aligned_realloc_new_auto(T *pts, std::size_t new_size,
                                               std::size_t old_size) {
  check_size_for_overflow<T>(new_size);
  check_size_for_overflow<T>(old_size);
 ...
 *
 * \note If no element of the array is well aligned or the requested alignment
 * is not a multiple of a scalar, the size of the array is returned. For example
 * with SSE, the requested alignment is typically 16-bytes. If packet size for
 * the given scalar type is 1, then everything is considered well-aligned.
 *
 * \note Otherwise, if the Alignment is larger that the scalar size, we rely on
 * the assumptions that sizeof(Scalar) is a power of 2. On the other hand, we do
 * not assume that the array address is a multiple of sizeof(Scalar), as that
 * fails for example with Scalar=double on certain 32-bit platforms, see bug
 * #79.
 *
 * There is also the variant first_aligned(const MatrixBase&) defined in
 * DenseCoeffsBase.h. \sa first_default_aligned()
 */
...
EIGEN_DEVICE_FUNC inline Index first_aligned(const Scalar *array, Index size) {
  ...
  if (AlignmentSize <= 1) {
    // Either the requested alignment if smaller than a scalar, or it exactly
    // match a 1 scalar so that all elements of the array have the same
    // alignment.
    return 0;
  } else if ((UIntPtr(array) & (sizeof(Scalar) - 1)) ||
             (Alignment % ScalarSize) != 0) {
    // The array is not aligned to the size of a single scalar, or the requested
    // alignment is not a multiple of the scalar size. Consequently, no element
    // of the array is well aligned.
    return size;
  } else {
    Index first = (AlignmentSize -
                   (Index((UIntPtr(array) / sizeof(Scalar))) & AlignmentMask)) &
                  AlignmentMask;
...
EIGEN_DEVICE_FUNC inline Index first_default_aligned(const Scalar *array,
                                                     Index size) {
  typedef typename packet_traits<Scalar>::type DefaultPacketType;
  return first_aligned<unpacket_traits<DefaultPacketType>::alignment>(array,
                                                                      size);
}
...
// use memcpy on trivial types, i.e., on types that does not require an
// initialization ctor.
template <typename T, bool UseMemcpy> struct smart_copy_helper;

template <typename T>
EIGEN_DEVICE_FUNC void smart_copy(const T *start, const T *end, T *target) {
  smart_copy_helper<T, !NumTraits<T>::RequireInitialization>::run(start, end,
                                                                  target);
}

template <typename T> struct smart_copy_helper<T, true> {
  EIGEN_DEVICE_FUNC static inline void run(const T *start, const T *end,
                                           T *target) {
...
// intelligent memmove. falls back to std::memmove for POD types, uses std::copy
// otherwise.
template <typename T, bool UseMemmove> struct smart_memmove_helper;

template <typename T>
void smart_memmove(const T *start, const T *end, T *target) {
  smart_memmove_helper<T, !NumTraits<T>::RequireInitialization>::run(start, end,
                                                                     target);
}
...
// you can overwrite Eigen's default behavior regarding alloca by defining
// EIGEN_ALLOCA to the appropriate stack allocation function
...
// This helper class construct the allocated memory, and takes care of
// destructing and freeing the handled data at destruction time. In practice
// this helper class is mainly useful to avoid memory leak in case of
// exceptions.
...
public:
   ...
   *NumTraits<T>::RequireInitialization). In this case, the buffer elements will
   *also be destructed when this handler will be destructed. Finally, if \a
   *dealloc is true, then the pointer \a ptr is freed.
   **/
  EIGEN_DEVICE_FUNC
...
template <typename Xpr, int NbEvaluations,
          bool MapExternalBuffer = nested_eval<Xpr, NbEvaluations>::Evaluate &&
                                   Xpr::MaxSizeAtCompileTime == Dynamic>
...
struct local_nested_eval_wrapper<Xpr, NbEvaluations, true> {
  ...
  local_nested_eval_wrapper(const Xpr &xpr, Scalar *ptr)
      : object(ptr == 0
                   ? reinterpret_cast<Scalar *>(Eigen::internal::aligned_malloc(
                         sizeof(Scalar) * xpr.size()))
                   : ptr,
               xpr.rows(), xpr.cols()),
        m_deallocate(ptr == 0) {
    if (NumTraits<Scalar>::RequireInitialization && object.data())
      Eigen::internal::construct_elements_of_array(object.data(),
                                                   object.size());
    object = xpr;
  }
 ...
 * declares, allocates, and construct an aligned buffer named NAME of SIZE
 * elements of type TYPE on the stack if the size in bytes is smaller than
 * EIGEN_STACK_ALLOCATION_LIMIT, and if stack allocation is supported by the
 * platform (currently, this is Linux, OSX and Visual Studio only). Otherwise
 * the memory is allocated on the heap. The allocated buffer is automatically
 * deleted when exiting the scope of this declaration. If BUFFER is non null,
 * then the declared variable is simply an alias for BUFFER, and no
 * allocation/deletion occurs. Here is an example: \code
 * {
 *   ei_declare_aligned_stack_constructed_variable(float,data,size,0);
 *   // use data[0] to data[size-1]
 * }
 * \endcode
 * The underlying stack allocation function can controlled with the EIGEN_ALLOCA
 * preprocessor token.
 *
 ...
 * \endcode
 * with the advantage of using aligned stack allocation even if the maximal size
 * of XPR at compile time is unknown. This is accomplished through alloca if
 * this later is supported and if the required number of bytes is below
...
// If alloca is already aligned, the compiler should be smart enough to optimize
// away the re-alignment.
#define EIGEN_ALIGNED_ALLOCA(SIZE)                                             \
  reinterpret_cast<void *>(                                                    \
      (internal::UIntPtr(EIGEN_ALLOCA(SIZE + EIGEN_DEFAULT_ALIGN_BYTES - 1)) + \
       EIGEN_DEFAULT_ALIGN_BYTES - 1) &                                        \
      ~(std::size_t(EIGEN_DEFAULT_ALIGN_BYTES - 1)))
#else
#define EIGEN_ALIGNED_ALLOCA(SIZE) EIGEN_ALLOCA(SIZE)
#endif

#define ei_declare_aligned_stack_constructed_variable(TYPE, NAME, SIZE,        \
                                                      BUFFER)                  \
  ...
  TYPE *NAME =                                                                 \
      (BUFFER) != 0                                                            \
          ...
          : reinterpret_cast<TYPE *>(                                          \
                (sizeof(TYPE) * SIZE <= EIGEN_STACK_ALLOCATION_LIMIT)          \
                    ? EIGEN_ALIGNED_ALLOCA(sizeof(TYPE) * SIZE)                \
                    : Eigen::internal::aligned_malloc(sizeof(TYPE) * SIZE));   \
  Eigen::internal::aligned_stack_memory_handler<TYPE> EIGEN_CAT(               \
      NAME, _stack_memory_destructor)((BUFFER) == 0 ? NAME : 0, SIZE,          \
                                      sizeof(TYPE) * SIZE >                    \
                                          EIGEN_STACK_ALLOCATION_LIMIT)

#define ei_declare_local_nested_eval(XPR_T, XPR, N, NAME)                      \
  Eigen::internal::local_nested_eval_wrapper<XPR_T, N> EIGEN_CAT(NAME,         \
                                                                 _wrapper)(    \
      XPR, reinterpret_cast<typename XPR_T::Scalar *>(                         \
               ((Eigen::internal::local_nested_eval_wrapper<                   \
                    XPR_T, N>::NeedExternalBuffer) &&                          \
                ((sizeof(typename XPR_T::Scalar) * XPR.size()) <=              \
                 EIGEN_STACK_ALLOCATION_LIMIT))                                \
                   ? EIGEN_ALIGNED_ALLOCA(sizeof(typename XPR_T::Scalar) *     \
                                          XPR.size())                          \
                   : 0));                                                      \
  typename Eigen::internal::local_nested_eval_wrapper<XPR_T, N>::ObjectType    \
  NAME(EIGEN_CAT(NAME, _wrapper).object)
...
#define ei_declare_aligned_stack_constructed_variable(TYPE, NAME, SIZE,        \
  ...
  TYPE *NAME = (BUFFER) != 0                                                   \
                   ? BUFFER                                                    \
                   : reinterpret_cast<TYPE *>(Eigen::internal::aligned_malloc( \
                         sizeof(TYPE) * SIZE));                                \
  Eigen::internal::aligned_stack_memory_handler<TYPE> EIGEN_CAT(               \
      NAME, _stack_memory_destructor)((BUFFER) == 0 ? NAME : 0, SIZE, true)
...
#define EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF_VECTORIZABLE_FIXED_SIZE(Scalar,     \
  ...
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF(bool(                                     \
      ...
      (((EIGEN_MAX_ALIGN_BYTES >= 16) &&                                       \
        ((sizeof(Scalar) * (Size)) % (EIGEN_MAX_ALIGN_BYTES) == 0)) ||         \
       ((EIGEN_MAX_ALIGN_BYTES >= 32) &&                                       \
        ((sizeof(Scalar) * (Size)) % (EIGEN_MAX_ALIGN_BYTES / 2) == 0)) ||     \
       ((EIGEN_MAX_ALIGN_BYTES >= 64) &&                                       \
        ((sizeof(Scalar) * (Size)) % (EIGEN_MAX_ALIGN_BYTES / 4) == 0)))))
 ...
 * // Vector3f does not require 16 bytes alignment, no need to use Eigen's
 * allocator: std::map< int, Vector3f > my_map_vec3; \endcode
 *
...
#define EIGEN_CPUID(abcd, func, id)                                            \
  __asm__ __volatile__("xchgl %%ebx, %k1;cpuid; xchgl %%ebx,%k1"               \
                       : "=a"(abcd[0]), "=&r"(abcd[1]), "=c"(abcd[2]),         \
                         "=d"(abcd[3])                                         \
                       : "a"(func), "c"(id));
#elif defined(__PIC__) && EIGEN_ARCH_x86_64
// Case for x64 with PIC. In theory this is only a problem with recent gcc and
// with medium or large code model, not with the default small code model.
// However, we cannot detect which code model is used, and the xchg overhead is
// negligible anyway.
#define EIGEN_CPUID(abcd, func, id)                                            \
...
inline void queryCacheSizes_intel_codes(int &l1, int &l2, int &l3) {
  ...
  for (int i = 0; i < 14; ++i) {
    ...
    case 0x22:
      l3 = 512;
      break; // code and data L3 cache, 512 KB, 4 ways (!), 64 byte lines,
             // dual-sectored
    case 0x23:
      l3 = 1024;
      break; // code and data L3 cache, 1024 KB, 8 ways, 64 byte lines,
             // dual-sectored
    case 0x25:
      l3 = 2048;
      break; // code and data L3 cache, 2048 KB, 8 ways, 64 byte lines,
             // dual-sectored
    case 0x29:
      l3 = 4096;
      break; // code and data L3 cache, 4096 KB, 8 ways, 64 byte lines,
             // dual-sectored
    case 0x39:
      l2 = 128;
    ...
    case 0x49:
      ...
      break; // code and data L3 cache, 4096 KB, 16 ways, 64 byte lines (P4) or
             // L2 for core2
    case 0x4A:
      l3 = 6144;
    ...
    case 0x79:
      l2 = 128;
      break; // code and data L2 cache, 128 KB, 8 ways, 64 byte lines,
             // dual-sectored
    case 0x7A:
      l2 = 256;
      break; // code and data L2 cache, 256 KB, 8 ways, 64 byte lines,
             // dual-sectored
    case 0x7B:
      l2 = 512;
    ...
    case 0x7E:
      l2 = 256;
      break; // code and data L2 cache, 256 KB, 8 ways, 128 byte lines, sect.
             // (IA-64)
    case 0x7F:
      l2 = 512;
...
inline void queryCacheSizes_amd(int &l1, int &l2, int &l3) {
  ...
  if (static_cast<numext::uint32_t>(abcd[0]) >=
      static_cast<numext::uint32_t>(0x80000006)) {
    EIGEN_CPUID(abcd, 0x80000005, 0);
    l1 = (abcd[2] >> 24) * 1024; // C[31:24] = L1 size in KB
    abcd[0] = abcd[1] = abcd[2] = abcd[3] = 0;
    EIGEN_CPUID(abcd, 0x80000006, 0);
    l2 = (abcd[2] >> 16) * 1024; // C[31;16] = l2 cache size in KB
    l3 = ((abcd[3] & 0xFFFC000) >> 18) * 512 *
         1024; // D[31;18] = l3 cache size in 512KB
  } else {
    l1 = l2 = l3 = 0;
  }
}
...
inline void queryCacheSizes(int &l1, int &l2, int &l3) {
  ...
  const int AMDisbetter_[] = {0x69444d41, 0x74656273,
                              0x21726574}; // "AMDisbetter!"
  ...
  else if (cpuid_is_vendor(abcd, AuthenticAMD) ||
           cpuid_is_vendor(abcd, AMDisbetter_))
