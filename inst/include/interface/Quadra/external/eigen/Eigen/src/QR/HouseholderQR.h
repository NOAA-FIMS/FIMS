 ...
 * decomposition
 *
 * This class performs a QR decomposition of a matrix \b A into matrices \b Q
 * and \b R such that \f[ \mathbf{A} = \mathbf{Q} \, \mathbf{R} \f] by using
 * Householder transformations. Here, \b Q a unitary matrix and \b R an upper
 * triangular matrix. The result is stored in a compact way compatible with
 * LAPACK.
 *
 * Note that no pivoting is performed. This is \b not a rank-revealing
 * decomposition. If you want that feature, use FullPivHouseholderQR or
 * ColPivHouseholderQR instead.
 *
 * This Householder QR decomposition is faster, but less numerically stable and
 * less feature-full than FullPivHouseholderQR or ColPivHouseholderQR.
 *
...
public:
  ...
  HouseholderQR(Index rows, Index cols)
      : m_qr(rows, cols), m_hCoeffs((std::min)(rows, cols)), m_temp(cols),
  ...
  explicit HouseholderQR(const EigenBase<InputType> &matrix)
      : m_qr(matrix.rows(), matrix.cols()),
        ...
        m_temp(matrix.cols()), m_isInitialized(false) {
   ...
   * can also be assigned to a dense Matrix object. Here is an example showing
   * how to recover the full or thin matrix Q, as well as how to perform matrix
...
void householder_qr_inplace_unblocked(MatrixQR &mat, HCoeffs &hCoeffs,
  ...
  for (Index k = 0; k < size; ++k) {
    ...
    mat.col(k)
        .tail(remainingRows)
        .makeHouseholderInPlace(hCoeffs.coeffRef(k), beta);
...
template <typename MatrixQR, typename HCoeffs,
          ...
          bool InnerStrideIsOne = (MatrixQR::InnerStrideAtCompileTime == 1 &&
                                   HCoeffs::InnerStrideAtCompileTime == 1)>
struct householder_qr_inplace_blocked {
  // This is specialized for LAPACK-supported Scalar types in
  ...
  static void run(MatrixQR &mat, HCoeffs &hCoeffs, Index maxBlockSize = 32,
    ...
    typedef Matrix<Scalar, Dynamic, 1, ColMajor, MatrixQR::MaxColsAtCompileTime,
                   1>
        TempType;
    TempType tempVector;
    if (tempData == 0) {
    ...
    for (k = 0; k < size; k += blockSize) {
      ...
      if (tcols) {
        ...
        apply_block_householder_on_the_left(A21_22, A11_21, hCoeffsSegment,
...
template <typename _MatrixType>
template <typename RhsType, typename DstType>
void HouseholderQR<_MatrixType>::_solve_impl(const RhsType &rhs,
                                             DstType &dst) const {
...
template <typename _MatrixType>
template <bool Conjugate, typename RhsType, typename DstType>
void HouseholderQR<_MatrixType>::_solve_impl_transposed(const RhsType &rhs,
                                                        DstType &dst) const {
  const Index rank = (std::min)(rows(), cols());
  ...
  dst.applyOnTheLeft(
      householderQ().setLength(rank).template conjugateIf<!Conjugate>());
}
#endif
...
void HouseholderQR<MatrixType>::computeInPlace() {
  ...
  internal::householder_qr_inplace_blocked<MatrixType, HCoeffsType>::run(
      m_qr, m_hCoeffs, 48, m_temp.data());
