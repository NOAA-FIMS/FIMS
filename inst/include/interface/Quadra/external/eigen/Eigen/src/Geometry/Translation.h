 ...
 * \tparam _Dim the  dimension of the space, can be a compile time value or
 * Dynamic
 *
 * \note This class is not aimed to be used to store a translation
 * transformation, but rather to make easier the constructions and updates of
 * Transform objects.
 *
...
public:
  ...
  /** Constructs and initialize the translation transformation from a vector of
  ...
  EIGEN_DEVICE_FUNC inline AffineTransformType
  ...
  EIGEN_DEVICE_FUNC inline IsometryTransformType
   ...
   * translation \a t */
  // its a nightmare to define a templated friend function outside its
  // declaration
  template <typename OtherDerived>
  friend EIGEN_DEVICE_FUNC inline AffineTransformType
  operator*(const EigenBase<OtherDerived> &linear, const Translation &t) {
  ...
  EIGEN_DEVICE_FUNC inline
      typename internal::cast_return_type<Translation,
                                          Translation<NewScalarType, Dim>>::type
      cast() const {
    return typename internal::cast_return_type<
        Translation, Translation<NewScalarType, Dim>>::type(*this);
  }

  /** Copy constructor with scalar type conversion */
  template <typename OtherScalarType>
  EIGEN_DEVICE_FUNC inline explicit Translation(
  ...
  isApprox(const Translation &other,
