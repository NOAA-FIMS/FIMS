...
pcast(const SrcPacket &a, const SrcPacket & /*b*/, const SrcPacket & /*c*/,
      const SrcPacket & /*d*/) {
  return static_cast<TgtPacket>(a);
}
template <typename SrcPacket, typename TgtPacket>
...
EIGEN_DEVICE_FUNC inline Target preinterpret(
    const Packet &a); /* { return reinterpret_cast<const Target&>(a); } */
...
template <> EIGEN_DEVICE_FUNC inline bool padd(const bool &a, const bool &b) {
  return a || b;
}
...
// Avoid compiler warning for boolean algebra.
template <> EIGEN_DEVICE_FUNC inline bool pmul(const bool &a, const bool &b) {
  return a && b;
}
...
struct ptrue_impl<
    T, typename internal::enable_if<
           is_scalar<T>::value && NumTraits<T>::RequireInitialization>::type> {
...
template <typename Packet>
EIGEN_DEVICE_FUNC inline Packet pcmp_lt_or_nan(const Packet &a,
                                               const Packet &b) {
  return a >= b ? pzero(a) : ptrue(a);
}

template <typename T> struct bit_and {
  EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR EIGEN_ALWAYS_INLINE T
...
struct bitwise_helper<
    T, typename internal::enable_if<
           is_scalar<T>::value && (NumTraits<T>::IsInteger ||
...
template <typename Packet, typename EnableIf = void> struct pselect_impl {
  static EIGEN_DEVICE_FUNC inline Packet run(const Packet &mask,
...
template <typename Packet>
struct pselect_impl<
    Packet, typename internal::enable_if<is_scalar<Packet>::value>::type> {
  static EIGEN_DEVICE_FUNC inline Packet run(const Packet &mask,
                                             const Packet &a, const Packet &b) {
...
template <typename Packet>
EIGEN_DEVICE_FUNC inline Packet pselect(const Packet &mask, const Packet &a,
                                        const Packet &b) {
  return pselect_impl<Packet>::run(mask, a, b);
}

template <>
EIGEN_DEVICE_FUNC inline bool pselect<bool>(const bool &cond, const bool &a,
                                            const bool &b) {
  return cond ? a : b;
}
...
template <int NaNPropagation> struct pminmax_impl {
  ...
  static EIGEN_DEVICE_FUNC inline Packet run(const Packet &a, const Packet &b,
                                             Op op) {
    return op(a, b);
  }
};
...
#define EIGEN_BINARY_OP_NAN_PROPAGATION(Type, Func)                            \
  [](const Type &a, const Type &b) { return Func(a, b); }
#endif
...
EIGEN_DEVICE_FUNC inline Packet pmin(const Packet &a, const Packet &b) {
  return pminmax_impl<NaNPropagation>::run(
...
template <> EIGEN_DEVICE_FUNC inline unsigned int pabs(const unsigned int &a) {
  return a;
}
template <>
...
/** \internal \returns the significant and exponent of the underlying floating
 * point numbers See https://en.cppreference.com/w/cpp/numeric/math/frexp
 */
template <typename Packet>
 ...
 */
template <typename Packet>
EIGEN_DEVICE_FUNC inline Packet pldexp(const Packet &a,
...
/** \internal \returns a packet version of \a *from, from must be 16 bytes
 * aligned */
template <typename Packet>
 ...
 * There is no generic implementation. We only have implementations for
 * specialized cases. Generic case should not be called.
 */
template <typename Packet>
EIGEN_DEVICE_FUNC inline
    typename enable_if<unpacket_traits<Packet>::masked_load_available,
                       Packet>::type
    ploadu(const typename unpacket_traits<Packet>::type *from,
...
/** \internal \returns a packet with constant coefficients \a a[0], e.g.:
 * (a[0],a[0],a[0],a[0]) */
template <typename Packet>
EIGEN_DEVICE_FUNC inline Packet
 ...
 * For instance, for a packet of 8 elements, 4 scalars will be read from \a
 * *from and duplicated to form:
 * {from[0],from[0],from[1],from[1],from[2],from[2],from[3],from[3]} Currently,
 * this function is only used for scalar * complex products.
 */
template <typename Packet>
 ...
 * For instance, for a packet of 8 elements, 2 scalars will be read from \a
 * *from and replicated to form:
 * {from[0],from[0],from[0],from[0],from[1],from[1],from[1],from[1]} Currently,
 * this function is only used in matrix products. For packet-size smaller or
 * equal to 4, this function is equivalent to pload1
 */
template <typename Packet>
EIGEN_DEVICE_FUNC inline Packet
...
pbroadcast4(const typename unpacket_traits<Packet>::type *a, Packet &a0,
...
template <typename Packet>
EIGEN_DEVICE_FUNC inline void
pbroadcast2(const typename unpacket_traits<Packet>::type *a, Packet &a0,
            Packet &a1) {
  a0 = pload1<Packet>(a + 0);
  a1 = pload1<Packet>(a + 1);
}

/** \internal \brief Returns a packet with coefficients
 * (a,a+1,...,a+packet_size-1). */
template <typename Packet>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet
...
/** \internal \returns a packet with constant coefficients \a a, e.g.: (x, 0, x,
   0), where x is the value of all 1-bits. */
template <typename Packet>
...
template <typename Scalar, typename Packet>
EIGEN_DEVICE_FUNC inline
    typename enable_if<unpacket_traits<Packet>::masked_store_available,
                       void>::type
...
EIGEN_DEVICE_FUNC inline void prefetch(const Scalar *addr) {
  ...
  asm(" prefetch.L1 [ %1 ];" : "=r"(addr) : "r"(addr));
    ...
    (EIGEN_COMP_GNUC || EIGEN_COMP_CLANG || EIGEN_COMP_ICC)
...
/** \internal \returns \a a with real and imaginary part flipped (for complex
 * type only) */
template <typename Packet>
...
/** \internal \returns the sum of the elements of upper and lower half of \a a
 * if \a a is larger than 4. For a packet {a0, a1, a2, a3, a4, a5, a6, a7}, it
 * returns a half packet {a0+a4, a1+a5, a2+a6, a3+a7} For packet-size smaller or
 * equal to 4, this boils down to a noop.
 */
template <typename Packet>
EIGEN_DEVICE_FUNC inline
    typename conditional<(unpacket_traits<Packet>::size % 8) == 0,
...
predux_mul(const Packet &a) {
  typedef typename unpacket_traits<Packet>::type Scalar;
  return predux_helper(a,
                       EIGEN_BINARY_OP_NAN_PROPAGATION(Scalar, (pmul<Scalar>)));
}
...
predux_min(const Packet &a) {
  typedef typename unpacket_traits<Packet>::type Scalar;
  return predux_helper(a, EIGEN_BINARY_OP_NAN_PROPAGATION(
                              Scalar, (pmin<PropagateFast, Scalar>)));
}
...
predux_min(const Packet &a) {
  typedef typename unpacket_traits<Packet>::type Scalar;
  return predux_helper(a, EIGEN_BINARY_OP_NAN_PROPAGATION(
                              Scalar, (pmin<NaNPropagation, Scalar>)));
}
...
predux_max(const Packet &a) {
  typedef typename unpacket_traits<Packet>::type Scalar;
  return predux_helper(a, EIGEN_BINARY_OP_NAN_PROPAGATION(
                              Scalar, (pmax<PropagateFast, Scalar>)));
}
...
predux_max(const Packet &a) {
  typedef typename unpacket_traits<Packet>::type Scalar;
  return predux_helper(a, EIGEN_BINARY_OP_NAN_PROPAGATION(
                              Scalar, (pmax<NaNPropagation, Scalar>)));
}
 ...
 */
// not needed yet
// template<typename Packet> EIGEN_DEVICE_FUNC inline bool predux_all(const
// Packet& a) { return bool(a); }
...
EIGEN_DEVICE_FUNC inline bool predux_any(const Packet &a) {
  // Dirty but generic implementation where "true" is assumed to be non 0 and
  // all the sames. It is expected that "true" is either:
  //  - Scalar(1)
  //  - bits full of ones (NaN for floats),
  //  - or first bit equals to 1 (1 for ints, smallest denormal for floats).
  // For all these cases, taking the sum is just fine, and this boils down to a
  // no-op for scalars.
  typedef typename unpacket_traits<Packet>::type Scalar;
...
/** \internal copy a packet with constant coefficient \a a (e.g., [a,a,a,a]) to
 * \a *to. \a to must be 16 bytes aligned */
// NOTE: this function must really be templated on the packet type (think about
// different packet types for the same scalar type)
template <typename Packet>
inline void pstore1(typename unpacket_traits<Packet>::type *to,
...
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void pstoret(Scalar *to,
                                                   const Packet &from) {
  if (Alignment >= unpacket_traits<Packet>::alignment)
    pstore(to, from);
  else
...
template <>
inline std::complex<float> pmul(const std::complex<float> &a,
...
template <>
inline std::complex<double> pmul(const std::complex<double> &a,
