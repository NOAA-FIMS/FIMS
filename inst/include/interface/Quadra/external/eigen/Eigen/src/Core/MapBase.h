...
#define EIGEN_STATIC_ASSERT_INDEX_BASED_ACCESS(Derived)                        \
  EIGEN_STATIC_ASSERT(                                                         \
      (int(internal::evaluator<Derived>::Flags) & LinearAccessBit) ||          \
 ...
 *
 * This base class provides the const low-level accessors (e.g. coeff, coeffRef)
 * of dense Map and Block objects with direct access. Typical users do not have
 * to directly deal with this class.
 *
 * This class can be extended by through the macro plugin \c
 * EIGEN_MAPBASE_PLUGIN. See \link TopicCustomizing_Plugins customizing Eigen
 * \endlink for details.
 *
 * The \c Derived class has to provide the following two methods describing the
 * memory layout: \code Index innerStride() const; \endcode \code Index
 * outerStride() const; \endcode
 *
...
protected:
  ...
  checkSanity(typename internal::enable_if<(internal::traits<T>::Alignment > 0),
                                           void *>::type = 0) const {
 ...
 * access
 *
 * This base class provides the non-const low-level accessors (e.g. coeff and
 * coeffRef) of dense Map and Block objects with direct access. It inherits
 * MapBase<Derived, ReadOnlyAccessors> which defines the const variant for
...
public:
  ...
  // In theory we could simply refer to Base:Base::operator=, but MSVC does not
  // like Base::Base, see bugs 821 and 920.
