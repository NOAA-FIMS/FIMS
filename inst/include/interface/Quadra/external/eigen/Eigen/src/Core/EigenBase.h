 ...
 * and a constructor MatrixBase(T).
 *
 * In other words, an EigenBase object is an object that can be copied into a
 * MatrixBase.
 *
 * Besides MatrixBase-derived classes, this also includes special matrix classes
 * such as diagonal matrices, etc.
 *
 * Notice that this class is trivial, it is only used to disambiguate overloaded
 * functions.
 *
...
template <typename Derived> struct EigenBase {
   ...
   * deprecated. Use Eigen::Index instead. Deprecation is not marked with a
   * doxygen comment because there are too many existing usages to add the
 ...
 * write its own special matrix without having to modify MatrixBase
 *
