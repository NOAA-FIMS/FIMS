 ...
 * \class FullPivLU
 *
 * \brief LU decomposition of a matrix with complete pivoting, and related
 * features
 *
 * \tparam _MatrixType the type of the matrix of which we are computing the LU
 * decomposition
 *
 * This class represents a LU decomposition of any matrix, with complete
 * pivoting: the matrix A is decomposed as \f$ A = P^{-1} L U Q^{-1} \f$ where L
 * is unit-lower-triangular, U is upper-triangular, and P and Q are permutation
 * matrices. This is a rank-revealing LU decomposition. The eigenvalues
 * (diagonal coefficients) of U are sorted in such a way that any zeros are at
 * the end.
 *
 * This decomposition provides the generic approach to solving systems of linear
 * equations, computing the rank, invertibility, inverse, kernel, and
 * determinant.
 *
 ...
 * working with the SVD allows to select the smallest singular values of the
 * matrix, something that the LU decomposition doesn't see.
 *
 * The data of the LU decomposition can be directly accessed through the methods
...
public:
   ...
   * (column-space).
   *
   * \param originalMatrix the original matrix, of which *this is the LU
   * decomposition. The reason why it is needed to pass it here, is that this
   * allows a large optimization, as otherwise this method would need to
   * reconstruct it from the LU decomposition.
   *
   * \note If the image has dimension zero, then the returned matrix is a
   * column-vector filled with zeros.
   *
   * \note This method has to determine which pivots should be considered
   * nonzero. For that, it uses the threshold value that you can control by
   ...
   * matrix, the only requirement in order for the equation to make sense is
   * that b.rows()==A.rows(), where A is the matrix of which *this is the LU
   * decomposition.
   *
   * \returns a solution.
   *
   ...
   * greater than \f$ \vert pivot \vert \leqslant threshold \times \vert
   * maxpivot \vert \f$ where maxpivot is the biggest pivot.
   *
  ...
  RealScalar threshold() const {
    ...
    return m_usePrescribedThreshold
               ...
               // this formula comes from experimenting (see "LU precision
               // tuning" thread on the list) and turns out to be identical to
               // Higham's formula used already in LDLt.
...
FullPivLU<MatrixType>::FullPivLU(Index rows, Index cols)
    : m_lu(rows, cols), m_p(rows), m_q(cols), m_rowsTranspositions(rows),
      m_colsTranspositions(cols), m_isInitialized(false),
      m_usePrescribedThreshold(false) {}

template <typename MatrixType>
template <typename InputType>
FullPivLU<MatrixType>::FullPivLU(const EigenBase<InputType> &matrix)
    : m_lu(matrix.rows(), matrix.cols()), m_p(matrix.rows()),
      m_q(matrix.cols()), m_rowsTranspositions(matrix.rows()),
      m_colsTranspositions(matrix.cols()), m_isInitialized(false),
      m_usePrescribedThreshold(false) {
...
FullPivLU<MatrixType>::FullPivLU(EigenBase<InputType> &matrix)
    : m_lu(matrix.derived()), m_p(matrix.rows()), m_q(matrix.cols()),
      m_rowsTranspositions(matrix.rows()), m_colsTranspositions(matrix.cols()),
      m_isInitialized(false), m_usePrescribedThreshold(false) {
  computeInPlace();
}

template <typename MatrixType> void FullPivLU<MatrixType>::computeInPlace() {
  ...
  // can't accumulate on-the-fly because that will be done in reverse order for
  // the rows.
  ...
  for (Index k = 0; k < size; ++k) {
    ...
    // biggest coefficient in the remaining bottom-right corner (starting at row
    // k, col k)
    ...
    if (biggest_in_corner == Score(0)) {
      // before exiting, make sure to initialize the still uninitialized
      // transpositions in a sane state without destroying what we already have.
      m_nonzero_pivots = k;
      for (Index i = k; i < size; ++i) {
        m_rowsTranspositions.coeffRef(i) =
            internal::convert_index<StorageIndex>(i);
        m_colsTranspositions.coeffRef(i) =
            internal::convert_index<StorageIndex>(i);
      }
      break;
    }
    ...
    RealScalar abs_pivot = internal::abs_knowing_score<Scalar>()(
        m_lu(row_of_biggest_in_corner, col_of_biggest_in_corner),
        biggest_in_corner);
    ...
    m_rowsTranspositions.coeffRef(k) =
        internal::convert_index<StorageIndex>(row_of_biggest_in_corner);
    m_colsTranspositions.coeffRef(k) =
        internal::convert_index<StorageIndex>(col_of_biggest_in_corner);
  ...
  // the main loop is over, we still have to accumulate the transpositions to
  // find the permutations P and Q
...
struct kernel_retval<FullPivLU<_MatrixType>>
  ...
  template <typename Dest> void evalTo(Dest &dst) const {
    ...
    if (dimker == 0) {
      // The Kernel is just {0}, so it doesn't have a basis properly speaking,
      // but let's avoid crashing/asserting as that depends on floating point
      // calculations. Let's just return a single column vector filled with
      // zeros.
      dst.setZero();
      return;
    }
     ...
     * us use that to construct dimKer linearly independent vectors in Ker U.
     */
    ...
    // permuting the rows and cols to bring the nonnegligible pivots to the top
    // of the main diagonal. We need that to be able to apply our triangular
    // solvers.
    // FIXME when we get triangularView-for-rectangular-matrices, this can be
    // simplified
    ...
    for (Index i = 0; i < rank(); ++i) {
      ...
      m.row(i).tail(cols - i) =
          dec().matrixLU().row(pivots.coeff(i)).tail(cols - i);
    }
    ...
    // notice that the math behind this suggests that we should apply this to
    // the negative of the RHS, but for performance we just put the negative
    // sign elsewhere, see below.
    m.topLeftCorner(rank(), rank())
        .template triangularView<Upper>()
        .solveInPlace(m.topRightCorner(rank(), dimker));

    // now we must undo the column permutation that we had applied!
    for (Index i = rank() - 1; i >= 0; --i)
      m.col(i).swap(m.col(pivots.coeff(i)));

    // see the negative sign in the next line, that's what we were talking about
    // above.
    for (Index i = 0; i < rank(); ++i)
...
struct image_retval<FullPivLU<_MatrixType>>
  ...
  template <typename Dest> void evalTo(Dest &dst) const {
    using std::abs;
    if (rank() == 0) {
      // The Image is just {0}, so it doesn't have a basis properly speaking,
      // but let's avoid crashing/asserting as that depends on floating point
      // calculations. Let's just return a single column vector filled with
      // zeros.
      dst.setZero();
    ...
    for (Index i = 0; i < rank(); ++i)
      dst.col(i) = originalMatrix().col(
          dec().permutationQ().indices().coeff(pivots.coeff(i)));
  }
};
...
void FullPivLU<_MatrixType>::_solve_impl(const RhsType &rhs,
   ...
   * invertible. Step 3: replace c by the solution x to Ux = c. May or may not
   * exist. Step 4: result = Q * c;
   */
...
struct Assignment<DstXprType, Inverse<FullPivLU<MatrixType>>,
                  internal::assign_op<typename DstXprType::Scalar,
                                      typename FullPivLU<MatrixType>::Scalar>,
                  Dense2Dense> {
  ...
  static void run(DstXprType &dst, const SrcXprType &src,
    ...
    dst = src.nestedExpression().solve(
        MatrixType::Identity(src.rows(), src.cols()));
  }
};
} // end namespace internal
