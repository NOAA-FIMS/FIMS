  ...
  static void run(const Lhs &lhs, Rhs &other) {
    ...
    for (Index col = 0; col < other.cols(); ++col) {
      for (Index i = lhs.cols() - 1; i >= 0; --i) {
        ...
        {
          if (!(Mode & UnitDiag)) {
            // TODO replace this by a binary search. make sure the binary search
            // is safe for partially sorted elements
            LhsIterator it(lhsEval, i);
            while (it && it.index() != i)
              ++it;
            eigen_assert(it && it.index() == i);
...
void TriangularViewImpl<ExpressionType, Mode, Sparse>::solveInPlace(
    MatrixBase<OtherDerived> &other) const {
  eigen_assert(derived().cols() == derived().rows() &&
               derived().cols() == other.rows());
...
void TriangularViewImpl<ExpressionType, Mode, Sparse>::solveInPlace(
  ...
  internal::sparse_solve_triangular_sparse_selector<
      ExpressionType, OtherDerived, Mode>::run(derived().nestedExpression(),
                                               other.derived());
