 ...
 * \class LLT
 *
 * \brief Standard Cholesky decomposition (LL^T) of a matrix and associated
 * features
 *
 * \tparam _MatrixType the type of the matrix of which we are computing the LL^T
 ...
 * won't be read.
 *
 * This class performs a LL^T Cholesky decomposition of a symmetric, positive
 * definite matrix A such that A = LL^* = U^*U, where L is lower triangular.
 *
 ...
 * decomposition without square root which is more stable and even faster.
 * Nevertheless, this standard Cholesky decomposition remains useful in many
 ...
 * decomposition is only stable on positive definite matrices, use LDLT instead
 * for the semidefinite case. Also, do not use a Cholesky decomposition to
 * determine whether a system of equations has a solution.
 *
 * Example: \include LLT_example.cpp
 * Output: \verbinclude LLT_example.out
 *
 * \b Performance: for best performance, it is recommended to use a column-major
 ...
 * \endlink mechanism.
 *
 * Note that during the decomposition, only the lower (or upper, as defined by
 * _UpLo) triangular part of A is considered. Therefore, the strict lower part
 * does not have to store correct values.
 *
...
public:
   ...
   * solution theoretically exists and is unique regardless of b.
   *
...
static Index
llt_rank_update_lower(MatrixType &mat, const VectorType &vec,
...
template <typename Scalar> struct llt_inplace<Scalar, Lower> {
  ...
  template <typename MatrixType, typename VectorType>
  static Index rankUpdate(MatrixType &mat, const VectorType &vec,
...
LLT<_MatrixType, _UpLo>::rankUpdate(const VectorType &v,
  ...
  if (internal::llt_inplace<typename MatrixType::Scalar, UpLo>::rankUpdate(
          m_matrix, v, sigma) >= 0)
...
template <typename _MatrixType, int _UpLo>
template <typename RhsType, typename DstType>
void LLT<_MatrixType, _UpLo>::_solve_impl(const RhsType &rhs,
                                          DstType &dst) const {
...
template <typename _MatrixType, int _UpLo>
template <bool Conjugate, typename RhsType, typename DstType>
void LLT<_MatrixType, _UpLo>::_solve_impl_transposed(const RhsType &rhs,
                                                     DstType &dst) const {
  dst = rhs;
 ...
 *
 * This version avoids a copy when the right hand side matrix b is not needed
 * anymore.
 *
 * \warning The parameter is only marked 'const' to make the C++ compiler accept
 * a temporary expression here. This function will const_cast it, so constness
 * isn't honored here.
 *
...
void LLT<MatrixType, _UpLo>::solveInPlace(
    const MatrixBase<Derived> &bAndX) const {
  eigen_assert(m_isInitialized && "LLT is not initialized.");
