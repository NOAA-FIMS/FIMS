 ...
 * \tparam Rows the number of rows of the reshape we are taking at compile time
 * (optional) \tparam Cols the number of columns of the reshape we are taking at
 * compile time (optional) \tparam Order can be ColMajor or RowMajor, default is
 * ColMajor.
 *
 * This class represents an expression of either a fixed-size or dynamic-size
 * reshape. It is the return type of DenseBase::reshaped(NRowsType,NColsType)
 ...
 * you will need to use this class. In C++11, it is advised to use the \em auto
...
struct traits<Reshaped<XprType, Rows, Cols, Order>> : traits<XprType> {
  ...
  enum {
    ...
    XpxStorageOrder =
        ((int(traits<XprType>::Flags) & RowMajorBit) == RowMajorBit) ? RowMajor
                                                                     : ColMajor,
    ReshapedStorageOrder =
        (RowsAtCompileTime == 1 && ColsAtCompileTime != 1)   ? RowMajor
        : (ColsAtCompileTime == 1 && RowsAtCompileTime != 1) ? ColMajor
                                                             : XpxStorageOrder,
    HasSameStorageOrderAsXprType = (ReshapedStorageOrder == XpxStorageOrder),
    InnerSize = (ReshapedStorageOrder == int(RowMajor))
    ...
    MaskPacketAccessBit = (InnerSize == Dynamic ||
                           (InnerSize % packet_traits<Scalar>::size) == 0) &&
                                  (InnerStrideAtCompileTime == 1)
                              ? PacketAccessBit
                              : 0,
    // MaskAlignedBit = ((OuterStrideAtCompileTime!=Dynamic) &&
    // (((OuterStrideAtCompileTime * int(sizeof(Scalar))) % 16) == 0)) ?
    // AlignedBit : 0,
    ...
    FlagsRowMajorBit =
        (ReshapedStorageOrder == int(RowMajor)) ? RowMajorBit : 0,
    FlagsDirectAccessBit = HasDirectAccess ? DirectAccessBit : 0,
    Flags0 = traits<XprType>::Flags &
             ((HereditaryBits & ~RowMajorBit) | MaskPacketAccessBit),
...
template <typename XprType, int Rows, int Cols, int Order>
class Reshaped
    : public ReshapedImpl<XprType, Rows, Cols, Order,
                          typename internal::traits<XprType>::StorageKind> {
  typedef ReshapedImpl<XprType, Rows, Cols, Order,
                       typename internal::traits<XprType>::StorageKind>
      Impl;
...
// The generic default implementation for dense reshape simply forward to the
// internal::ReshapedImpl_dense that must be specialized for direct and
// non-direct access...
template <typename XprType, int Rows, int Cols, int Order>
class ReshapedImpl<XprType, Rows, Cols, Order, Dense>
    : public internal::ReshapedImpl_dense<
          XprType, Rows, Cols, Order,
          internal::traits<
              Reshaped<XprType, Rows, Cols, Order>>::HasDirectAccess> {
  typedef internal::ReshapedImpl_dense<
      XprType, Rows, Cols, Order,
...
public:
  ...
  EIGEN_DEVICE_FUNC inline ReshapedImpl(XprType &xpr, Index reshapeRows,
                                        Index reshapeCols)
      : Impl(xpr, reshapeRows, reshapeCols) {}
};
...
class ReshapedImpl_dense<XprType, Rows, Cols, Order, false>
    : public internal::dense_xpr_base<
          Reshaped<XprType, Rows, Cols, Order>>::type {
...
struct evaluator<Reshaped<ArgType, Rows, Cols, Order>>
    : reshaped_evaluator<
          ArgType, Rows, Cols, Order,
          traits<Reshaped<ArgType, Rows, Cols, Order>>::HasDirectAccess> {
  ...
  enum {
    ...
    //
    //     InnerStrideAtCompileTime =
    //     traits<XprType>::HasSameStorageOrderAsXprType
    //                              ?
    ...
    FlagsLinearAccessBit =
        (traits<XprType>::RowsAtCompileTime == 1 ||
         traits<XprType>::ColsAtCompileTime == 1 || HasDirectAccess)
            ? LinearAccessBit
            : 0,
    FlagsRowMajorBit = (traits<XprType>::ReshapedStorageOrder == int(RowMajor))
                           ? RowMajorBit
                           : 0,
    FlagsDirectAccessBit = HasDirectAccess ? DirectAccessBit : 0,
    Flags0 = evaluator<ArgType>::Flags & (HereditaryBits & ~RowMajorBit),
    Flags =
        Flags0 | FlagsLinearAccessBit | FlagsRowMajorBit | FlagsDirectAccessBit,

    PacketAlignment = unpacket_traits<PacketScalar>::alignment,
    Alignment = evaluator<ArgType>::Alignment
  };
  typedef reshaped_evaluator<ArgType, Rows, Cols, Order, HasDirectAccess>
...
struct reshaped_evaluator<ArgType, Rows, Cols, Order,
  ...
  inline Scalar &coeffRef(Index index) {
    ...
    const RowCol row_col =
        index_remap(Rows == 1 ? 0 : index, Rows == 1 ? index : 0);
    return m_argImpl.coeffRef(row_col.first, row_col.second);
  }
...
struct reshaped_evaluator<ArgType, Rows, Cols, Order,
  ...
  EIGEN_DEVICE_FUNC explicit reshaped_evaluator(const XprType &xpr)
    ...
    // TODO: for the 3.4 release, this should be turned to an internal
    // assertion, but let's keep it as is for the beta lifetime
    eigen_assert(
        ((internal::UIntPtr(xpr.data()) %
