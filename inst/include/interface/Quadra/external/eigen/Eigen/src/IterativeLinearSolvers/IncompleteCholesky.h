 ...
 * can be Lower or Upper. Default is Lower. \tparam _OrderingType The ordering
 * method to use, either AMDOrdering<> or NaturalOrdering<>. Default is
 * AMDOrdering<int>, unless EIGEN_MPL2_ONLY is defined, in which case the
 * default is NaturalOrdering<int>.
 *
 ...
 *
 */
template <typename Scalar, int _UpLo = Lower,
          typename _OrderingType = AMDOrdering<int>>
class IncompleteCholesky
    : public SparseSolverBase<
          IncompleteCholesky<Scalar, _UpLo, _OrderingType>> {
protected:
  typedef SparseSolverBase<IncompleteCholesky<Scalar, _UpLo, _OrderingType>>
      Base;
...
public:
   ...
   */
  IncompleteCholesky()
      : m_initialShift(1e-3), m_analysisIsOk(false),
...
private:
  inline void updateList(Ref<const VectorIx> colPtr, Ref<VectorIx> rowIdx,
                         Ref<VectorSx> vals, const Index &col, const Index &jk,
...
void IncompleteCholesky<Scalar, _UpLo, OrderingType>::factorize(
    const _MatrixType &mat) {
  using std::sqrt;
  eigen_assert(m_analysisIsOk && "analyzePattern() should be called first");

  // Dropping strategy : Keep only the p largest elements per column, where p is
  // the number of elements in the column of the original matrix. Other
  // strategies will be added

  // Apply the fill-reducing permutation computed in analyzePattern()
  if (m_perm.rows() == mat.rows()) // To detect the null permutation
  {
    // The temporary is needed to make sure that the diagonal entry is properly
    // sorted
    FactorType tmp(mat.rows(), mat.cols());
    tmp = mat.template selfadjointView<_UpLo>().twistedBy(m_perm);
    m_L.template selfadjointView<Lower>() =
  ...
  Map<VectorIx> colPtr(m_L.outerIndexPtr(),
                       n + 1); // Pointer to the beginning of each row
  VectorIx firstElt(n - 1); // for each j, points to the next entry in vals that
                            // will be used in the factorization
  VectorList listCol(
      n); // listCol(j) is a linked list of columns to update column j
  ...
  // TODO disable scaling if not needed, i.e., if it is roughly uniform? (this
  // will make solve() faster)
  ...
  do {
    ...
    for (; j < n; ++j) {
      ...
      Scalar diag =
          vals[colPtr[j]]; // It is assumed that only the lower part is stored
      col_nnz = 0;
      ...
      // p is the original number of elements in the column (without the
      // diagonal)
...
template <typename Scalar, int _UpLo, typename OrderingType>
inline void IncompleteCholesky<Scalar, _UpLo, OrderingType>::updateList(
    Ref<const VectorIx> colPtr, Ref<VectorIx> rowIdx, Ref<VectorSx> vals,
    const Index &col, const Index &jk, VectorIx &firstElt,
    VectorList &listCol) {
  if (jk < colPtr(col + 1)) {
    ...
    listCol[rowIdx(jk)].push_back(
        internal::convert_index<StorageIndex, Index>(col));
  }
}
