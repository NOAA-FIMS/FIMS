...
template <typename IndexType> IndexType ones_complement(const IndexType r) {
  return (-(r)-1);
}
...
template <typename IndexType> struct ColStructure {
...
template <typename IndexType> struct RowStructure {
  ...
  union {
    ...
    IndexType
        first_column; /* first column in row (used in garbage collection) */
  } shared2;
...
/*
  ...
  Explicit typecast to IndexType added Sept. 23, 2002, COLAMD version 2.2, to
  avoid gcc -pedantic warning messages.
*/
template <typename IndexType> inline IndexType colamd_c(IndexType n_col) {
  return IndexType(((n_col) + 1) * sizeof(ColStructure<IndexType>) /
                   sizeof(IndexType));
}

template <typename IndexType> inline IndexType colamd_r(IndexType n_row) {
  return IndexType(((n_row) + 1) * sizeof(RowStructure<IndexType>) /
                   sizeof(IndexType));
}
...
static IndexType init_rows_cols(IndexType n_row, IndexType n_col,
                                ...
                                ColStructure<IndexType> col[], IndexType A[],
                                IndexType p[], IndexType stats[NStats]);

template <typename IndexType>
static void
init_scoring(IndexType n_row, IndexType n_col, RowStructure<IndexType> Row[],
             ColStructure<IndexType> Col[], IndexType A[], IndexType head[],
             double knobs[NKnobs], IndexType *p_n_row2, IndexType *p_n_col2,
             IndexType *p_max_deg);
...
static IndexType find_ordering(IndexType n_row, IndexType n_col, IndexType Alen,
                               RowStructure<IndexType> Row[],
                               ColStructure<IndexType> Col[], IndexType A[],
                               IndexType head[], IndexType n_col2,
                               IndexType max_deg, IndexType pfree);

template <typename IndexType>
static void order_children(IndexType n_col, ColStructure<IndexType> Col[],
                           IndexType p[]);

template <typename IndexType>
static void detect_super_cols(ColStructure<IndexType> Col[], IndexType A[],
                              IndexType head[], IndexType row_start,
 ...
 * so be careful for side effects, if you pass expressions as arguments to
 * COLAMD_RECOMMENDED.
 *
...
inline IndexType recommended(IndexType nnz, IndexType n_row, IndexType n_col) {
  if ((nnz) < 0 || (n_row) < 0 || (n_col) < 0)
    ...
    return (2 * (nnz) + colamd_c(n_col) + colamd_r(n_row) + (n_col) +
            ((nnz) / 5));
}
...
/**
 * \brief  Computes a column ordering using the column approximate minimum
 * degree ordering
 *
...
static bool compute_ordering(IndexType n_row, IndexType n_col, IndexType Alen,
  ...
  Colamd::RowStructure<IndexType>
      *Row; /* pointer into A of Row [0..n_row] array */
  Colamd::ColStructure<IndexType>
      *Col;                     /* pointer into A of Col [0..n_col] array */
...
static void init_scoring(
  ...
  dense_row_count = numext::maxi(
      IndexType(0),
      numext::mini(IndexType(knobs[Colamd::DenseRow] * n_col), n_col));
  dense_col_count = numext::maxi(
      IndexType(0),
      numext::mini(IndexType(knobs[Colamd::DenseCol] * n_row), n_row));
  COLAMD_DEBUG1(
...
static IndexType find_ordering /* return the number of garbage collections */
  ...
  IndexType
      pivot_col_thickness; /* number of columns represented by pivot col */
