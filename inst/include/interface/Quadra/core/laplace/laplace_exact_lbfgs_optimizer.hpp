...
struct LaplaceExactLBFGSOptions {
  LaplaceExactGradientOptions gradient_m;
...
struct LaplaceExactLBFGSResult {
  ...
  LaplaceExactGradientResult gradient_result_m;
  std::vector<LaplaceLBFGSIteration> history_m;
};

template <class Model>
inline LaplaceExactLBFGSResult optimize_laplace_fixed_effects_exact_lbfgs(
  ...
  if (theta_initial.empty()) {
    throw std::invalid_argument("optimize_laplace_fixed_effects_exact_lbfgs: "
                                "theta_initial cannot be empty");
  }
  ...
  if (!current.converged_m || !current.logdet_ok_m ||
    ...
    result.message_m =
        "Failed: initial exact Laplace gradient evaluation failed.";
  ...
  for (int iter = 0; iter < options.max_iterations_m; ++iter) {
    ...
    if (current.gradient_norm_m <= options.gradient_tolerance_m) {
      ...
      result.message_m =
          "Converged: exact fixed-effect gradient norm below tolerance.";
      break;
    }
    ...
    if (result.step_norm_m <= options.step_tolerance_m) {
      result.converged_m = true;
      result.message_m =
          "Converged: exact fixed-effect step norm below tolerance.";
      break;
    }
  ...
  if (!result.converged_m && result.message_m.empty()) {
    result.message_m = "Stopped: maximum exact LBFGS iterations reached.";
  }
...
inline LaplaceExactLBFGSResult optimize_laplace_fixed_effects_exact_lbfgs(
    ...
    const LaplaceExactLBFGSOptions &options = LaplaceExactLBFGSOptions()) {
  return optimize_laplace_fixed_effects_exact_lbfgs(
