...
/**
 * @brief Rcpp interface for DSEM selectivity.
 */
class DSEMSelectivityInterface : public SelectivityInterfaceBase {
public:
    ParameterVector beta_z;

    DSEMSelectivityInterface() : SelectivityInterfaceBase() {
        std::shared_ptr<DSEMSelectivityInterface> dsem_sel = 
            std::make_shared<DSEMSelectivityInterface>(*this);
        FIMSRcppInterfaceBase::fims_interface_objects.push_back(dsem_sel);
        SelectivityInterfaceBase::live_objects[this->id] = dsem_sel;
    }

    DSEMSelectivityInterface(const DSEMSelectivityInterface& other)
        : SelectivityInterfaceBase(other), beta_z(other.beta_z) {}

    virtual ~DSEMSelectivityInterface() {}
    virtual uint32_t get_id() { return this->id; }
    virtual double evaluate(double x) { return 1.0; }
    ...
    bool add_to_fims_tmb_internal() {
        ...
        // Instantiate the computational functor
        std::shared_ptr<fims_popdy::DSEMSelectivity<Type>> selectivity = 
            std::make_shared<fims_popdy::DSEMSelectivity<Type>>();
        
        selectivity->id = this->id;

        /**
         * @brief Standardized registration.
         * @details Replaces manual loops to handle resizing, value copying, 
         * fixed/random effect registration, and variable_map wiring.
         */
        register_beta_z_standardized<Type>(this, selectivity, info, "beta_z_sel");

        info->selectivity_models[this->id] = selectivity;
        return true;
    }
    ...
    virtual void finalize() {
        ...
        std::shared_ptr<fims_info::Information<double>> info = 
            fims_info::Information<double>::GetInstance();
        auto it = info->selectivity_models.find(this->id);
        if (it != info->selectivity_models.end()) {
            auto sel = std::dynamic_pointer_cast<fims_popdy::DSEMSelectivity<double>>(it->second);
            for (size_t i = 0; i < this->beta_z.size(); ++i) {
                this->beta_z[i].final_value_m = sel->beta_z[i];
            }
        }
        this->finalized = true;
    }
