...
public:
  ...
  virtual double evaluate(double x)
  {
    ...
    LogisticSel.slope[0] = fims_math::log(this->slope[0].initial_value_m);
  ...
  virtual void finalize()
  {
    ...
    else
    {
      ...
      for (size_t i = 0; i < slope.size(); i++)
      {
        ...
        {
          this->slope[i].final_value_m = fims_math::exp(sel->slope[i]);
        }
      }
    }
  }
  ...
  bool add_to_fims_tmb_internal()
  {
    ...
    {
      if (this->slope[i].initial_value_m <= 0.0)
      {
        throw std::invalid_argument(
            "LogisticSelectivity slope must be positive for log-slope parameterization.");
      }
      selectivity->slope[i] = fims_math::log(this->slope[i].initial_value_m);
      if (this->slope[i].estimation_type_m.get() == "fixed_effects")
      {
        ss.str("");
        ss << "Selectivity." << this->id << ".log_slope."
           << this->slope[i].id_m;
        info->RegisterParameterName(ss.str());
...
public:
  ...
  virtual double evaluate(double x)
  {
    ...
    DoubleLogisticSel.slope_asc[0] =
        fims_math::log(this->slope_asc[0].initial_value_m);
    ...
    DoubleLogisticSel.slope_desc[0] =
        fims_math::log(this->slope_desc[0].initial_value_m);
  ...
  virtual void finalize()
  {
    ...
    else
    {
      ...
      for (size_t i = 0; i < slope_asc.size(); i++)
      {
        ...
        {
          this->slope_asc[i].final_value_m = fims_math::exp(sel->slope_asc[i]);
        }
      }
      ...
      for (size_t i = 0; i < slope_desc.size(); i++)
      {
        ...
        {
          this->slope_desc[i].final_value_m = fims_math::exp(sel->slope_desc[i]);
        }
      }
    }
  }
  ...
  bool add_to_fims_tmb_internal()
  {
    ...
    {
      if (this->slope_asc[i].initial_value_m <= 0.0)
      {
        throw std::invalid_argument(
            "DoubleLogisticSelectivity slope_asc must be positive for log-slope parameterization.");
      }
      selectivity->slope_asc[i] = fims_math::log(this->slope_asc[i].initial_value_m);

      if (this->slope_asc[i].estimation_type_m.get() == "fixed_effects")
      {
        ss.str("");
        ss << "Selectivity." << this->id << ".log_slope_asc."
    ...
    {
      if (this->slope_desc[i].initial_value_m <= 0.0)
      {
        throw std::invalid_argument(
            "DoubleLogisticSelectivity slope_desc must be positive for log-slope parameterization.");
      }
      selectivity->slope_desc[i] = fims_math::log(this->slope_desc[i].initial_value_m);

      if (this->slope_desc[i].estimation_type_m.get() == "fixed_effects")
      {
        ss.str("");
        ss << "Selectivity." << this->id << ".log_slope_desc."
