...
class DnormDistributionsInterface : public DistributionsInterfaceBase {
 public:
  ...
  /**
   * @brief Evaluate the probability density function (pdf) for this
   * distribution.
   * @return The natural log of the pdf is returned.
   */
   ...
   * object, which allows the information to be passed to R.
   */
   ...
   * @return A string is returned specifying that the module_name, module_id,
   * module_type, observed_data_id, input_type, density_component, lpdf_value,
   * and specific information about the values; all formatted for a json file.
   */
...
class DlnormDistributionsInterface : public DistributionsInterfaceBase {
 public:
  ...
  /**
   * @copydoc DnormDistributionsInterface::evaluate
   */
  virtual double evaluate() {
  ...
  /**
   * @copydoc DnormDistributionsInterface::finalize
   */
  virtual void finalize() {
    if (this->finalized) {
      // log warning that finalize has been called more than once.
  ...
  /**
   * @copydoc DnormDistributionsInterface::to_json
   */
  virtual std::string to_json() {
    std::stringstream ss;
...
class DDirichletMultinomialDistributionsInterface
 ...
 public:
   ...
   * @brief The destructor.
   */
  virtual ~DDirichletMultinomialDistributionsInterface() {}
  /**
   * @brief Gets the ID of the interface base object.
   * @return The ID.
   */
  ...
  virtual double evaluate() {
    ...
    dirichlet_multinomial.dims[1] = this->dims[1];
  ...
  void finalize() {
    ...
    if (it == info->density_components.end()) {
      ...
      size_t n_x = dirichlet_multinomial->lpdf_vec.size();
      this->lpdf_vec = Rcpp::NumericVector(n_x);
      if (this->expected_values.size() != n_x) {
        this->expected_values.resize(n_x);
      }
      ...
      if (dirichlet_multinomial->input_type == "data") {
        dims.resize(2);
        dims[0] = dirichlet_multinomial->dims[0];
        dims[1] = dirichlet_multinomial->dims[1];
        for (size_t i = 0; i < dims[0]; i++) {
          for (size_t j = 0; j < dims[1]; j++) {
            size_t idx = (i * dims[1]) + j;
  ...
  virtual std::string to_json() {
    ...
    ss << " \"module_name\": \"density\",\n";
    ss << " \"module_id\": " << this->id_m << ",\n";
    ss << " \"module_type\": \"Dirichlet-multinomial\",\n";
    ss << "\"observed_data_id\" : " << this->interface_observed_data_id_m
       << ",\n";
    ss << " \"input_type\" : \"" << this->input_type_m << "\",\n";
    ss << " \"density_component\": {\n";
  ...
  bool add_to_fims_tmb_internal() {
    ...
    info->density_components[distribution->id] = distribution;
    FIMS_INFO_LOG("Done adding Dirichlet-multinomial to FIMS.");
    return true;
  }
