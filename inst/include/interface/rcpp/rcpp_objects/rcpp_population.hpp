...
public:
    ...
    void SetMaturity(uint32_t id) {
        this->maturity_id = id;
    }

    void SetGrowth(uint32_t id) {
        this->growth_id = id;
    }

    void SetRecruitment(uint32_t id) {
    ...
    virtual bool add_to_fims_tmb() {
        ...
        d0->nages = this->nages;
        d0->nyears = this->nyears;
        d0->ages.resize(this->nages);
        for (size_t i = 0; i < this->nages; i++) {
            d0->ages[i] = this->ages[i];
        }
        ...
        d1->nages = this->nages;
        d1->nyears = this->nyears;
        d1->ages.resize(this->nages);
        for (size_t i = 1; i < this->nages; i++) {
            d1->ages[i] = this->ages[i];
        }
        ...
        d2->nages = this->nages;
        d2->nyears = this->nyears;
        d2->ages.resize(this->nages);
        for (size_t i = 2; i < this->nages; i++) {
            d2->ages[i] = this->ages[i];
        }
        ...
        d3->nages = this->nages;
        d3->nyears = this->nyears;
        d3->ages.resize(this->nages);
        for (size_t i = 3; i < this->nages; i++) {
            d3->ages[i] = this->ages[i];
        }
