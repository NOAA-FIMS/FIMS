...
public:
    ...
    Rcpp::NumericVector estimated_proportion_female;
    std::string name;

    PopulationInterface() : PopulationInterfaceBase() {
        std::stringstream ss;
        ss << "population_" << this->id;
        this->name = ss.str();
    }
    ...
    virtual std::string to_json() {
        ...
        if (this->log_M.size() == 0) {
            ss << "],\n";
        } else {
            for (size_t i = 0; i < this->log_M.size() - 1; i++) {
                ss << this->log_M[i] << ", ";
            }
            ss << this->log_M[this->log_M.size() - 1] << "],\n";
        }

        ss << " \"estimated_values\":[";
        if (this->estimated_log_M.size() == 0) {
            ss << "],\n";
        } else {
            for (size_t i = 0; i < this->estimated_log_M.size() - 1; i++) {
                ss << this->estimated_log_M[i] << ", ";
            }
            ss << this->estimated_log_M[this->estimated_log_M.size() - 1] << "],\n";
        }

        ss << " \"is_estimated\":" << this->estimate_M << ",\n";
        ...
        if (this->log_init_naa.size() == 0) {
            ss << "],\n";
        } else {
            for (size_t i = 0; i < this->log_init_naa.size() - 1; i++) {
                ss << this->log_init_naa[i] << ", ";
            }
            ss << this->log_init_naa[this->log_init_naa.size() - 1] << "],\n";
        }

        ss << "  \"estimated_values\":[";
        if (this->estimated_log_init_naa.size() == 0) {
            ss << "],\n";
        } else {
            for (size_t i = 0; i < this->estimated_log_init_naa.size() - 1; i++) {
                ss << this->estimated_log_init_naa[i] << ", ";
            }
            ss << this->estimated_log_init_naa[this->estimated_log_init_naa.size() - 1] << "],\n";
        }

        ss << "  \"is_estimated\":" << this->estimate_initNAA << ",\n";
        ss << "  \"is_random_effect\":" << 0 << "\n },\n";
        ...
        if (this->proportion_female.size() == 0) {
            ss << "],\n";
        } else {
            for (size_t i = 0; i < this->proportion_female.size() - 1; i++) {
                ss << this->proportion_female[i] << ", ";
            }
            ss << this->proportion_female[this->proportion_female.size() - 1] << "],\n";
        }

        ss << "  \"estimated_values\":[";
        if (this->estimated_proportion_female.size() == 0) {
            ss << "],\n";
        } else {
            for (size_t i = 0; i < this->estimated_proportion_female.size() - 1; i++) {
                ss << this->estimated_proportion_female[i] << ", ";
            }
            ss << this->estimated_proportion_female[this->estimated_proportion_female.size() - 1] << "],\n";
        }
