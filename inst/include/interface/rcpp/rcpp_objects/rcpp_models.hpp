...
public:
   ...
   *
   * @details Returns true if reporting is enabled, false otherwise. The implementation may depend on TMB_MODEL.
    ...
    if (has_se)
    {
      ...
      try {
        ...
        if (se_vals_vector.size() < dq.size()) {
          throw std::runtime_error("Standard error vector size is smaller than derived quantity size for derived quantity " + name);
        }
        ...
        throw std::runtime_error("Error processing uncertainty for derived quantity " + name + ": " + e.what());
      }
    }
    else
    {
  ...
  fleet_to_json(FleetInterface *fleet_interface)
  {
    ...
    if (fit != info->fleets.end())
    {
      ...
      {
        ss << " ,{\n";
        fims::Vector<double> age_to_length_conversion_uncertainty(fleet->age_to_length_conversion.size(), -999);
        this->get_se_values("age_to_length_conversion", this->se_values, age_to_length_conversion_uncertainty);
        ...
        ss << " \"type\": \"vector\",\n";
        ss << " \"dimensionality\": {\n";
        ss << "  \"header\": [" << "\"nages\", \"nlengths\"" << "],\n";
        ss << "  \"dimensions\": [" << fleet_interface->nages.get() << ", " << fleet_interface->nlengths.get() << "]\n},\n";
   ...
   *
   * @details Returns a numeric vector containing the fixed effect parameters used in the model.
   ...
   *
   * @details Returns a numeric vector containing the random effect parameters used in the model.
   ...
   *
   * @details Returns a list containing the report results for the CatchAtAge model, including derived quantities and diagnostics.
         ...
         ++it)
    {
      ...
      derived_quantities_dim_info["numbers_at_age"] =
          fims_popdy::DimensionInfo("numbers_at_age",
                                    ...
                                    fims::Vector<std::string>{"nyears+1", "nages"});
