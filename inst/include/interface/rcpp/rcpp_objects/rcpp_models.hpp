...
public:
  ...
  CatchAtAgeInterface(const CatchAtAgeInterface &other)
      : FisheryModelInterfaceBase(other), population_ids(other.population_ids) {
  }
  ...
  template <typename Type> bool add_to_fims_tmb_internal() {
    ...
    for (fleet_ids_iterator it = fleet_ids.begin(); it != fleet_ids.end();
      ...
      derived_quantities["selectivity_at_age"] =
          fims::Vector<Type>(fleet_interface->n_ages.get());
      derived_quantities_dim_info["selectivity_at_age"] =
          fims_popdy::DimensionInfo(
              "selectivity_at_age",
              fims::Vector<int>{fleet_interface->n_ages.get()},
              fims::Vector<std::string>{"n_ages"});

      derived_quantities["selectivity_at_length"] =
          fims::Vector<Type>(fleet_interface->n_lengths.get());
