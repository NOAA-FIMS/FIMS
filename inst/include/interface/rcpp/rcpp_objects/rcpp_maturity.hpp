...
/**
 * @brief Rcpp interface for DSEM maturity.
 */
class DSEMMaturityInterface : public MaturityInterfaceBase {
public:
    ParameterVector beta_z;

    DSEMMaturityInterface() : MaturityInterfaceBase() {
        std::shared_ptr<DSEMMaturityInterface> dsem_mat = 
            std::make_shared<DSEMMaturityInterface>(*this);
        FIMSRcppInterfaceBase::fims_interface_objects.push_back(dsem_mat);
        MaturityInterfaceBase::live_objects[this->id] = dsem_mat;
    }

    DSEMMaturityInterface(const DSEMMaturityInterface& other)
        : MaturityInterfaceBase(other), beta_z(other.beta_z) {}

    virtual ~DSEMMaturityInterface() {}
    virtual uint32_t get_id() { return this->id; }
    ...
    bool add_to_fims_tmb_internal() {
        std::shared_ptr<fims_info::Information<Type>> info = 
            fims_info::Information<Type>::GetInstance();
        std::shared_ptr<fims_popdy::DSEMMaturity<Type>> maturity = 
            std::make_shared<fims_popdy::DSEMMaturity<Type>>();
        
        maturity->id = this->id;

        register_beta_z_standardized<Type>(this, maturity, info, "beta_z_mat");

        info->maturity_models[this->id] = maturity;
        return true;
    }
    ...
    virtual void finalize() {
        ...
        std::shared_ptr<fims_info::Information<double>> info = 
            fims_info::Information<double>::GetInstance();
        auto it = info->maturity_models.find(this->id);
        if (it != info->maturity_models.end()) {
            auto mat = std::dynamic_pointer_cast<fims_popdy::DSEMMaturity<double>>(it->second);
            for (size_t i = 0; i < this->beta_z.size(); ++i) {
                this->beta_z[i].final_value_m = mat->beta_z[i];
            }
        }
        this->finalized = true;
    }
