 ...
 * define different Rcpp interfaces for each possible recruitment negative
 * log-likelihood
 *
 */
 ...
 public:
  ...
  virtual double evaluate_nll() {
    ...
    NLL.recruit_deviations.resize(
        recruit_deviations.size());  // Vector from TMB
    ...
    Rcout << "Rec devs being passed to C++ are " << recruit_deviations
          << std::endl;
