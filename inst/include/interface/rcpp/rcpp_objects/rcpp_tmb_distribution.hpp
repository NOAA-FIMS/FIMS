...
public:
  ...
  /** @brief evaluate method for child distribution interface objects to
   * inherit
   * **/
...
public:
  ...
  /**
   * @brief Evaluate lognormal probability density function, default returns
   * the log of the pdf
   *
   * @tparam T
   * @return log pdf
...
public:
  ...
  {
    typedef
        typename fims::ModelTraits<TMB_FIMS_REAL_TYPE>::EigenVector Vector0;
    std::shared_ptr<fims::Information<TMB_FIMS_REAL_TYPE> > d0
        = fims::Information<TMB_FIMS_REAL_TYPE>::GetInstance ();
