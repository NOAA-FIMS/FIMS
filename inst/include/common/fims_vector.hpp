...
namespace fims
{
  ...
  public:
     ...
     */
    Vector(const Vector<Type> &other) : name_m(other.name_m)
    {
     ...
     */
    Vector(const std::vector<Type> &other, const std::string &name = "unnamed")
        : name_m(name)
    {
      this->vec_m = other;
    }
     ...
     */
    Vector(std::initializer_list<Type> init, const std::string &name = "unnamed")
        : name_m(name)
    {
    ...
    inline Type &operator[](size_t pos)
    {
      try
      {
        if (pos >= this->size())
        {
          throw std::out_of_range("index is greater than or equal to vector size");
        }
      ...
      catch (const std::exception &e)
      {
        ...
        ss << "fims::Vector out of bounds";
        if (!this->name_m.empty())
        {
          ss << " for vector '" << this->name_m << "'";
        }
        ss << " (index=" << pos << ", size=" << this->size() << "). "
           << e.what();
    ...
    /**
     * @brief Gets the debug name for the vector.
     * @return The debug name.
     */
    std::string get_name() const { return this->name_m; }

    /**
     * @brief Sets the debug name for the vector.
     */
    void set_name(const std::string &name) { this->name_m = name; }
  ...
  private:
    std::string name_m; /*!< Debug name for error reporting. */
