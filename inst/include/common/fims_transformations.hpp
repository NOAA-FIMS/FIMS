...
namespace fims_transformations {
  ...
  inline Type ApplyBackTransformation(
    ...
    switch (label) {
      ...
      default:
        ...
        std::string(". Valid transformations are identity, exp, log, logit, square, and sqrt."));
        break;
    }
  ...
  inline fims::Vector<Type> TransformPrior(const fims::Vector<Type>& input_value, 
    ...
    if (input.label == prior.label) {
      ...
      for(size_t i = 0; i < n; i ++) {
        natural_parameter[i] = 
          ApplyBackTransformation(input_value[i], input);
        prior_parameter[i] = 
          ApplyTransformation(natural_parameter[i], prior);
      }
  ...
  */
  ...
  inline Type TransformPrior(const Type& input_value,
    ...
    return ApplyTransformation(natural_parameter, 
                              prior);
  }
    ...
    if (n == spjac.i.size()) {
      ...
      for (size_t k = 0; k < jac_vals.size(); k++) {
        triplets.push_back(Eigen::Triplet<double>(
            spjac.i[k], spjac.j[k], jac_vals[k]));
      }
