...
class Information {
 public:
   ...
   * errors.
   */
  bool CreateLengthModel() {
    bool valid_model = true;
    INFO_LOG << "" << std::endl;
    INFO_LOG
        << "Beginning to create FIMS model with length composition data in information.hpp CreateModel(). "
        << std::endl;
    INFO_LOG << "Initializing fleet objects for " << this->fleets.size()
             << " fleets." << std::endl;
    for (fleet_iterator it = this->fleets.begin(); it != this->fleets.end();
    ...
    for (population_iterator it = this->populations.begin();
      ...
      INFO_LOG << "Initializing population " << p->id << "." << std::endl;
      p->Initialize(p->nyears, p->nseasons, p->nages, p->nlengths);
