...
namespace fims_info {
    ...
    public:
        ...
        bool CreateModel() {
            ...
            FIMS_INFO_LOG("Initializing fleet objects for "
                    + fims::to_string(this->fleets.size())
                    + " fleets.");
            ...
            for (fleet_iterator it = this->fleets.begin(); it != this->fleets.end();
                ...
                FIMS_INFO_LOG("Initializing fleet  "
                        + fims::to_string(f->id) + ".");

                f->Initialize(f->nyears, f->nages);


                FIMS_INFO_LOG("Checking for available fleet index data objects.");
                ...
                FIMS_INFO_LOG("Checking for available fleet age comp data objects.");
                ...
                FIMS_INFO_LOG("Checking for available fleet selectivity pattern.");
                ...
                FIMS_INFO_LOG("Checking for available index likelihood function.");
                ...
                if (f->fleet_index_likelihood_id_m != -999) {
                    ...
                    if (it != this->distribution_models.end()) {
                        ...
                        //                        FIMS_ERROR_LOG("index likelihood function not defined in FIMS for fleet " + fims::to_string(f->id) +
                        //                                " ,looking for index likelihood function with id " + fims::to_string(f->fleet_index_likelihood_id_m));
                    ...
                    //          data." << std::endl;
                    // exit(1);
                    FIMS_WARNING_LOG("index likelihood function not defined for fleet " + fims::to_string(f->id));
                }
                // end set index likelihood

                FIMS_INFO_LOG("Checking for available age comp likelihood function.");
                ...
                if (f->fleet_agecomp_likelihood_id_m != -999) {
                    ...
                    if (it != this->distribution_models.end()) {
                        ...
                        //                        FIMS_ERROR_LOG("age comp likelihood function not defined in FIMS for fleet "
                        //                                + fims::to_string(f->id) +
                        //                                " ,looking for age comp likelihood function with id "
                        //                                + fims::to_string(f->fleet_agecomp_likelihood_id_m));
                    }
                    ...
                    //          data." << std::endl;
                    // exit(1);
                    FIMS_WARNING_LOG("age comp likelihood function not defined for fleet " + fims::to_string(f->id));
                }
                // end set agecomp likelihood

                FIMS_INFO_LOG("Completed initialization for fleet " + fims::to_string(f->id) + ".");

            } // close fleet iterator loop
            FIMS_INFO_LOG("Completed initialization of all fleets.");

            FIMS_INFO_LOG("Initializing population objects.");
            for (population_iterator it = this->populations.begin();
                ...
                FIMS_INFO_LOG("Checking for available recruitment function.");
                // set recruitment
                ...
                FIMS_INFO_LOG("Checking for available growth function.");
                // set growth
                if (p->growth_id != -999) {
                    ...
                    if (it != this->growth_models.end()) {
                        ...
                        valid_model = false;
                        FIMS_ERROR_LOG("Expected growth function not defined in FIMS for population "
                ...
                FIMS_INFO_LOG("Checking for available maturity function.");
                // set maturity
                if (p->maturity_id != -999) {
                    ...
                    if (it != this->maturity_models.end()) {
                        ...
                        valid_model = false;
                        FIMS_ERROR_LOG("Expected maturity function not defined in FIMS for population "
                                + fims::to_string(p->id)
                                + ", maturity function " + fims::to_string(maturity_uint) + ".");
                    }
                ...
                FIMS_INFO_LOG("Completed initialization for population "
                        + fims::to_string(p->id) + ".");
            }
            ...
            FIMS_INFO_LOG("Completed FIMS model creation with "
                    ...
                    + " warnings and "
                    + fims::to_string(fims::FIMSLog::fims_log->get_error_count())
                    + " errors.");
