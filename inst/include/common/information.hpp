    ...
    public:
        ...
        void SetupRandomEffects()
        {
            for (density_components_iterator it = this->density_components.begin(); it != this->density_components.end(); ++it)
            {
                ...
                if (d->input_type == "random_effects")
                {
                    ...
                    variable_map_iterator vmit;
                    FIMS_INFO_LOG("Link random effects from distribution " + fims::to_string(d->id) + " to derived value " + fims::to_string(d->key[0]));
        ...
        void SetupData()
        {
            for (density_components_iterator it = this->density_components.begin(); it != this->density_components.end(); ++it)
            {
                ...
                if (d->input_type == "data")
                {
                    ...
                    variable_map_iterator vmit;
                    FIMS_INFO_LOG("Link expected value from distribution " + fims::to_string(d->id) + " to derived value " + fims::to_string(d->key[0]));
                    vmit = this->variable_map.find(d->key[0]);
                    d->expected_values = *(*vmit).second;
                    FIMS_INFO_LOG("Expected value size for distribution " + fims::to_string(d->id) + " is: " + fims::to_string(d->expected_values.size()));
                }
            }
        }
            ...
            std::shared_ptr<fims_popdy::Fleet<Type>> f)
        {
            if (f->fleet_observed_landings_data_id_m != -999)
            {
                ...
                if (it != this->data_objects.end())
                {
                    ...
                    FIMS_INFO_LOG("Landings data for fleet " + fims::to_string(f->id) + " successfully set to " + fims::to_string(f->observed_landings_data->at(1)));
                }
                else
                {
                    valid_model = false;
                    FIMS_ERROR_LOG("Expected landings data not defined for fleet " + fims::to_string(f->id) + ", index " + fims::to_string(observed_landings_id));
                }
            }
        }
            ...
            std::shared_ptr<fims_popdy::Fleet<Type>> f)
        {
            if (f->fleet_observed_index_data_id_m != static_cast<Type>(-999))
            {
                ...
                if (it != this->data_objects.end())
                {
                    ...
                    FIMS_INFO_LOG("Index data for fleet " + fims::to_string(f->id) + " successfully set to " + fims::to_string(f->observed_index_data->at(1)));
                }
                else
                {
                    valid_model = false;
                    FIMS_ERROR_LOG("Expected index data not defined for fleet " + fims::to_string(f->id) + ", index " + fims::to_string(observed_index_id));
                }
            }
        }
            ...
            std::shared_ptr<fims_popdy::Fleet<Type>> f)
        {
            if (f->fleet_observed_agecomp_data_id_m != static_cast<Type>(-999))
            {
                ...
                if (it != this->data_objects.end())
                {
                    ...
                    FIMS_INFO_LOG("Observed input age-composition data for fleet " + fims::to_string(f->id) + " successfully set to " + fims::to_string(f->observed_agecomp_data->at(1)));
                }
                else
                {
                    valid_model = false;
                    FIMS_ERROR_LOG("Expected age-composition observations not defined for fleet " + fims::to_string(f->id));
                }
            }
        }
            ...
            std::shared_ptr<fims_popdy::Fleet<Type>> f)
        {
            if (f->fleet_observed_lengthcomp_data_id_m != static_cast<Type>(-999))
            {
                ...
                if (it != this->data_objects.end())
                {
                    ...
                    FIMS_INFO_LOG("Observed input length-composition data for fleet " + fims::to_string(f->id) + " successfully set to " + fims::to_string(f->observed_lengthcomp_data->at(1)));
                }
                else
                {
                    valid_model = false;
                    FIMS_ERROR_LOG("Expected length-composition observations not defined for fleet " + fims::to_string(f->id));
                }
            }
        }
            ...
            std::shared_ptr<fims_popdy::Fleet<Type>> f)
        {
            if (f->fleet_selectivity_id_m != static_cast<Type>(-999))
            {
                ...
                if (it != this->selectivity_models.end())
                {
                    ...
                    FIMS_INFO_LOG("Selectivity model " + fims::to_string(f->fleet_selectivity_id_m) + " successfully set to fleet " + fims::to_string(f->id));
                }
                else
                {
                    valid_model = false;
                    FIMS_ERROR_LOG("Expected selectivity pattern not defined for fleet " + fims::to_string(f->id) + ", selectivity pattern " + fims::to_string(sel_id));
                }
            }
            else
            {
                valid_model = false;
                FIMS_ERROR_LOG("Error: No selectivity pattern defined for fleet " + fims::to_string(f->id) + ". FIMS requires selectivity be defined for all fleets.");
            }
        }
            ...
            std::shared_ptr<fims_popdy::Population<Type>> p)
        {
            if (p->recruitment_id != static_cast<Type>(-999))
            {
                ...
                if (it != this->recruitment_models.end())
                {
                    ...
                    FIMS_INFO_LOG("Recruitment model " + fims::to_string(recruitment_uint) + " successfully set to population " + fims::to_string(p->id));
                }
                else
                {
                    valid_model = false;
            ...
            {
                valid_model = false;
                FIMS_ERROR_LOG("No recruitment function defined for population " + fims::to_string(p->id) + ". FIMS requires recruitment functions be defined for all "
                                                                                                            "populations.");
            }
        }
            ...
            std::shared_ptr<fims_popdy::Population<Type>> p)
        {
            ...
            if (r->process_id != -999)
            {
                ...
                if (it != this->recruitment_process_models.end())
                {
                    ...
                    FIMS_INFO_LOG("Recruitment Process model " + fims::to_string(process_uint) + " successfully set to population " + fims::to_string(p->id));
                ...
                else
                {
                    ...
                    FIMS_ERROR_LOG("Expected recruitment process function not defined for "
                                   "population " +
                                   fims::to_string(p->id) + ", recruitment process function " + fims::to_string(process_uint));
                }
            }
            else
            {
                valid_model = false;
                FIMS_ERROR_LOG("No recruitment process function defined for population " + fims::to_string(p->id) + ". FIMS requires recruitment process functions be defined for all "
                                                                                                                    "recruitments.");
            }
        }
            ...
            std::shared_ptr<fims_popdy::Population<Type>> p)
        {
            if (p->growth_id != static_cast<Type>(-999))
            {
                ...
                if (it != this->growth_models.end())
                {
                    ...
                    FIMS_INFO_LOG("Growth model " + fims::to_string(growth_uint) + " successfully set to population " + fims::to_string(p->id));
                }
                else
                {
                    valid_model = false;
                    FIMS_ERROR_LOG("Expected growth function not defined for population " + fims::to_string(p->id) + ", growth function " + fims::to_string(growth_uint));
                }
            }
            else
            {
                valid_model = false;
                FIMS_ERROR_LOG("No growth function defined for population " + fims::to_string(p->id) + ". FIMS requires growth functions be defined for all "
                                                                                                       "populations.");
            }
        }
            ...
            std::shared_ptr<fims_popdy::Population<Type>> p)
        {
            if (p->maturity_id != static_cast<Type>(-999))
            {
                ...
                if (it != this->maturity_models.end())
                {
                    ...
                    FIMS_INFO_LOG("Maturity model " + fims::to_string(maturity_uint) + " successfully set to population " + fims::to_string(p->id));
                }
                else
                {
                    valid_model = false;
                    FIMS_ERROR_LOG("Expected maturity function not defined for population " + fims::to_string(p->id) + ", maturity function " + fims::to_string(maturity_uint));
                }
            }
            else
            {
                ...
                valid_model = false;
                FIMS_ERROR_LOG("No maturity function defined for population " + fims::to_string(p->id) + ". FIMS requires maturity functions be defined for all "
                                                                                                         "populations.");
            }
        }
                ...
                if (d->input_type == "data")
                {
                    if (d->observed_data_id_m != static_cast<Type>(-999))
                    {
                        ...
                        if (it != this->data_objects.end())
                        {
                            ...
                            FIMS_INFO_LOG("Observed data " + fims::to_string(observed_data_id) + " successfully set to density component " + fims::to_string(d->id));
                        }
                        else
                        {
                            valid_model = false;
                            FIMS_ERROR_LOG("Expected data observations not defined for density component " + fims::to_string(d->id) + ", observed data " + fims::to_string(observed_data_id));
                        }
                    }
                    else
                    {
        ...
        void CreatePopulationObjects(bool &valid_model)
        {
                 ...
                 it != this->populations.end(); ++it)
            {
                ...
                for(fit = p->fleet_ids.begin(); fit != p->fleet_ids.end(); ++fit) {
                    fleet_iterator fleet_it = this->fleets.find(*fit);
                    if (fleet_it != this->fleets.end()) {
                        std::shared_ptr<fims_popdy::Fleet<Type>> f = (*fleet_it).second;
                        p->fleets.push_back(f);
                    } else {
                        valid_model = false;
                        FIMS_ERROR_LOG("Expected fleet not defined for population " + fims::to_string(p->id) + ", fleet " + fims::to_string(*fit));
                    }
                }
