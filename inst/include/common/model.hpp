...
class Model {  // may need singleton
 public:
  ...
  const Type Evaluate() {
    ...
    FIMS_INFO_LOG(std::string("Starting Evaluate(): ") +
                  fims::to_string(models_count) + " models, " +
    ...
    // Evaluate each registered model with defensive logging
    for (m_it = this->fims_information->models_map.begin();
      ...
      FIMS_INFO_LOG(std::string("Preparing/Evaluating model ") +
                    fims::to_string(model_id));
      ...
      try {
        ...
        FIMS_ERROR_LOG(std::string("Exception evaluating model ") +
                       fims::to_string(model_id) + ": " +
                       std::string(e.what()));
        throw e;  // rethrow after logging
      } catch (...) {
        FIMS_ERROR_LOG(std::string("Unknown exception evaluating model ") +
                       fims::to_string(model_id));
