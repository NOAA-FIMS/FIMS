...
struct MultinomialLPMF : public DensityComponentBase<Type> {
  Type lpdf = 0.0; /**< total negative log-likelihood contribution of the
                      distribution */
      ...
      for (size_t i = 0; i < dims[0]; i++) {
        ...
        for (size_t j = 0; j < dims[1]; j++) {
          if (this->input_type == "data") {
            // if data, check if there are any NA values and skip lpdf
            // calculation for entire row if there are
        ...
        if (!containsNA) {
          this->lpdf_vec[i] = dmultinom((vector<Type>)x_vector,
                                        (vector<Type>)prob_vector, true);
        } else {
          this->lpdf_vec[i] = 0;
        }
