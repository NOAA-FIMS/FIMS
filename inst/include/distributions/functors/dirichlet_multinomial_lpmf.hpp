 ...
 */
#ifndef FIMS_DISTRIBUTIONS_DIRICHLET_MULTINOMIAL_LPMF
#define FIMS_DISTRIBUTIONS_DIRICHLET_MULTINOMIAL_LPMF
...
namespace fims_distributions
{
    ...
    struct DirichiletMultinomialLPMF : public DensityComponentBase<Type>
    {
        ...
        Type theta;                /**< Dirichlet parameter */

        /** @brief Constructor.
         */
        DirichiletMultinomialLPMF() : DensityComponentBase<Type>()
        {
        }

        /** @brief Destructor.
         */
        virtual ~DirichiletMultinomialLPMF() {}
        ...
        virtual const Type evaluate()
        {
            ...
            {
                FIMS_ERROR_LOG("Observed age comp is of size " + fims::to_string(dims[0] * dims[1]) + " and expected is of size " + fims::to_string(this->expected_values.size()));
            }
            else
            {
                for (size_t i = 0; i < dims[0]; i++)
                {
                    ...
                    if (!containsNA)
                    {
                        this->lpdf_vec[i] = fims_math::ddirichlet_multinomial(x_vector,
                                                                              prob_vector,
                                                                              theta,
                                                                              true);
                    }
