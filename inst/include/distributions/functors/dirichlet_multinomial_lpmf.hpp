 ...
 */
#ifndef Dirichlet_multinomial_LPMF
#define Dirichlet_multinomial_LPMF
...
struct Dirichlet_multinomialLPMF : public DensityComponentBase<Type> {
  /**
  ...
  /** @brief Constructor.
   */
  Dirichlet_multinomialLPMF() : DensityComponentBase<Type>() {}

  /** @brief Destructor.
   */
  virtual ~Dirichlet_multinomialLPMF() {}
  ...
  virtual const Type evaluate() {
    ...
    if (this->input_type == "data") {
      if (this->data_expected_values) {
        if (dims[0] * dims[1] != this->data_expected_values->size()) {
          throw std::invalid_argument(
              ...
              "dimension of the number of rows times the number of columns is "
              "of size " +
      ...
      if (dims[0] * dims[1] != this->observed_values.size()) {
        throw std::invalid_argument(
            "Dirichlet_multinomialLPDF: Vector index out of bounds. The "
            "dimension of the number of rows times the number of columns is of "
            "size " +
            fims::to_string(dims[0] * dims[1]) +
      ...
      if (this->observed_values.size() != this->expected_values.size()) {
        throw std::invalid_argument(
            "Dirichlet_multinomialLPDF: Vector index out of bounds. The "
            "dimension of the observed vector of size " +
            fims::to_string(this->observed_values.size()) +
            " and the expected vector is of size " +
    ...
    for (size_t i = 0; i < dims[0]; i++) {
      ...
      if (!containsNA) {
        std::fill(this->lpdf_vec.begin() + lpdf_vec_idx,
                  ...
                  fims_math::ddirichlet_multinom(
                      observed_values_vector, prob_vector, this->theta, true));
