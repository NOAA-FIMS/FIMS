...
struct NormalLPDF : public DensityComponentBase<Type> {
  ...
  Type lpdf = 0.0; /**< total log probability density contribution of the
                      distribution */
    ...
    for (size_t i = 0; i < n_x; i++) {
      ...
      if (this->input_type == "data") {
        ...
        if (this->observed_values->at(i) != this->observed_values->na_value) {
          this->lpdf_vec[i] =
              dnorm(this->observed_values->at(i),
                    this->expected_values.get_force_scalar(i),
        ...
        this->lpdf_vec[i] =
            dnorm(this->x[i], this->expected_values.get_force_scalar(i),
                  fims_math::exp(log_sd.get_force_scalar(i)), true);
      }
        ...
        FIMS_SIMULATE_F(this->of) {
          if (this->input_type == "data") {
            this->observed_values->at(i) =
                rnorm(this->expected_values.get_force_scalar(i),
                      fims_math::exp(log_sd.get_force_scalar(i)));
          } else {
            this->x[i] = rnorm(this->expected_values.get_force_scalar(i),
                               fims_math::exp(log_sd.get_force_scalar(i)));
          }
        }
      }
      ...
      /* osa not working yet
        ...
        this->expected_values.get_force_scalar(i), sd[i]) ); this->lpdf_vec[i] =
        this->keep.cdf_upper[i] * log( 1.0 - pnorm(this->x[i],
        this->expected_values.get_force_scalar(i), sd[i]) );
        } */
    }
#ifdef TMB_MODEL
