...
class CatchAtAge : public FisheryModelBase<Type> {
 ...
 public:
  ...
  /**
   * @brief Computes population-level mean weight-at-age using the
   * growth-derived path.
   *
   * This helper uses the population-level growth-derived path only. It prefers
   * the canonical fleet/bin-aware growth-derived weight path when one shared
   * fleet-bin definition can be reused safely at the population level. If no
   * such growth-derived path is available, the function fails.
   *
   * @param population Shared pointer to the population object.
   * @param year Year index.
   * @param age Age index.
  ...
  std::shared_ptr<fims_popdy::Fleet<Type>> GetCanonicalGrowthDerivedWeightFleet(
    ...
    const std::shared_ptr<fims_popdy::Fleet<Type>>& canonical_fleet =
        population->fleets[0];
    std::shared_ptr<fims_popdy::GrowthDerivedALK<Type>> canonical_growth_alk =
        std::dynamic_pointer_cast<fims_popdy::GrowthDerivedALK<Type>>(
            canonical_fleet->alk);

    if (canonical_growth_alk == nullptr || !canonical_growth_alk->IsActive() ||
