  ...
  public:
    ...
    virtual void Prepare()
    {
           ...
           ++fit)
      {
        ...
        for (size_t l = 0;
             l < fleet->age_to_length_conversion.size();
             l++)
        {
          this->fleet_derived_quantities[fleet->GetId()]
                                        ["age_to_length_conversion"]
                                        [l] =
              fleet->age_to_length_conversion[l];
        }
        ...
        for (size_t a = 0; a < fleet->nages; a++)
        {
               ...
               l++)
          {
            ...
            fleet->selectivity_at_age[a] =
                fleet->selectivity_age->evaluate(fleet->ages[a]);
            if (fleet->nlengths > 0)
            {
              fleet->selectivity_at_length[a] = fleet->age_to_length_conversion[a] *
                                                fleet->selectivity_length->evaluate(fleet->ages[a]);
            }
          }
        }
      }
        ...
        size_t i_age_year, size_t year, size_t age)
    {
        ...
        if (this->fleets[fleet_]->nlengths > 0)
        {
          for (size_t i_length = 0; i_length < this->fleets[fleet_]->nlengths; i_length++)
          {
            ...
            this->fleet_derived_quantities[population->fleets[fleet_]->GetId()]
                ...
                this->population_derived_quantities[population->GetId()]
                                                   ["numbers_at_age"][i_age_year] *
                (1 - fims_math::exp(-(this->population_derived_quantities[population->GetId()]
                                                                         ["mortality_Z"][i_age_year]))) *
      ...
      void CalculateIndexNumbersAL(std::shared_ptr<fims_popdy::Population<Type>> & population,
                                   size_t i_age_year, size_t year, size_t age)
      {
        for (size_t fleet_ = 0; fleet_ < population->nfleets; fleet_++)
        {
          ...
          {
            for (size_t i_length = 0; i_length < population->fleets[fleet_]->nlengths; i_length++)
            {
              // iterate through all lengths within an age and sum the selectivity
              // to get numbers at length
              size_t i_length_age = age * population->fleets[fleet_]->nlengths + i_length;
              size_t i_length_year = year * population->fleets[fleet_]->nlengths + i_length;
              this->fleet_derived_quantities[population->fleets[fleet_]->GetId()]
