...
Typically, Rcpp exposes C++ functions to R using `Rcpp::export` placed before the C++ function code. This enables automatic wrapper generation and type conversion. Unfortunately, we cannot use this functionality within a TMB model, and thus, the code here may be slightly more complicated than a typical Rcpp interface. The FIMS interface is primarily made up of C++ classes. Rather than expose each class individually as an S4 object, `RCPP_MODULE(fims)` in `interface/rcpp/fims_modules.hpp` registers the R-facing interface explicitly, almost entirely as functions rather than classes, and C++ modules are wrapped in `Rcpp::XPtr`, a type of pointer that allows information to pass across the R-C++ language boundary.  

R creates a module by calling a function such as `create_selectivity_()`, then reads and writes that module's fields by name through functions such as `set_variable_vector_()` and `get_variable_vector_()`; the module answers to a name only if it is listed in its own `get_variable_vector()` or `get_numeric_vector()`. Only `Variable` and `VariableVector` are still registered as classes, with `Rcpp::class_<>` in `src/rcpp_variable.cpp`. For templates, direct export isn't supported, but individual instantiations can be wrapped in type-specific functions.

#### inst/include/interface/interface.hpp

The `interface/interface.hpp` is the only file sitting directly in the interface directory, alongside the `rcpp` and `TMB` subdirectories. It defines the TMB reporting macros --- `FIMS_REPORT_F`, `ADREPORT_F`, and `FIMS_SIMULATE_F` --- together with the no-op versions used when FIMS is built without TMB, so the same population dynamics code compiles either way.

#### inst/include/interface/rcpp

This directory contains the Rcpp module registration layer for FIMS. The files in this directory and its `rcpp_objects` subdirectory define the interface classes, the `get_variable_vector()` and `get_numeric_vector()` functions that R uses to access variable values, helper functions, and registration declarations that expose FIMS C++ objects to R. These Rcpp interface files live under `inst/include` rather than in `src/` so that their declarations are available both to the package source files in `src/` and to other files that need to reference the R-facing interface.
