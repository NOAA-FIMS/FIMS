 ...
 *        (1 - exp(-von_bertalanffy_coefficient_K * (a -
 * reference_age_for_length_1))) / (1 - exp(-von_bertalanffy_coefficient_K *
...
struct VonBSchnuteGrowth : public GrowthBase<Type> {
  ...
  void ValidateLengthParameters() const {
    ...
    if (mean_length_old <= Type(0.0)) {
      throw std::runtime_error("VonBSchnuteGrowth mean_length_old must be > 0");
    }
  ...
  void ValidateWeightParameters() const {
    if (length_weight_a <= Type(0.0)) {
      throw std::runtime_error("VonBSchnuteGrowth length_weight_a must be > 0");
    }

    if (length_weight_b <= Type(0.0)) {
      throw std::runtime_error("VonBSchnuteGrowth length_weight_b must be > 0");
    }
  }
  ...
  Type length_at_age(const Type& age) const {
    ...
    const Type denom =
        Type(1.0) - fims_math::exp(-von_bertalanffy_coefficient_K *
