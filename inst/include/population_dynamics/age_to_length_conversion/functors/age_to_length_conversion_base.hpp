 ...
 * define the age-to-length conversion hpp file if not already defined.
...
struct AgeToLengthConversionBase : public fims_model_object::FIMSObject<Type> {
  ...
  AgeToLengthConversionBase() {
    this->id = AgeToLengthConversionBase<Type>::id_g++;
  }

  virtual ~AgeToLengthConversionBase() {}

  /**
   * @brief Returns whether this age-to-length conversion is active and valid
   * for use.
   ...
   * @return True if the age-to-length conversion is ready to build rows for the
   * current state.
   */
...
template <typename Type>
uint32_t AgeToLengthConversionBase<Type>::id_g =
    0; /**< global counter for age-to-length conversion ids */
