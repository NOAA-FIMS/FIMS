 ...
 * @brief Shared runtime helpers for constructing and validating fleet age-to-length conversions.
...
/**
 * @brief Build the active age-to-length conversion for a fleet from the current population and fleet
 * state.
 *
 * Tries the growth-derived age-to-length conversion path when the linked population growth object
 ...
 * @return Shared pointer to an active age-to-length conversion implementation, or nullptr when no
 * usable age-to-length conversion path exists.
 */
template <typename Type>
std::shared_ptr<AgeToLengthConversionBase<Type>> BuildAgeToLengthConversionFleet(
  ...
  if (std::shared_ptr<GrowthDerivedObservationBase<Type>> growth_observation =
          std::dynamic_pointer_cast<GrowthDerivedObservationBase<Type>>(
              population->growth)) {
    std::shared_ptr<AgeToLengthConversionBase<Type>> age_to_length_conversion_derived =
    ...
    if (age_to_length_conversion_derived->IsActive() &&
      ...
      FIMS_INFO_LOG("Growth-derived age-to-length conversion successfully set to fleet " +
  ...
  std::shared_ptr<AgeToLengthConversionBase<Type>> age_to_length_conversion_fixed =
...
/**
 * @brief Ensure a fleet has an active age-to-length conversion before length-based calculations.
 *
 * Reuses the current fleet age-to-length conversion when it matches the current population growth
 ...
 * age-to-length conversion from the current population and fleet state. Throws when the fleet has
 * length bins but no usable age-to-length conversion path can be constructed.
 *
 * @param population Shared pointer to the owning population.
 * @param fleet Shared pointer to the fleet.
 */
template <typename Type>
void EnsureAgeToLengthConversionFleet(const std::shared_ptr<Population<Type>>& population,
                    const std::shared_ptr<Fleet<Type>>& fleet) {
  if (fleet == nullptr) {
    throw std::runtime_error("Fleet pointer was null while resolving age-to-length conversion.");
  }

  if (population == nullptr) {
    throw std::runtime_error("Population pointer was null while resolving age-to-length conversion.");
  }
  ...
  if (fleet->age_to_length_conversion_model != nullptr) {
    if (growth_observation != nullptr) {
      std::shared_ptr<AgeToLengthConversionDerived<Type>> age_to_length_conversion_derived =
          std::dynamic_pointer_cast<AgeToLengthConversionDerived<Type>>(fleet->age_to_length_conversion_model);
      ...
      if (age_to_length_conversion_derived != nullptr &&
        ...
        return;
               ...
               fleet->age_to_length_conversion_model->PrepareForCurrentState()) {
      return;
    }
  }

  fleet->age_to_length_conversion_model = BuildAgeToLengthConversionFleet<Type>(population, fleet);
  ...
  if (growth_observation != nullptr) {
    ss << " This population uses a growth-derived-capable growth object, so "
       << "the growth-derived age-to-length conversion path with population size-provider support "
...
/**
 * @brief Ensure all fleets linked to a population have a usable age-to-length conversion.
 *
 * Applies EnsureAgeToLengthConversionFleet() to each fleet linked to the population so runtime
 * length-based calculations can safely assume age-to-length conversion availability.
 *
...
void EnsurePopulationFleetAgeToLengthConversion(
    const std::shared_ptr<Population<Type>>& population) {
  if (population == nullptr) {
    throw std::runtime_error("Population pointer was null while resolving age-to-length conversions.");
  }
