 ...
 * fix selectivity for at least one age to a value equal or close to 1. This can
 * be achieved by setting at least one age-specific parameter (logit_sel_at_age)
...
struct AgeSpecificSelectivity : public SelectivityBase<Type> {
  fims::Vector<Type> logit_sel_at_age; /**< age-specific selectivity parameters
                                          on the logit scale */
  size_t
      n_ages; /**< number of modeled ages, based on user-provided input data */
