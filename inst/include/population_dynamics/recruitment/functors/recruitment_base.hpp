...
template <class Type> struct RecruitmentBase : public FIMSObject<Type>
{
  ...
  bool use_recruit_bias_adjustment
      = true; /*!< A flag to indicate if recruitment deviations are bias
               * adjusted
               */
  ...
  bool estimate_recruit_deviations
      = true; /*!< A flag to indicate if recruitment deviations are estimated
       or not */
  ...
  virtual const Type evaluate (const Type &spawners,
                               const Type &ssbzero)
      = 0; // need to add input parameter values
  ...
  PrepareConstrainedDeviations ()
  {
    ...
    for (size_t i = 0; i < this->recruit_deviations.size (); i++)
      {
        this->recruit_deviations[i]
            -= sum / (this->recruit_deviations.size ());
