...
template <typename Type> struct Population : public FIMSObject<Type>
{
  using ParameterVector =
      typename ModelTraits<Type>::ParameterVector; /*!< the vector of
   ...
   */
  void
  Initialize (int nyears, int nseasons, int nages)
  {
    this->nyears = nyears;
  ...
  Prepare ()
  {
    ...
    std::fill (expected_recruitment.begin (), expected_recruitment.end (),
               0.0);
    std::fill (proportion_mature_at_age.begin (),
               proportion_mature_at_age.end (), 0.0);
        ...
        for (size_t year = 0; year < this->nyears; year++)
          {
            ...
            // mortality_F is a ParameterVector and therefore needs to be
            // filled within a loop
   ...
   */
  inline void
  CalculateInitialNumbersAA (size_t index_ya, size_t a)
   ...
   */
  void
  CalculateMortality (size_t index_ya, size_t year, size_t age)
  {
        ...
        if (this->fleets[fleet_]->is_survey == false)
          {
            ...
            FIMS_LOG << " sel age " << ages[age] << "for fleet " << fleet_
                     << " is "
                     << this->fleets[fleet_]->selectivity->evaluate (ages[age])
                     << " F mort year " << year << " "
   ...
   */
  inline void
  CalculateNumbersAA (size_t index_ya, size_t index_ya2, size_t age)
  {
   ...
   */
  inline void
  CalculateUnfishedNumbersAA (size_t index_ya, size_t index_ya2, size_t age)
  {
   ...
   */
  void
  CalculateBiomass (size_t index_ya, size_t year, size_t age)
  {
    this->biomass[year]
   ...
   */
  void
  CalculateUnfishedBiomass (size_t index_ya, size_t year, size_t age)
  {
   ...
   * @param age the age who's biomass is being added into total spawning
   * biomass
   */
  void
  CalculateSpawningBiomass (size_t index_ya, size_t year, size_t age)
  {
   ...
   */
  void
  CalculateUnfishedSpawningBiomass (size_t index_ya, size_t year, size_t age)
  {
   ...
   */
  void
  CalculateRecruitment (size_t index_ya, size_t year)
  {
    ...
    FIMS_LOG << "spawning biomass = " << this->spawning_biomass[year]
             << std::endl;
    FIMS_LOG << "rec devs = "
             << this->recruitment->recruit_deviations[year - 1] << std::endl;
    FIMS_LOG << "rec eval = "
   ...
   */
  void
  CalculateCatch (size_t year, size_t age)
  {
    for (size_t fleet_ = 0; fleet_ < this->nfleets; fleet_++)
      {
        if (this->fleets[fleet_]->is_survey == false)
          {
            ...
            FIMS_LOG << " fleet " << fleet_ << " year " << year << " age "
                     << age << std::endl;
   ...
   */
  void
  CalculateIndex (size_t index_ya, size_t year, size_t age)
  {
    for (size_t fleet_ = 0; fleet_ < this->nfleets; fleet_++)
      {
        Type index_;
   ...
   */
  void
  CalculateCatchNumbersAA (size_t index_ya, size_t year, size_t age)
  {
    for (size_t fleet_ = 0; fleet_ < this->nfleets; fleet_++)
      {
   ...
   */
  void
  CalculateCatchWeightAA (size_t year, size_t age)
  {
    ...
    for (size_t fleet_ = 0; fleet_ < this->nfleets; fleet_++)
      {
        ...
        FIMS_LOG << " catchnaa "
                 << this->fleets[fleet_]->catch_numbers_at_age[year]
                 << std::endl;
        FIMS_LOG << " weight " << this->growth->evaluate (ages[age])
                 << std::endl;
        this->fleets[fleet_]->catch_weight_at_age[index_ya]
            = this->fleets[fleet_]->catch_numbers_at_age[index_ya]
              * this->growth->evaluate (
                  ages[age]); // this->weight_at_age[age];
        FIMS_LOG << " catch_waa "
                 << this->fleets[fleet_]->catch_weight_at_age[index_ya]
                 << std::endl;
      }
   ...
   */
  void
  CalculateMaturityAA (size_t index_ya, size_t age)
  {
  ...
  Evaluate ()
  {
    ...
    /*
     ...
     here year 0 is the estimated initial stock structure and age 0 are
     recruits loops start at zero with if statements inside to specify unique
     code for initial structure and recruitment 0 loops. Could also have
     started loops at 1 with initial structure and recruitment setup outside
     the loops.

     year loop is extended to <= nyears because SSB is calculted as the start
     ...
     would be to keep initial numbers at age in it's own vector and each year
     to include the population structure at the end of the year. This is likely
     a null point given that we are planning to modify to an event/stanza based
        ...
        for (size_t a = 0; a < this->nages; a++)
          {
            ...
            /*
             ...
             used to show expected stock structure at the end of the model
             period. This is because biomass in year i represents biomass at
             the start of the year. Should we add complexity to track more
             values such as start, mid, and end biomass in all years where,
             start biomass=end biomass of the previous year? Referenced above,
             this is probably not worth exploring as later milestone changes
             will eliminate this confusion.
             */
            if (y < this->nyears)
              {
                /*
                 First thing we need is total mortality aggregated across all
                 fleets to inform the subsequent catch and change in numbers at
                 age calculations. This is only calculated for years < nyears
                 as these are the model estimated years with data. The year
                 loop extends to y=nyears so that population numbers at age and
                 SSB can be calculated at the end of the last year of the model
                 */
            ...
            if (y == 0)
              {
                // Initial numbers at age is a user input or estimated
                // parameter vector.
                ...
                /*
                 Fished and unfished spawning biomass vectors are summing
                 biomass at age across ages to allow calculation of recruitment
                 in the next year.
                 */
                ...
                this->expected_recruitment[index_ya]
                    = this->numbers_at_age[index_ya];
              }
            else
              {
                if (a == 0)
                  {
                    // Set the nrecruits for age a=0 year y (use pointers
                    // instead of functional returns) assuming fecundity = 1
                    // and 50:50 sex ratio
            ...
            /*
            Here composition, total catch, and index values are calculated for
            all years with reference data. They are not calculated for y=nyears
            as there is this is just to get final population structure at the
            end of the terminal year.
             */
            if (y < this->nyears)
              {
    ...
    // maturity base ADREPORT_F(this->maturity->median, of); can't access
    // median b/c not in maturity base
#endif
  }
};
