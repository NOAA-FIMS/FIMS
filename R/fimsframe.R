...
#'   exactly match strings in the column `"fleet"` of `get_data(x)`.
#' @return
...
methods::setMethod(
  ...
  function(x) {
    ...
    fleet_names <- unique(model_data[["fleet"]])
...
#' must have composition entries for every integer between your first and last
