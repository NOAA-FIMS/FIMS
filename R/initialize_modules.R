...
initialize_recruitment <- function(parameters, data) {
  # A model uses either log_devs or log_r for the recruitment process, never
  # both, and the tibble can name both without anything downstream objecting.
  recruitment_pars <- parameters |>
    dplyr::filter(.data$module_name == "Recruitment") |>
    dplyr::pull(.data$label) |>
    unique() |>
    na.omit()

  if (all(c("log_r", "log_devs") %in% recruitment_pars)) {
