...
initialize_module <- function(parameters, data, module_name) {
  ...
  if (!is.list(parameters)) {
    ...
    cli::cli_abort("The {.var parameters} argument must contain both parameters and modules lists.")
  }
  ...
  if (!module_name %in% c(names(parameters[["parameters"]]), names(parameters[["modules"]]))) {
  ...
  module_class_name <-
    if (module_name == "population") {
      ...
      parameters[["modules"]][["fleets"]][[module_name]][[names(module_name)]][["form"]]
    } else if (!(module_name %in% names(parameters[["modules"]])) &
  ...
  for (field in module_fields) {
    if (field %in% non_standard_field) {
      ...
      module[[field]] <- switch(field,
        ...
        "random_q" = FALSE,
        cli::cli_abort("`{field}` is not a valid field in the `{module_class_name}` module.")
      )
    } else {
      set_param_vector(
        field = field,
...
#' @param module_input A list. Contains parameters for initializing the distribution.
#' @param distribution_name A character. Name of the distribution to initialize.
#' @param distribution_type A character. Type of distribution, either "data" or "process".
#' @param linked_ids A vector. Named vector of linked IDs required for the distribution,
...
initialize_distribution <- function(module_input, distribution_name,
  ...
  if (!is.vector(linked_ids) || !all(c("data_link", "fleet_link") %in% names(linked_ids))) {
    cli::cli_abort("{.var linked_ids} must be a named vector containing 'data_link' and 'fleet_link' for 'data' distribution types.")
  }
  ...
  distribution_input_names <- grep(distribution_name, names(module_input), value = TRUE)
  ...
  cli::cli_inform(c(
    "i" = "{distribution_name} initialized successfully for {names(distribution_name)}."
  ))
...
initialize_index <- function(data, fleet_name) {
  ...
  module <- methods::new(Index, data@n_years)
...
initialize_age_comp <- function(data, fleet_name) {
  ...
  if (is.null(age_comp_data) || length(age_comp_data) == 0) {
    cli::cli_abort("Age composition data for fleet `{fleet_name}` is unavailable or empty.")
  }
...
initialize_fims <- function(parameters, data) {
  ...
  for (i in seq_along(fleet_names)) {
    ...
    if (length(parameter_value_name) == 0 || length(parameter_estimated_name) == 0) {
