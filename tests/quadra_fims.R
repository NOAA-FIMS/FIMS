...
load("/Users/matthew.supernaw/FIMS-Testing/FIMS_Quadra/FIMS/tests/testthat/fixtures/integration_test_data.RData")
...
recruitment$log_devs$set_estimation_types("random_effects")
...
max(abs(quadra_fit$gradient))

diagnostics <- quadra_model_diagnostics(
  quadra_fit$par,
  quadra_fit$random
)
...
writeLines(markdown, "quadra-diagnostics.md")
