...
load("FIMS/tests/testthat/fixtures/integration_test_data.RData")
...
fishing_fleet_index_distribution$set_distribution_links("data", fishing_fleet$log_index_expected$get_id())
...
fishing_fleet_agecomp_distribution$set_distribution_links("data", fishing_fleet$agecomp_expected$get_id())
...
fishing_fleet_lengthcomp_distribution$set_distribution_links("data", fishing_fleet$lengthcomp_expected$get_id())
...
# )
alcm <- c(t(em_input[["age_to_length_conversion"]]))
...
for (i in 1:(om_input[["nages"]] * om_input[["nlengths"]])) {
  fishing_fleet$age_to_length_conversion[i]$value <- alcm[i]
}
...
survey_fleet_length_comp$length_comp_data$fromRVector(c(t(survey_lengthcomp)) * em_input[["n.survey.lengthcomp"]][["survey1"]])
# Fleet
...
survey_fleet_agecomp_distribution$set_distribution_links(
  "data", survey_fleet$agecomp_expected$get_id())
...
survey_fleet_lengthcomp_distribution$set_distribution_links("data", survey_fleet$lengthcomp_expected$get_id()) # Set age to length conversion matrix
...
for (i in 1:(om_input[["nages"]] * om_input[["nlengths"]])) {
  survey_fleet$age_to_length_conversion[i]$value <- alcm[i]
}
# Turn off estimation for length-at-age
