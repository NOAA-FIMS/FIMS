test_that("create_FIMSFit creates an object of class 'FIMSFit'", {
  ...
  result <- create_FIMSFit(
    input = list(),
    timing = c(time_total = as.difftime(10, units = "secs")),
    obj = list(
      par = NA, fn = NA, gr = NA, he = NA, hessian = NA, method = NA,
      retape = NA,
      env = list("last.par.best" = 1, "parList" = function(x) {TRUE}),
      report = function(x) {list(TRUE)}, simulate = NA
    )
  )
