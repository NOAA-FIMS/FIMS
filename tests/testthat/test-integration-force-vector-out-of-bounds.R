...
iter_id = 1
load("fixtures/integration_test_data.RData")

 test_that("Test that R doesn't crash when force vector is out of bounds", {
 # Load operating model data for the current iteration
  ...
  expect_error(obj <- TMB::MakeADFun(
    data = list(), parameters, DLL = "FIMS",
    silent = TRUE, map = NULL, random = "re"
