...
test_that("posterior equals prior with no data using likelihood objects", {
  ...
  # Set up likelihood priors for selectivity parameters and link to both
  # fishery and survey selectivity. The new likelihood interface links one
  # input vector per component, so each selectivity parameter gets its own
  # NormalLikelihood object.
  ...
  expect_length(report_nll, 4)
  fishing_slope_input <- om_input[["sel_fleet"]][["fleet1"]][["slope.sel1"]]
  survey_slope_input <- om_input[["sel_survey"]][["survey1"]][["slope.sel1"]]
  fishing_inflection_point_input <- om_input[["sel_fleet"]][["fleet1"]][["A50.sel1"]]
  survey_inflection_point_input <- om_input[["sel_survey"]][["survey1"]][["A50.sel1"]]
  #' @description Test the fishing fleet slope prior nll
  expect_equal(
    report_nll[1], -dnorm(fishing_slope_input, mean = slope_mean, sd = sqrt(slope_var), log = TRUE)
  )
  #' @description Test the survey fleet slope prior nll
  expect_equal(
    report_nll[2], -dnorm(survey_slope_input, mean = slope_mean, sd = sqrt(slope_var), log = TRUE)
  )
  #' @description Test the fishing fleet inflection point prior nll
  expect_equal(
    report_nll[3], -dnorm(fishing_inflection_point_input, mean = inflection_point_mean, sd = sqrt(inflection_point_var), log = TRUE)
  )
  #' @description Test the survey fleet inflection point prior nll
  expect_equal(
    report_nll[4], -dnorm(survey_inflection_point_input, mean = inflection_point_mean, sd = sqrt(inflection_point_var), log = TRUE)
  )
