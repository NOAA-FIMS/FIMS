...
test_that("test new_process_distribution", {
    ...
    expect_equal(length(recruitment_distribution$x), length(recruitment_distribution$expected_values))
  ...
  fishing_fleet_index_distribution <-
    new_data_distribution(data_type = "cpue", module = fishing_fleet,
                        ...
                        sd = list(value = rep(sqrt(log(em_input$cv.L$fleet1^2 + 1)), om_input$nyr),
                                  estimated = FALSE))
   expect_equal(log(sqrt(log(em_input$cv.L$fleet1^2 + 1))), fishing_fleet_index_distribution$log_logsd[1]$value)
   expect_error(new_data_distribution(data_type = "cpue", module = fishing_fleet,
                        family = multinomial(),
                        sd = list(value = rep(sqrt(log(em_input$cv.L$fleet1^2 + 1)), om_input$nyr),
                                  estimated = FALSE)))
    expect_error(new_data_distribution(data_type = "index", module = fishing_fleet,
                        family = multinomial(),
                        sd = list(value = rep(sqrt(log(em_input$cv.L$fleet1^2 + 1)), om_input$nyr),
                                  estimated = FALSE)))
    expect_error(new_data_distribution(data_type = "agecomp", module = fishing_fleet,
                        family = gaussian(),
                        sd = list(value = rep(sqrt(log(em_input$cv.L$fleet1^2 + 1)), om_input$nyr),
                                  estimated = FALSE)))
    expect_error(new_data_distribution(data_type = "lengthcomp", module = fishing_fleet,
                        family = lognormal(),
                        sd = list(value = rep(sqrt(log(em_input$cv.L$fleet1^2 + 1)), om_input$nyr),
                                  estimated = FALSE)))
   clear()
