...
test_that("test new_process_distribution", {
  ...
  recruitment$log_devs <- methods::new(ParameterVector, om_input$logR.resid[-1],
                                       om_input$nyr - 1)

  # set up logR_sd using the normal log_sd parameter
  recruitment_distribution <-
    new_process_distribution(module = recruitment, par = "log_devs",
                             family = gaussian(),
                             sd = list(value = om_input$logR_sd,
                                       estimated = FALSE),
  ...
  expect_error(
    new_process_distribution(module = recruitment, par = "log_devs",
                             family = gaussian(),
                             sd = list(value = rep(om_input$logR_sd, 3), 
