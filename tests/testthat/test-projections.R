...
if(n_projection_years>0){
  ...
  projected_landings <- c(rep(-999,(n_projection_years-1)),4*max(em_input[["L.obs"]][["fleet1"]]))
  projected_landings_sd <- rep(2,(n_projection_years))
  projected_F <- rep(om_output[["f"]][om_input$nyr],n_projection_years)
  estim_projected_F <- rep("constant",n_projection_years)
  projected_index <- rep(-999,n_projection_years)
  ...
  if(n_projection_years>0){
    for (y in (om_input[["nyr"]] + 1):(om_input[["nyr"]] + n_projection_years)) {
      # Compute lognormal SD from OM coefficient of variation (CV)
      fishing_fleet_landings_distribution$log_sd[y]$value <- projected_landings_sd[(y-om_input[["nyr"]])]
    }
  ...
  if(n_projection_years>0){
    ...
    for (i in (om_input[["nyr"]] + max(1,(n_projection_years - 30))):(om_input[["nyr"]] + n_projection_years)) {
      F_mult_distribution$x[i]$value <- 0
      F_mult_distribution$expected_values[i]$value <- 0
      F_mult_distribution$log_sd[i]$value <- -1
      F_mult_distribution$log_sd[i]$estimation_type$set("constant")
    }
  ...
  sdr_report_20_year_project_MSY_target <- sdr_report
  sdr_fixed_20_year_project_MSY_target <- sdr_fixed
  report_20_year_project_MSY_target <- report
  ...
  n_projection_years <- 100
  projected_landings <- c(rep(-999,(n_projection_years)))
  projected_landings_sd <- rep(2,(n_projection_years))
  projected_F <- rep(om_output[["f"]][om_input$nyr],n_projection_years)
  estim_projected_F <- rep("constant",n_projection_years)
  projected_index <- rep(-999,n_projection_years)
  ...
  sdr_report_20_year_project_SSB_ratio_target <- sdr_report
  sdr_fixed_20_year_project_SSB_ratio_target <- sdr_fixed
  report_20_year_project_SSB_ratio_target <- report

  #Compare results across model runs

  comparison_results <- sdr_fixed_20_year_project_SSB_ratio_target[-50,]/sdr_fixed_no_project
  max(comparison_results)
  min(comparison_results)

  sdr_fixed_20_year_project_SSB_ratio_target/sdr_fixed_20_year_project_MSY_target
