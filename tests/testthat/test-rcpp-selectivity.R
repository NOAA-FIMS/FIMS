...
test_that("age-specific selectivity works through XPtr", {
  clear()
  on.exit(clear())
  sel <- create_selectivity("AgeSpecific")
  set_selectivity_ages(sel, 1:3)
  set_variable_vector(sel, "logit_sel_at_age", c(-2, 0, 2), "fixed_effects")
  expect_equal(vapply(1:3, function(age) evaluate_selectivity(sel, age), 0),
               plogis(c(-2, 0, 2)))
  expect_error(evaluate_selectivity(sel, 10), "out of bounds")
  expect_equal(get_module_id(sel), 1)
  clear()
  expect_error(evaluate_selectivity(sel, 1))
})
