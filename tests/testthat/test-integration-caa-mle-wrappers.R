...
test_that("deterministic run returns correct nlls", {
  ...
  expect_equal(deterministic_age_length_comp@number_of_parameters["fixed_effects"] |> unname(), 77)
  #' @description Veryify the number of random effects are correct
  expect_equal(deterministic_age_length_comp@number_of_parameters["random_effects"] |> unname(), 0)
})
})
