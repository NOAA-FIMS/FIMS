...
test_that("catch-at-age model (estimation MLE with wrappers) works with mixed estimation statuss", {
  # Load setup data
  ...
  fit_mixed_estimation_statuses <- modified_parameters |>
    dplyr::mutate(
  ...
  mixed_output <- get_estimates(fit_mixed_estimation_statuses) |>
  ...
  #' @description Test that the output from FIMS matches the model comparison project OM values when Fmort estimation statuss are mixed.
  validate_fims(
    report = get_report(fit_mixed_estimation_statuses),
    estimates = get_estimates(fit_mixed_estimation_statuses),
    om_input = om_input_list[[iter_id]],
    om_output = om_output_list[[iter_id]],
    em_input = em_input_list[[iter_id]],
    use_fimsfit = TRUE
...
test_that("catch-at-age model (estimation MLE with wrappers) returns an error when there are no estimated parameters for optimization", {
  ...
  # Set all non-NA estimation statuss to "assumed_known" and initialize the model
