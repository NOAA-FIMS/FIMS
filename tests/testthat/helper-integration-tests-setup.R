...
setup_and_run_FIMS <- function(iter_id,
  ...
  # To close all connections if using doParallel::registerDoParallel()
  # closeAllConnections()
}
...
# Function to run FIMS in parallel with {snowfall}
setup_and_run_FIMS_by_core <- function(core_id,
                                        om_input_list,
                                        om_output_list,
                                        em_input_list,
                                        begin_id,
  ...
  results_index <- 1
  # Load operating model data
  for (iter_id in begin_id[core_id]:end_id[core_id]){
