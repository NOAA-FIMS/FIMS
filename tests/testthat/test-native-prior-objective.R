test_that("native prior objectives and gradients match analytic normal densities", {
  ...
  means <- ifelse(inflection, inflection_mean, log_slope_mean)
  sds <- ifelse(inflection, inflection_sd, log_slope_sd)
  for (shift in c(0, 0.1, -0.1)) {
    p <- obj$par + shift
    expected <- -sum(dnorm(p, means, sds, log = TRUE))
    gradient <- (p - means) / sds^2
    expect_equal(obj$fn(p), expected, tolerance = 1e-10)
    expect_equal(as.numeric(obj$gr(p)), unname(gradient), tolerance = 1e-10)
    if ("native_quadra_objective" %in% getNamespaceExports("FIMS")) {
      expect_equal(native_quadra_objective(p, numeric()), expected, tolerance = 1e-10)
      expect_equal(as.numeric(native_quadra_gradient(p, numeric())),
                   unname(gradient), tolerance = 1e-10)
    }
  }
})
