...
validate_comp_nll <- function(observed_prop, sample_size, expected_prop, theta = NULL) {
  ...
  if (is.null(theta)) {
    ...
    return(FIMS_ddirichlet_multinom(observed_counts, expected_prop, theta))
  }
}
...
validate_fims <- function(
  ...
  validate_error <- function(expected,
    ...
    # Validate z-scores such that max < 4 (indicating no extreme outliers)
  ...
  if (use_fimsfit) {
    out_of_tolerance_parameters <- estimates |>
      ...
      # (input, estimate, and uncertainty)
      dplyr::filter(
        !is.na(input),
        !is.na(estimated),
        !is.na(uncertainty),
