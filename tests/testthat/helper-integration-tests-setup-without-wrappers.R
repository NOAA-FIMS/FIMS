...
# FIMS models without wrapper functions. The functions are sourced by
# devtools::load_all().
...
setup_and_run_FIMS_without_wrappers <- function(iter_id,
  ...
  tryCatch({
    ...
    cli::cli_abort(
      c(
        "x" = "Failed to create a FIMS model without wrapper functions.",
        "i" = "Please check the code logic in `setup_and_run_FIMS_without_wrappers()`",
        "i" = "File to check: `tests/testthat/helper-integration-tests-setup-without-wrapper.R`.",
