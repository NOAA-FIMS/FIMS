...
for (i in 2:(nrow(tuna.dat))) {
  init_z[i] <- (log(inits$depletion[i]) - log(pella_tomlinson(
    inits$depletion[i - 1],
    inits$r, inits$K, 2, tuna.dat$C[i - 1]
  ))) / sqrt(inits$sigma2)
}
...
for (i in 2:(nrow(tuna.dat) + 1)) {
  init_z_p1[i] <- (log(inits_p1$depletion[i]) - log(pella_tomlinson(
    inits_p1$depletion[i - 1],
    inits_p1$r, inits_p1$K, 2, tuna.dat$C[i - 1]
  ))) / sqrt(inits_p1$sigma2)
}

pars_nc_sig <- list(
  log_r = log(inits$r), log_K = log(inits$K),
...
pars_nc_sig_p1 <- list(
  log_r = log(inits_p1$r), log_K = log(inits_p1$K),
  log_sigma2 = log(inits_p1$sigma2),
  log_tau2 = log(inits_p1$tau2),
  z = init_z_p1
)
pars_nc_isig_p1 <- list(
  log_r = log(inits_p1$r), log_K = log(inits_p1$K),
...
for (i in 1:10) {
  set.seed(1234 + i)
  fit_nc_sig[[i]] <- tmbstan::tmbstan(obj_nc_sig,
    init = "last.par.best",
    warmup = 5000, iter = 10000, chains = 4,
    control = list(adapt_delta = 0.99)
  )
  fit_c_sig[[i]] <- tmbstan::tmbstan(obj_c_sig,
    init = "last.par.best",
    warmup = 5000, iter = 10000, chains = 4,
    control = list(adapt_delta = 0.99)
  )
  fit_nc_isig[[i]] <- tmbstan::tmbstan(obj_nc_isig,
    init = "last.par.best",
    warmup = 5000, iter = 10000, chains = 4,
    control = list(adapt_delta = 0.99)
  )
  fit_c_isig[[i]] <- tmbstan::tmbstan(obj_c_isig,
    init = "last.par.best",
    warmup = 5000, iter = 10000, chains = 4,
    control = list(adapt_delta = 0.99)
  )
...
fit_rtmb_stan <- tmbstan::tmbstan(obj,
  init = "last.par.best",
...
for (i in 1:10) {
  ...
  fit_nc_sig_p1[[i]] <- tmbstan::tmbstan(obj_nc_sig_p1,
    init = "last.par.best",
    warmup = 5000, iter = 10000, chains = 4,
  ...
  fit_nc_isig_p1[[i]] <- tmbstan::tmbstan(obj_nc_isig_p1,
    init = "last.par.best",
    warmup = 5000, iter = 10000, chains = 4,
...
init_funs <- function(chain_id) {
  sigma2 <- 1 / rgamma(1, 3.785518, rate = 0.010223)
  out_par <- c(
    r = rlnorm(1, -1.38, 1 / sqrt(3.845)),
    K = rlnorm(1, 5.042905, 1 / sqrt(3.7603664)),
    sigma2 = sigma2,
    tau2 = 1 / rgamma(1, 1.708603, rate = 0.008613854),
    P = rlnorm(
      (tuna.dat |> nrow()) + 1,
      log(tuna.dat$I / tuna.dat$I[1]),
      sqrt(sigma2)
    )
  )
  names(out_par) <- c("r", "K", "sigma2", "tau2", paste0("P[", 1:24, "]"))
  return(out_par)
}
...
fit <- tmbstan::tmbstan(obj_inits,
  init = init_funs(), # "last.par.best",
  warmup = 5000, iter = 30000, chains = 4,
...
model_c_sig <- function(par) {
  ...
  # Process likelihood
  nll_P <- -sum(dlnorm(P[1:Plen], meanlog = log(P_med[1:Plen]), sd = sigma, log = TRUE))
...
fit_nc_sig_p1 <- tmbstan::tmbstan(obj_nc_sig_p1,
  init = "last.par.best",
  warmup = 5000, iter = 10000, chains = 4,
  lower = lower_nc_sig_p1, upper = upper_nc_sig_p1,
  control = list(adapt_delta = 0.99)
...
out_par_nc_sig_p1 <-
  apply(as.matrix(fit_nc_sig_p1)[, -ncol(as.matrix(fit_nc_sig_p1))], 2, median)
out_par_nc_sig_p1
