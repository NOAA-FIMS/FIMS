    ...
    if (input.GetType() == fims::JsonValueType::Object &&
      ...
      if (it != obj.end()) {
        ...
        for (size_t i = 0; i < n_fleets; i++) {
          std::shared_ptr<fims_popdy::Fleet<double>> f =
              std::make_shared<fims_popdy::Fleet<double>>();
          f->log_q.resize(1);
          this->InitializeFleetObject(*f, n_years, n_ages);
          //  f->observed_index_data =
          //  std::make_shared<fims_data_object::DataObject<double> >(n_years);
              ...
              if ((*fsel).second.GetType() == fims::JsonValueType::Object) {
                ...
                if ((*it).second.GetType() == fims::JsonValueType::JArray) {
                  ...
                  if (sel_pattern[0].GetInt() == 1) { // logistic
                    ...
                    std::shared_ptr<fims_popdy::LogisticSelectivity<double>>
                        selectivity = std::make_shared<
                            fims_popdy::LogisticSelectivity<double>>();
      ...
      if (it != obj.end()) {
        ...
        for (size_t i = 0; i < nsurveys; i++) {
          std::shared_ptr<fims_popdy::Fleet<double>> s =
              std::make_shared<fims_popdy::Fleet<double>>();
          s->log_q.resize(1);
          ...
          if ((*it).second.GetType() == fims::JsonValueType::Object) {
            //                        f->log_q =
            //                        fims_math::log((*it).second.GetDouble());
      ...
      if (it != obj.end()) {
        if ((*it).second.GetType() == fims::JsonValueType::JArray) {
          ...
          rec->logit_steep[0] = fims_math::logit(
