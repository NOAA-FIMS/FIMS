...
class IntegrationTest {
 public:
  ...
  bool ReadJson(const std::string &path, fims::JsonValue &result) {
    std::stringstream ss;
    std::ifstream infile;
  ...
  void InitializeFleetObject(fims::popdy::Fleet<double> &fleet, size_t n_years,
                             size_t n_ages) {
  ...
  void InitializePopulationObject(fims::popdy::Population<double> &pop,
  ...
  bool ConfigurePopulationModel(fims::popdy::Population<double> &pop,
                                fims::JsonValue &input,
                                fims::JsonValue &output) {
    ...
    if (input.GetType() == fims::JsonValueType::Object &&
        output.GetType() == fims::JsonValueType::Object) {
      ...
      if (it != obj.end()) {
        ...
        for (size_t i = 0; i < n_fleets; i++) {
          std::shared_ptr<fims::popdy::Fleet<double>> f =
              std::make_shared<fims::popdy::Fleet<double>>();
          f->log_q.resize(1);
          this->InitializeFleetObject(*f, n_years, n_ages);
          //  f->observed_index_data =
          //  std::make_shared<fims::data_object::DataObject<double> >(n_years);
              ...
              if ((*fsel).second.GetType() == fims::JsonValueType::Object) {
                ...
                if ((*it).second.GetType() == fims::JsonValueType::JArray) {
                  ...
                  if (sel_pattern[0].GetInt() == 1) {  // logistic
                    ...
                    std::shared_ptr<fims::popdy::LogisticSelectivity<double>>
                        selectivity = std::make_shared<
                            fims::popdy::LogisticSelectivity<double>>();
                    ...
                    std::shared_ptr<
                        fims::popdy::DoubleLogisticSelectivity<double>>
                        selectivity = std::make_shared<
                            fims::popdy::DoubleLogisticSelectivity<double>>();
                    ...
                    if ((*it).second.GetType() == fims::JsonValueType::JArray) {
                      ...
                      if (print_statements) {
                        std::cout << "A50 asc "
                                  << selectivity->inflection_point_asc[0]
                                  << "\n";
                      }
                    }
                    ...
                    if ((*it).second.GetType() == fims::JsonValueType::JArray) {
                      ...
                      selectivity->inflection_point_desc[0] =
                          a50[0].GetDouble();
                      if (print_statements) {
                        std::cout << "A50 desc "
                                  << selectivity->inflection_point_desc[0]
      ...
      if (it != obj.end()) {
        ...
        for (size_t i = 0; i < nsurveys; i++) {
          std::shared_ptr<fims::popdy::Fleet<double>> s =
              std::make_shared<fims::popdy::Fleet<double>>();
          s->log_q.resize(1);
          this->InitializeFleetObject(*s, n_years, n_ages);
          //   s->observed_index_data =
          //   std::make_shared<fims::data_object::DataObject<double>
          //   >(n_years);
          //  s->observed_agecomp_data =
          //  std::make_shared<fims::data_object::DataObject<double> >(n_years,
          //  n_ages);
          ...
          if ((*it).second.GetType() == fims::JsonValueType::Object) {
            //                        f->log_q =
            //                        fims::math::log((*it).second.GetDouble());
            ...
            if ((*qit).second.GetType() == fims::JsonValueType::JArray) {
              ...
              s->log_q[0] = fims::math::log(a[0].GetDouble());
      ...
      // set recruitment
      std::shared_ptr<fims::popdy::SRBevertonHolt<double>> rec =
          std::make_shared<fims::popdy::SRBevertonHolt<double>>();
      // set recruitment process
      std::shared_ptr<fims::popdy::LogDevs<double>> log_devs =
          std::make_shared<fims::popdy::LogDevs<double>>();
      ...
      if (it != obj.end()) {
        if ((*it).second.GetType() == fims::JsonValueType::JArray) {
          ...
          rec->logit_steep[0] = fims::math::logit(
              0.2, 1.0, (*it).second.GetArray()[0].GetDouble());
          if (print_statements) {
      ...
      std::fill(rec->log_recruit_devs.begin(), rec->log_recruit_devs.end(),
                0.0);
      std::fill(rec->log_expected_recruitment.begin(),
                rec->log_expected_recruitment.end(), 0.0);
      if (it != obj.end()) {
        if ((*it).second.GetType() == fims::JsonValueType::JArray) {
      ...
      std::shared_ptr<fims::popdy::LogisticMaturity<double>> mat =
          std::make_shared<fims::popdy::LogisticMaturity<double>>();
      ...
      if (it != obj.end()) {
        if ((*it).second.GetType() == fims::JsonValueType::JArray) {
          ...
          for (size_t i = 0; i < pop.ages.size(); i++) {
            growth->ewaa[static_cast<double>(pop.ages[i])] =
                wt[i].GetDouble() / 1000.0;
            if (print_statements) {
  ...
  std::vector<double> RunModelLoop(fims::popdy::Population<double> &pop,
                                   const fims::JsonValue &input) {
    fims::JsonObject output;
    fims::JsonArray array;

    fims::popdy::CatchAtAge<double> model_loop;
  ...
  bool CheckModelOutput(fims::popdy::Population<double> &pop,
                        fims::JsonValue &output) {
    return true;
  }
};
