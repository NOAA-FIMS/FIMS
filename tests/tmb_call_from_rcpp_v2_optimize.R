...
cat("Native objective at start:", objective_at_start, "\n")
cat(
    "Native n_fixed:", length(parameters[["p"]]),
    "n_random:", length(parameters[["re"]]), "\n"
)
print(obj[["par"]])

if (length(parameter_names) > 0) {
    cat("\nNative fixed parameter registration order:\n")
    print(data.frame(
        index = seq_along(fixed_parameters),
        name = parameter_names[seq_len(min(length(parameter_names), length(fixed_parameters)))],
        value = fixed_parameters[seq_len(min(length(parameter_names), length(fixed_parameters)))],
        row.names = NULL
    ))
}
...
cat("\nInitial diagnostics\n")
cat("-------------------\n")
cat("objective:", objective_at_start, "\n")
cat("n_parameters:", length(par_start), "\n")
cat("gradient max abs:", max(abs(grad_start)), "\n")
cat("gradient L2 norm:", sqrt(sum(grad_start^2)), "\n")
...
if (!is.null(initial_report) &&
    !is.null(initial_report[["nll_components"]])) {
    cat("\nInitial NLL components:\n")
    print(initial_report[["nll_components"]])
}

# -----------------------------------------------------------------------------
# Optimize using the same nlminb setup as tmb_Rcpp.R
# -----------------------------------------------------------------------------
cat("\nStarting nlminb optimization...\n")
...
cat("\nFinal diagnostics\n")
cat("-----------------\n")
cat("objective at start:", objective_at_start, "\n")
cat("objective at optimum:", objective_final, "\n")
cat("objective decrease:", objective_at_start - objective_final, "\n")
cat("convergence code:", opt[["convergence"]], "\n")
cat("message:", opt[["message"]], "\n")
cat("iterations:", opt[["iterations"]], "\n")
cat("evaluations:\n")
print(opt[["evaluations"]])
cat("gradient max abs:", max(abs(grad_final)), "\n")
cat("gradient L2 norm:", sqrt(sum(grad_final^2)), "\n")
...
if (!is.null(final_report) &&
    !is.null(final_report[["nll_components"]])) {
    cat("\nFinal NLL components:\n")
    print(final_report[["nll_components"]])
}
