...
TEST_F(CAAPrepareTestFixture, HandlesCorrectInput_Fleet_Initialize) {
  ...
  for (auto fit = catch_at_age_model->fleets.begin();
    ...
    EXPECT_EQ(dq["catch_numbers"], fims::Vector(n_years, 0.0));
    EXPECT_EQ(dq["catch_expected"], fims::Vector(n_years, 0.0));
    EXPECT_EQ(dq["log_catch_expected"], fims::Vector(n_years, 0.0));
