...
TEST_F(CAAEvaluateTestFixture,
       CalculateLandingsNumbersAA_CalculateLandingsWeightAA_works) {
  ...
  test_naa[i_age_year] =
      test_naa[i_agem1_yearm1] * exp(-dq_pop["mortality_Z"][i_agem1_yearm1]);
  ...
  for (size_t fleet_index = 0; fleet_index < population->n_fleets;
    ...
    // \inst\include\population_dynamics\population\population.hpp
    landings_temp = (population->fleets[fleet_index]->Fmort[year] *
                     population->fleets[fleet_index]->selectivity->evaluate(
                         population->ages[age])) /
                    dq_pop["mortality_Z"][i_age_year] * test_naa[i_age_year] *
    ...
    // test value
    EXPECT_EQ(dq_fleet["landings_numbers_at_age"][i_age_year],
              test_landings_naa[i_age_yearf]);
    EXPECT_EQ(dq_fleet["landings_weight_at_age"][i_age_year],
              test_landings_waa[i_age_yearf]);
  }
}
} // namespace