...
TEST_F(CAAEvaluateTestFixture, HandlesCorrectInput) {
  ...
  for (size_t fleet_index = 0; fleet_index < population->n_fleets;
    ...
    // \inst\include\population_dynamics\population\population.hpp
    catch_temp = (population->fleets[fleet_index]->Fmort[year] *
    ...
    test_catch_waa[i_age_yearf] +=
        catch_temp * population->growth->evaluate(year, population->ages[age]);

    // test value
    EXPECT_EQ(dq_fleet["catch_numbers_at_age"][i_age_year],
              test_catch_naa[i_age_yearf]);
    EXPECT_EQ(dq_fleet["catch_weight_at_age"][i_age_year],
              test_catch_waa[i_age_yearf]);
    // Default pooled demand leaves partitioned catch untouched.
    EXPECT_EQ(dq_fleet["catch_numbers_at_age_by_partition"],
              fims::Vector(dq_fleet["catch_numbers_at_age_by_partition"].size(),
                           0.0));
    EXPECT_EQ(dq_fleet["catch_weight_at_age_by_partition"],
              fims::Vector(dq_fleet["catch_weight_at_age_by_partition"].size(),
                           0.0));
  }
}
...
TEST_F(CAAEvaluateTestFixture, FemaleDemandWritesCatchByPartition) {
  ...
  for (size_t fleet_index = 0; fleet_index < population->n_fleets;
    ...
    EXPECT_DOUBLE_EQ(dq_fleet["catch_numbers_at_age_by_partition"][male_idx],
                     0.0);
    EXPECT_DOUBLE_EQ(
        dq_fleet["catch_weight_at_age_by_partition"][female_idx],
        dq_fleet["catch_weight_at_age"][i_age_year] * p_female);
    EXPECT_DOUBLE_EQ(dq_fleet["catch_weight_at_age_by_partition"][male_idx],
                     0.0);
  }
}
}  // namespace
