// Instructions ----
// This file follows the format generated by FIMS:::use_gtest_template().
// Necessary tests include input and output (IO) correctness [IO
// correctness], edge-case handling [Edge handling], and built-in errors and
// warnings [Error handling]. See `?FIMS:::use_gtest_template` for more
// information. Every test should have a description comment.
// More assertion macros provided by GoogleTest can be found at
// https://google.github.io/googletest/reference/assertions.html.

#include "gtest/gtest.h"
#include "fims_math.hpp"

namespace {

// fimsMath_log
// IO correctness
// Test log using multiple input values and types
// Not worth to write many tests when testing thin database wrappers,
// third-party libraries, or basic variable assignments.

TEST(fimsMath_log, HandlesCorrectInput) {
  // Test log using large double value
  EXPECT_EQ(fims::math::log(1000000.0), std::log(1000000.0));

  // Test log using double value 3.0
  // R code that generates true values for the test
  // log(3.0): 1.098612
  EXPECT_NEAR(fims::math::log(3.0), 1.098612, 0.0001);
}

// Edge handling
TEST(fimsMath_log, HandlesEdgeCase) {
  // Test log using large positive integer value
  int large_int = 1000000;
  EXPECT_EQ(fims::math::log<double>(large_int), std::log(large_int));

  // log(3.0): 1.098612
  int small_int = 3;
  EXPECT_EQ(fims::math::log<double>(small_int), std::log(small_int));
}

// Error handling
// log(-2.5): NaN
TEST(fimsMath_log, CaptureError) {
  // Test log using negative value -2.5 and expect return of NaN
  EXPECT_TRUE(std::isnan(fims::math::log(-2.5)));
}

}  // namespace
