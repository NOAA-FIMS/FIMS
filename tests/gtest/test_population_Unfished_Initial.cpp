...
TEST_F(CAAPrepareTestFixture, CalculateMortality_works) {
  ...
  for (int year = 0; year < population->n_years; year++) {
    for (int age = 0; age < population->n_ages; age++) {
      ...
      // Call FIMS CalculateMortality() function to compare FIMS mortality
      // values with "true" values later
  ...
  std::vector<double> mortality_F(
      ...
      0.0); // n_ages should be defined appropriately
  ...
  for (int year = 0; year < population->n_years; year++) {
    for (int age = 0; age < population->n_ages; age++) {
      ...
      for (int fleet_index = 0; fleet_index < population->n_fleets;
           fleet_index++) {
        // Known values were used to generate "true" value and test
        // CalculateMortality()
...
TEST_F(CAAEvaluateTestFixture,
       CalculateUnfishedNumbersAAandUnfishedSpawningBiomass_works) {
  ...
  for (int year = 0; year < (population->n_years + 1); year++) {
    for (int age = 0; age < population->n_ages; age++) {
      ...
      if (year > 0 && age > 0) {
        ...
        // unfished_numbers_at_age[i_age_year-1] *
        // fims_math::exp(-fims_math::exp(population.log_M[i_age_year-1]));
        test_unfished_numbers_at_age[i_age_year] =
