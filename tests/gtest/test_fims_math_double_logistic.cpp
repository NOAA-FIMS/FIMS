...
TEST(DoubleLogistic, UseMultipleInputValues) {
  ...
  // 1.0/(1.0+exp(-(2.0-1.0)*0.0)) * (1.0 - 1.0/(1.0+exp(-(2.0-6.0)*0.0))) =
  // 0.25 1.0/(1.0+exp(-(20.0-10.0)*0.2)) * (1.0
  // - 1.0/(1.0+exp(-(20.0-15.0)*0.2))) = 0.2368828
  // 1.0/(1.0+exp(-(40.5-20.5)*0.05)) * (1.0 - 1.0/(1.0+exp(-(40.5-23.5)*0.05)))
  // = 0.218903

  std::vector<double> expect_value = {0.25, 0.2368828, 0.218903};

  for (int i = 0; i < expect_value.size(); ++i) {
    EXPECT_NEAR(fims_math::double_logistic(inflection_point_asc_value[i],
                                           slope_asc_value[i],
                                           inflection_point_desc_value[i],
                                           slope_desc_value[i], x_value[i]),
