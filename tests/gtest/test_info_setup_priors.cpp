...
TEST(SetupPriors, UseDoubleValues) {
  ...
  std::shared_ptr<fims_distributions::NormalLPDF<double> >
      normal_inflection_point =
  ...
  // Set up inflection point key. Keys 1 and 3 will reference variable map 1 and
  // 3
  ...
  // Call function that links key ID to variable map pointers given variable map
  // ID This function will set the density component, priors, to the respective
  // parameters
  info->SetupPriors();

  EXPECT_EQ((*normal_inflection_point->priors[0])[0],
            selectivity1->inflection_point[0]);
  EXPECT_EQ((*normal_inflection_point->priors[1])[0],
            selectivity2->inflection_point[0]);
  EXPECT_EQ((*lognormal_slope->priors[0])[0], selectivity1->slope[0]);
  EXPECT_EQ((*lognormal_slope->priors[1])[0], selectivity2->slope[0]);
  EXPECT_EQ((normal_inflection_point->get_observed(0)),
            selectivity1->inflection_point[0]);
  EXPECT_EQ((normal_inflection_point->get_observed(1)),
            selectivity2->inflection_point[0]);
