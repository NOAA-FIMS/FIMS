...
#include "selectivity/functors/double_logistic.hpp"
...
TEST(DoubleLogisticSelectivity_Evaluate, HandlesCorrectInput) {
  ...
  fishery_selectivity.slope_asc[0] = std::log(0.2);
  fishery_selectivity.inflection_point_desc[0] = 15.0;
  fishery_selectivity.slope_desc[0] = std::log(0.05);
  double fishery_x = 34.5;
  // 1.0/(1.0+exp(-(34.5-10.5)*0.2)) * (1.0 - 1.0/(1.0+exp(-(34.5-15.0)*0.05)))
  // = 0.2716494
...
TEST(DoubleLogisticSelectivity_Evaluate, HandlesThreeTimeSteps) {
  ...
  fishery_selectivity.inflection_point_asc[0] = 10.5;
  fishery_selectivity.slope_asc[0] = std::log(0.2);
  fishery_selectivity.slope_asc[1] = std::log(0.3);
  fishery_selectivity.slope_asc[2] = std::log(0.4);
  fishery_selectivity.inflection_point_desc[0] = 15.0;
  fishery_selectivity.slope_desc[0] = std::log(0.05);
  ...
  // pos 0: 1.0/(1.0+exp(-(34.5-10.5)*0.2)) * (1.0
  // - 1.0/(1.0+exp(-(34.5-15.0)*0.05))) = 0.2716494 pos
  // 1: 1.0/(1.0+exp(-(34.5-10.5)*0.3)) * (1.0
  // - 1.0/(1.0+exp(-(34.5-15.0)*0.05))) = 0.2736807 pos
  // 2: 1.0/(1.0+exp(-(34.5-10.5)*0.4)) * (1.0
  // - 1.0/(1.0+exp(-(34.5-15.0)*0.05))) = 0.2738665
