...
TEST_F(CAAInitializeTestFixture, Initialize_works) {
  ...
  EXPECT_EQ(catch_at_age_model->populations[0]->proportion_female.size(),
            n_ages);
...
TEST_F(CAAPrepareTestFixture, Prepare_works) {
  ...
  EXPECT_EQ(dq["total_landings_weight"], fims::Vector<double>(n_years, 0));
  EXPECT_EQ(dq["total_landings_numbers"], fims::Vector<double>(n_years, 0));
  EXPECT_EQ(dq["mortality_F"], fims::Vector<double>(n_years * n_ages, 0));
  EXPECT_EQ(dq["mortality_Z"], fims::Vector<double>(n_years * n_ages, 0));
  EXPECT_EQ(dq["numbers_at_age"],
            fims::Vector<double>((n_years + 1) * n_ages, 0));
  EXPECT_EQ(dq["unfished_numbers_at_age"],
            fims::Vector<double>((n_years + 1) * n_ages, 0));
  EXPECT_EQ(dq["biomass"], fims::Vector<double>(n_years + 1, 0));
  EXPECT_EQ(dq["spawning_biomass"], fims::Vector<double>(n_years + 1, 0));
  EXPECT_EQ(dq["unfished_biomass"], fims::Vector<double>(n_years + 1, 0));
  EXPECT_EQ(dq["unfished_spawning_biomass"],
            fims::Vector<double>(n_years + 1, 0));
  EXPECT_EQ(dq["proportion_mature_at_age"],
            fims::Vector<double>((n_years + 1) * n_ages, 0));
  EXPECT_EQ(dq["expected_recruitment"], fims::Vector<double>(n_years + 1, 0));
  EXPECT_EQ(dq["sum_selectivity"], fims::Vector<double>(n_years * n_ages, 0));
  ...
  for (int i = 0; i < n_ages; i++) {
    EXPECT_EQ(catch_at_age_model->populations[0]->proportion_female[i],
              p_female[i]);
  }
}
} // namespace
