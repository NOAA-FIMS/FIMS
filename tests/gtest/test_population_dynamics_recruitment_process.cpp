...
TEST(EvaluateRecruitmentProcess, UseMultipleInputs) {
  ...
  //  # x = spawners
  //  recruits <- (0.8 * R0 * h * x) / (0.2 * 100.0 * (1.0 - h) + x * (h //  -
  //  0.2)) return(recruits)
  // }
  // (0.8 * 1000.0 * 0.75 * 30.0) / (0.2 * 100.0 * (1.0 - 0.75) + 30.0 * (0.75 -
  // 0.2)) = 837.2093 log(837.2093) - 1.0 = 5.730074
  double log_expect_recruit1_plus_devs = 5.730074;
  recruit1->log_expected_recruitment.resize(1);
  recruit1->log_expected_recruitment[0] =
      log(recruit1->evaluate_mean(spawners, phi_0));
  EXPECT_NEAR(
      recruit1->process->evaluate_process(0),
      recruit1->log_expected_recruitment[0] + recruit1->log_recruit_devs[0],
      0.0001);
  EXPECT_NEAR(recruit1->process->evaluate_process(0),
              log_expect_recruit1_plus_devs, 0.0001);
  ...
  //  # phi0 = unfished spawners per recruit
  //  # x = spawners
  //  recruits <- (0.8 * R0 * h * x) / (0.2 * ssb0 * (1.0 - h) + x * (h //  -
  //  0.2)) return(recruits)
  // }
  // log(200) = 5.298317
  // log_r = log(200) + 1
  ...
  EXPECT_NEAR(recruit2->process->evaluate_process(0), recruit2->log_r[0],
              0.0001);
