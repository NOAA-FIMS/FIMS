...
namespace {
// SRBevertonHolt_evaluateProcess
// IO correctness
TEST(SRBevertonHolt_evaluateProcess, HandlesCorrectInput) {
  ...
  auto recruit1 = std::make_shared<fims::popdy::SRBevertonHolt<double>>();
  ...
  recruit1->logit_steep[0] = fims::math::logit(0.2, 1.0, 0.7500);
  ...
  auto recruit2 = std::make_shared<fims::popdy::SRBevertonHolt<double>>();
  auto log_r = std::make_shared<fims::popdy::LogR<double>>();
  recruit2->process = log_r;
  recruit2->process->recruitment = recruit2;
  recruit2->logit_steep.resize(1);
  recruit2->logit_steep[0] = fims::math::logit(0.2, 1.0, 0.200);
  ...
  //  # phi_0 = unfished spawners per recruit
  //  # x = spawners
  //  recruits <- (0.8 * R0 * h * x) / (0.2 * phi_0 * R0 * (1.0 - h) + x * (h //
  //  - 0.2)) return(recruits)
  // }
  // log(200) = 5.298317
  // log_r = log(200) + 1
