#include <cmath>
#include "gtest/gtest.h"
#include "common/fims_math.hpp"
#include "population_dynamics/growth/functors/vonb_schnute.hpp"

namespace {
fims_popdy::VonBertalanffySchnuteGrowth<double> MakeValidVonB() {
  fims_popdy::VonBertalanffySchnuteGrowth<double> vb;

  vb.mean_length_young = 275.0;
  vb.mean_length_old = 725.0;
  vb.growth_coefficient = 0.18;
  vb.reference_age_for_length_young = 1.0;
  vb.reference_age_for_length_old = 12.0;
  vb.length_weight_a = 2.5e-11;
  vb.length_weight_b = 3.0;

  return vb;
}

TEST(VonBertalanffySchnuteEvaluate, BasicSanity) {
  auto vb = MakeValidVonB();

  // expected values from the formula (guarded denominator)
  const double denom_raw = 1.0 - std::exp(-vb.growth_coefficient *
                                          (vb.reference_age_for_length_old -
                                           vb.reference_age_for_length_young));
  const double denom = fims_math::ad_max(fims_math::ad_fabs(denom_raw), 1e-8);

  double L1 = vb.length_at_age(1.0);
  double L5 = vb.length_at_age(5.0);
  double L12 = vb.length_at_age(12.0);

  const double L1_expected =
      vb.mean_length_young +
      (vb.mean_length_old - vb.mean_length_young) *
          (1.0 - std::exp(-vb.growth_coefficient *
                          (1.0 - vb.reference_age_for_length_young))) /
          denom;
  const double L12_expected =
      vb.mean_length_young +
      (vb.mean_length_old - vb.mean_length_young) *
          (1.0 - std::exp(-vb.growth_coefficient *
                          (12.0 - vb.reference_age_for_length_young))) /
          denom;

  // sanity checks
  EXPECT_NEAR(L1, L1_expected, 1e-12);
  EXPECT_GT(L5, 275.0);
  EXPECT_LT(L5, 725.0);
  EXPECT_NEAR(L12, L12_expected, 1e-12);

  double W5 = vb.evaluate(0, 5.0);
  EXPECT_NEAR(W5, 2.5e-11 * std::pow(L5, 3.0), 1e-10);

  fims_popdy::VonBertalanffySchnuteGrowth<double> vb2;
  EXPECT_EQ(vb2.GetId(), vb.GetId() + 1);
}

TEST(VonBertalanffySchnuteEvaluate, UsesLinearRampBelowFirstReferenceAge) {
  auto vb = MakeValidVonB();

  // This setup reproduces the age-0 back-calculation problem:
  // age at length young is above the youngest model age, so age 0 is below
  // the first reference age.
  vb.mean_length_young = 6.0;
  vb.mean_length_old = 67.1;
  vb.growth_coefficient = 0.242;
  vb.reference_age_for_length_young = 1.0;
  vb.reference_age_for_length_old = 21.0;

  // Use a non-integer length-weight exponent because negative length raised
  // to a non-integer power is the pathway that can produce NaN.
  vb.length_weight_a = 1.75e-8;
  vb.length_weight_b = 2.99;

  // Ages below age at length young should now use the early-age linear ramp
  // rather than back-extrapolating the curved growth equation.
  EXPECT_NEAR(vb.length_at_age(0.0), 0.0, 1e-12);
  EXPECT_NEAR(vb.length_at_age(0.5), 3.0, 1e-12);
  EXPECT_NEAR(vb.length_at_age(1.0), 6.0, 1e-12);
  EXPECT_GT(vb.length_at_age(2.0), vb.mean_length_young);

  // The key failure mode was NaN weight-at-age at age 0. This should now be
  // finite and non-negative.
  const double W0 = vb.weight_at_age(0.0);
  const double W_half = vb.weight_at_age(0.5);

  EXPECT_TRUE(std::isfinite(W0));
  EXPECT_TRUE(std::isfinite(W_half));
  EXPECT_GE(W0, 0.0);
  EXPECT_GE(W_half, 0.0);
  EXPECT_NEAR(W_half, vb.length_weight_a * std::pow(3.0, vb.length_weight_b),
              1e-12);

  // log-length gradient follows the early-age ramp below age at length young.
  // At age 0.5, mean length is 3 and dL/dL1 is 0.5. The implementation uses
  // the same smooth safety floor as other log-length calculations, so compute
  // the expected value with that same safe mean length.
  double d_log_laa_d_length_young = 0.0;
  double d_log_laa_d_length_old = 0.0;
  double d_log_laa_d_k = 0.0;

  vb.log_length_at_age_gradient(0.5, d_log_laa_d_length_young,
                                d_log_laa_d_length_old, d_log_laa_d_k);

  const double ratio_half = 0.5 / vb.reference_age_for_length_young;
  const double mean_length_half = vb.mean_length_young * ratio_half;
  const double mean_length_half_safe =
      fims_math::ad_max(mean_length_half, 1e-8);

  EXPECT_NEAR(d_log_laa_d_length_young, ratio_half / mean_length_half_safe,
              1e-12);
  EXPECT_NEAR(d_log_laa_d_length_old, 0.0, 1e-12);
  EXPECT_NEAR(d_log_laa_d_k, 0.0, 1e-12);

  // On the log scale, multiply the natural-scale derivative by length young.
  double d_log_laa_d_log_length_young = 0.0;
  double d_log_laa_d_log_length_old = 0.0;
  double d_log_laa_d_log_k = 0.0;

  vb.log_length_at_age_logscale_gradient(0.5, d_log_laa_d_log_length_young,
                                         d_log_laa_d_log_length_old,
                                         d_log_laa_d_log_k);

  EXPECT_NEAR(d_log_laa_d_log_length_young,
              vb.mean_length_young * ratio_half / mean_length_half_safe, 1e-12);
  EXPECT_NEAR(d_log_laa_d_log_length_old, 0.0, 1e-12);
  EXPECT_NEAR(d_log_laa_d_log_k, 0.0, 1e-12);
}

}  // namespace
