...
class SPEvaluateTestFixture : public testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    surplus_production_model->populations[0]->logit_init_depletion.resize(1);
    surplus_production_model->populations[0]->logit_init_depletion[0] =
          log(init_depletion / (1 - init_depletion)); //logit transformation
    ...
    depletion->logit_depletion.resize(nyears-1);
    for(int i=0; i<depletion->logit_depletion.size(); i++){
      double input_depletion = depletion_distribution(generator);
      //logit transformation
      depletion->logit_depletion[i] = log(input_depletion / (1 - input_depletion));
    }
