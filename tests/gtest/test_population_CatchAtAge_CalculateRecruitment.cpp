...
namespace
{
    ...
    // IO correctness
    TEST_F(CAAEvaluateTestFixture, HandlesCorrectInput_CatchAtAge_CalculateRecruitment)
    {
        ...
        (0.2 * phi_0 * rzero * (1.0 - steep) + population->spawning_biomass[sb_year] * (steep - 0.2)) * fims_math::exp(population->recruitment->log_recruit_devs[r_year-1]);
        ...
        EXPECT_DOUBLE_EQ(population->numbers_at_age[r_i_age_year],
                         expect_recruitment[r_i_age_year]);
