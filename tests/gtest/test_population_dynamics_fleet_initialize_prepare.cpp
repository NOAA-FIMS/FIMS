...
TEST_F(CAAPrepareTestFixture, FleetPrepareWorks) {
  ...
  for (auto fit = catch_at_age_model->fleets.begin();
    ...
    EXPECT_EQ(dq["landings_weight"], fims::Vector(n_years, 0.0));
    EXPECT_EQ(dq["landings_numbers"], fims::Vector(n_years, 0.0));
    EXPECT_EQ(dq["landings_expected"], fims::Vector(n_years, 0.0));
    EXPECT_EQ(dq["log_landings_expected"], fims::Vector(n_years, 0.0));
    EXPECT_EQ(dq["agecomp_proportion"], fims::Vector(n_years * n_ages, 0.0));
    ...
    EXPECT_EQ(dq["index_numbers"], fims::Vector(n_years, 0.0));
    EXPECT_EQ(dq["index_expected"], fims::Vector(n_years, 0.0));
    EXPECT_EQ(dq["log_index_expected"], fims::Vector(n_years, 0.0));
    EXPECT_EQ(dq["catch_index"], fims::Vector(n_years, 0.0));
    EXPECT_EQ(dq["expected_catch"], fims::Vector(n_years, 0.0));
    EXPECT_EQ(dq["agecomp_expected"], fims::Vector(n_years * n_ages, 0.0));
