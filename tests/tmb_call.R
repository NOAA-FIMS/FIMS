...
match_rcpp_setup <- identical(
    Sys.getenv("FIMS_TMB_CALL_MATCH_RCPP_SETUP", "0"),
    "1"
)
...
fishing_log_fmort_estimation_type <- if (isTRUE(match_rcpp_setup)) {
    rep("fixed_effects", nyr)
} else {
    c("fixed_effects", rep("constant", nyr - 1))
}

fishing_log_q_estimation_type <- if (isTRUE(match_rcpp_setup)) {
    "constant"
} else {
    c("random_effects", rep("constant", nyr - 1))
}

survey_log_q_estimation_type <- if (isTRUE(match_rcpp_setup)) {
    "fixed_effects"
} else {
    "constant"
}
...
maturity_id <- maturity_logistic_create(
    ...
    inflection_point_estimation_type = if (isTRUE(match_rcpp_setup)) "constant" else "fixed_effects",
    slope_estimation_type = if (isTRUE(match_rcpp_setup)) "constant" else "random_effects"
)
...
recruitment_id <- recruitment_beverton_holt_create(
    logit_steep = logit_steep,
    log_rzero = log_rzero,
    log_devs = if (isTRUE(match_rcpp_setup)) as.numeric(om_input[["logR.resid"]][-1]) else numeric(),
    logit_steep_estimation_type = if (isTRUE(match_rcpp_setup)) "constant" else "fixed_effects",
    log_rzero_estimation_type = if (isTRUE(match_rcpp_setup)) "fixed_effects" else "random_effects"
)

population_log_m <- if (isTRUE(match_rcpp_setup)) {
    rep(log(as.numeric(om_input[["M.age"]][1])), nyr * nages)
} else {
    rep(log(as.numeric(om_input[["M.age"]][1])), nyr)
}

population_log_m_estimation_type <- if (isTRUE(match_rcpp_setup)) {
    "constant"
} else {
    c("fixed_effects", rep("constant", nyr - 1))
}

population_log_f_multiplier_estimation_type <- if (isTRUE(match_rcpp_setup)) {
    "constant"
} else {
    c("random_effects", rep("constant", nyr - 1))
}

population_log_init_naa_estimation_type <- if (isTRUE(match_rcpp_setup)) {
    rep("fixed_effects", nages)
} else {
    c("fixed_effects", rep("constant", nages - 2), "random_effects")
}
...
build_default_likelihood <- function() {
    ...
    .Call(
        ...
        if (isTRUE(match_rcpp_setup)) as.numeric(om_input[["logR_sd"]]) else NA_real_,
        as.integer(nyr),
        as.integer(nages),
