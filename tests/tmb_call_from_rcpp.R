...
#
# Rcpp creates Landings/AgeComp/LengthComp/Index and all distributions before
# CreateTMBModel(). Do the native equivalent BEFORE fims_call_create_model().
#
# The native likelihood builder is expected to reproduce the Rcpp transforms:
...
# -----------------------------------------------------------------------------
# Catch-at-age/model assembly -- equivalent point to CreateTMBModel()
# -----------------------------------------------------------------------------
...
# Set FIMS_TMB_NATIVE_SKIP_OPTIMIZATION=1 to stop after the equivalence check.
skip_optimization <- identical(
    Sys.getenv("FIMS_TMB_NATIVE_SKIP_OPTIMIZATION", "0"),
    "1"
)
...
if (isTRUE(skip_optimization)) {
    result <- list(
        ...
        create_model_result = create_model_result
    )
    print(result)
} else {
    opt <- stats::nlminb(
    ...
    print(opt)
    cat("max(abs(obj$gr())):", max(abs(obj$gr())), "\n")
}
