...
> **Interface update:** FIMS now constructs runtime model objects through the
> native `.Call` interface in `inst/include/interface/call/` and `src/call_*.cpp`.
> Sections below that discuss Rcpp classes document the former interface and
> remain useful only as historical architectural context. Current R code should
> use `maturity_logistic_create()`, `population_create()`, and
> `initialize_fims()` rather than instantiate Rcpp module classes.

The following diagram represents the former Rcpp path from R to C++ for the
maturity module. It is retained to explain the core object relationships while
the diagram is updated for the native registry architecture.
...
# Create the native maturity object and retain its integer handle
maturity_id <- maturity_logistic_create(
  inflection_point = 2.25,
  slope = 3,
...
The same native constructor can be called directly when building a lower-level
model. It returns an integer handle rather than an R reference object.
...
For lower-level development, create the population through its native wrapper
and link components by integer handle.

```{r,  class.source = "rchunk", eval = FALSE}
population_id <- population_create(
  log_m = log(0.2),
  log_f_multiplier = 0,
  log_init_naa = log(rep(1000, 10)),
...
Native FIMS objects live in backend registries, so removing an R integer handle
does not clear the corresponding C++ object. Use `clear()` (an alias of
`native_clear()`) between independent models. Restarting R also releases this
memory.
```{r,  class.source = "rchunk", eval = TRUE}
# Clear C++ memory
clear()
