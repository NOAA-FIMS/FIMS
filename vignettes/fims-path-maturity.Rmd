...
```{Rcpp, class.source = "rcppchunk", eval = FALSE}
 Rcpp::class_<Variable>("Variable")
  ...
  .field("min", &Variable::min)
  .field("max", &Variable::max)
  .field("estimation_type", &Variable::estimation_type);
```

Each field (i.e., parameter) from `maturity` (i.e., the maturity module we defined in R) will therefore inherit the five fields defined in the Variable class: `value`, `min`, `max`, `estimation_type`. That is, two parameter fields and 10 fields within those parameters.
