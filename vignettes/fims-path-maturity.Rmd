...
For power users, the module can be created directly, without using lists of parameters or any wrapper functions. The following code also creates the same maturity module.
...
# Populate the maturity module with parameter values. Each call sets a whole
# field at once, giving its values and how each one is to be estimated.
set_variable_vector(maturity, "inflection_point", 10, "assumed_known")
set_variable_vector(maturity, "slope", 0.2, "assumed_known")
```
...
population <- population_defaults |>
  ...
  FIMS:::initialize_population(
    data = fims_frame,
    maturity = maturity,
    # Growth and recruitment are given as bare IDs because this example never
    # creates those modules; normally you would pass the modules themselves.
    growth = 2,
    recruitment = 3,
    fleets = list()
  )
...
For power users, the population module can be created directly, without using lists of parameters or any wrapper functions. One call links all three processes.
...
R does not hold the maturity module as an object with fields of its own. It holds an external pointer to the C++ interface object, and reaches that object's fields by name. Each module declares which names it answers to, in `get_variable_vector()`, defined for each module in the .hpp files under [inst/include/interface/rcpp/rcpp_objects](https://github.com/NOAA-FIMS/FIMS/tree/main/inst/include/interface/rcpp/rcpp_objects). A field with no entry there cannot be reached from R, whatever else is true of it.
...
The code above shows that there are two fields specific to the maturity module, `inflection_point` and `slope`. `get_module_id()` returns a unique ID for a created module: you can have multiple instances of a maturity module in memory, but only one can be used per model. To see which fields a module has, ask it -- `has_variable_vector(maturity, "slope")` answers for one name, and the `get_variable_vector()` code above lists every name it will answer to.
...
All Rcpp interface classes from FIMS define parameters (e.g., `inflection_point`, `slope`) using the `VariableVector` class defined in [rcpp_interface_base.hpp](https://github.com/NOAA-FIMS/FIMS/blob/main/inst/include/interface/rcpp/rcpp_objects/rcpp_interface_base.hpp) in the directory [inst/include/interface/rcpp/rcpp_objects](https://github.com/NOAA-FIMS/FIMS/blob/main/inst/include/interface/rcpp/rcpp_objects). VariableVectors allow parameters to vary with time. Whereas, the Variable class is only used for time-invariant parameters. `Variable` and `VariableVector` are themselves still registered as Rcpp classes, in [src/rcpp_variable.cpp](https://github.com/NOAA-FIMS/FIMS/blob/main/src/rcpp_variable.cpp), which is what makes their fields reachable once a module hands one back. For example,
...
Rcpp::class_<Variable>(
    ...
    .field("estimation_status", &Variable::estimation_status_m);
```

Each field (i.e., parameter) from `maturity` (i.e., the maturity module we defined in R) will therefore inherit the four fields defined in the Variable class: `value`, `estimated_value`, `id`, `estimation_status`. That is, two parameter fields and 8 fields within those parameters.
...
std::shared_ptr<fims_info::Information<TMB_FIMS_REAL_TYPE> > info =
 ...
 if (this->inflection_point.estimation_status == "random_effects") {
        /*
  ...
  if (this->inflection_point.estimation_status == "fixed_effects") {
        /*
...
The `maturity_id` was set from R using `set_population_processes()`, which as a reminder is done internally within `FIMS::initialize_population` but can also be done by hand. Where both options are shown, again, below.
```{r,  class.source = "rchunk", eval = FALSE}
# Helper function
recruitment_defaults <- FIMS::setup_default_Recruitment(data = fims_frame)
log_M <- log(0.2)
...
# Manual
set_population_processes(population, maturity = maturity)
```
...
# set_variable_vector(maturity1, "inflection_point", 10, "assumed_known")
# set_variable_vector(maturity1, "slope", 0.2, "assumed_known")
# maturity2 <- create_maturity("Logistic")
# set_variable_vector(maturity2, "inflection_point", 8, "assumed_known")
# set_variable_vector(maturity2, "slope", 0.3, "assumed_known")
# population1 <- create_population()
# population2 <- create_population()
# population3 <- create_population()
```

Next, the maturity IDS are assigned to each `maturity_id` in one of the three populations. Which is actually done using `set_population_processes()` in R (i.e., the commented out code).
```{r}
...
# set_population_processes(population1, maturity = maturity1)
# set_population_processes(population2, maturity = maturity1)
# set_population_processes(population3, maturity = maturity2)
```
