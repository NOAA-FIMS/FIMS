...
#| message: false
#| fig.alt: >
#|   Catches are increasing over time and the index is decreasing over time.
#| fig.cap: >
...
# Look at default Logistic parameters
default_parameters |>
  dplyr::filter(fleet == "survey1", module_name == "Selectivity") |>
  dplyr::select(module_type, label, age, value, estimation_type)

# Update default to AgeSpecificSelectivity
...
updated_parameters <- default_parameters |>
  dplyr::filter(!(fleet == "survey1" & module_name == "Selectivity")) |>
  dplyr::bind_rows(new_selectivity)

# Look at updated parameters
updated_parameters |>
  dplyr::filter(fleet == "survey1", module_name == "Selectivity") |>
  dplyr::select(module_type, label, age, value, estimation_type)
```
...
```{r parameters-update}
parameters_4_model <- updated_parameters |>
  ...
  dplyr::rows_update(
    tibble::tibble(
      fleet = "survey1",
      label = c("log_q"),
      value = c(log(3.315143e-07))
    ),
    by = c("fleet", "label")
  ) |>
  # Update selectivity parameters for survey1
  # AJ: need to figure out how to change this from default Logistic to AgeSpecific
  # All but first two ages are fixed to true values
  dplyr::rows_update(
    tibble::tibble(
      fleet = "survey1",
...
# Look at updated AgeSpecific selectivity parameters
parameters_4_model |>
  dplyr::filter(fleet == "survey1", module_name == "Selectivity") |>
  dplyr::select(module_type, label, age, value, estimation_type)
parameters_4_model |>
  dplyr::filter(fleet == "survey1", label == "log_q") |>
  dplyr::select(module_type, label, age, value, estimation_type)
```
...
fit@obj$gr()
fit@obj$par
sel_estimates <- get_estimates(fit) |> dplyr::filter(module_name == "Selectivity")
```
