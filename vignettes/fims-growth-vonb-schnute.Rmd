...
The final plot is not meant to duplicate the test suite. It shows the realized growth-derived age-to-length conversion in the final modeled year. This is useful for checking whether the fitted growth curve and length-at-age variability imply reasonable length distributions across ages.
...
#| fig.height: 8
#| fig.cap: >
#|   Realized growth-derived age-to-length conversion in the final modeled
#|   year, showing length-bin probabilities by age.
#| fig.alt: >
#|   Mirrored distributions of length-bin probabilities by age. Each age has
#|   one vertical distribution, with wider areas showing higher probability.
...
# Reshape the flattened tensor into length x age x year form for plotting.
...
realized_age_to_length_conversion_dataframe <- expand.grid(
  length_i = seq_along(demo$lengths),
  age_i = seq_along(demo$ages),
  year_i = seq_along(demo$years)
) |>
  tibble::as_tibble() |>
  dplyr::mutate(
    length = demo$lengths[.data[["length_i"]]],
    age = demo$ages[.data[["age_i"]]],
    year = demo$years[.data[["year_i"]]],
    probability = realized_age_to_length_conversion_array[
      cbind(.data[["length_i"]], .data[["age_i"]], .data[["year_i"]])
    ]
  )

age_to_length_plot_data <- realized_age_to_length_conversion_dataframe |>
...
ggplot2::ggplot(
  ...
  ggridges::geom_density_ridges(
    stat = "binline",
    bins = dplyr::n_distinct(age_to_length_plot_data[["length"]]),
    scale = 1.5,
    alpha = 0.8
  ) +
  ggplot2::coord_flip() +
  ggplot2::labs(
    ...
    subtitle = paste("Length distributions by age in the modeled year", final_year),
    x = "Length",
    y = "Age"
  ) +
...
Each age-specific distribution should sum to one in the plotted year:

```{r downstream-check}
age_to_length_plot_data |>
  dplyr::group_by(.data[["age"]]) |>
  dplyr::summarize(
    probability_sum = sum(.data[["probability"]]),
    .groups = "drop"
  ) |>
  dplyr::summarize(
