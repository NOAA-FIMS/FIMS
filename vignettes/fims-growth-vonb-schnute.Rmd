...
#| fig.alt: >
#|   Ridgeline plot of length-bin probabilities by age. Each age has one
#|   distribution, with wider areas showing higher probability.
...
realized_age_to_length_conversion_dataframe <- expand.grid(
  ...
  dplyr::mutate(
    ...
    timing = demo$years[.data[["year_i"]]],
    estimated = realized_age_to_length_conversion_array[
...
plot_age_to_length_conversion(
  data = realized_age_to_length_conversion_dataframe |>
    dplyr::filter(timing == 2)
)
```
...
age_to_length_plot_data |>
  ...
  dplyr::summarize(
    proportion_sum = sum(.data[["proportion"]]),
    .groups = "drop"
  ) |>
  dplyr::summarize(
    min_proportion_sum = min(.data[["proportion_sum"]]),
    max_proportion_sum = max(.data[["proportion_sum"]])
  )
```
