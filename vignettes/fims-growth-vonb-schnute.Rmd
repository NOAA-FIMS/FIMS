...
This vignette walks through the `VonBertalanffySchnute` growth pathway in FIMS fitting to simulated data with known growth parameters. The example shows how to create data, set up model-derived growth output, fit the model, compare estimated growth parameters to true parameters, and inspect the Growth outputs that feed length-based fleet calculations.

The `VonBertalanffySchnute` growth module uses two reference ages to estimate the specific lengths on the growth curve instead of the standard $t_0$ and $L_{\infty}$. Thus, the growth curve is defined by length at two reference ages, young and old, and the growth coefficient, $K$. In this example, the following three Growth parameters are estimated:
- `mean_length_young`
- `mean_length_old`
- `growth_coefficient`
...
This vignette uses the interpolation-based variability pathway that supplies the standard deviation at the two reference ages through `length_at_age_sd_at_reference_ages`.
...
Data is simulated for this vignette rather than using `data_big` to allow for ages 1--30, rather than a maximum age of 12. The wider age range makes the shape of the fitted curve easy to inspect, and thus the properties of the module can be better explained than if we were to use `data_big`. But, the model does take longer to run because it is fitting to more data bins than if there were less ages.
...
Initialize and fit the model using the prepared parameter table. The reporting flag includes the realized Growth-derived age-to-length conversion tensor in standard output for later inspection.
...
# Request the realized age-to-length conversion tensor in standard output.
...
# Save standard output for the Growth comparison and later derived-output plot.
model_estimates <- FIMS::get_estimates(fit)

# Keep only the core VonBertalanffySchnute Growth estimates.
growth_estimates <- model_estimates |>
  dplyr::filter(
    module_name == "Growth",
...
### Inspect the Realized Age-to-Length Conversion

The final plot shows the realized growth-derived age-to-length conversion across modeled time steps. This is useful for checking whether the fitted growth curve and length-at-age variability imply reasonable length-bin distributions across ages.
...
#| fig.height: 8
#| fig.cap: >
#|   Realized growth-derived age-to-length conversion across modeled time steps,
#|   showing length-bin probabilities by age.
#| fig.alt: >
#|   Ridgeline panels of length-bin probabilities by age and modeled time step.
#|   Each age has one distribution, with wider areas showing higher probability.

# The path flags remain available through the TMB report.
report <- FIMS::get_report(fit)

# Standard output expands the requested derived tensor into long model output.
# The current JSON parser returns dimension positions, so year_i, age_i, and
# length_i are indices rather than the original year, age, or length-bin values.
age_to_length_plot_data <- model_estimates |>
  dplyr::filter(
    .data[["label"]] == "age_to_length_conversion_derived"
  )

plot_age_to_length_conversion(
  data = age_to_length_plot_data
)
```

Each age-specific distribution should sum to one for every modeled year:

```{r downstream-check}
age_to_length_plot_data |>
  dplyr::group_by(
    .data[["year_i"]],
    .data[["age_i"]]
  ) |>
  dplyr::summarize(
    probability_sum = sum(.data[["estimated"]]),
    .groups = "drop"
  ) |>
  dplyr::summarize(
    min_probability_sum = min(.data[["probability_sum"]]),
    max_probability_sum = max(.data[["probability_sum"]])
  )
```
