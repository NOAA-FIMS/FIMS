---
title: "FIMS Growth With VonB-Schnute"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{FIMS Growth With VonB-Schnute}
...
This vignette walks through the `VonB-Schnute` growth pathway in FIMS using one generated-data example with known Growth truth. The example shows how to create data, set up model-derived Growth through the standard FIMS workflow, fit the model, compare estimated Growth to truth, and inspect the Growth outputs that feed length-based fleet calculations.

The `VonB-Schnute` module uses a two-reference-age growth form. The growth curve is defined by length at two reference ages and the growth coefficient `K`. In this example, the following three Growth parameters are estimated:
- `mean_length_young`
- `mean_length_old`
...
## Main Example: Estimable VonB-Schnute With Generated Data
...
make_vonb_demo_data <- function(
  ...
  true_curve <- tibble::tibble(
    ...
    true_mean_length = vonb_two_reference_age(
      ...
      von_bertalanffy_coefficient_K = von_bertalanffy_coefficient_K,
...
# Build the generated VonB-Schnute example data.
...
# Identify the three estimable VonB-Schnute curve parameters used for
...
# Replace only the Growth rows with VonB-Schnute Growth defaults.
...
```{r}
# Pull out the default VonB-Schnute interpolation SD rows.
...
# Keep only the core VonB-Schnute Growth estimates.
...
# Show the side-by-side comparison for the core VonB-Schnute parameters.
growth_comparison
```
...
#| fig.height: 6
#| fig.cap: >
#|   Observed lengths-at-age, the true `VonB-Schnute` curve used to generate
#|   the data, and the fitted `VonB-Schnute` curve from FIMS.
#| fig.alt: >
...
true_plot_curve <- tibble::tibble(
  ...
  mean_length = vonb_two_reference_age(
    ...
    von_bertalanffy_coefficient_K = true_growth_values$von_bertalanffy_coefficient_K,
    reference_age_for_length_1 =
...
estimated_plot_curve <- tibble::tibble(
  ...
  mean_length = vonb_two_reference_age(
    ...
    von_bertalanffy_coefficient_K = estimated_growth_values$von_bertalanffy_coefficient_K,
    reference_age_for_length_1 =
...
ggplot2::ggplot() +
  ...
  ggplot2::labs(
    title = "Observed and predicted VonB-Schnute growth curve",
    x = "Age",
    y = "Length"
  ) +
...
#|   generated `VonB-Schnute` example.
#| fig.alt: >
#|   Residual plot with age on the x-axis and observed minus predicted length
#|   on the y-axis for the generated `VonB-Schnute` example.

# Build residuals from the fitted VonB-Schnute curve
...
```text
estimated VonB-Schnute Growth parameters
        |
        v
...
This vignette shows the baseline `VonB-Schnute` growth-derived pathway. Small changes can be made to the generated example to see how Growth starting values, fleet length bins, length-at-age variability, reference ages, report settings, and downstream model outputs respond.
