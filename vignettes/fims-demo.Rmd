...
The default_configurations are just a starting point. Functions (e.g., `rows_*()`) from `{dplyr}` can be used to modify the default configurations as needed. For example, logistic selectivity for survey1 can be changed to age-based selectivity.
...
updated_configurations <- default_configurations_unnested |>
  dplyr::rows_update(
    y = tibble::tibble(
      ...
      module_type = c("SelectivityatAge")
    ),
...
# Look at default SelectivityatAge parameters
default_parameters_unnested |> 
  dplyr::filter(fleet_name == "survey1", module_name == "Selectivity") |>
...
parameters_4_model <- default_parameters |>
  ...
  dplyr::rows_update(
    tibble::tibble(
      ...
      value = c(-1.000000, 1.000000, 2.999999, 4.999993, 6.999946,
                8.999956, 11.000085, 12.982599, 15.019483, 18.420681,
                18.420681, 18.420681),
      estimation_type = c(rep("constant", 12))
      #, rep("fixed_effects", 5),
      #                    rep("constant", 5))
    ),
...
parameters_4_model |>
  ...
  dplyr::select(module_type, label, age, value, estimation_type)
parameters_4_model |>
  dplyr::filter(fleet_name == "survey1", label == "log_q") |>
  dplyr::select(module_type, label, age, value, estimation_type)
...
test_fit@obj$hessian
fit@obj$hessian
fit@obj$gr()
fit@obj$par
ests <- get_estimates(fit) #fails
```
