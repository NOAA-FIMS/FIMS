...
nyears <- age_frame@nyrs  # the number of years which we have data for.
nseanons<-1 # the number of seasons in each year.
ages <- age_frame@ages # age vector.
nages <- max(age_frame@ages)   # the number of age groups.
...
```{r fit_model}
  ...
  opt<-nlminb(fims$get_fixed(), obj$fn, obj$gr)
