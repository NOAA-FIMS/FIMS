...
parameters <- default_parameters |>
  ...
  #   module = "recruitment",   
  #   form = "BevertonHolt",   
  #   process = process_model(par = "log_devs", type = "iid", method = "Penalized MLE",
  #       sd = 0.4)) |>
#  update_process(
#    parent_modules = list(fleet1)
#    module = "selectivity",   
#    form = "LogisticSelectivity",   
#    process = process_model(par = log_devs, type = iid, method = "REFull",
#        log_sd = 0, init = 0)
#  ) |>
  update_parameters(
...
default_fleet1 <- default_parameters[["parameters"]][["fleet1"]] |>
  purrr::list_flatten() |>
  purrr::map_vec(
  \(x) length(x)
)
updated_fleet1 <- parameters[["parameters"]][["fleet1"]] |>
  purrr::list_flatten() |>
  purrr::map_vec(
  \(x) length(x)
)
...
```{r parameter-length-table}
  ...
  purrr::list_flatten() |>
  purrr::map_df( \(x) purrr::map_vec(x, length),
  .id = "module"
) |>
