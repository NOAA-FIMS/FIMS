...
Calling `library(FIMS)` loads the R package and its compiled native library. FIMS
uses registered `.Call` entry points internally; users do not need to load C++
modules or construct C++ objects directly.
...
Use the standard R help system for both the high-level modeling workflow and
the lower-level native wrappers. For example:
```r
?FIMS::fit_fims
?FIMS::native_create_model
?FIMS::native_build_default_likelihood
```

Most users should use `initialize_fims()` and `fit_fims()`. The native wrappers
are primarily useful for interface development and focused backend tests.
...
The tibble returned by `create_default_parameters()` is a data frame containing
model specifications. `initialize_fims()` validates those specifications,
creates the native backend objects, registers fixed and random effects, links
the model components, and returns the parameter lists needed by TMB.
...
The former class-based interface exposed the C++ log through `get_log()`. That accessor
is not yet exposed by the native interface. Errors and warnings raised during
native model construction are still propagated as ordinary R conditions.
Native log retrieval will be documented here when its `.Call` wrapper is added.
...
The native result table currently omits some dimensional labels expected by
{stockplotr}. The plotting calls below show the intended workflow and will be
enabled once that metadata is exposed.

```{r fit-plot-spawning-biomass, eval=FALSE}
#| warning: false
#| message: false
#| fig.alt: >
...
```{r fit-plot-log-fishing-mortality, eval=FALSE}
#| fig.alt: >
...
```{r fit-plot-index-of-abundance, eval=FALSE}
#| fig.alt: >
...
```{r fit-plot-landings, eval=FALSE}
#| fig.alt: >
