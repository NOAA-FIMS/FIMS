...
# mapping the module list to environment
# list2env(initialization$module_list, globalenv())

parameter_list <- list(parameters = initialization$parameter_list, version = "FIMS run without optimization")

fit <- fit_fims(input = parameter_list, optimize = TRUE)
...
catch_results <- data.frame(
  observed = m_landings(fims_frame),
