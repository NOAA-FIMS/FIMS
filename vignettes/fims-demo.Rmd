...
Calling `library(FIMS)` loads the R package and its registered native interface into the R environment.
...
Use the traditional R help system, for example `?FIMS::fit_fims`, for user-facing functions. C++ implementation details are available in the generated Doxygen documentation.

For example, `?FIMS::native_create_model` documents native model construction.
...
This vignette selects `backend = "TMB"` because its plots use derived-quantity standard errors. The default Quadra backend currently reports parameter uncertainty but does not include those derived quantities in its uncertainty report.

The list returned from `initialize_fims()` can be passed to the parameter of `fit_fims()` called `input` to run a FIMS model. If `optimize = FALSE`, the model will not actually be optimized but instead just checked to ensure it is a viable model. When `optimize = TRUE`, the model will be fit using `stats::nlminb()` with the explicitly selected TMB backend and an object of the class `FIMSFit` will be returned.
...
test_fit <- parameters_4_model |>
  ...
  fit_fims(optimize = FALSE, backend = "TMB")
clear()
# Run the  model with optimization
fit <- parameters_4_model |>
  initialize_fims(data = data_4_model) |>
  fit_fims(optimize = TRUE, backend = "TMB")
```
...
output <- get_estimates(fit) |>
  dplyr::mutate(
    ...
    year = year_i,
    estimate = dplyr::coalesce(estimated, expected)
  )
```
...
stockplotr::plot_spawning_biomass(
  output |>
    ...
    dplyr::mutate(year = seq.int(
      get_start_year(data_4_model), length.out = dplyr::n()
    ))
) +
...
stockplotr::plot_timeseries(
  stockplotr::filter_data(
    output |> dplyr::filter(fleet == "survey1"),
...
stockplotr::plot_timeseries(
  stockplotr::filter_data(
    output |> dplyr::filter(fleet == "fleet1"),
