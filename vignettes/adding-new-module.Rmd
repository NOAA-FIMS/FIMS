...
> **Native-interface transition:** Runtime R integration now uses registered
> `.Call` routines and native registries. The early architecture and directory
> guidance below reflects that interface. Later Rcpp-specific examples are
> retained temporarily as historical implementation notes and should not be
> copied for new modules; new modules should follow the corresponding files in
> `inst/include/interface/call/`, `src/call_*.cpp`, and `R/*_call.R`.
...
- Understanding of R's native `.Call` interface and `SEXP` conversion
...
2. **R Interface Layer** (`inst/include/interface/call/`): Declares native entry points and registries
3. **R Wrapper Layer** (`R/`): R functions for initialization and data management
4. **Routine Registration** (`inst/include/interface/TMB/init_tmb.hpp`): Registers native routines with R
