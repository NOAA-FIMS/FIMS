...
```{r, eval = FALSE}
 # deviations approach
  ...
  nll <- nll - sum(dnorm(y, lamda[oi], sdo, TRUE))
  ...
  nll <- nll - sum(dnorm(y, lambda[oi], sdo, TRUE))
...
the fixed effects. The Laplace approximation requires up to the third order partial 
...
**Figure 1.** Hessian matrix for AR1 process using deviations approach (left)
...
for year $y$ and for age 1 and written as the following:

<!--  TODO: fix deviations approach equation -->
1. deviations approach: $log(N_{y,1}) = \hat{r} + z_{y}, \quad where z_{y} \sim N(z_{y-1},\sigma^{2}_{r})$
2. process approach: $N_{pred} = log(N_{y-1, 1}), \quad where log(N_{y, 1}) \sim N(\hat{r} + \phi*(N_{pred} - \hat{r}))$
...
<!--  TODO: flush this out more or just skip to results? -->
Results compared model parameter estimates and computational times. 
