...
# survey lengthcomp not set up in fimsframe yet
...
Each fleet is required to have data in order to evaluate the objective function. Currently FIMS only has a fleet module that is used to set up both fleets and surveys. FIMS contains an Index module, AgeComp module, LengthComp module, and length-at-age conversion module to pass data objects into the fleet module. Each of these data modules require a dimension be added to indicate the dimensions of the raw data (e.g. nyears x nages matrix). Any years with missing data should be specified with a value set to -999. Given this information, FIMS is able to correctly apply dimension folding for model output.
...
We'll create index, age composition, and length composition modules for the fleet using the `methods::new()` function and pass in the data defined above from the Model Comparison project.
```{r fleet-set-data}
# fleet index data
...
We will use the Lognormal distribution for fitting the fishing fleet cpue data
...
for(y in 1:nyears){
  fishing_fleet_index_distribution$log_logsd[y]$value <- log(sqrt(log(0.01^2 + 1)))
}
...
The next step is to link this distribution to the expected value of the fleet, log expected_index, using the 
set_distribution_links function. The first argument assigns the distribution type ("data", "random_effects", or "prior").
The second argument takes the id of the parameter or derived value being linked to the distribution. 
```{r fleet-expected}
...
Now we will set up the distribution for the fishery length composition data using the Multinomial distribution
```{r fleet-age-comp-distribution}
...
survey_fleet_length_comp$length_comp_data <- survey_lengthcomp * 200 # unit: number at age; proportion at age also works
```
...
survey_fleet_index_distribution$log_logsd <- new(ParameterVector, nyears)
for(y in 1:nyears){
  survey_fleet_index_distribution$log_logsd[y]$value <- log(sqrt(log(0.2^2 + 1)))
}
...
survey_fleet_lengthcomp_distribution <- methods::new(TMBDmultinomDistribution)
survey_fleet_lengthcomp_distribution$set_observed_data(survey_fleet_length_comp$get_id())
survey_fleet_lengthcomp_distribution$set_distribution_links("data", survey_fleet$proportion_catch_numbers_at_length$get_id())
```
