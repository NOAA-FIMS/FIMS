...
comp_lengths <- seq(0,1100,50) # length vector.
nlengths <- 23 # the number of length bins.
```
...
# use FIMS data frame
data(package = "FIMS")
fims_frame <- FIMSFrame(data_mile2)
```
...
fishery_lengthcomp <- FIMS::m_lengthcomp(fims_frame, "fleet1")
fishery_length_at_age <- FIMS::m_length_at_age(fims_frame, "fleet1")

survey_lengthcomp <- FIMS::m_lengthcomp(fims_frame, "survey1")
survey_length_at_age <- FIMS::m_length_at_age(fims_frame, "survey1")
...
# Set to not be time-varying but else statement shows how to make the
# parameter be time-varying, with a different value for every year
time_varying <- FALSE
if (!time_varying) {
  ...
  fishing_fleet_selectivity$slope$resize(nyears)
  for (i in 1:nyears) {
    fishing_fleet_selectivity$slope[i]$value <- 1.0
    fishing_fleet_selectivity$slope[i]$is_random_effect <- FALSE
    fishing_fleet_selectivity$slope[i]$estimated <- TRUE
  }
}
```
...
fishing_fleet$nlengths <- nlengths
# Set values for log_Fmort
fishing_fleet$log_Fmort <- new(ParameterVector, log(c(
...
We will use the Lognormal distribution for fitting the fishing fleet CPUE data. We indicate a log link function so that the expected value is the log of the expected CPUE. 
```{r fleet-index-distribution}
...
Now we will set up the distribution for the fishery age composition data using the Multinomial distribution
```{r fleet-age-comp-distribution}
fishing_fleet_agecomp_distribution <- new_data_distribution(
  module = fishing_fleet,
  family = multinomial(link = "logit"),
  data_type = "agecomp"
)
fishing_fleet_lengthcomp_distribution <- new_data_distribution(
  module = fishing_fleet,
  family = multinomial(link = "logit"),
  data_type = "lengthcomp"
)
#Set length-at-age conversion matrix
fishing_fleet$age_length_conversion_matrix <- new(ParameterVector,fishery_length_at_age,nages*nlengths)
...
survey_fleet_length_comp <- methods::new(LengthComp, nyears, nlengths)
# Effective sampling size is 200
survey_fleet_length_comp$length_comp_data <- survey_lengthcomp * 200 # unit:
...
survey_fleet$nlengths <- nlengths
...
survey_fleet_lengthcomp_distribution <- new_data_distribution(
  module = survey_fleet,
  family = multinomial(link = "logit"),
  data_type = "lengthcomp"
)
#Set length-at-age conversion matrix
survey_fleet$age_length_conversion_matrix <- new(ParameterVector,survey_length_at_age,nages*nlengths)
# Turn off estimation for length-at-age
...
recruitment$log_devs <- new(ParameterVector, c(
...
fishery_age_comp_results <- data.frame(
  observed = fishing_fleet_age_comp$age_comp_data/200,
  expected = fit@report$pcnaa[[1]]
)
print(fishery_age_comp_results)
plot(x=fishery_age_comp_results$observed,y=fishery_age_comp_results$expected)
#lines(x=c(0,1),y=c(0,1))

survey_age_comp_results <- data.frame(
  observed = survey_fleet_age_comp$age_comp_data/200,
  expected = fit@report$pcnaa[[2]]
)
print(survey_age_comp_results)
plot(x=survey_age_comp_results$observed,y=survey_age_comp_results$expected)
#lines(x=c(0,1),y=c(0,1))

fishery_length_comp_results <- data.frame(
  observed = fishing_fleet_length_comp$length_comp_data/200,
  expected = fit@report$pcnal[[1]]
)
print(fishery_length_comp_results)
plot(x=fishery_length_comp_results$observed,y=fishery_length_comp_results$expected)
#lines(x=c(0,1),y=c(0,1))

survey_length_comp_results <- data.frame(
  observed = survey_fleet_length_comp$length_comp_data/200,
  expected = fit@report$pcnal[[2]]
)
print(survey_length_comp_results)
plot(x=survey_length_comp_results$observed,y=survey_length_comp_results$expected)
#lines(x=c(0,1),y=c(0,1))
