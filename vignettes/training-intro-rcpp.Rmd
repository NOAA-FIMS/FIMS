...
<!-- The section below creates a collapse/expand all button -->
...
```{r, echo = FALSE}
# cleanup environment for Rcpp::sourceCpp to work
if (isNamespaceLoaded("TMB")) {
  devtools::unload(package = "TMB")
}
if (isNamespaceLoaded("Rcpp")) {
  devtools::unload(package = "Rcpp")
}
```
...
FIMS previously exposed its runtime model classes through Rcpp modules. The
current R-facing model interface uses registered native `.Call` routines. Rcpp
is still used within the package build and remains useful background for C++/R
interoperability, but new FIMS runtime wrappers belong in
`inst/include/interface/call/` with implementations in `src/call_*.cpp`.

In FIMS, most C++ code is organized into headers under
[`inst/include/`](https://github.com/NOAA-FIMS/FIMS/tree/main/inst/include).
Native interface declarations and registries live in `inst/include/interface/call/`.
...
Within FIMS, we first use `RCPP_EXPOSED_CLASS()` to expose all of our new type classes, e.g., `RCPP_EXPOSED_CLASS(Variable)` in [`src/fims_modules.hpp`](https://github.com/NOAA-FIMS/FIMS/blob/main/src/fims_modules.hpp). Once all of the type classes are exposed, we then use a single instance of `RCPP_MODULE` to expose the C++ to R in that same file.
