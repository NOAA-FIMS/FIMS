...
[Rcpp Sugar](https://dirk.eddelbuettel.com/code/rcpp/Rcpp-sugar.pdf) uses expression templates to allow you to write C++ code at a higher level of abstraction, making it more similar to R syntax. This syntactic sugar makes your C++ code more readable and often more efficient through vectorized code and allows one to eliminate many for loops from their code.

Within FIMS, we do not use Rcpp Sugar as you will find many for loops in the C++ code where we iterate through vectors. FIMS is structured as a large, modular C++ library, making standard C++ types and patterns more appropriate for its maintainability rather than using shortcuts found through Rcpp Sugar.

Some example Rcpp Sugar functions are as follows:
