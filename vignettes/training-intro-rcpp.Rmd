...
## Type conversion with wrap and as
...
These functions are particularly useful when you want to use standard C++ types (like `std::vector<double>`) in your C++ code while still maintaining compatibility with R.
...
```{r wrap_as_example, eval=FALSE}
# After compilation with sourceCpp
mean_wrap(c(42, 34))
```
</details>
...
Under the hood, all R objects are stored in a common datatype called `SEXP` (S-expression). Technically, a `SEXP` is a pointer to a structure of type `SEXPREC`. This is a variant type with subvariants corresponding to the R types you're familiar with (e.g., real, integer, logical, character).

When you use Rcpp types like `NumericVector`, Rcpp handles the conversion between `SEXP` and C++ types automatically. However, when writing more advanced C++ code or working with the R API directly, you may encounter `SEXP` types explicitly.
...
After compiling with `sourceCpp()`, you can use this class from R:

```{r uniform_usage, eval=FALSE}
...
[Rcpp Sugar](https://dirk.eddelbuettel.com/code/rcpp/Rcpp-sugar.pdf) uses expression templates to allow you to write C++ code at a higher level of abstraction, making it more similar to R syntax. This syntactic sugar makes your C++ code more readable and often more efficient through vectorization.

## Sugar Functions

Rcpp Sugar provides many useful functions similar to those in R:

* Logical functions: `all()`, `any()`
* Sequence generation: `seq_len()`, `seq_along()`
* Statistical functions: `mean()`, `var()`, `sd()`
* Mathematical functions: `abs()`, `exp()`, `log()`, `sqrt()`
* Statistical distributions: `dnorm()`, `pnorm()`, `qnorm()`, `rnorm()` (and similar for other distributions)

<details>
<summary><strong>Example: Rcpp Sugar</strong></summary>

```{Rcpp sugar_example, eval=FALSE}
#include <Rcpp.h>
using namespace Rcpp;
...
void sugar_demo() {
  ...
  LogicalVector check1 = x * x < 3;
  
  // Logical functions
  bool all_check = all(check1);
  bool any_check = any(check1);
  
  Rcout << "All values less than 3: " << all_check << std::endl;
  Rcout << "Any values less than 3: " << any_check << std::endl;
}
```

You can also use sugar with statistical distributions:

```{Rcpp sugar_dist, eval=FALSE}
#include <Rcpp.h>
using namespace Rcpp;

// [[Rcpp::export]]
void distribution_demo() {
  NumericVector y1 = NumericVector::create(0.0, 1.0, 2.0);
  NumericVector y2 = NumericVector::create(0.25, 0.5, 0.75);
  NumericVector y3 = NumericVector::create(-1.0, 0.0, 1.0);
  
  // Statistical functions (all assume standard normal)
  NumericVector x1 = dnorm(y1, 0, 1); // density of y1 at m=0, sd=1
  NumericVector x2 = qnorm(y2, 0, 1); // quantiles of y2
  NumericVector x3 = pnorm(y3, 0, 1); // distribution function of y3
  NumericVector x4 = rnorm(10, 0, 1); // 10 RNG draws of N(0, 1)
}
```

Sugar also supports `sapply` for applying functions:

```{Rcpp sugar_sapply, eval=FALSE}
#include <Rcpp.h>
using namespace Rcpp;
...
To learn more about Rcpp, we recommend the following resources:

* ["Advanced R"](http://adv-r.had.co.nz/Rcpp.html) by Hadley Wickham - Chapter on Rcpp
* ["Rcpp: Seamless R and C++ integration"](https://www.jstatsoft.org/article/view/v040i08) by Dirk Eddelbuettel and Romain Francois - Journal of Statistical Software article
* ["Rcpp for everyone"](https://teuder.github.io/rcpp4everyone_en/) by Masaki E. Tsuda - Comprehensive online book
* [Rcpp Modules vignette](https://cran.rstudio.com/web/packages/Rcpp/vignettes/Rcpp-modules.pdf) by Eddelbuettel and Francois
* [Rcpp Sugar vignette](https://dirk.eddelbuettel.com/code/rcpp/Rcpp-sugar.pdf)
