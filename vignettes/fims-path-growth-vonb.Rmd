...
This vignette describes the path from R to C++ for the **Growth v1** module using the fixed-parameter **VonBertalanffy** implementation. The goal is to show how values set in R propagate through the Rcpp interface and into the C++ core where length-at-age (LAA), SD LAA, and weight-at-age (WAA) are computed and cached for downstream use. The VonB parameterization used here is the SS3-style **two-reference-age** form (`length_at_ref_age_1`, `length_at_ref_age_2` at `reference_age_for_length_1`, `reference_age_for_length_2`), sometimes referred to informally as a "two-point" or "length-at-two-ages" parameterization.
