...
data_4_projections <- data_big_with_extra_year |>
  # Add on weight_at_age and age_to_length_conversion data for 10 more years
  # because you don't want these filled with -999
  dplyr::bind_rows(
    dplyr::filter(
      data_big,
      type == "weight_at_age",
      ...
      dplyr::select(-timing) |>
      merge(
        data.frame(timing = max(data_big[["timing"]]):(max(data_big[["timing"]]) + years_of_projection))
      )
  ) |>
