---
title: "A priojection example using FIMS advanced user functions"
...
```{r default-projection-settings}
#In this control run there are no projections this provides expected parameter
#estimates to compare to when performing projections to ensure that the 
#projection period did not inform the historic period estimates.

#Set projection years to zero
n_projection_years <- 0
#Add missing values for projection period landings and survey fleet
projected_landings <- rep(-999,n_projection_years)
projected_index <- rep(-999,n_projection_years)
#Set projection period fishing mortality to final year estimate
projected_F <- rep(om_output[["f"]][om_input$nyr],n_projection_years)
#Set projection period F to constant to not estimate 
estim_projected_F <- rep("constant",n_projection_years)
```

## Setup fishing fleet data
```{r fishing-fleet}
...
# set projection years landings if projections implemented
...
# set projection years age comps to missing values projections implemented
if(n_projection_years>0){
...
## Setup fishing fleet SetSelectivity
```{r fishing-fleet-selectivity}
# Create the fishing fleet selectivity
...
# set projection years fishing mortality if projections implemented
if(n_projection_years>0){
...
## Setup the fishing fleet distribution
```{r fishing-fleet-distribution}
...
# Create the survey fleet selectivity
...
## Setup recruitment distribution
...
## Get output and standard errors
```{r output}
...
## Run FIMS with 20 year projection targeting 30%SPR 
```{r default-projection-settings}
## Setup model to produce 20 projection years with no
## index, composition, or catch data and recruitment deviations
## in projection years set as constant values. SSB_ratio priors
## are provided with very high log_sd=200 in all but terminal
## projection year when log_sd=-5. F is not estimated in projection 
## years but a multiplier on annual F is estimated with all years in the
## estimation period set a constant log(multiplier)=0 and multiplier in the 
## projection years estimated. A prior on the log multiplier is set with 
## a log_sd=20 in estimation period and log_sd=-5 in projection years.
...
# Set projection period landings log_sd to large value 5
if(n_projection_years>0){
  for (y in (om_input[["nyr"]] + 1):(om_input[["nyr"]] + n_projection_years)) {
    # Compute lognormal SD from OM coefficient of variation (CV)
...
#Add missing values for projection year survey ages comps
projected_survey_age_comps <- em_input[["survey.age.obs"]][["survey1"]]* em_input[["n.survey"]][["survey1"]]
projected_survey_age_comps <- rbind(
projected_survey_age_comps,
matrix(-999, nrow = n_projection_years, ncol = om_input[["nages"]])
...
#Add missing values for projection year survey length comps
projected_survey_length_comps <- em_input[["survey.length.obs"]][["survey1"]]* em_input[["n.survey.lengthcomp"]][["survey1"]]
projected_survey_length_comps <- rbind(
projected_survey_length_comps,
matrix(-999, nrow = n_projection_years, ncol = om_input[["nlengths"]])
...
#Initialize the x and expected values vectors to the length of the estimated
#model period so projection period values don't impact the calculation
recruitment_distribution$x$resize(om_input[["nyr"]] - 1)
recruitment_distribution$expected_values$resize(om_input[["nyr"]] - 1)
for (i in 1:(om_input[["nyr"]] - 1)) {
recruitment_distribution$x[i]$value <- (0)
...
#stock status targets.

#This projection uses a log F multiplier to scale all fleets proportionally 
#Allowing the user to define relative fleet effort through the base log_F_mort
#values input as constants for each fleet.

population$log_f_multiplier$resize(om_input[["nyr"]] + n_projection_years)
#The multiplier is set at log(0) so that it has no impact on the estimation
#period fishing mortality. If not set the multiplier will also be set at log(0)
#in all years by default.
for (i in 1:(om_input[["nyr"]]) ) {
  population$log_f_multiplier[i]$value <- 0.0
  population$log_f_multiplier[i]$estimation_type$set("constant")
}
#Set the F multiplier and it prior for the projection period
if(n_projection_years>0){
  #Start estimation of the multiplier at log(0) in projection years and make
  #a random effect
  ...
  #Define new density component to add mean constraint to F multiplier
  F_mult_distribution <- methods::new(DnormDistribution)
  
  #Initialize the distribution mean this sets both the initial value and 
  #turns on an internal flag to use the mean instead of the expected_values
  #vector defined below which is required to set the distribution length.
  F_mult_distribution$set_distribution_mean(-2)
  
  #Set x and expected_values to define the distribution length
  ...
  for (i in 1:(om_input[["nyr"]])) {
    #x and expected values are not used just set at default values to 
    #define distribution length
    F_mult_distribution$x[i]$value <- 0
    F_mult_distribution$expected_values[i]$value <- 0
    #Set log_sd in the estimation period at a large number so that it does not 
  ...
  #Fill in the projection period distribution vectors
  for (i in (om_input[["nyr"]] + 1):(om_input[["nyr"]] + n_projection_years)) {
    F_mult_distribution$x[i]$value <- 0
    F_mult_distribution$expected_values[i]$value <- 0
    #Set log_sd at very small -5 to force all F_mult to equal the estimated mean
...
#Setup projection population target prior
#This is setting a ssb_ratio target which is also the SPR target for steepness=1
SSB_ratio_prior <- methods::new(DnormDistribution)
#Set all distribution vector lengths
...
for(y in 1:(om_input[["nyr"]] + 1)){
  #Set expected values to the target 
  SSB_ratio_prior$x[y]$value <- .3
  SSB_ratio_prior$expected_values[y]$value <- .3
  #For estimation period set the log_sd extremely large so the prior does not
  #impact estimation
...
if(n_projection_years>0){
  for(y in (om_input[["nyr"]] + 2):(om_input[["nyr"]] + 1 + n_projection_years)){
    SSB_ratio_prior$x[y]$value <- .3
    SSB_ratio_prior$expected_values[y]$value <- .3
    #Similarly to estimation period set all but the final value to a very large
    #number. 
    SSB_ratio_prior$log_sd[y]$value <- 200
  }
  ...
  for(y in (om_input[["nyr"]] + 1 + n_projection_years)){
    SSB_ratio_prior$x[y]$value <- .3
    SSB_ratio_prior$expected_values[y]$value <- .3
    #Set the log_sd very small in the final year so the model adjusts F_mult to
    #achieve this goal
...
# Set up catch at age model
```{r caa}
# NOTE: Not needed yet will fill out once we transition to new dev branch
# caa <- methods::new(CatchAtAge)
# caa$AddPopulation(population$get_id())
```
...
#Save control results for comparison with projection results
sdr_report_20_year_project <- sdr_report
sdr_fixed_20_year_project <- sdr_fixed
```
  
## Compare projection results with control
```{r output}

#Compare ratio of estimated parameters between model runs excluding the 
#estimated mean and f_mult parameters
comparison_results <- sdr_fixed_20_year_project[-50,]/sdr_fixed_no_project
comparison_results
