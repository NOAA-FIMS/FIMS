...
Here we specify how many years beyond the terminal data year projections should be run for. This could be short (<10) for constant catch scenarios or long (1 or more generations) in order to calculate equilibrium reference points such as Fmsy and Bmsy.
...
parameters_projection <- create_default_parameters(
  ...
  # In this simple example we are simply projecting the population forward with
  # the terminal year mortality values. More advanced approaches are included
  # in the test file test-projections-looped.R
  dplyr::rows_update(
    tibble::tibble(
      fleet_name = "fleet1",
      label = "log_Fmort",
...
Setting up projections to achieve a reference point target
  
Reference point targets are currently achieved by specifying a prior on the derived population quantity spawning_biomass_ratio in the final year/s of the projection period as well as a mean target for the parameter log_f_multiplier which is a simple annual multiplier of F_mort that allows the population total F to be adjusted while keeping relative fleet effort constant by fixing log_Fmort.
  
This approach allows annual log_f_multiplier to be estimated while ensuring that total F is held constant during the projection period and achieves the spawning biomass target.
  
The wrapper functions used in this vignette are not yet capable of setting up priors so the full projection approach is not included here. This is a high priority development goal that is in progress. Examples of setting up constraints on log_f_multiplier and a prior on spawning biomass ratio can be found in the test-projections-looped.R test files used to validate this approach.  
