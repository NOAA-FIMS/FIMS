...
This guide helps FIMS users set up a standardized Fisheries Integrated Modeling System (FIMS) environment. By using cloud-based workstations and containerized development environments, we ensure cross-platform consistency and eliminate the complexities of manual dependency management.

Please follow the section below that matches how you plan to set up FIMS:
  - ☁️[Google Cloud Workstations](#google-cloud-workstations) for managed cloud IDEs (internal NOAA)
  - 🌐[GitHub Codespaces](#github-codespaces) for browser-based containerized environments
  - 🐧[Windows Subsystem for Linux 2 (WSL2)](#wsl2) for a containerized Linux environment running on Windows hardware
  - 🧑‍💻[Local](#local) installation for direct deployment on your machine's primary operating system.
...
> **Note:** The dependency installation process typically takes 15 to 30 minutes. If using `Positron Pro`, a restart of the R session is required to initialize the new libraries. Please run `library(FIMS)` in the R session to confirm the successful installation of FIMS.
